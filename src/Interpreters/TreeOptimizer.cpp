...
const std::unordered_set<String> possibly_injective_function_names{
    "dictGet",      "dictGetString", "dictGetUInt8", "dictGetUInt16",  "dictGetUInt32",  "dictGetUInt64", "dictGetInt8",
    "dictGetInt16", "dictGetInt32",  "dictGetInt64", "dictGetFloat32", "dictGetFloat64", "dictGetDate",   "dictGetDateTime"};

/** You can not completely remove GROUP BY. Because if there were no aggregate functions, then it turns out that there will be no
 * aggregation. Instead, leave `GROUP BY const`. Next, see deleting the constants in the analyzeAggregation method.
 */
...
void optimizeGroupBy(ASTSelectQuery *select_query, ContextPtr context) {
  ...
  if (!select_query->groupBy()) return;

  const auto is_literal = [](const ASTPtr &ast) -> bool { return ast->as<ASTLiteral>(); };

  auto &group_exprs = select_query->groupBy()->children;

  /// removes expression at index idx by making it last one and calling .pop_back()
  const auto remove_expr_at_index = [&group_exprs](const size_t idx) {
    if (idx < group_exprs.size() - 1) std::swap(group_exprs[idx], group_exprs.back());
  ...
  for (size_t i = 0; i < group_exprs.size();) {
    if (const auto *function = group_exprs[i]->as<ASTFunction>()) {
      ...
      if (possibly_injective_function_names.contains(function->name)) {
        ...
        FunctionOverloadResolverPtr function_builder = UserDefinedExecutableFunctionFactory::instance().tryGet(
            function->name, context);  /// NOLINT(readability-static-accessed-through-instance)

        if (!function_builder) function_builder = function_factory.get(function->name, context);
      ...
      (void)std::remove_copy_if(std::begin(args_ast->children), std::end(args_ast->children), std::inserter(group_exprs, insert_position),
                                is_literal);
      ...
      if (settings[Setting::enable_positional_arguments]) {
        ...
        if (value.getType() == Field::Types::UInt64) {
          ...
          if (pos > 0 && pos <= select_query->select()->children.size()) keep_position = true;
        }
      }
  ...
  if (group_exprs.empty()) appendUnusedGroupByColumn(select_query);
}
...
GroupByKeysInfo getGroupByKeysInfo(const ASTs &group_by_keys) {
  ...
  for (const auto &group_key : group_by_keys) {
    ...
    if (group_key->as<ASTExpressionList>()) {
      ...
      if (group_key->as<ASTFunction>()) data.has_function = true;
...
void optimizeDuplicatesInOrderBy(const ASTSelectQuery *select_query) {
  if (!select_query->orderBy()) return;
  ...
  if (unique_elems.size() < elems.size()) elems = std::move(unique_elems);
}
...
std::unordered_set<String> getDistinctNames(const ASTSelectQuery &select) {
  ...
  if (!select.distinct) {
    ...
    if (const ASTSelectQuery *subselect = getSimpleSubselect(select)) implicit_distinct = getDistinctNames(*subselect);

    if (implicit_distinct.empty()) return {};
  }
  ...
  if (!select.distinct && names.size() != implicit_distinct.size()) return {};
...
void optimizeRedundantFunctionsInOrderBy(const ASTSelectQuery *select_query, ContextPtr context) {
  ...
  for (const auto &child : order_by->children) {
    ...
    if (order_by_element->with_fill) return;
  }
  ...
  for (auto &order_by_element : order_by->children) {
    ...
    if (name_or_function->as<ASTFunction>()) {
      if (!prev_keys.empty()) {
        ...
        if (data.redundant) continue;
      }
    }

    /// @note Leave duplicate keys unchanged. They would be removed in optimizeDuplicatesInOrderBy()
    if (auto *identifier = name_or_function->as<ASTIdentifier>()) prev_keys.emplace(getIdentifierName(identifier));

    modified.push_back(order_by_element);
  }

  if (modified.size() < order_by->children.size()) order_by->children = std::move(modified);
}

/// Remove duplicate items from LIMIT BY.
void optimizeLimitBy(const ASTSelectQuery *select_query) {
  if (!select_query->limitBy()) return;
  ...
  for (const auto &elem : elems) {
    if (elems_set.emplace(elem->getColumnName()).second) unique_elems.emplace_back(elem);
  }
...
void optimizeWithConstraints(ASTSelectQuery *select_query, Aliases & /*aliases*/, const NameSet & /*source_columns_set*/,
                             ...
                             const StorageMetadataPtr &metadata_snapshot, const bool optimize_append_index) {
...
void optimizeSubstituteColumn(ASTSelectQuery *select_query, Aliases & /*aliases*/, const NameSet & /*source_columns_set*/,
                              const std::vector<TableWithColumnNamesAndTypes> & /*tables_with_columns*/,
                              const StorageMetadataPtr &metadata_snapshot, const ConstStoragePtr &storage) {
...
void optimizeUsing(const ASTSelectQuery *select_query) {
  if (!select_query->join()) return;

  const auto *table_join = select_query->join()->table_join->as<ASTTableJoin>();
  if (!(table_join && table_join->using_expression_list)) return;
  ...
  if (uniq_expressions_list.size() < expression_list.size()) expression_list = uniq_expressions_list;
}
...
void optimizeDateFilters(ASTSelectQuery *select_query, const std::vector<TableWithColumnNamesAndTypes> &tables_with_columns,
                         ContextPtr context) {
...
void TreeOptimizer::optimizeIf(ASTPtr &query, Aliases &aliases, bool if_chain_to_multiif, bool multiif_to_if) {
  if (multiif_to_if) optimizeMultiIfToIf(query);
  ...
  if (if_chain_to_multiif) OptimizeIfChainsVisitor().visit(query);
}

void TreeOptimizer::optimizeCountConstantAndSumOne(ASTPtr &query, ContextPtr context) { RewriteCountVariantsVisitor(context).visit(query); }
...
void TreeOptimizer::optimizeGroupByFunctionKeys(ASTSelectQuery *select_query) {
  ...
  if (!group_by_keys_data.has_function) return;
  ...
  /// filling the result
  for (const auto &group_key : group_by_keys)
    if (group_by_keys_data.key_names.contains(group_key->getColumnName())) modified.push_back(group_key);

  /// modifying the input
  group_by->children = modified;
}

void TreeOptimizer::apply(ASTPtr &query, TreeRewriterResult &result, const std::vector<TableWithColumnNamesAndTypes> &tables_with_columns,
                          ContextPtr context) {
  const auto &settings = context->getSettingsRef();
  ...
  /// Move arithmetic operations out of aggregation functions
  if (settings[Setting::optimize_arithmetic_operations_in_aggregate_functions]) optimizeAggregationFunctions(query);

  bool converted_to_cnf = false;
  if (settings[Setting::convert_query_to_cnf]) converted_to_cnf = convertQueryToCNF(select_query);

  if (converted_to_cnf && settings[Setting::optimize_using_constraints] && result.storage_snapshot) {
    optimizeWithConstraints(select_query, result.aliases, result.source_columns_set, tables_with_columns, result.storage_snapshot->metadata,
                            settings[Setting::optimize_append_index]);

    if (settings[Setting::optimize_substitute_columns])
      optimizeSubstituteColumn(select_query, result.aliases, result.source_columns_set, tables_with_columns,
                               result.storage_snapshot->metadata, result.storage);
  }
  ...
  if (settings[Setting::optimize_time_filter_with_preimage]) optimizeDateFilters(select_query, tables_with_columns, context);

  /// GROUP BY injective function elimination.
  optimizeGroupBy(select_query, context);

  /// GROUP BY functions of other keys elimination.
  if (settings[Setting::optimize_group_by_function_keys]) optimizeGroupByFunctionKeys(select_query);

  if (settings[Setting::optimize_normalize_count_variants]) optimizeCountConstantAndSumOne(query, context);

  if (settings[Setting::optimize_rewrite_array_exists_to_has]) optimizeArrayExistsFunctions(query);

  /// Remove injective functions inside uniq
  if (settings[Setting::optimize_injective_functions_inside_uniq]) optimizeInjectiveFunctionsInsideUniq(query, context);

  /// Eliminate min/max/any aggregators of functions of GROUP BY keys
  if (settings[Setting::optimize_aggregators_of_group_by_keys] && !select_query->group_by_with_totals &&
  ...
  if (settings[Setting::optimize_redundant_functions_in_order_by]) optimizeRedundantFunctionsInOrderBy(select_query, context);
  ...
  if (settings[Setting::optimize_if_transform_strings_to_enum]) transformIfStringsIntoEnum(query);
  ...
  if (settings[Setting::optimize_or_like_chain] && settings[Setting::allow_hyperscan] &&
      settings[Setting::max_hyperscan_regexp_length] == 0 && settings[Setting::max_hyperscan_regexp_total_length] == 0) {
