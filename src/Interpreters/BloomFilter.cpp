...
BloomFilterParameters::BloomFilterParameters(size_t filter_size_, size_t filter_hashes_, size_t seed_)
  ...
  if (filter_size == 0) throw Exception(ErrorCodes::BAD_ARGUMENTS, "The size of bloom filter cannot be zero");
  if (filter_hashes == 0) throw Exception(ErrorCodes::BAD_ARGUMENTS, "The number of hash functions for bloom filter cannot be zero");
...
BloomFilter::BloomFilter(const BloomFilterParameters &params) : BloomFilter(params.filter_size, params.filter_hashes, params.seed) {}
...
bool BloomFilter::find(const char *data, size_t len) const {
  ...
  for (size_t i = 0; i < hashes; ++i) {
    ...
    if (!(filter[pos / word_bits] & (1ULL << (pos % word_bits)))) return false;
    acc += hash2;
  }
  return true;
}
...
void BloomFilter::clear() { filter.assign(words, 0); }
...
size_t BloomFilter::memoryUsageBytes() const { return filter.capacity() * sizeof(UnderType); }

bool operator==(const BloomFilter &a, const BloomFilter &b) {
  for (size_t i = 0; i < a.words; ++i)
    if (a.filter[i] != b.filter[i]) return false;
  return true;
}
...
DataTypePtr BloomFilter::getPrimitiveType(const DataTypePtr &data_type) {
  if (const auto *array_type = typeid_cast<const DataTypeArray *>(data_type.get())) {
    if (!typeid_cast<const DataTypeArray *>(array_type->getNestedType().get())) return getPrimitiveType(array_type->getNestedType());
...
ColumnPtr BloomFilter::getPrimitiveColumn(const ColumnPtr &column) {
  if (const auto *array_col = typeid_cast<const ColumnArray *>(column.get())) return getPrimitiveColumn(array_col->getDataPtr());
