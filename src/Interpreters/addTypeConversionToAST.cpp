...
ASTPtr addTypeConversionToAST(ASTPtr &&ast, const String &type_name, const NamesAndTypesList &all_columns, ContextPtr context) {
  ...
  const auto actions = ExpressionAnalyzer(ast, syntax_analyzer_result, const_pointer_cast<Context>(context)).getActions(true);
  ...
  if (desc_type->getName() != type_name) return addTypeConversionToAST(std::move(ast), type_name);
