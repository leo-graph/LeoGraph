...
ColumnsDescription QueryMetricLogElement::getColumnsDescription() {
  ...
  result.add({"query_id", std::make_shared<DataTypeString>(),
              parseQuery(codec_parser, "(ZSTD(1))", 0, DBMS_DEFAULT_MAX_PARSER_DEPTH, DBMS_DEFAULT_MAX_PARSER_BACKTRACKS), "Query ID."});
  result.add({"hostname", std::make_shared<DataTypeLowCardinality>(std::make_shared<DataTypeString>()),
              parseQuery(codec_parser, "(ZSTD(1))", 0, DBMS_DEFAULT_MAX_PARSER_DEPTH, DBMS_DEFAULT_MAX_PARSER_BACKTRACKS),
              "Hostname of the server executing the query."});
  result.add({"event_date", std::make_shared<DataTypeDate>(),
              parseQuery(codec_parser, "(Delta(2), ZSTD(1))", 0, DBMS_DEFAULT_MAX_PARSER_DEPTH, DBMS_DEFAULT_MAX_PARSER_BACKTRACKS),
              "Event date."});
  result.add({"event_time", std::make_shared<DataTypeDateTime>(),
              parseQuery(codec_parser, "(Delta(4), ZSTD(1))", 0, DBMS_DEFAULT_MAX_PARSER_DEPTH, DBMS_DEFAULT_MAX_PARSER_BACKTRACKS),
              "Event time."});
  result.add({"event_time_microseconds", std::make_shared<DataTypeDateTime64>(6),
              parseQuery(codec_parser, "(Delta(4), ZSTD(1))", 0, DBMS_DEFAULT_MAX_PARSER_DEPTH, DBMS_DEFAULT_MAX_PARSER_BACKTRACKS),
              "Event time with microseconds resolution."});
  result.add({"memory_usage", std::make_shared<DataTypeUInt64>(),
              "Amount of RAM the query uses. It might not include some types of dedicated memory."});
  result.add({"peak_memory_usage", std::make_shared<DataTypeUInt64>(), "Maximum amount of RAM the query used."});
...
void QueryMetricLogElement::appendToBlock(MutableColumns& columns) const {
  ...
  for (size_t i = 0, end = ProfileEvents::end(); i < end; ++i) columns[column_idx++]->insert(profile_events[i]);
}

void QueryMetricLog::shutdown() { Base::shutdown(); }
...
void QueryMetricLog::collectMetric(const ProcessList& process_list, String query_id) {
  ...
  if (elem) add(std::move(elem.value()));
}
...
void QueryMetricLog::startQuery(const String& query_id, TimePoint start_time, UInt64 interval_milliseconds) TSA_NO_THREAD_SAFETY_ANALYSIS {
  ...
  info.task = context->getSchedulePool().createTask(StorageID::createEmpty(), "QueryMetricLog",
                                                    [this, &process_list, query_id] { collectMetric(process_list, query_id); });
...
void QueryMetricLog::finishQuery(const String& query_id, TimePoint finish_time, QueryStatusInfoPtr query_info) {
  ...
  if (thread_id != query_status.thread_id || query_status.finished) {
    LOG_TEST(logger,
             "Query {} finished from a different thread_id than the one it started it: "
...
void QueryMetricLog::setQueryFinished(QueryMetricLogStatus& status) { status.finished = true; }

bool QueryMetricLog::getQueryFinished(QueryMetricLogStatus& status) { return status.finished; }
...
void QueryMetricLogStatus::scheduleNext(String query_id) {
  ...
  if (info.next_collect_time > now) {
    ...
    LOG_TEST(logger, "The next collecting task for query {} should have already run at {}. Scheduling it right now", query_id,
             timePointToString(info.next_collect_time));
...
std::optional<QueryMetricLogElement> QueryMetricLogStatus::createLogMetricElement(const String& query_id, const QueryStatusInfo& query_info,
                                                                                  TimePoint query_info_time, bool schedule_next) {
  ...
  if (schedule_next) scheduleNext(query_id);
