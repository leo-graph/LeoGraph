...
class AccumulatedBlockReader {
 public:
  AccumulatedBlockReader(TemporaryBlockStreamReaderHolder reader_, std::mutex& mutex_, size_t result_block_size_ = 0)
      : reader(std::move(reader_)), mutex(mutex_), result_block_size(result_block_size_) {
    if (!reader) throw Exception(ErrorCodes::LOGICAL_ERROR, "Reader is nullptr");
  }
...
void retryForEach(std::deque<size_t> indices, Fn<bool(size_t)> auto callback) {
  while (!indices.empty()) {
    ...
    if (!callback(bucket_index)) indices.push_back(bucket_index);
  }
}
}  // namespace
...
class GraceHashJoin::FileBucket : boost::noncopyable {
 ...
 public:
  ...
  explicit FileBucket(size_t bucket_index_, TemporaryBlockStreamHolder left_file_, TemporaryBlockStreamHolder right_file_, LoggerPtr log_)
      : idx(bucket_index_), left_file(std::move(left_file_)), right_file(std::move(right_file_)), state(State::WRITING_BLOCKS), log(log_) {}
...
void flushBlocksToBuckets(Blocks& blocks, const GraceHashJoin::Buckets& buckets, size_t except_index = 0) {
  ...
  retryForEach(generateRandomPermutation(1, buckets.size()),  // skipping 0 block, since we join it in memory w/o spilling on disk
               [&](size_t i) {
                 /// Skip empty and current bucket
                 if (!blocks[i].rows() || i == except_index) return true;

                 bool flushed = false;
                 if constexpr (table_side == JoinTableSide::Left) flushed = buckets[i]->tryAddLeftBlock(blocks[i]);
                 if constexpr (table_side == JoinTableSide::Right) flushed = buckets[i]->tryAddRightBlock(blocks[i]);

                 if (flushed) blocks[i].clear();
...
GraceHashJoin::GraceHashJoin(size_t initial_num_buckets_, size_t max_num_buckets_, std::shared_ptr<TableJoin> table_join_,
...
void GraceHashJoin::initBuckets() {
  ...
  size_t initial_rounded_num_buckets = roundUpToPowerOfTwoOrZero(std::clamp<size_t>(initial_num_buckets, 1, max_num_buckets));

  addBuckets(initial_rounded_num_buckets);

  if (buckets.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "No buckets created");
...
bool GraceHashJoin::addBlockToJoin(const Block& block, bool /*check_limits*/) {
  if (current_bucket == nullptr) throw Exception(ErrorCodes::LOGICAL_ERROR, "GraceHashJoin is not initialized");
...
bool GraceHashJoin::hasMemoryOverflow(size_t total_rows, size_t total_bytes) const {
  if (force_spill) return true;
  /// One row can't be split, avoid loop
  if (total_rows < 2) return false;
  bool has_overflow = !table_join->sizeLimits().softCheck(total_rows, total_bytes);

  if (has_overflow)
    LOG_TRACE(log, "Memory overflow, size exceeded {} / {} bytes, {} / {} rows", ReadableSize(total_bytes),
              ReadableSize(table_join->sizeLimits().max_bytes), total_rows, table_join->sizeLimits().max_rows);
...
void GraceHashJoin::addBuckets(const size_t bucket_count) {
  ...
  for (size_t i = 0; i < bucket_count; ++i) try {
      ...
      LOG_ERROR(getLogger("GraceHashJoin"), "Can't create bucket {} due to error: {}", current_size + i, getCurrentExceptionMessage(false));
      throw;
    }

  buckets.reserve(buckets.size() + bucket_count);
  for (auto& bucket : tmp_buckets) buckets.emplace_back(std::move(bucket));
}
...
JoinResultPtr GraceHashJoin::joinBlock(Block block) {
  if (rightTableCanBeReranged()) tryRerangeRightTableData();

  if (block.rows() == 0) return hash_join->joinBlock(block);
...
bool GraceHashJoin::alwaysReturnsEmptySet() const {
  if (!isInnerOrRight(table_join->kind())) return false;
  ...
  if (!file_buckets_are_empty) return false;
...
IBlocksStreamPtr GraceHashJoin::getNonJoinedBlocks(const Block& left_sample_block_, const Block& result_sample_block_,
                                                   UInt64 max_block_size_) const {
...
class GraceHashJoin::DelayedBlocks : public IBlocksStream {
 public:
  explicit DelayedBlocks(size_t current_bucket_, Buckets buckets_, InMemoryJoinPtr hash_join_, const Names& left_key_names_,
  ...
  Block nextImpl() override {
    ...
    while (true) {
      ...
      std::lock_guard lock(extra_block_mutex);
      if (not_processed_results.empty()) break;
    ...
    do {
      ...
      if (block.empty()) {
        ...
        bool there_are_still_might_be_rows_to_process = false;
        {
          ...
          std::lock_guard lock(extra_block_mutex);
          if (!not_processed_results.empty()) there_are_still_might_be_rows_to_process = true;
        }
      ...
      for (size_t bucket_idx = 0; bucket_idx < num_buckets; ++bucket_idx) {
        if (blocks[bucket_idx].rows() == 0) continue;
...
IBlocksStreamPtr GraceHashJoin::getDelayedBlocks() {
  ...
  if (current_bucket == nullptr) return nullptr;
  ...
  for (bucket_idx = bucket_idx + 1; bucket_idx < buckets.size(); ++bucket_idx) {
    ...
    LOG_TRACE(log, "Loaded bucket {} with {}(/{}) rows, {}", bucket_idx, hash_join->getTotalRowCount(), num_rows,
...
Block GraceHashJoin::prepareRightBlock(const Block& block) { return HashJoin::prepareRightBlock(block, hash_join_sample_block); }
...
void GraceHashJoin::addBlockToJoinImpl(Block block) {
  ...
  if (current_block.rows() > 0) {
    std::lock_guard lock(hash_join_mutex);
    if (!hash_join) hash_join = makeInMemoryJoin(fmt::format("grace{}", bucket_index));
    ...
    if (!hasMemoryOverflow(hash_join_total_keys, hash_join_total_bytes)) return;
    ...
    if (current_block.rows() > 0) hash_join->addBlockToJoin(current_block, /* check_limits = */ false);
  }
}
...
bool GraceHashJoin::rightTableCanBeReranged() const {
  if (hash_join && getNumBuckets() <= 1) return hash_join->rightTableCanBeReranged();
  return false;
}

void GraceHashJoin::tryRerangeRightTableData() {
  std::lock_guard lock(hash_join_mutex);
  if (hash_join) hash_join->tryRerangeRightTableData();
}
...
void GraceHashJoin::onBuildPhaseFinish() {
  ...
  if (hash_join) hash_join->onBuildPhaseFinish();
}
}  // namespace DB
