...
static bool isUnlimitedQuery(const IAST* ast) {
  ...
  if (ast->as<ASTKillQueryQuery>() || ast->getQueryKind() == IAST::QueryKind::AsyncInsertFlush) return true;
  ...
  if (const auto* ast_selects = ast->as<ASTSelectWithUnionQuery>()) {
    if (!ast_selects->list_of_selects || ast_selects->list_of_selects->children.empty()) return false;

    const auto* ast_select = ast_selects->list_of_selects->children[0]->as<ASTSelectQuery>();
    if (!ast_select) return false;
...
ProcessList::EntryPtr ProcessList::insert(const String& query_, UInt64 normalized_query_hash, const IAST* ast,
                                          ContextMutablePtr query_context, UInt64 watch_start_nanoseconds, bool is_internal) {
  EntryPtr res;

  const ClientInfo& client_info = query_context->getClientInfo();
  const Settings& settings = query_context->getSettingsRef();

  if (client_info.current_query_id.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Query id cannot be empty");
...

  {
    ...
    if (!is_unlimited_query && max_size && non_internal_processes >= max_size) {
      if (queue_max_wait_ms) LOG_WARNING(getLogger("ProcessList"), "Too many simultaneous queries, will wait {} ms.", queue_max_wait_ms);
      if (!queue_max_wait_ms ||
          !have_space.wait_for(lock, std::chrono::milliseconds(queue_max_wait_ms), [&] { return non_internal_processes < max_size; }))
    ...
    if (!is_unlimited_query) {
      ...
      if (max_insert_queries_amount && query_kind == IAST::QueryKind::Insert && amount >= max_insert_queries_amount)
        throw Exception(ErrorCodes::TOO_MANY_SIMULTANEOUS_QUERIES, "Too many simultaneous insert queries. Maximum: {}, current: {}",
      ...
      if (max_select_queries_amount && query_kind == IAST::QueryKind::Select && amount >= max_select_queries_amount)
        throw Exception(ErrorCodes::TOO_MANY_SIMULTANEOUS_QUERIES, "Too many simultaneous select queries. Maximum: {}, current: {}",
...

    {
       ...
       *
       * With `max_concurrent_queries_for_all_users` it is possible to configure an additional, runtime configurable, limit for query
       * concurrency. Usually it should be configured just once for `default_profile` which is inherited by all users. DBAs can override
      ...
      if (!is_unlimited_query && settings[Setting::max_concurrent_queries_for_all_users] &&
        ...
        throw Exception(ErrorCodes::TOO_MANY_SIMULTANEOUS_QUERIES,
                        ...
                        "Current: {}, maximum: {}",
                        non_internal_processes, settings[Setting::max_concurrent_queries_for_all_users].toString());
    }
      ...
      if (user_process_list != user_to_queries.end()) {
        if (!is_unlimited_query && settings[Setting::max_concurrent_queries_for_user] &&
          ...
          throw Exception(ErrorCodes::TOO_MANY_SIMULTANEOUS_QUERIES,
                          ...
                          "Current: {}, maximum: {}",
                          client_info.current_user, user_process_list->second.non_internal_queries,
        ...
        if (running_query != user_process_list->second.queries.end()) {
          if (!settings[Setting::replace_running_query])
            throw Exception(ErrorCodes::QUERY_WITH_SAME_ID_IS_ALREADY_RUNNING, "Query with id = {} is already running.",
          ...
          if (!replace_running_query_max_wait_ms ||
              !have_space.wait_for(lock, std::chrono::milliseconds(replace_running_query_max_wait_ms), [&] {
                running_query = user_process_list->second.queries.find(client_info.current_query_id);
                if (running_query == user_process_list->second.queries.end()) return true;
                running_query->second->is_killed.store(true, std::memory_order_relaxed);
                return false;
              })) {
            throw Exception(ErrorCodes::QUERY_WITH_SAME_ID_IS_ALREADY_RUNNING, "Query with id = {} is already running and can't be stopped",
                            client_info.current_query_id);
          }
        }
      }
    ...
    if (auto query_user = queries_to_user.find(client_info.current_query_id);
        query_user != queries_to_user.end() && query_user->second != client_info.current_user) {
      throw Exception(ErrorCodes::QUERY_WITH_SAME_ID_IS_ALREADY_RUNNING, "Query with id = {} is already running by user {}",
    ...
    if (user_process_list_it == user_to_queries.end()) {
      user_process_list_it = user_to_queries
                                 .emplace(std::piecewise_construct, std::forward_as_tuple(client_info.current_user),
                                          std::forward_as_tuple(query_context->getGlobalContext(), this))
                                 .first;
    }
    ...
    if (thread_group) {
      ...
      if (user_process_list.user_temp_data_on_disk) {
        ...
        if (user_process_list.user_temp_data_on_disk)
          query_context->setTempDataOnDisk(std::make_shared<TemporaryDataOnDiskScope>(user_process_list.user_temp_data_on_disk,
    ...
    query = std::make_shared<QueryStatus>(
        query_context, query_, normalized_query_hash, client_info,
        ...
        std::move(query_slot), std::move(thread_group), query_kind, settings, watch_start_nanoseconds, is_internal);

    auto process_it = processes.emplace(processes.end(), query);
    ...
    bool registered_in_cancellation_checker = CancellationChecker::getInstance().appendTask(
        query, query_context->getSettingsRef()[Setting::max_execution_time].totalMilliseconds(),
        query_context->getSettingsRef()[Setting::timeout_overflow_mode]);
...
ProcessListEntry::~ProcessListEntry() {
  if (registered_in_cancellation_checker) {
    /// We need to block the overcommit tracker here to avoid lock inversion because OvercommitTracker takes a lock on the
    /// ProcessList::mutex. When task is added, we lock the ProcessList::mutex, and then the CancellationChecker mutex.
  ...
  if (user_process_list.queries.empty()) user_process_list.resetTrackers();
}

QueryStatus::QueryStatus(ContextPtr context_, const String& query_, UInt64 normalized_query_hash_, const ClientInfo& client_info_,
                         QueryPriorities::Handle&& priority_handle_, QuerySlotPtr&& query_slot_, ThreadGroupPtr&& thread_group_,
                         IAST::QueryKind query_kind_, const Settings& query_settings_, UInt64 watch_start_nanoseconds, bool is_internal_)
  ...
  if (!is_internal) num_non_internal_queries_increment.emplace(CurrentMetrics::QueryNonInternal);
...
QueryStatus::~QueryStatus() {
  ...
  for (const auto& [_, e] : executors) assert(!e->executor);
#endif

  if (auto* memory_tracker = getMemoryTracker()) {
    if (user_process_list) user_process_list->user_overcommit_tracker.onQueryStop(memory_tracker);
    if (global_overcommit_tracker) global_overcommit_tracker->onQueryStop(memory_tracker);
  }
}
...
CancellationCode QueryStatus::cancelQuery(CancelReason reason, std::exception_ptr exception) {
  {
    ...
    if (is_killed) return CancellationCode::CancelSent;
...

  {
    ...
    for (const auto& [_, e] : executors) executors_snapshot.push_back(e);
  }
  ...
  for (const auto& e : executors_snapshot) e->cancel();
...
void QueryStatus::throwProperExceptionIfNeeded(const UInt64& max_execution_time_ms, const UInt64& elapsed_ns) {
  {
    ...
    if (is_killed) {
      ...
      if (elapsed_ns) additional_error_part = fmt::format("elapsed {} ms, ", static_cast<double>(elapsed_ns) / 1000000ULL);
...
std::shared_ptr<ProcessListEntry> QueryStatus::getProcessListEntry() const { return process_list_entry.lock(); }

ThrottlerPtr QueryStatus::getUserNetworkThrottler() {
  if (!user_process_list) return {};
...
QueryStatusPtr ProcessList::tryGetProcessListElement(const String& current_query_id, const String& current_user) {
  ...
  if (user_it != user_to_queries.end()) {
    ...
    if (query_it != user_queries.end()) return query_it->second;
  }
...
CancellationCode ProcessList::sendCancelToQuery(const String& current_query_id, const String& current_user) {
  ...
  /// The ProcessListEntry cannot be destroy if is_cancelling is true.
  {
    ...
    if (!elem) return CancellationCode::NotFound;
...
void ProcessList::killAllQueries() {
  ...
  SCOPE_EXIT({
    LockAndBlocker lock(mutex);
    for (auto& cancelled_process : cancelled_processes) cancelled_process->is_cancelling = false;
    cancelled_cv.notify_all();
  });
  ...
  for (auto& cancelled_process : cancelled_processes) cancelled_process->cancelQuery(CancelReason::CANCELLED_BY_USER);
}
...
QueryStatusPtr ProcessList::getProcessListElement(const String& query_id) const {
  ...
  for (const auto& process : processes) {
    if (process->client_info.current_query_id == query_id) return process;
  }

  return nullptr;
}

QueryStatusInfoPtr ProcessList::getQueryInfo(const String& query_id, bool get_thread_list, bool get_profile_events,
                                             bool get_settings) const {
  auto process = getProcessListElement(query_id);
  if (process) return std::make_shared<QueryStatusInfo>(process->getInfo(get_thread_list, get_profile_events, get_settings));

  return nullptr;
}

ProcessListForUser::ProcessListForUser(ProcessList* global_process_list) : ProcessListForUser(nullptr, global_process_list) {}
  ...
  if (global_context) {
    ...
    if (auto shared_temp_data = global_context->getSharedTempDataOnDisk())
      user_temp_data_on_disk =
          std::make_shared<TemporaryDataOnDiskScope>(std::move(shared_temp_data), std::move(temporary_data_on_disk_settings));
  }
}
...
ProcessList::UserInfo ProcessList::getUserInfo(bool get_profile_events) const {
  ...
  for (const auto& [user, user_queries] : user_to_queries) per_user_infos.emplace(user, user_queries.getInfo(get_profile_events));
...
ProcessList::QueryAmount ProcessList::getQueryKindAmount(const IAST::QueryKind& query_kind) const {
  auto found = query_kind_amounts.find(query_kind);
  if (found == query_kind_amounts.end()) return 0;
  return found->second;
}
