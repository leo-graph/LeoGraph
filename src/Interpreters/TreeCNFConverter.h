 ...
 public:
  ...
  CNFQuery& filterAlwaysTrueGroups(P predicate_is_unknown)  /// delete always true groups
  {
    ...
    for (const auto& or_group : statements) {
      if (predicate_is_unknown(or_group)) filtered.insert(or_group);
    }
  ...
  CNFQuery& filterAlwaysFalseAtoms(P predicate_is_unknown)  /// delete always false atoms
  {
    ...
    for (const auto& or_group : statements) {
      ...
      for (const auto& ast : or_group) {
        if (predicate_is_unknown(ast)) filtered_group.insert(ast);
      }
  ...
  const CNFQuery& iterateGroups(F func) const {
    for (const auto& group : statements) func(group);
    return *this;
  }

  CNFQuery& appendGroup(AndGroup&& and_group) {
    for (auto&& or_group : and_group) statements.emplace(or_group);
    return *this;
  }
  ...
  CNFQuery& transformAtoms(F func) {
    transformGroups([func](const OrGroup& group) -> OrGroup {
      ...
      for (const auto& atom : group) {
        ...
        if (new_atom.ast) result.insert(std::move(new_atom));
      }
      return result;
    });
    return *this;
...
class TreeCNFConverter {
 public:
  ...
  /// 0 - means unlimited.
  static std::optional<CNFQuery> tryConvertToCNF(const IAST* query, size_t max_growth_multiplier = DEFAULT_MAX_GROWTH_MULTIPLIER);

  static CNFQuery toCNF(const IAST* query, size_t max_growth_multiplier = DEFAULT_MAX_GROWTH_MULTIPLIER);
...
TAndGroup reduceOnceCNFStatements(const TAndGroup& groups) {
  ...
  for (const auto& group : groups) {
    ...
    if (!inserted) result.insert(group);
  }
  return result;
}
...
bool isCNFGroupSubset(const TOrGroup& left, const TOrGroup& right) {
  ...
  for (const auto& elem : left)
    if (!right.contains(elem)) return false;
  return true;
}
