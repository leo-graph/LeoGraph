 ...
 public:
  ...
  struct InsertQuery {
   public:
    ...
    InsertQuery(const ASTPtr& query_, const std::optional<UUID>& user_id_, const std::vector<UUID>& current_roles_,
                const Settings& settings_, AsynchronousInsertQueueDataKind data_kind_);
 ...
 private:
  struct DataChunk : public std::variant<StringWithMemoryTracking, Block> {
    ...
    AsynchronousInsertQueueDataKind getDataKind() const {
      if (std::holds_alternative<Block>(*this)) return AsynchronousInsertQueueDataKind::Preprocessed;
  ...
  struct InsertData {
    struct Entry {
     public:
      ...
      Entry(DataChunk&& chunk_, String&& query_id_, const String& async_dedup_token_, const String& format_,
    ...
    InsertData() : ready_future(ready_promise.get_future().share()) {}

    explicit InsertData(Milliseconds timeout_ms_) : ready_future(ready_promise.get_future().share()), timeout_ms(timeout_ms_) {}
  ...
  Milliseconds getBusyWaitTimeoutMs(const Settings& settings, const QueueShard& shard,
  ...
  void scheduleDataProcessingJob(const InsertQuery& key, InsertDataPtr data, ContextPtr global_context, size_t shard_num,
                                 ThreadGroupPtr current_query_thread_group = nullptr);

  static void processData(InsertQuery key, InsertDataPtr data, ContextPtr global_context, ThreadGroupPtr current_query_thread_group,
                          QueueShardFlushTimeHistory& queue_shard_flush_time_history);

  template <typename LogFunc>
  static Chunk processEntriesWithParsing(const InsertQuery& key, const InsertDataPtr& data, const Block& header,
                                         const ContextPtr& insert_context, LoggerPtr logger, LogFunc&& add_to_async_insert_log);

  template <typename LogFunc>
  static Chunk processPreprocessedEntries(const InsertDataPtr& data, const Block& header, const ContextPtr& context_, LoggerPtr logger,
