...
WhereConstraintsOptimizer::WhereConstraintsOptimizer(ASTSelectQuery* select_query_, const StorageMetadataPtr& metadata_snapshot_,
                                                     bool optimize_append_index_)
    : select_query(select_query_), metadata_snapshot(metadata_snapshot_), optimize_append_index(optimize_append_index_) {}
...
bool checkIfGroupAlwaysTrueGraph(const CNFQuery::OrGroup& group, const ComparisonGraph<ASTPtr>& graph) {
  ...
  for (const auto& atom : group) {
    ...
    if (func && func->arguments->children.size() == 2) {
      ...
      if (graph.isAlwaysCompare(expected, func->arguments->children[0], func->arguments->children[1])) return true;
    }
  }
...
bool checkIfAtomAlwaysFalseFullMatch(const CNFQueryAtomicFormula& atom, const ConstraintsDescription& constraints_description) {
  ...
  if (constraint_atom_ids) {
    for (const auto& constraint_atom : constraints_description.getAtomsById(*constraint_atom_ids)) {
      ...
      if (match_result == MatchState::NOT_MATCH) return true;
    }
  }
...
void replaceToConstants(ASTPtr& term, const ComparisonGraph<ASTPtr>& graph) {
  ...
  if (equal_constant) {
    ...
    for (auto& child : term->children) replaceToConstants(child, graph);
  }
}
  ...
  if (select_query->where() && metadata_snapshot) {
    ...
    cnf.pullNotOutFunctions()
        ...
        .filterAlwaysFalseAtoms([&compare_graph, this](const auto& atom) {
          ...
          return !checkIfAtomAlwaysFalseFullMatch(atom, metadata_snapshot->getConstraints()) &&
                 !checkIfAtomAlwaysFalseGraph(atom, compare_graph);
        })
        .transformAtoms([&compare_graph](const auto& atom) { return replaceTermsToConstants(atom, compare_graph); })
        .reduce()
        .pushNotInFunctions();

    if (optimize_append_index) AddIndexConstraintsOptimizer(metadata_snapshot).perform(cnf);
