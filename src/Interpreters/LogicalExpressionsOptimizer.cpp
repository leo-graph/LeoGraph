...
LogicalExpressionsOptimizer::OrWithExpression::OrWithExpression(const ASTFunction *or_function_, const IASTHash &expression_,
...
LogicalExpressionsOptimizer::LogicalExpressionsOptimizer(ASTSelectQuery *select_query_, const TablesWithColumns &tables_with_columns_,
                                                         UInt64 optimize_min_equality_disjunction_chain_length)
...
void LogicalExpressionsOptimizer::perform() {
  if (select_query == nullptr) return;
  if (visited_nodes.contains(select_query)) return;
  ...
  for (auto &chain : disjunctive_equality_chains_map) {
    if (!mayOptimizeDisjunctiveEqualityChain(chain)) continue;
  ...
  while (!to_visit.empty()) {
    ...
    if (found_chain) {
      if (from_node != nullptr) {
        ...
        if (!res.second) throw Exception(ErrorCodes::LOGICAL_ERROR, "LogicalExpressionsOptimizer: parent node information is corrupted");
      }
    } else {
...
inline ASTs &getFunctionOperands(const ASTFunction *or_function) { return or_function->children[0]->children; }
...
bool LogicalExpressionsOptimizer::isLowCardinalityEqualityChain(const std::vector<ASTFunction *> &functions) const {
  ...
  if (first_operands.empty()) return false;

  if (!first_operands[0]) return false;

  const auto *identifier = first_operands.at(0)->as<ASTIdentifier>();
  if (!identifier) return false;

  auto pos = IdentifierSemantic::getMembership(*identifier);
  if (!pos) pos = IdentifierSemantic::chooseTableColumnMatch(*identifier, tables_with_columns, true);

  if (!pos) return false;

  if (*pos >= tables_with_columns.size()) return false;

  if (auto data_type_and_name = tables_with_columns.at(*pos).columns.tryGetByName(identifier->shortName())) {
    if (typeid_cast<const DataTypeLowCardinality *>(data_type_and_name->type.get())) return true;
  }
...
bool LogicalExpressionsOptimizer::mayOptimizeDisjunctiveEqualityChain(const DisjunctiveEqualityChain &chain) const {
  ...
  if (settings.optimize_min_equality_disjunction_chain_length == 0) return false;
  ...
  for (size_t i = 1; i < equality_functions.size(); ++i) {
    ...
    if (literal->value.getType() != first_literal->value.getType()) return false;
  }
  return true;
}
...
void LogicalExpressionsOptimizer::cleanupOrExpressions() {
  ...
  for (const auto &chain : disjunctive_equality_chains_map) {
    if (!chain.second.is_processed) continue;
  ...
  for (const auto &chain : disjunctive_equality_chains_map) {
    const auto &equalities = chain.second;
    if (!equalities.is_processed) continue;
    ...
    if (it == garbage_map.end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "LogicalExpressionsOptimizer: garbage map is corrupted");
  ...
  for (const auto &chain : disjunctive_equality_chains_map) {
    ...
    if (operands.size() == 1) {
      ...
      if (it2 != column_to_position.end()) {
        ...
        if (!inserted) throw Exception(ErrorCodes::LOGICAL_ERROR, "LogicalExpressionsOptimizer: internal error");
      ...
      for (auto &parent : parents) {
        ...
        std::replace_if(
            parent->children.begin(), parent->children.end(), [or_function](const ASTPtr &ptr) { return ptr.get() == or_function; },
            operands[0]);
      }
