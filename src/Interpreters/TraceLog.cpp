...
ColumnsDescription TraceLogElement::getColumnsDescription() {
  ...
  auto entry_type_enum = std::make_shared<DataTypeEnum8>(DataTypeEnum8::Values{
  ...
  return ColumnsDescription{
      ...
      {"trace_type", std::make_shared<TraceDataType>(trace_values),
       ...
       "`MemoryAllocatedWithoutCheck` represents collection of significant allocations (>16MiB) that is done with ignoring any memory "
       "limits (for ClickHouse developers only)."
      ...
      {"size", std::make_shared<DataTypeInt64>(),
       "For trace types Memory, MemorySample, MemoryAllocatedWithoutCheck or MemoryPeak is the amount of memory allocated, for other trace "
       "types is 0."},
      {"ptr", std::make_shared<DataTypeUInt64>(), "The address of the allocated chunk."},
      {"memory_context", std::make_shared<ContextDataType>(context_values),
       fmt::format("Memory Tracker context (only for Memory/MemoryPeak): {}", context_description)},
      {"memory_blocked_context", std::make_shared<ContextDataType>(context_values),
       fmt::format("Context for which memory tracker is blocked (for ClickHouse developers only): {}", context_description)},
      ...
      {"lines", symbolized_type,
       "If the symbolization is enabled, contains strings with file names with line numbers, corresponding to the `trace`."},
      {"function_id", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeInt32>()),
       "For trace type Instrumentation, ID assigned to the function in xray_instr_map section of elf-binary."},
      {"function_name", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeString>()),
       "For trace type Instrumentation, name of the instrumented function."},
      {"handler", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeString>()),
       "For trace type Instrumentation, handler of the instrumented function."},
      {"entry_type", std::make_shared<DataTypeNullable>(entry_type_enum),
       "For trace type Instrumentation, entry type of the instrumented function."},
      {"duration_nanoseconds", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()),
       "For trace type Instrumentation, time the function was running for in nanoseconds."},
  };
}
 ...
 private:
  ...
  std::string_view impl(uintptr_t addr) {
    ...
    if (const auto *object = symbol_index.thisObject()) {
      ...
      if (!std::filesystem::exists(object->name)) return {};
  ...
  std::string_view implCached(uintptr_t addr) {
    ...
    if (inserted) it->getMapped() = impl(addr);
    return it->getMapped();
  }
...
void TraceLogElement::appendToBlock(MutableColumns &columns) const {
  ...
  typeid_cast<ColumnNullable &>(*columns[i++])
      .insertData(entry_type.has_value() ? reinterpret_cast<const char *>(&entry_type.value()) : nullptr,
                  sizeof(Instrumentation::EntryType));
  typeid_cast<ColumnNullable &>(*columns[i++])
      .insertData(duration_nanoseconds.has_value() ? reinterpret_cast<const char *>(&duration_nanoseconds.value()) : nullptr,
                  sizeof(UInt64));
}
