...
void TraceCollector::initialize(std::shared_ptr<TraceLog> trace_log_) {
  if (is_trace_log_initialized) throw DB::Exception(ErrorCodes::LOGICAL_ERROR, "TraceCollector is already initialized");
...
std::shared_ptr<TraceLog> TraceCollector::getTraceLog() {
  if (!is_trace_log_initialized.load(std::memory_order_acquire)) return nullptr;
    ...
    while (true) {
      ...
      if (auto trace_log = getTraceLog()) {
        ...
        TraceLogElement element{
            ...
            .size = size,
            .ptr = ptr,
            .memory_context = memory_context == TraceSender::MEMORY_CONTEXT_UNKNOWN
                                  ? std::nullopt
                                  : std::make_optional<VariableContext>(static_cast<VariableContext>(memory_context)),
            .memory_blocked_context = memory_blocked_context == TraceSender::MEMORY_CONTEXT_UNKNOWN
                                          ? std::nullopt
                                          : std::make_optional<VariableContext>(static_cast<VariableContext>(memory_blocked_context)),
            .event = event,
            .increment = increment,
