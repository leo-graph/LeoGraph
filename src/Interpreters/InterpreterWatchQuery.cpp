...
QueryPipelineBuilder InterpreterWatchQuery::buildQueryPipeline() {
  ...
  if (!storage) throw Exception(ErrorCodes::UNKNOWN_TABLE, "Table {} does not exist.", table_id.getNameForLogs());
  ...
  if (settings[Setting::max_columns_to_read] && required_columns.size() > settings[Setting::max_columns_to_read])
    throw Exception(ErrorCodes::TOO_MANY_COLUMNS,
                    ...
                    "Requested: {}, maximum: {}",
                    required_columns.size(), settings[Setting::max_columns_to_read].toString());
