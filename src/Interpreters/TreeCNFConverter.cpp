...
bool isLogicalFunction(const ASTFunction& func) { return func.name == "and" || func.name == "or" || func.name == "not"; }

size_t countAtoms(const ASTPtr& node) {
  checkStackSize();
  if (node->as<ASTIdentifier>() || node->as<ASTLiteral>()) return 1;

  const auto* func = node->as<ASTFunction>();
  if (func && !isLogicalFunction(*func)) return 1;

  size_t num_atoms = 0;
  for (const auto& child : node->children) num_atoms += countAtoms(child);
  return num_atoms;
}
...
void splitMultiLogic(ASTPtr& node) {
  ...
  if (func && (func->name == "and" || func->name == "or")) {
    ...
    for (auto& child : new_func->arguments->children) splitMultiLogic(child);
  } else if (func && func->name == "not") {
    for (auto& child : func->arguments->children) splitMultiLogic(child);
  }
}
...
void traversePushNot(ASTPtr& node, bool add_negation) {
  ...
  if (func && (func->name == "and" || func->name == "or")) {
    if (add_negation) {
      ...
      node = makeASTOperator((func->name == "and" ? "or" : "and"), func->arguments->children[0]->clone(),
                             func->arguments->children[1]->clone());
    }

    auto* new_func = node->as<ASTFunction>();
    for (auto& child : new_func->arguments->children) traversePushNot(child, add_negation);
  } else if (func && func->name == "not") {
    if (func->arguments->children.size() != 1) throw Exception(ErrorCodes::INCORRECT_QUERY, "Bad NOT function. Expected 1 argument");
    /// delete NOT
    node = func->arguments->children[0]->clone();

    traversePushNot(node, !add_negation);
  } else {
    if (add_negation) node = makeASTOperator("not", node->clone());
  }
}
...
bool traversePushOr(ASTPtr& node, size_t num_atoms, size_t max_atoms) {
  ...
  if (func && (func->name == "or" || func->name == "and")) {
    for (auto& child : func->arguments->children)
      if (!traversePushOr(child, num_atoms, max_atoms)) return false;
  }
  ...
  if (func && func->name == "or") {
    ...
    for (size_t i = 0; i < func->arguments->children.size(); ++i) {
      ...
      if (and_func && and_func->name == "and") and_node_id = i;
    }

    if (and_node_id == func->arguments->children.size()) return true;
    ...
    node = makeASTOperator("and", makeASTOperator("or", a->clone(), b), makeASTOperator("or", a, c));
...
void traverseCNF(const ASTPtr& node, CNFQuery::AndGroup& and_group, CNFQuery::OrGroup& or_group) {
  ...
  if (func && func->name == "and") {
    for (auto& child : func->arguments->children) {
      ...
      if (!group.empty()) and_group.insert(std::move(group));
    }
...
void traverseCNF(const ASTPtr& node, CNFQuery::AndGroup& result) {
  ...
  if (!or_group.empty()) result.insert(or_group);
}

}  // namespace

std::optional<CNFQuery> TreeCNFConverter::tryConvertToCNF(const IAST* query, size_t max_growth_multiplier) {
  ...
  size_t max_atoms = max_growth_multiplier ? std::max(MAX_ATOMS_WITHOUT_CHECK, num_atoms * max_growth_multiplier) : 0;

  if (!traversePushOr(cnf, num_atoms, max_atoms)) return {};
...
CNFQuery TreeCNFConverter::toCNF(const IAST* query, size_t max_growth_multiplier) {
...
ASTPtr TreeCNFConverter::fromCNF(const CNFQuery& cnf) {
  ...
  if (or_groups.size() == 1) return or_groups.front();

  ASTPtr res = makeASTOperator("and");
  auto* func = res->as<ASTFunction>();
  for (const auto& group : or_groups) func->arguments->children.push_back(group);
