...
class GlobalSubqueriesMatcher {
 public:
  struct Data : WithContext {
    ...
    Data(ContextPtr context_, size_t subquery_depth_, bool is_remote_, bool is_explain_, TemporaryTablesMapping &tables,
         PreparedSetsPtr prepared_sets_, bool &has_global_subqueries_, TableJoin *table_join_)
  ...
  static void visit(ASTPtr &ast, Data &data) {
    if (auto *t = ast->as<ASTFunction>()) visit(*t, ast, data);
    if (auto *t = ast->as<ASTTablesInSelectQueryElement>()) visit(*t, ast, data);
  }
