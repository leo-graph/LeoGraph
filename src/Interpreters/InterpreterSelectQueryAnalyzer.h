...
class InterpreterSelectQueryAnalyzer : public IInterpreter {
 public:
   ...
   */
  InterpreterSelectQueryAnalyzer(const ASTPtr& query_, const ContextPtr& context_, const SelectQueryOptions& select_query_options_,
                                 const Names& column_names = {}, const ActionsDAG* post_filter_ = nullptr);
   ...
   */
  InterpreterSelectQueryAnalyzer(const ASTPtr& query_, const ContextPtr& context_, const SelectQueryOptions& select_query_options_,
                                 const StoragePtr& storage_, const Names& column_names = {});

  /** Initialize interpreter with query tree.
   * No query tree passes are applied.
   */
  InterpreterSelectQueryAnalyzer(const QueryTreeNodePtr& query_tree_, const ContextPtr& context_,
  ...
  static SharedHeader getSampleBlock(const ASTPtr& query, const ContextPtr& context, const SelectQueryOptions& select_query_options = {});

  static SharedHeader getSampleBlock(const QueryTreeNodePtr& query_tree, const ContextPtr& context_,
