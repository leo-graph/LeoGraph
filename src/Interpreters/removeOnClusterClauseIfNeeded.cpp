...
static bool isUserDefinedFunctionQuery(const ASTPtr &query) {
  return query->as<ASTCreateSQLFunctionQuery>() || query->as<ASTCreateWasmFunctionQuery>() || query->as<ASTDropFunctionQuery>();
}

static bool isAccessControlQuery(const ASTPtr &query) {
  return query->as<ASTCreateUserQuery>() || query->as<ASTCreateQuotaQuery>() || query->as<ASTCreateRoleQuery>() ||
         query->as<ASTCreateRowPolicyQuery>() || query->as<ASTCreateSettingsProfileQuery>() || query->as<ASTDropAccessEntityQuery>() ||
         query->as<ASTGrantQuery>();
}

static bool isNamedCollectionQuery(const ASTPtr &query) {
  return query->as<ASTCreateNamedCollectionQuery>() || query->as<ASTDropNamedCollectionQuery>() ||
...
ASTPtr removeOnClusterClauseIfNeeded(const ASTPtr &query, ContextPtr context, const WithoutOnClusterASTRewriteParams &params) {
  ...
  if (!query_on_cluster || query_on_cluster->cluster.empty()) return query;

  if ((isUserDefinedFunctionQuery(query) && context->getSettingsRef()[Setting::ignore_on_cluster_for_replicated_udf_queries] &&
       context->getUserDefinedSQLObjectsStorage().isReplicated()) ||
      (isAccessControlQuery(query) && context->getSettingsRef()[Setting::ignore_on_cluster_for_replicated_access_entities_queries] &&
       context->getAccessControl().containsStorage(ReplicatedAccessStorage::STORAGE_TYPE)) ||
      (isNamedCollectionQuery(query) && context->getSettingsRef()[Setting::ignore_on_cluster_for_replicated_named_collections_queries] &&
