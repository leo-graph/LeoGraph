...
std::optional<EvaluateConstantExpressionResult> evaluateConstantExpressionImpl(const ASTPtr &node, const ContextPtr &context,
                                                                               bool no_throw) {
  if (ASTLiteral *literal = node->as<ASTLiteral>()) return getFieldAndDataTypeFromLiteral(literal);
  ...
  if (context->getSettingsRef()[Setting::allow_experimental_analyzer]) {
    ...
    auto [actions, correlated_subtrees] =
        buildActionsDAGFromExpressionNode(expression,
                                          /*input_columns=*/{}, planner_context, empty_correlated_columns_set);
    ...
    if (actions.getOutputs().size() != 1) {
      ...
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Constant expression returns more than 1 column: {}",
                      ast->formatForLogging());
    }
    ...
    if (output->column) {
      ...
      if (result_column->empty() && isColumnConst(*result_column)) result_column = result_column->cloneResized(1);
    }
  } else {
    ...
    if (ASTLiteral *literal = ast->as<ASTLiteral>()) return getFieldAndDataTypeFromLiteral(literal);
  ...
  if (!result_column)
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    ...
                    "is not a constant expression (result column not found): {}",
                    result_name);
  ...
  if (!isColumnConst(*result_column))
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    "Element of set in IN, VALUES, or LIMIT, or aggregate function parameter, or a table function argument "
                    "is not a constant expression (result column is not const): {}",
                    result_name);
...
EvaluateConstantExpressionResult evaluateConstantExpression(const ASTPtr &node, const ContextPtr &context) {
  ...
  if (!res) throw Exception(ErrorCodes::LOGICAL_ERROR, "evaluateConstantExpression expected to return a result or throw an exception");
  return *res;
}

ASTPtr evaluateConstantExpressionAsLiteral(const ASTPtr &node, const ContextPtr &context) {
  /// If it's already a literal.
  if (node->as<ASTLiteral>()) return node;
...
ASTPtr evaluateConstantExpressionOrIdentifierAsLiteral(const ASTPtr &node, const ContextPtr &context) {
  if (const auto *id = node->as<ASTIdentifier>()) return make_intrusive<ASTLiteral>(id->name());
...
std::optional<ConjunctionMap> andConjunctions(const ConjunctionMap &lhs, const ConjunctionMap &rhs) {
  ...
  for (const auto &[node, column] : rhs) {
    ...
    if (it != lhs.end() && column.column->compareAt(0, 0, *it->second.column, 1)) return {};

    if (it == lhs.end()) res.emplace(node, column);
  }
...
DisjunctionList andDisjunctions(const DisjunctionList &lhs, const DisjunctionList &rhs) {
  ...
  for (const auto &lhs_map : lhs)
    for (const auto &rhs_map : rhs)
      if (auto conj = andConjunctions(lhs_map, rhs_map)) res.emplace_back(std::move(*conj));
...
const ActionsDAG::Node *findMatch(const ActionsDAG::Node *key, const MatchedTrees::Matches &matches) {
  ...
  if (!match.node || match.monotonicity) return nullptr;
...
ColumnPtr tryCastColumn(ColumnPtr col, const DataTypePtr &from_type, const DataTypePtr &to_type) {
  ...
  if (!to_type_no_lc->canBeInsideNullable()) return {};

  auto res = castColumnAccurateOrNull({col, from_type, std::string()}, makeNullable(to_type_no_lc));
  if (res->onlyNull()) return nullptr;

  if (!typeid_cast<const ColumnNullable *>(res.get())) return nullptr;

  return res;
}

std::optional<ConjunctionMap::value_type> analyzeConstant(const ActionsDAG::Node *key, const ActionsDAG::Node *value,
                                                          const MatchedTrees::Matches &matches) {
  if (value->type != ActionsDAG::ActionType::COLUMN) return {};
  ...
  if (const auto *col = typeid_cast<const ColumnConst *>(value->column.get())) {
    if (const auto *node = findMatch(key, matches)) {
      ...
      if (!value->result_type->equals(*node->result_type)) {
        ...
        if (!inner || inner->isNullAt(0)) return {};
...
std::optional<DisjunctionList> analyzeSet(const ActionsDAG::Node *key, const ActionsDAG::Node *value, const MatchedTrees::Matches &matches,
                                          const ContextPtr &context, size_t max_elements) {
  if (value->type != ActionsDAG::ActionType::COLUMN) return {};

  auto col = value->column;
  if (const auto *col_const = typeid_cast<const ColumnConst *>(col.get())) col = col_const->getDataColumnPtr();

  const auto *col_set = typeid_cast<const ColumnSet *>(col.get());
  if (!col_set || !col_set->getData()) return {};
  ...
  if (column->size() > max_elements) return {};
...
std::optional<DisjunctionList> analyze(const ActionsDAG::Node *node, const MatchedTrees::Matches &matches, const ContextPtr &context,
  ...
  if (node->type == ActionsDAG::ActionType::FUNCTION) {
    if (node->function_base->getName() == "equals") {
      ...
      if (auto val = analyzeConstant(lhs_node, rhs_node, matches)) return DisjunctionList{ConjunctionMap{std::move(*val)}};

      if (auto val = analyzeConstant(rhs_node, lhs_node, matches)) return DisjunctionList{ConjunctionMap{std::move(*val)}};
      ...
      for (const auto *child : node->children) {
        ...
        if (val->size() + res.size() > max_elements) return {};
      ...
      std::sort(lists.begin(), lists.end(), [](const auto &lhs, const auto &rhs) { return lhs.size() < rhs.size(); });
      ...
      for (auto &list : lists) {
        ...
        if (res.size() * list.size() > max_elements) break;
    ...
    if (isColumnConst(*node->column) && node->result_type->canBeUsedInBooleanContext()) {
      if (!node->column->getBool(0)) return DisjunctionList{};
    }
  }
...
std::optional<ColumnsWithTypeAndName> evaluateConjunction(const ActionsDAG::NodeRawConstPtrs &target_expr, ConjunctionMap &&conjunction) {
...
std::optional<ConstantVariants> evaluateExpressionOverConstantCondition(const ActionsDAG::Node *predicate,
                                                                        const ActionsDAG::NodeRawConstPtrs &expr, const ContextPtr &context,
                                                                        size_t max_elements) {
  if (!predicate) return {};
...
std::optional<Blocks> evaluateExpressionOverConstantCondition(const ASTPtr &node, const ExpressionActionsPtr &target_expr, size_t &limit) {
  ...
  if (const auto *fn = node->as<ASTFunction>()) {
      ...
      for (const auto &elem : conjunct) {
        if (!block.has(elem.name)) {
          ...
          if (isColumnConst(*prev.column) && isColumnConst(*elem.column)) {
            ...
            if (always_false) break;
          }
        }
      }

      if (always_false) continue;
    ...
    if (literal->value.getType() == Field::Types::UInt64 && literal->value.safeGet<UInt64>() == 0) return {result};
    return {};
  }
