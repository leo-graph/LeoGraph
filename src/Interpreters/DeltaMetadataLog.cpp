...
ColumnsDescription DeltaMetadataLogElement::getColumnsDescription() {
  return ColumnsDescription{{"event_date", std::make_shared<DataTypeDate>(), "Date of the entry."},
...
void insertDeltaRowToLogTable(const ContextPtr& local_context, String row, const String& table_path, const String& file_path) {
  if (!local_context->getSettingsRef()[Setting::delta_lake_log_metadata].value) return;
  ...
  if (!delta_lake_metadata_log) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Delta metadata log table is not configured");
  delta_lake_metadata_log->add(DB::DeltaMetadataLogElement{.current_time = spec.tv_sec,
