...
using HandleExceptionInOutputFormatFunc =
    std::function<void(IOutputFormat &output_format, const String &format_name, const ContextPtr &context,
                       const std::optional<FormatSettings> &format_settings)>;
using QueryFinishCallback = std::function<void()>;
using HTTPContinueCallback = std::function<void()>;

/// Parse and execute a query.
void executeQuery(ReadBufferUniquePtr istr,                 /// Where to read query from (and data for INSERT, if present).
                  ...
                  SetResultDetailsFunc set_result_details,  /// If a non-empty callback is passed, it will be called with the query id, the
                                                            /// content-type, the format, and the timezone, as well as additional headers.
                  QueryFlags flags = {},
                  ...
                  QueryFinishCallback query_finish_callback =
                      {},  /// Use it to do everything you need to before the QueryFinish entry will be dumped to query_log
                           /// NOTE: It will not be called in case of exception (i.e. ExceptionWhileProcessing)
                  HTTPContinueCallback http_continue_callback = {}
                  /// If a non-empty callback is passed, it will be called after quota checks to send HTTP 100 Continue.
);

void executeQuery(ReadBuffer &istr,                         /// Where to read query from (and data for INSERT, if present).
                  WriteBuffer &ostr,                        /// Where to write query output to.
                  ContextMutablePtr context,                /// DB, tables, data types, storage engines, functions, aggregate functions...
                  SetResultDetailsFunc set_result_details,  /// If a non-empty callback is passed, it will be called with the query id, the
                                                            /// content-type, the format, and the timezone, as well as additional headers.
