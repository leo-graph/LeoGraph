...
bool less(const Field& lhs, const Field& rhs, int direction) {
  if (direction == -1) return accurateLess(rhs, lhs);
...
bool equals(const Field& lhs, const Field& rhs) {
  ...
  if (lhs.getType() == rhs.getType()) return lhs == rhs;

  return accurateEquals(lhs, rhs);
}

FillingRow::FillingRow(const SortDescription& sort_description_) : sort_description(sort_description_) {
  row.resize(sort_description.size());

  constraints.reserve(sort_description.size());
  for (size_t i = 0; i < size(); ++i) constraints.push_back(getFillDescription(i).fill_to);
}

bool FillingRow::operator<(const FillingRow& other) const {
  for (size_t i = 0; i < sort_description.size(); ++i) {
    if ((*this)[i].isNull() || other.row[i].isNull() || equals(row[i], other.row[i])) continue;
...
bool FillingRow::operator==(const FillingRow& other) const {
  for (size_t i = 0; i < sort_description.size(); ++i)
    if (!equals(row[i], other.row[i])) return false;
  return true;
}

bool FillingRow::operator>=(const FillingRow& other) const { return !(*this < other); }
...
Field FillingRow::doLongJump(const FillColumnDescription& descr, size_t column_ind, const Field& to) {
  ...
  for (; step_no < 500 && step_len > 0; ++step_no) {
    ...
    logDebug("doLongJump: shifted_value: {}, next_value: {}, to: {}, step_no: {}, step_len: {}", shifted_value, next_value, to, step_no,
             step_len);
...
bool FillingRow::hasSomeConstraints(size_t pos) const { return !constraints[pos].isNull(); }
...
static const Field& findBorder(const Field& constraint, const Field& next_original, int direction) {
  if (constraint.isNull()) return next_original;  /// NOLINT(bugprone-return-const-ref-from-parameter)

  if (next_original.isNull()) return constraint;  /// NOLINT(bugprone-return-const-ref-from-parameter)

  if (less(constraint, next_original, direction)) return constraint;  /// NOLINT(bugprone-return-const-ref-from-parameter)
...
bool FillingRow::next(const FillingRow& next_original_row, bool& value_changed) {
  ...
  for (; pos < row_size; ++pos) {
    if (row[pos].isNull()) continue;

    const Field& border = findBorder(constraints[pos], next_original_row[pos], getDirection(pos));
    logDebug("border: {}", border);

    if (!border.isNull() && !equals(row[pos], border)) break;
  }

  logDebug("pos: {}", pos);

  if (pos == row_size) return false;

  if (next_original_row[pos].isNaN() || next_original_row[pos].isInf()) return false;

  if (!next_original_row[pos].isNull() && less(next_original_row[pos], row[pos], getDirection(pos))) return false;

  if (!constraints[pos].isNull() && !less(row[pos], constraints[pos], getDirection(pos))) return false;
  ...
  for (size_t i = row_size - 1; i > pos; --i) {
    ...
    if (constraints[i].isNull()) continue;

    Field next_value = row[i];
    fill_column_desc.step_func(next_value, 1);

    if (!less(next_value, constraints[i], getDirection(i))) continue;
  ...
  if (!next_original_row[pos].isNull() && less(next_original_row[pos], next_value, getDirection(pos))) return false;

  if (!constraints[pos].isNull() && !less(next_value, constraints[pos], getDirection(pos))) return false;
  ...
  if (equals(row[pos], next_original_row[pos])) {
    ...
    for (size_t i = pos + 1; i < row_size; ++i) {
      ...
      is_less |= ((next_original_row[i].isNull() || less(row[i], next_original_row[i], getDirection(i))) &&
...
bool FillingRow::shift(const FillingRow& next_original_row, bool& value_changed) {
  ...
  for (size_t pos = 0; pos < size(); ++pos) {
    if (row[pos].isNull() || next_original_row[pos].isNull() || equals(row[pos], next_original_row[pos])) continue;

    if (less(next_original_row[pos], row[pos], getDirection(pos))) return false;
...
bool FillingRow::hasSomeConstraints() const {
  for (size_t pos = 0; pos < size(); ++pos)
    if (hasSomeConstraints(pos)) return true;
...
bool FillingRow::isConstraintsSatisfied() const {
  for (size_t pos = 0; pos < size(); ++pos) {
    if (row[pos].isNull() || !hasSomeConstraints(pos)) continue;
...
void FillingRow::initUsingFrom(size_t from_pos) {
  for (size_t i = from_pos; i < sort_description.size(); ++i) row[i] = getFillDescription(i).fill_from;
}

void FillingRow::initUsingTo(size_t from_pos) {
  for (size_t i = from_pos; i < sort_description.size(); ++i) row[i] = getFillDescription(i).fill_to;
}
...
struct fmt::formatter<DB::FillingRow> : fmt::formatter<string_view> {
  constexpr auto format(const DB::FillingRow& row, format_context& ctx) const { return fmt::format_to(ctx.out(), "{}", row.dump()); }
};
