...
#define LIST_OF_ALL_SYSTEM_LOGS(M)                                                                                                         \
  ...
  M(QueryThreadLog, query_thread_log,                                                                                                      \
  ...
  M(PartLog, part_log,                                                                                                                     \
    "This table contains information about events that occurred with data parts in the MergeTree family tables, such as adding or "        \
    "merging data.")                                                                                                                       \
  ...
  M(CrashLog, crash_log,                                                                                                                   \
    "Contains information about stack traces for fatal errors. The table does not exist in the database by default, it is created only "   \
    "when fatal errors occur.")                                                                                                            \
  M(TextLog, text_log, "Contains logging entries which are normally written to a log file or to stdout.")                                  \
  M(MetricLog, metric_log,                                                                                                                 \
    "Contains history of metrics values from tables system.metrics and system.events, periodically flushed to disk.")                      \
  M(TransposedMetricLog, transposed_metric_log,                                                                                            \
    "Contains history of metrics values from tables system.metrics and system.events. Periodically flushed to disk. Transposed form of "   \
    "system.metric_log.")                                                                                                                  \
  ...
  M(AsynchronousMetricLog, asynchronous_metric_log,                                                                                        \
    "Contains the historical values for system.asynchronous_metrics, once per time interval (one second by default).")                     \
  M(OpenTelemetrySpanLog, opentelemetry_span_log, "Contains information about trace spans for executed queries.")                          \
  M(QueryViewsLog, query_views_log,                                                                                                        \
  ...
  M(ProcessorsProfileLog, processors_profile_log,                                                                                          \
  ...
  M(DeadLetterQueue, dead_letter_queue,                                                                                                    \
    "Contains messages that came from a streaming engine (e.g. Kafka) and were parsed unsuccessfully.")                                    \
  M(ZooKeeperConnectionLog, zookeeper_connection_log, "Contains history of ZooKeeper connections.")                                        \
  M(AggregatedZooKeeperLog, aggregated_zookeeper_log,                                                                                      \
    "Contains statistics (number of operations, latencies, errors) of ZooKeeper operations grouped by session_id, parent_path and "        \
    "operation. Periodically flushed to disk.")                                                                                            \
...
class StorageWithComment : public IAST {
 public:
  ...
  ASTPtr clone() const override { throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method clone is not supported"); }
...
class ParserStorageWithComment : public IParserBase {
 protected:
  ...
  bool parseImpl(Pos &pos, ASTPtr &node, Expected &expected) override {
    ...
    if (!storage_p.parse(pos, storage, expected)) return false;

    ParserKeyword s_comment(Keyword::COMMENT);
    ParserStringLiteral string_literal_parser;
    ASTPtr comment;

    if (s_comment.ignore(pos, expected)) string_literal_parser.parse(pos, comment, expected);
...
#define FORWARD_DECLARATION(log_type, member, descr) class log_type;
...
#define DECLARE_PUBLIC_MEMBERS(log_type, member, descr) std::shared_ptr<log_type> member;
...
class SystemLog : public SystemLogBase<LogElement>, private boost::noncopyable, public WithContext {
 public:
   ...
   */
  SystemLog(ContextPtr context_, const SystemLogSettings &settings_, std::shared_ptr<SystemLogQueue<LogElement>> queue_ = nullptr);
