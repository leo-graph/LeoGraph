...
ASTPtr createIdentifierFromColumnName(const String &column_name) {
  ...
  if (!res || getIdentifierName(res) != column_name) return make_intrusive<ASTIdentifier>(column_name);
  return res;
}
...
ASTPtr normalizeAndValidateQuery(const ASTPtr &query, const Names &column_names) {
  ...
  if (query->as<ASTSelectWithUnionQuery>() || query->as<ASTSelectQuery>())
    ...
    throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Expected ASTSelectWithUnionQuery, ASTSelectQuery or ASTSubquery. Actual {} ({})",
                    query->formatForErrorMessage(), query->getID());

  if (column_names.empty()) return result_query;
...
ContextMutablePtr buildContext(const ContextPtr &context, const SelectQueryOptions &select_query_options) {
  ...
  if (select_query_options.shard_num)
    result_context->addSpecialScalar("_shard_num", Block{{DataTypeUInt32().createColumnConst(1, *select_query_options.shard_num),
                                                          std::make_shared<DataTypeUInt32>(), "_shard_num"}});
  if (select_query_options.shard_count)
    result_context->addSpecialScalar("_shard_count", Block{{DataTypeUInt32().createColumnConst(1, *select_query_options.shard_count),
...
QueryPlanPtr buildQueryPlanForAutomaticParallelReplicas(const ASTPtr &ast, const ContextMutablePtr &ctx,
                                                        const SelectQueryOptions &select_options, Args &&...interpreter_args) {
  ...
  /// If the query is executed by remote*/cluster* function, the following attempt to build a plan with parallel replicas may result in
  /// exceptions
  if (ctx->getClientInfo().query_kind == ClientInfo::QueryKind::SECONDARY_QUERY) return QueryPlanPtr{};
  ...
  // We should build sets and create `CreatingSetsStep` only in the original plan. The automatic parallel replicas optimization happens
  // before building sets, so even if we decide to use the plan with parallel replicas, we will substitute it in place of the original plan
  // and then build sets.
...
void replaceStorageInQueryTree(QueryTreeNodePtr &query_tree, const ContextPtr &context, const StoragePtr &storage) {
  ...
  for (auto &node : nodes) {
    ...
    if (table_node.getStorageID().getFullNameNotQuoted() != storage->getStorageID().getFullNameNotQuoted()) continue;
...
static QueryTreeNodePtr buildQueryTreeAndRunPasses(const ASTPtr &query, const SelectQueryOptions &select_query_options,
  ...
  if (storage) replaceStorageInQueryTree(query_tree, context, storage);

  return query_tree;
}

InterpreterSelectQueryAnalyzer::InterpreterSelectQueryAnalyzer(const ASTPtr &query_, const ContextPtr &context_,
                                                               const SelectQueryOptions &select_query_options_, const Names &column_names,
...
InterpreterSelectQueryAnalyzer::InterpreterSelectQueryAnalyzer(const ASTPtr &query_, const ContextPtr &context_,
                                                               const SelectQueryOptions &select_query_options_, const StoragePtr &storage_,
    ...
    : query(normalizeAndValidateQuery(query_, column_names)),
      ...
      query_plan_with_parallel_replicas_builder(
          [ast = query_->clone(), ctx = Context::createCopy(context_), storage = storage_, select_options = select_query_options_,
           column_names]() { return buildQueryPlanForAutomaticParallelReplicas(ast, ctx, select_options, storage, column_names); }) {}

InterpreterSelectQueryAnalyzer::InterpreterSelectQueryAnalyzer(const QueryTreeNodePtr &query_tree_, const ContextPtr &context_,
...
SharedHeader InterpreterSelectQueryAnalyzer::getSampleBlock(const ASTPtr &query, const ContextPtr &context,
...
std::pair<SharedHeader, PlannerContextPtr> InterpreterSelectQueryAnalyzer::getSampleBlockAndPlannerContext(
    const QueryTreeNodePtr &query_tree, const ContextPtr &context, const SelectQueryOptions &select_query_options) {
  auto select_query_options_copy = select_query_options;
  select_query_options_copy.only_analyze = true;
...
SharedHeader InterpreterSelectQueryAnalyzer::getSampleBlock(const QueryTreeNodePtr &query_tree, const ContextPtr &context,
                                                            const SelectQueryOptions &select_query_options) {
...
void InterpreterSelectQueryAnalyzer::addStorageLimits(const StorageLimitsList &storage_limits) { planner.addStorageLimits(storage_limits); }
...
void InterpreterSelectQueryAnalyzer::extendQueryLogElemImpl(QueryLogElement &elem, const ASTPtr & /*ast*/, ContextPtr /*context*/) const {
  for (const auto &used_row_policy : planner.getUsedRowPolicies()) elem.used_row_policies.emplace(used_row_policy);
}
