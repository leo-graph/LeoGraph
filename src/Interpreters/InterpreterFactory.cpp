...
InterpreterFactory::InterpreterPtr InterpreterFactory::get(ASTPtr& query, ContextMutablePtr context, const SelectQueryOptions& options) {
  ...
  if (query->as<ASTSelectQuery>()) {
    if (context->getSettingsRef()[Setting::allow_experimental_analyzer]) interpreter_name = "InterpreterSelectQueryAnalyzer";
  ...
  } else if (query->as<ASTExistsDatabaseQuery>() || query->as<ASTExistsTableQuery>() || query->as<ASTExistsViewQuery>() ||
             query->as<ASTExistsDictionaryQuery>()) {
    interpreter_name = "InterpreterExistsQuery";
  } else if (query->as<ASTShowCreateTableQuery>() || query->as<ASTShowCreateViewQuery>() || query->as<ASTShowCreateDatabaseQuery>() ||
    ...
    if (kind == ASTExplainQuery::ParsedAST) context->setSetting("allow_experimental_analyzer", false);
