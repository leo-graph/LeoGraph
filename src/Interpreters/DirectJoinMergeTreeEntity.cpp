...
DirectJoinMergeTreeEntity::DirectJoinMergeTreeEntity(QueryPlan&& lookup_plan_, ActionsDAG filter_dag_, ContextPtr context_)
...
Names DirectJoinMergeTreeEntity::getPrimaryKey() const { return filter_dag.getNames(); }
...
Block DirectJoinMergeTreeEntity::getSampleBlock(const Names& required_columns) const {
  ...
  if (required_columns.empty()) return *sample_block;

  Block result_block;
  for (const auto& column_name : required_columns) result_block.insert(sample_block->getByName(column_name));

  return result_block;
}

static std::unique_ptr<FilterStep> buildFilterStepWithIn(const ColumnWithTypeAndName& key_column, ActionsDAG filter_dag,
                                                         const SharedHeader& source_header) {
  if (filter_dag.getOutputs().size() != 1)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Filter DAG for DirectJoinMergeTreeEntity must have single output, got DAG: {}",
                    filter_dag.dumpDAG());
  ...
  auto future_set =
      std::make_shared<FutureSetFromTuple>(CityHash_v1_0_2::uint128{}, ASTPtr{}, ColumnsWithTypeAndName{key_column}, false, SizeLimits{});
  ...
  auto filter_step = std::make_unique<FilterStep>(source_header, std::move(actions_for_filter->dag), filter_column_name,
                                                  actions_for_filter->remove_filter);
...
Chunk DirectJoinMergeTreeEntity::executePlan(QueryPlan& plan) const {
  ...
  pipeline.setProgressCallback(
      [](const auto& progress) { ProfileEvents::increment(ProfileEvents::JoinBuildTableRowCount, progress.read_rows); });
...
Chunk DirectJoinMergeTreeEntity::getByKeys(const ColumnsWithTypeAndName& keys, const Names& required_columns,
                                           PaddedPODArray<UInt8>& out_null_map, IColumn::Offsets& out_offsets) const {
  if (keys.size() != 1)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "DirectJoinMergeTreeEntity only supports single-column keys, got: [{}]",
                    Block(keys).dumpStructure());

  if (!lookup_plan.isInitialized()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Query plan for direct join is not initialized");

  auto sample_block = getSampleBlock(required_columns);

  const size_t num_keys = keys[0].column->size();
  if (num_keys == 0) return Chunk(sample_block.cloneEmptyColumns(), 0);

  if (filter_dag.getOutputs().size() != 1) throw Exception(ErrorCodes::LOGICAL_ERROR, "Filter DAG must have single output");
  ...
  if (!found_chunk) {
    ...
    for (auto& col : result_columns) col->insertManyDefaults(num_keys);
  ...
  if (func_cast) {
    found_key_column = func_cast->execute(ColumnsWithTypeAndName{{found_key_column, header_key_column.type, ""}}, keys[0].type,
                                          found_key_column->size(), false);
  }
  ...
  for (const auto& col : found_columns) col->insertDefault();

  MutableColumns result_columns;
  for (auto&& col : found_columns) result_columns.push_back(IColumn::mutate(col->index(*selector, 0)));
