...
IInterpreterUnionOrSelectQuery::IInterpreterUnionOrSelectQuery(const ASTPtr& query_ptr_, const ContextPtr& context_,
                                                               const SelectQueryOptions& options_)
    : IInterpreterUnionOrSelectQuery(query_ptr_, Context::createCopy(context_), options_) {}

IInterpreterUnionOrSelectQuery::IInterpreterUnionOrSelectQuery(const ASTPtr& query_ptr_, const ContextMutablePtr& context_,
                                                               const SelectQueryOptions& options_)
  ...
  if (context->getSettingsRef()[Setting::allow_experimental_analyzer]) {
    LOG_TRACE(getLogger("IInterpreterUnionOrSelectQuery"),
              "The analyzer is enabled, but the old interpreter is used. It can be a bug, please report it. Will disable "
              "'allow_experimental_analyzer' setting (for query: {})",
  ...
  if (options.shard_num)
    context->addSpecialScalar(
        "_shard_num", Block{{DataTypeUInt32().createColumnConst(1, *options.shard_num), std::make_shared<DataTypeUInt32>(), "_shard_num"}});
  if (options.shard_count)
    context->addSpecialScalar("_shard_count", Block{{DataTypeUInt32().createColumnConst(1, *options.shard_count),
...
void IInterpreterUnionOrSelectQuery::setQuota(QueryPipeline& pipeline) const {
  ...
  if (!options.ignore_quota && (options.to_stage == QueryProcessingStage::Complete)) quota = context->getQuota();
...
static ActionsDAG makeAdditionalPostFilter(ASTPtr& ast, ContextPtr context, const Block& header) {
  ...
  for (const auto* node : dag.getInputs()) outputs.push_back(node);
...
void IInterpreterUnionOrSelectQuery::addAdditionalPostFilter(QueryPlan& plan) const {
  if (options.subquery_depth != 0) return;
  ...
  auto filter_step = std::make_unique<FilterStep>(plan.getCurrentHeader(), std::move(dag), std::move(filter_name), true);
...
void IInterpreterUnionOrSelectQuery::addStorageLimits(const StorageLimitsList& limits) {
  for (const auto& val : limits) storage_limits.push_back(val);
}
