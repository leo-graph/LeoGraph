...
BlockIO InterpreterCreateWorkloadQuery::execute() {
  ...
  if (create_workload_query.or_replace) access_rights_elements.emplace_back(AccessType::DROP_WORKLOAD);
  ...
  current_context->getWorkloadEntityStorage().storeEntity(current_context, WorkloadEntityType::Workload, workload_name, query_ptr,
                                                          throw_if_exists, replace_if_exists, current_context->getSettingsRef());
