...
class PasteJoin : public IJoin {
 public:
  explicit PasteJoin(std::shared_ptr<TableJoin> table_join_, SharedHeader& right_sample_block_)
      : table_join(table_join_), right_sample_header(right_sample_block_) {
  ...
  void checkTypesOfKeys(const Block& /*left_block*/) const override {
    if (!isSupported(table_join)) throw DB::Exception(ErrorCodes::NOT_IMPLEMENTED, "PasteJoin doesn't support specified query");
  }

  /// Used just to get result header
  JoinResultPtr joinBlock(Block block) override {
    for (const auto& col : *right_sample_header) block.insert(col);
