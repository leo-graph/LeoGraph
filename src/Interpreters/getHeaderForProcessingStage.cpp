...
bool removeJoin(ASTSelectQuery &select, TreeRewriterResult &rewriter_result, ContextPtr context) {
  if (!hasJoin(select)) return false;
  ...
  auto replace_where = [&membership_collector](ASTSelectQuery &query, ASTSelectQuery::Expression expr) {
    ...
    for (auto &&node : splitConjunctionsAst(where)) {
      if (membership_collector.getIdentsMembership(node) == left_table_pos) new_conj->arguments->children.push_back(std::move(node));
    }
...
SharedHeader getHeaderForProcessingStage(const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
                                         const SelectQueryInfo &query_info, ContextPtr context,
    ...
    case QueryProcessingStage::MAX: {
      ...
      if (const auto *select = query_info.query->as<ASTSelectQuery>(); select && hasJoin(*select)) {
        if (!query_info.syntax_analyzer_result) {
          if (!query_info.planner_context) throw Exception(ErrorCodes::LOGICAL_ERROR, "Query is not analyzed: no planner context");
      ...
      if (context->getSettingsRef()[Setting::allow_experimental_analyzer]) {
        auto storage = std::make_shared<StorageDummy>(storage_snapshot->storage.getStorageID(),
                                                      storage_snapshot->getAllColumnsDescription(), storage_snapshot);
