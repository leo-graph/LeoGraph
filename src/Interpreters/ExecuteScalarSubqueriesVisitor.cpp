...
bool ExecuteScalarSubqueriesMatcher::needChildVisit(ASTPtr &node, const ASTPtr &child) {
  ...
  if (auto *tables = node->as<ASTTablesInSelectQueryElement>()) {
    ...
    if (child == tables->array_join) return true;
    return false;
  }
...
void ExecuteScalarSubqueriesMatcher::visit(ASTPtr &ast, Data &data) {
  if (const auto *t = ast->as<ASTSubquery>()) visit(*t, ast, data);
...
static auto getQueryInterpreter(const ASTSubquery &subquery, ExecuteScalarSubqueriesMatcher::Data &data) {
  ...
  if (subquery_context->hasQueryContext()) {
    ...
    if (!data.only_analyze) {
      ...
      for (const auto &it : data.scalars) context->addScalar(it.first, it.second);
    }
  }
...
void ExecuteScalarSubqueriesMatcher::visit(const ASTSubquery &subquery, ASTPtr &ast, Data &data) {
  ...
  if (!hit) {
    if (!interpreter) interpreter = getQueryInterpreter(subquery, data);
    ...
    if (data.only_analyze) {
      ...
      if (block.rows() == 0) {
        ...
        if (types.size() != 1) types = {std::make_shared<DataTypeTuple>(types)};

        auto &type = types[0];
        if (!type->isNullable()) {
          if (!type->canBeInsideNullable())
            throw Exception(ErrorCodes::INCORRECT_RESULT_OF_SCALAR_SUBQUERY,
                            "Scalar subquery returned empty result of type {} which cannot be Nullable", type->getName());
        ...
        block = Block({ColumnWithTypeAndName(type->createColumnConstWithDefaultValue(1)->convertToFullColumnIfConst(), type, column_name)});
      }

      if (block.rows() != 1) throw Exception(ErrorCodes::INCORRECT_RESULT_OF_SCALAR_SUBQUERY, "Scalar subquery returned more than one row");
    ...
    if (columns == 1) {
      ...
      scalar.insert({ColumnTuple::create(block.getColumns()), std::make_shared<DataTypeTuple>(block.getDataTypes()), "tuple"});
    }
  }
  ...
  if (data.only_analyze || !settings[Setting::enable_scalar_subquery_optimization] ||
      worthConvertingScalarToLiteral(scalar, data.max_literal_size) || !data.getContext()->hasQueryContext()) {
  ...
  if (checkFunctionIsInOrGlobalInOperator(func)) {
    for (auto &child : ast->children) {
      if (child != func.arguments)
        ...
        for (size_t i = 0, size = func.arguments->children.size(); i < size; ++i)
          if (i != 1 || !func.arguments->children[i]->as<ASTSubquery>()) out.push_back(&func.arguments->children[i]);
    }
    ...
    for (auto &child : ast->children) {
      if (child != func.arguments)
        ...
        for (size_t i = 0, size = func.arguments->children.size(); i < size; ++i)
          if (i != 0 || !func.arguments->children[i]->as<ASTSubquery>()) out.push_back(&func.arguments->children[i]);
    }
  } else
    for (auto &child : ast->children) out.push_back(&child);

  for (ASTPtr *add_node : out) Visitor(data).visit(*add_node);
}
...
bool worthConvertingScalarToLiteral(const Block &scalar, std::optional<size_t> max_literal_size) {
  ...
  static const std::set<std::string_view> maybe_large_literal_types = {"Array",    "Tuple", "AggregateFunction",
                                                                       "Function", "Set",   "LowCardinality"};

  if (!maybe_large_literal_types.contains(scalar_type_name)) return true;

  if (!max_literal_size) return false;

  /// Size of serialized literal cannot be less than size in bytes.
  if (scalar.bytes() > *max_literal_size) return false;
