...
class ActionsDAG {
 ...
 public:
  ...
  const Node &addFunction(const FunctionOverloadResolverPtr &function, NodeRawConstPtrs children, std::string result_name);
  const Node &addFunction(const FunctionNode &function, NodeRawConstPtrs children, std::string result_name);
  const Node &addFunction(const FunctionBasePtr &function_base, NodeRawConstPtrs children, std::string result_name);
  ...
  NameSet foldActionsByProjection(const NameSet &required_columns, const Block &projection_block_for_keys,
                                  const String &predicate_column_name = {}, bool add_missing_keys = true);
  ...
  ///            \      /
  ///            c * d - e
  static ActionsDAG foldActionsByProjection(const std::unordered_map<const Node *, const Node *> &new_inputs,
  ...
  static ColumnsWithTypeAndName evaluatePartialResult(IntermediateExecutionResult &node_to_column, const NodeRawConstPtrs &outputs,
  ...
  static ActionsDAG makeConvertingActions(const ColumnsWithTypeAndName &source, const ColumnsWithTypeAndName &result, MatchColumnsMode mode,
                                          ContextPtr context, bool ignore_constant_values = false, bool add_cast_columns = false,
                                          NameToNameMap *new_names = nullptr, NameSet *columns_contain_compiled_function = nullptr);
  ...
  SplitResult split(std::unordered_set<const Node *> split_nodes, bool create_split_nodes_mapping = false,
                    bool avoid_duplicate_inputs = false) const;
  ...
  std::optional<ActionsForFilterPushDown> splitActionsForFilterPushDown(const std::string &filter_name, bool removes_filter,
  ...
  bool isSortingPreserved(const Block &input_header, const SortDescription &sort_description,
  ...
  static NodeRawConstPtrs filterNodesByAllowedInputs(NodeRawConstPtrs nodes, const std::unordered_set<const Node *> &allowed_inputs);
   ...
   */
  static std::optional<ActionsForFilterPushDown> createActionsForConjunction(NodeRawConstPtrs conjunction,
                                                                             const ColumnsWithTypeAndName &all_inputs);
 ...
 private:
  ...
  const Node &addFunctionImpl(const FunctionBasePtr &function_base, NodeRawConstPtrs children, ColumnsWithTypeAndName arguments,
                              std::string result_name, DataTypePtr result_type, bool all_const);
