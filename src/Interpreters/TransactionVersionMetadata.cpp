...
TransactionID VersionMetadata::getRemovalTID() const {
  ...
  if (removal_lock) {
    if (removal_lock == Tx::PrehistoricTID.getHash()) return Tx::PrehistoricTID;
    if (auto txn = TransactionLog::instance().tryGetRunningTransaction(removal_lock)) return txn->tid;
  }
...
void VersionMetadata::lockRemovalTID(const TransactionID& tid, const TransactionInfoContext& context) {
  ...
  TIDHash locked_by = 0;
  if (tryLockRemovalTID(tid, context, &locked_by)) return;
...
bool VersionMetadata::tryLockRemovalTID(const TransactionID& tid, const TransactionInfoContext& context, TIDHash* locked_by_id) {
  ...
  if (!locked) {
    ...
    if (locked_by_id) *locked_by_id = expected_removal_lock_value;
    return false;
  }
...
void VersionMetadata::unlockRemovalTID(const TransactionID& tid, const TransactionInfoContext& context) {
  ...
  auto throw_cannot_unlock = [&]() {
    ...
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot unlock removal_tid, it's a bug. Current: {} {}, actual: {} {}", removal_lock_value,
                    tid, locked_by, locked_by_txn ? locked_by_txn->tid : Tx::EmptyTID);
  };

  if (locked_by != removal_lock_value) throw_cannot_unlock();

  removal_tid = Tx::EmptyTID;
  bool unlocked = removal_tid_lock.compare_exchange_strong(locked_by, 0);
  if (!unlocked) throw_cannot_unlock();

  tryWriteEventToSystemLog(log, TransactionsInfoLogElement::UNLOCK_PART, tid, context);
}

bool VersionMetadata::isRemovalTIDLocked() const { return removal_tid_lock.load() != 0; }
...
void VersionMetadata::setCreationTID(const TransactionID& tid, TransactionInfoContext* context) {
  ...
  creation_tid = tid;
  if (context) tryWriteEventToSystemLog(log, TransactionsInfoLogElement::ADD_PART, tid, *context);
}

bool VersionMetadata::isVisible(const MergeTreeTransaction& txn) { return isVisible(txn.getSnapshot(), txn.tid); }
...
bool VersionMetadata::isVisible(CSN snapshot_version, TransactionID current_tid) {
  ...
  if (unlikely(snapshot_version == Tx::EverythingVisibleCSN)) return true;
  ...
  if (creation && snapshot_version < creation) return false;
  if (removal && removal <= snapshot_version) return false;
  if (!current_tid.isEmpty() && removal_lock && removal_lock == current_tid.getHash()) return false;
  ...
  if (creation && creation <= snapshot_version && !removal_lock) return true;
  if (creation && creation <= snapshot_version && removal && snapshot_version < removal) return true;
  if (!current_tid.isEmpty() && creation_tid == current_tid) return true;
  ...
  if (snapshot_version <= creation_tid.start_csn) return false;
  ...
  if (removal_lock) {
    ...
    if (removal) removal_csn.store(removal, std::memory_order_release);  /// Pairs with acquire load at the top of isVisible
  }
...
bool VersionMetadata::canBeRemoved() {
  if (creation_tid == Tx::PrehistoricTID) {
    ...
    if (creation_csn.load(std::memory_order_relaxed) == Tx::RolledBackCSN) return true;

    TIDHash removal_lock = removal_tid_lock.load(std::memory_order_relaxed);
    if (!removal_lock) return false;

    if (removal_lock == Tx::PrehistoricTID.getHash()) return true;
  }
...
bool VersionMetadata::canBeRemovedImpl(CSN oldest_snapshot_version) {
  ...
  if (creation == Tx::RolledBackCSN) return true;
  ...
  if (oldest_snapshot_version < creation) return false;
...
String VersionMetadata::toString(bool one_line) const {
  ...
  write(buf);
  String res = buf.str();
  if (one_line) std::replace(res.begin(), res.end(), '\n', ' ');
  return res;
}
