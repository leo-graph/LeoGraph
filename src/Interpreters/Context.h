...
class ContextData {
 ...
 protected:
  ...
  struct StorageCache {
    ...
    struct Shard {
      std::mutex mutex;
      std::unordered_set<StorageID, StorageID::DatabaseAndTableNameHash, StorageID::DatabaseAndTableNameEqual> set;
    };

    std::array<Shard, NumShards> shards;

    static size_t shardIndex(const StorageID &id) { return StorageID::DatabaseAndTableNameHash{}(id) & (NumShards - 1); }
  };
 ...
 protected:
  ...
  ZooKeeperMetadataTransactionPtr
      metadata_transaction;               /// Distributed DDL context. I'm not sure if it's a suitable place for this,
...
class Context : public ContextData, public std::enable_shared_from_this<Context> {
 ...
 public:
  ...
  void checkAccess(const AccessFlags &flags, std::string_view database, std::string_view table,
  ...
  void setClientVersion(UInt64 client_version_major, UInt64 client_version_minor, UInt64 client_version_patch,
  ...
  void setConnectionClientVersion(UInt64 client_version_major, UInt64 client_version_minor, UInt64 client_version_patch,
                                  unsigned client_tcp_protocol_version);
  ...
  void addQueryAccessInfo(const StorageID &table_id, const Names &column_names);
  ...
  void setInsertionTable(StorageID db_and_table, std::optional<Names> column_names = std::nullopt,
  ...
  const std::shared_ptr<ColumnsDescription> &getInsertionTableColumnsDescription() const {
    return insertion_table_info.columns_description;
  }
  ...
  OutputFormatPtr getOutputFormat(const String &name, WriteBuffer &buf, const Block &sample,
                                  const std::optional<FormatSettings> &format_settings = std::nullopt) const;
  OutputFormatPtr getOutputFormatParallelIfPossible(const String &name, WriteBuffer &buf, const Block &sample,
                                                    const std::optional<FormatSettings> &format_settings = std::nullopt) const;
  ...
  bool isBackgroundContext() const { return is_background_operation; }
  ...
  /// call. If no ZooKeeper configured, throws an exception.
  ...
  void setPageCache(std::chrono::milliseconds history_window, const String &cache_policy, double size_ratio, size_t min_size_in_bytes,
                    size_t max_size_in_bytes, double free_memory_ratio, size_t num_shards);
 ...
 private:
  ...
  void setCurrentProfilesWithLock(const SettingsProfilesInfo &profiles_info, bool check_constraints,
  ...
  ExternalUserDefinedExecutableFunctionsLoader &getExternalUserDefinedExecutableFunctionsLoaderWithLock(
      const std::lock_guard<std::mutex> &lock);

  void setUserID(const UUID &user_id_);
  void setCurrentRolesImpl(const std::vector<UUID> &new_current_roles, bool throw_if_not_granted, bool skip_if_not_granted,
