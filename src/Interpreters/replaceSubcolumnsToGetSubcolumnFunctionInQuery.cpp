...
void replaceSubcolumnsToGetSubcolumnFunctionInQuery(ASTPtr& ast, const NamesAndTypesList& columns) {
  if (auto* identifier = ast->as<ASTIdentifier>()) {
    ...
    if (node->arguments) {
      for (auto& child : node->arguments->children) replaceSubcolumnsToGetSubcolumnFunctionInQuery(child, columns);
    }
  } else {
    for (auto& child : ast->children) replaceSubcolumnsToGetSubcolumnFunctionInQuery(child, columns);
  }
}
