...
static std::unordered_set<const ActionsDAG::Node *> processShortCircuitFunctions(
    const ActionsDAG &actions_dag, ShortCircuitFunctionEvaluation short_circuit_function_evaluation);
...
ExpressionActions::ExpressionActions(ActionsDAG actions_dag_, const ExpressionActionsSettings &settings_, bool project_inputs_)
    : actions_dag(std::move(actions_dag_)), project_inputs(project_inputs_), settings(settings_) {
  ...
  if (settings.max_temporary_columns && num_columns > settings.max_temporary_columns)
    throw Exception(ErrorCodes::TOO_MANY_TEMPORARY_COLUMNS, "Too many temporary columns: {}. Maximum: {}", actions_dag.dumpNames(),
...
ExpressionActionsPtr ExpressionActions::clone() const {
  ...
  for (auto &action : copy->actions) action.node = copy_map[action.node];
...
static ActionsDAGReverseInfo getActionsDAGReverseInfo(const std::list<ActionsDAG::Node> &nodes, const ActionsDAG::NodeRawConstPtrs &index) {
  ...
  for (const auto *node : index) result_info.nodes_info[result_info.reverse_index[node]].used_in_result = true;

  for (const auto &node : nodes) {
    for (const auto &child : node.children) result_info.nodes_info[result_info.reverse_index[child]].parents.emplace_back(&node);
  }
...
static void setLazyExecutionInfo(
    const ActionsDAG::Node *node, const ActionsDAGReverseInfo &reverse_info,
  ...
  if (lazy_execution_infos.contains(node)) return;
  ...
  for (const auto &parent : node_info.parents) {
    ...
    if (short_circuit_nodes.contains(parent)) {
      ...
      for (size_t i = 0; i != parent->children.size(); ++i) {
        if (node == parent->children[i]) indexes.insert(i);
      }
      ...
      for (auto idx : short_circuit_nodes.at(parent).arguments_with_disabled_lazy_execution) {
        if (idx < parent->children.size() && node == parent->children[idx]) {
          ...
          if (!lazy_execution_infos[parent].can_be_lazy_executed) lazy_execution_info.can_be_lazy_executed = false;
        }
      }
  ...
  if (!lazy_execution_info.can_be_lazy_executed) return;
  ...
  for (const auto &[short_circuit_node, indexes] : lazy_execution_info.short_circuit_ancestors_info) {
    ...
    if (lazy_execution_infos[short_circuit_node].can_be_lazy_executed) continue;
...
static bool findLazyExecutedNodes(const ActionsDAG::NodeRawConstPtrs &children,
                                  std::unordered_map<const ActionsDAG::Node *, LazyExecutionInfo> &lazy_execution_infos,
                                  bool force_enable_lazy_execution, std::unordered_set<const ActionsDAG::Node *> &lazy_executed_nodes_out) {
  ...
  for (const auto *child : children) {
    ...
    if (!lazy_execution_infos[child].can_be_lazy_executed) continue;
    ...
    switch (child->type) {
      case ActionsDAG::ActionType::FUNCTION: {
        ...
        if (force_enable_lazy_execution || has_lazy_child ||
            child->function_base->isSuitableForShortCircuitArgumentsExecution(getDataTypesWithConstInfoFromNodes(child->children))) {
          has_lazy_node = true;
...
static std::unordered_set<const ActionsDAG::Node *> processShortCircuitFunctions(
    const ActionsDAG &actions_dag, ShortCircuitFunctionEvaluation short_circuit_function_evaluation) {
  if (short_circuit_function_evaluation == ShortCircuitFunctionEvaluation::DISABLE) return {};
...
void ExpressionActions::linearizeActions(const std::unordered_set<const ActionsDAG::Node *> &lazy_executed_nodes) {
  ...
  for (const auto &node : nodes) data.push_back({.node = &node});
  ...
  for (const auto &node : nodes) {
    if (node.children.empty()) ready_nodes.emplace(&node);
  }
  ...
  while (!ready_nodes.empty() || !ready_array_joins.empty()) {
    ...
    for (const auto *child : cur.node->children) {
      ...
      if (arg.position < 0) throw Exception(ErrorCodes::LOGICAL_ERROR, "Argument was not calculated for {}", child->result_name);
      ...
      if (!argument.needed_later) free_positions.push(argument.pos);
  ...
  for (const auto &node : outputs) {
    ...
    if (pos < 0) throw Exception(ErrorCodes::LOGICAL_ERROR, "Action for {} was not calculated", node->result_name);
...
void ExpressionActions::checkLimits(const ColumnsWithTypeAndName &columns) const {
  if (settings.max_temporary_non_const_columns) {
    ...
    for (const auto &column : columns)
      if (column.column && !isColumnConst(*column.column)) ++non_const_columns;

    if (non_const_columns > settings.max_temporary_non_const_columns) {
      WriteBufferFromOwnString list_of_non_const_columns;
      for (const auto &column : columns)
        if (column.column && !isColumnConst(*column.column)) list_of_non_const_columns << "\n" << column.name;

      throw Exception(ErrorCodes::TOO_MANY_TEMPORARY_NON_CONST_COLUMNS, "Too many temporary non-const columns:{}. Maximum: {}",
...
static void replicateColumns(ColumnsWithTypeAndName &columns, const IColumn::Offsets &offsets) {
  for (auto &column : columns)
    if (column.column) column.column = column.column->replicate(offsets);
}
...
static void executeAction(const ExpressionActions::Action &action, ExecutionContext &execution_context, bool dry_run,
                          bool allow_duplicates_in_input, bool enable_lazy_columns_replication) {
  ...
  switch (action.node->type) {
    case ActionsDAG::ActionType::FUNCTION: {
      ...
      if (res_column.type || res_column.column) throw Exception(ErrorCodes::LOGICAL_ERROR, "Result column is not empty");
      ...
      if (action.node->column) {
        ...
        for (const auto &argument : action.arguments) {
          if (!argument.needed_later) columns[argument.pos] = {};
        }
        break;
      }
      ...
      if (action.is_lazy_executed)
        ...
        if (action.node->is_function_compiled) ProfileEvents::increment(ProfileEvents::CompiledFunctionExecute);
        ...
        if (res_column.column->getDataType() != res_column.type->getColumnType()) {
          throw Exception(ErrorCodes::LOGICAL_ERROR,
                          ...
                          action.node->function->getName(), res_column.type->getName(), res_column.column->getName(), action.toString(),
    ...
    case ActionsDAG::ActionType::ARRAY_JOIN: {
      ...
      if (!action.arguments.front().needed_later) columns[array_join_key_pos] = {};
      ...
      if (!array) throw Exception(ErrorCodes::TYPE_MISMATCH, "ARRAY JOIN of not array nor map: {}", action.node->result_name);
    ...
    case ActionsDAG::ActionType::ALIAS: {
      ...
      if (action.result_position != arg.pos) {
        ...
        if (!arg.needed_later) columns[arg.pos] = {};
      }
...
void ExpressionActions::execute(Block &block, size_t &num_rows, bool dry_run, bool allow_duplicates_in_input) const {
    ...
    if (it != input_positions.end()) {
      for (auto input_pos : it->second) {
        if (execution_context.inputs_pos[input_pos] < 0) {
          ...
          if (!allow_duplicates_in_input) break;
        }
      }
    }
  }
  ...
  if (project_inputs) {
    ...
    for (auto input : execution_context.inputs_pos)
      if (input >= 0) block.erase(input);
  }

  Block res;

  for (auto pos : result_positions)
    if (execution_context.columns[pos].column) res.insert(execution_context.columns[pos]);

  for (auto &&item : block) res.insert(std::move(item));
...
void ExpressionActions::execute(Block &block, bool dry_run, bool allow_duplicates_in_input) const {
  ...
  if (block.empty()) block.insert({DataTypeUInt8().createColumnConst(num_rows, 0), std::make_shared<DataTypeUInt8>(), "_dummy"});
}

Names ExpressionActions::getRequiredColumns() const {
  Names names;
  for (const auto &input : required_columns) names.push_back(input.name);
  return names;
}

bool ExpressionActions::hasArrayJoin() const { return getActionsDAG().hasArrayJoin(); }

void ExpressionActions::assertDeterministic() const { getActionsDAG().assertDeterministic(); }
...
NameAndTypePair ExpressionActions::getSmallestColumn(const NamesAndTypesList &columns) {
  ...
  for (const auto &column : columns) {
    ...
    if (column.isSubcolumn()) continue;
  ...
  if (!min_size) throw Exception(ErrorCodes::LOGICAL_ERROR, "No available columns");
...
std::string ExpressionActions::dumpActions() const {
  ...
  ss << "input:\n";
  for (const auto &input_column : required_columns) ss << input_column.name << " " << input_column.type->getName() << "\n";
  ...
  ss << "\noutput positions:";
  for (auto pos : result_positions) ss << " " << pos;
  ss << "\n";
...
void ExpressionActions::describeActions(WriteBuffer &out, std::string_view prefix) const {
  ...
  for (const auto &pos : result_positions) out << ' ' << pos;
  out << '\n';
}
...
JSONBuilder::ItemPtr ExpressionActions::toTree() const {
  ...
  for (const auto &input_column : required_columns) {
    ...
    if (input_column.type) map->add("Type", input_column.type->getName());
  ...
  for (const auto &output_column : sample_block) {
    ...
    if (output_column.type) map->add("Type", output_column.type->getName());

    outputs_array->add(std::move(map));
  }

  auto actions_array = std::make_unique<JSONBuilder::JSONArray>();
  for (const auto &action : actions) actions_array->add(action.toTree());

  auto positions_array = std::make_unique<JSONBuilder::JSONArray>();
  for (auto pos : result_positions) positions_array->add(pos);
        ...
        if (const auto *column_set = checkAndGetColumn<const ColumnSet>(action.node->column.get()))
          if (auto future_set = column_set->getData())
            if (auto set = future_set->get())
              if (set->getTotalRowCount() == 0) return true;
    }
  }
...
void ExpressionActionsChain::addStep(NameSet non_constant_inputs) {
  if (steps.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot add action to empty ExpressionActionsChain");

  ColumnsWithTypeAndName columns = steps.back()->getResultColumns();
  for (auto &column : columns)
    if (column.column && isColumnConst(*column.column) && non_constant_inputs.contains(column.name)) column.column = nullptr;
...
void ExpressionActionsChain::finalize() {
  ...
  for (int i = static_cast<int>(steps.size()) - 1; i >= 0; --i) {
    ...
    for (const auto &output : required_output) required_names.insert(output.first);
...
std::string ExpressionActionsChain::dumpChain() const {
  ...
  for (size_t i = 0; i < steps.size(); ++i) {
    ss << "step " << i << "\n";
    ss << "required output:\n";
    for (const auto &it : steps[i]->required_output) ss << it.first << "\n";
    ss << "\n" << steps[i]->dump() << "\n";
  }
...
void ExpressionActionsChainSteps::ArrayJoinStep::finalize(const NameSet &required_output_) {
  ...
  for (const auto &column : result_columns) {
    if (array_join_columns.contains(column.name) || required_output_.contains(column.name)) new_result_columns.emplace_back(column);
  }
  for (const auto &column : required_columns) {
    if (array_join_columns.contains(column.name) || required_output_.contains(column.name)) new_required_columns.emplace_back(column);
  }
...
ExpressionActionsChainSteps::JoinStep::JoinStep(std::shared_ptr<TableJoin> analyzed_join_, JoinPtr join_,
                                                const ColumnsWithTypeAndName &required_columns_)
    : Step({}), analyzed_join(std::move(analyzed_join_)), join(std::move(join_)) {
  for (const auto &column : required_columns_) required_columns.emplace_back(column.name, column.type);
...
void ExpressionActionsChainSteps::JoinStep::finalize(const NameSet &required_output_) {
  ...
  for (const auto &name : analyzed_join->getAllNames(JoinTableSide::Left)) required_names.emplace(name);

  for (const auto &onexpr : analyzed_join->getClauses())
    if (const auto &cond_name = onexpr.condColumnNames().first; !cond_name.empty()) required_names.emplace(cond_name);

  for (const auto &column : required_columns) {
    if (required_names.contains(column.name)) new_required_columns.emplace_back(column);
  }

  /// Result will also contain joined columns.
  for (const auto &column : analyzed_join->columnsAddedByJoin()) required_names.emplace(column.name);

  for (const auto &column : result_columns) {
    if (required_names.contains(column.name)) new_result_columns.emplace_back(column);
  }
