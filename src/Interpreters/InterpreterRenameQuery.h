...
struct UniqueTableName {
  ...
  UniqueTableName(const String& database_name_, const String& table_name_) : database_name(database_name_), table_name(table_name_) {}
...
class InterpreterRenameQuery : public IInterpreter, WithContext {
 ...
 private:
  ...
  enum class RenameType : uint8_t { RenameTable, RenameDatabase };
