...
void ApplyWithSubqueryVisitor::visit(ASTSelectQuery &ast, const Data &data) {
  ...
  for (auto &child : ast.children) {
    if (child != ast.with()) visit(child, new_data ? *new_data : data);
  }
}
  ...
  if (table.database_and_table_name) {
    ...
    if (table_id.database_name.empty()) {
      ...
      if (subquery_it != data.subqueries.end()) {
        ...
        if (!old_alias.empty()) table.subquery->setAlias(old_alias);
    ...
    if (const auto *identifier = ast->as<ASTIdentifier>()) {
      if (identifier->isShort()) {
        ...
        if (subquery_it != data.subqueries.end()) {
          ...
          if (!old_alias.empty()) func.arguments->children[1]->setAlias(old_alias);
        } else {
