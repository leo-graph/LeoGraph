  ...
  if (auto* identifier = ast->as<ASTIdentifier>()) {
    ...
    if (node->arguments) {
      ...
      for (size_t arg = 0; arg < num_arguments; ++arg) {
        ...
        if (replaceMissedSubcolumnsInFunction(child, column_name)) is_replaced = true;
      }
    }
  } else {
...
void replaceMissedSubcolumnsInQuery(ASTPtr& ast, const String& column_name) {
  if (auto* identifier = ast->as<ASTIdentifier>()) {
    ...
    if (replaceMissedSubcolumnsInFunction(ast, column_name)) ast->setAlias(function_alias);
  } else {
    for (auto& child : ast->children) replaceMissedSubcolumnsInQuery(child, column_name);
  }
}
