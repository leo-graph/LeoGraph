...
PredicateRewriteVisitorData::PredicateRewriteVisitorData(ContextPtr context_, const ASTs &predicates_,
                                                         const TableWithColumnNamesAndTypes &table_columns_, bool optimize_final_,
                                                         bool optimize_with_)
...
void PredicateRewriteVisitorData::visitOtherInternalSelect(ASTSelectQuery &select_query, ASTPtr &) {
  ...
  const Names &internal_columns =
      InterpreterSelectQuery(temp_internal_select, const_pointer_cast<Context>(getContext()), SelectQueryOptions().analyze())
          .getSampleBlock()
          ->getNames();
...
static void cleanAliasAndCollectIdentifiers(ASTPtr &predicate, std::vector<ASTIdentifier *> &identifiers) {
  ...
  if (!predicate->as<ASTSubquery>()) {
    for (auto &children : predicate->children) cleanAliasAndCollectIdentifiers(children, identifiers);
  }

  if (const auto alias = predicate->tryGetAlias(); !alias.empty()) predicate->setAlias({});

  if (ASTIdentifier *identifier = predicate->as<ASTIdentifier>()) identifiers.emplace_back(identifier);
}
...
static void useAliasInsteadOfIdentifier(const ASTPtr &predicate) {
  if (!predicate->as<ASTSubquery>()) {
    for (auto &children : predicate->children) useAliasInsteadOfIdentifier(children);
  }

  if (const auto alias = predicate->tryGetAlias(); !alias.empty()) {
    if (ASTIdentifier *identifier = predicate->as<ASTIdentifier>()) identifier->setShortName(alias);
    predicate->setAlias({});
  }
}
...
bool PredicateRewriteVisitorData::rewriteSubquery(ASTSelectQuery &subquery, const Names &inner_columns) {
  if ((!optimize_final && subquery.final()) ||
      (subquery.with() && (!optimize_with || hasNonRewritableFunction(subquery.with(), getContext()))) || subquery.withFill() ||
      subquery.limitBy() || subquery.limitLength() || subquery.limitByLength() || subquery.limitByOffset() ||
      hasNonRewritableFunction(subquery.select(), getContext()) || (subquery.orderBy() && subquery.limitOffset()))
    return false;
  ...
  for (const auto &predicate : predicates) {
    ...
    if (hashes.contains(predicate_hash)) continue;
