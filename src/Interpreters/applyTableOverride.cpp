...
void applyTableOverrideToCreateQuery(const ASTTableOverride& override, ASTCreateQuery* create_query) {
  if (auto* columns = override.columns) {
    if (!create_query->columns_list) create_query->set(create_query->columns_list, make_intrusive<ASTColumns>());
    if (columns->columns) {
      for (const auto& override_column_ast : columns->columns->children) {
        ...
        auto exists = std::find_if(dest_children.begin(), dest_children.end(),
        ...
        if (exists == dest_children.end()) {
          if (override_column->default_specifier == ColumnDefaultSpecifier::Alias) dest_children.emplace_back(override_column_ast);
        } else
    ...
    if (columns->constraints) {
      for (const auto& override_constraint_ast : columns->constraints->children) {
        ...
        if (!override_constraint) continue;
    ...
    if (columns->projections) {
      for (const auto& override_projection_ast : columns->projections->children) {
        ...
        if (!override_projection) continue;
  ...
  if (auto* storage = override.storage) {
    if (!create_query->storage) create_query->set(create_query->storage, make_intrusive<ASTStorage>());
    if (storage->partition_by) create_query->storage->set(create_query->storage->partition_by, storage->partition_by->clone());
    if (storage->primary_key) create_query->storage->set(create_query->storage->primary_key, storage->primary_key->clone());
    if (storage->order_by) create_query->storage->set(create_query->storage->order_by, storage->order_by->clone());
    if (storage->sample_by) create_query->storage->set(create_query->storage->sample_by, storage->sample_by->clone());
    if (storage->ttl_table) create_query->storage->set(create_query->storage->ttl_table, storage->ttl_table->clone());
