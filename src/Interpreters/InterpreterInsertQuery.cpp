...
InterpreterInsertQuery::InterpreterInsertQuery(const ASTPtr &query_ptr_, ContextMutablePtr context_, bool allow_materialized_,
                                               bool no_squash_, bool no_destination_, bool async_insert_)
  ...
  checkStackSize();
  if (auto quota = getContext()->getQuota()) quota->checkExceeded(QuotaType::WRITTEN_BYTES);
      ...
      if (current_context->getSettingsRef()[Setting::allow_experimental_analyzer]) {
        ...
        if (input_function)
          throw Exception(
              ...
              "Schema inference is not supported with allow_experimental_analyzer=0 for INSERT INTO FUNCTION ... SELECT FROM input()");

        InterpreterSelectWithUnionQuery interpreter_select{query.select, current_context, select_query_options};
...
Block InterpreterInsertQuery::getSampleBlock(const ASTInsertQuery &query, const StoragePtr &table,
                                             const StorageMetadataPtr &metadata_snapshot, ContextPtr context_, bool no_destination,
                                             bool allow_materialized) {
  /// If the query does not include information about columns
  if (!query.columns) {
    if (auto *window_view = dynamic_cast<StorageWindowView *>(table.get())) return window_view->getInputHeader();
    if (no_destination) return metadata_snapshot->getSampleBlockWithVirtuals(table->getVirtualsList());
...
Block InterpreterInsertQuery::getSampleBlock(const Names &names, const StoragePtr &table, const StorageMetadataPtr &metadata_snapshot,
                                             bool allow_virtuals, bool allow_materialized) {
  ...
  for (size_t i = 0; i < names.size(); i++) {
    ...
    if (!inserted_names.insert(current_name).second)
      throw Exception(ErrorCodes::DUPLICATE_COLUMN, "Column {} in table {} specified more than once", current_name,
  ...
  if (!missing_positions.empty()) {
    ...
    if (allow_virtuals) table_sample_virtuals = table->getVirtualsHeader();
    ...
    for (auto pos : missing_positions) {
      ...
      if (table_sample_physical.has(current_name)) {
        ...
        throw Exception(ErrorCodes::NO_SUCH_COLUMN_IN_TABLE, "No such column {} in table {}", current_name,
                        table->getStorageID().getNameForLogs());
      }
    }
  }
...
static bool hasAggregateFunctions(const IAST *ast) {
  if (const auto *func = typeid_cast<const ASTFunction *>(ast))
    if (AggregateUtils::isAggregateFunction(*func)) return true;

  for (const auto &child : ast->children)
    if (hasAggregateFunctions(child.get())) return true;
...
static bool isTrivialSelect(const ASTPtr &select) {
  if (auto *select_query = select->as<ASTSelectQuery>()) {
    ...
    if (tables_in_select_query.children.size() != 1) return false;
    ...
    if (table_expr.subquery) return false;

    /// Note: how to write it in more generic way?
    return (!select_query->distinct && !select_query->limit_with_ties && !select_query->prewhere() && !select_query->where() &&
...
static std::pair<QueryPipelineBuilder, ParallelReplicasReadingCoordinatorPtr> getLocalSelectPipelineForInserSelectWithParallelReplicas(
    const ASTPtr &select, const ContextPtr &context) {
...
QueryPipeline InterpreterInsertQuery::addInsertToSelectPipeline(ASTInsertQuery &query, StoragePtr table, QueryPipelineBuilder &pipeline) {
    ...
    if (input_columns.size() == query_columns.size()) {
      for (size_t col_idx = 0; col_idx < query_columns.size(); ++col_idx) {
        ...
        if (isNullableOrLowCardinalityNullable(input_columns[col_idx].type) &&
            !isNullableOrLowCardinalityNullable(query_columns[col_idx].type) && !isVariant(query_columns[col_idx].type) &&
            !isDynamic(query_columns[col_idx].type) && output_columns.has(query_columns[col_idx].name)) {
          query_sample_block.setColumn(col_idx, ColumnWithTypeAndName(makeNullableOrLowCardinalityNullable(query_columns[col_idx].column),
  ...
  auto actions_dag =
      ActionsDAG::makeConvertingActions(pipeline.getHeader().getColumnsWithTypeAndName(), query_sample_block.getColumnsWithTypeAndName(),
  ...
  pipeline.addSimpleTransform(
      [&](const SharedHeader &in_header) -> ProcessorPtr { return std::make_shared<ExpressionTransform>(in_header, actions); });
  ...
  if (select_streams != 1) pipeline.resize(1);
  ...
  auto insert_dependencies =
      InsertDependenciesBuilder::create(table, query_ptr, std::make_shared<const Block>(std::move(query_sample_block)), async_insert,
                                        /*skip_destination_table*/ no_destination, max_insert_threads, context);

  pipeline.addSimpleTransform([&](const SharedHeader &in_header) -> ProcessorPtr {
    return std::make_shared<AddDeduplicationInfoTransform>(
        insert_dependencies, insert_dependencies->getRootViewID(), context->getSettingsRef()[Setting::insert_deduplication_token].value,
        context->getServerSettings()[ServerSetting::insert_deduplication_version].value, in_header);
  });
  ...
  if (should_squash) {
    pipeline.addSimpleTransform([&](const SharedHeader &in_header) -> ProcessorPtr {
      return std::make_shared<PlanSquashingTransform>(
          in_header,
          table->prefersLargeBlocks() ? context->getSettingsRef()[Setting::min_insert_block_size_rows]
  ...
  if (should_squash) {
    pipeline.addSimpleTransform(
        [&](const SharedHeader &in_header) -> ProcessorPtr { return std::make_shared<ApplySquashingTransform>(in_header); });
  }
...
bool queryHasOrderByAll(const ASTPtr &select) {
  if (auto *select_query = select->as<ASTSelectQuery>()) {
    ...
    if (union_query->list_of_selects->children.size() != 1) return false;
...
std::pair<QueryPipeline, ParallelReplicasReadingCoordinatorPtr> InterpreterInsertQuery::buildLocalInsertSelectPipelineForParallelReplicas(
    ASTInsertQuery &query, const StoragePtr &table) {
  ContextPtr select_context = getContext();
  applyTrivialInsertSelectOptimization(query, table->prefersLargeBlocks(), select_context);

  auto [pipeline_builder, coordinator] = getLocalSelectPipelineForInserSelectWithParallelReplicas(query.select, select_context);
...
static bool isInsertSelectTrivialEnoughForDistributedExecution(const ASTInsertQuery &query) {
  ...
  if (auto *select_query = selects.front()->as<ASTSelectQuery>()) {
    ...
    return (!select_query->distinct && !select_query->limit_with_ties && !select_query->groupBy() && !select_query->having() &&
            !select_query->orderBy() && !select_query->limitBy() && !select_query->limitLength() && !hasAggregateFunctions(select_query));
  }
  return false;
}
...
std::optional<QueryPipeline> InterpreterInsertQuery::buildInsertSelectPipelineParallelReplicas(ASTInsertQuery &query, StoragePtr table) {
  ...
  if (!settings[Setting::allow_experimental_analyzer]) return {};

  if (!context_ptr->canUseParallelReplicasOnInitiator()) return {};

  if (settings[Setting::parallel_distributed_insert_select] != 2) return {};

  // NOTE: should we limit it more here?
  if (auto storage = getTable(query); storage->isMergeTree() && !storage->supportsReplication()) return {};

  if (!isInsertSelectTrivialEnoughForDistributedExecution(query)) return {};

  auto select = query.select->as<ASTSelectWithUnionQuery &>().list_of_selects->children.front();
  if (!ClusterProxy::isSuitableForParallelReplicas(select, context_ptr)) return {};
...
QueryPipeline InterpreterInsertQuery::buildInsertPipeline(ASTInsertQuery &query, StoragePtr table) {
  ...
  if (query_sample_block->empty()) throw Exception(ErrorCodes::EMPTY_LIST_OF_COLUMNS_PASSED, "Empty list of columns to insert");
  ...
  auto insert_dependencies = InsertDependenciesBuilder::create(table, query_ptr, query_sample_block, async_insert,
  ...
  chain.addSource(std::make_shared<AddDeduplicationInfoTransform>(
      insert_dependencies, insert_dependencies->getRootViewID(), settings[Setting::insert_deduplication_token].value,
      context->getServerSettings()[ServerSetting::insert_deduplication_version].value, chain.getInputSharedHeader()));
  ...
  if (query.hasInlinedData() && !async_insert) {
    ...
    if (settings[Setting::enable_parsing_to_custom_serialization]) format->setSerializationHints(table->getSerializationHints());
...
std::optional<QueryPipeline> InterpreterInsertQuery::distributedWriteIntoReplicatedMergeTreeOrDataLakeFromClusterStorage(
  ...
  if (query.table_id.empty()) return {};

  StoragePtr dst_storage = DatabaseCatalog::instance().getTable(query.table_id, local_context);
  if (!(dst_storage->isMergeTree() || dst_storage->isDataLake()) || !dst_storage->supportsReplication()) return {};
  ...
  for (const auto &shard : src_cluster->getShardsInfo()) {
    ...
    auto remote_query_executor = std::make_shared<RemoteQueryExecutor>(
        pools.at(0).pool, query_str, std::make_shared<const Block>(Block{}), query_context,
...
BlockIO InterpreterInsertQuery::execute() {
  ...
  if (context->getServerSettings()[ServerSetting::disable_insertion_and_mutation] &&
    ...
    bool is_external_storage = table->isObjectStorage() ||   /// S3, Azure, GCS, HDFS, etc.
    ...
    if (!is_external_storage) throw Exception(ErrorCodes::QUERY_IS_PROHIBITED, "Insert queries are prohibited");
  }
  ...
  if (!query.table_function) context->checkAccess(AccessType::INSERT, query.table_id, query_sample_block.getNames());
  ...
  if (query.select) {
    if (settings[Setting::parallel_distributed_insert_select]) {
      ...
      if (!res.pipeline.initialized() && context->canUseParallelReplicasOnInitiator()) {
        ...
        if (pipeline) res.pipeline = std::move(*pipeline);
      }

      query.select = std::move(saved_select);
    }
    if (!res.pipeline.initialized()) res.pipeline = buildInsertSelectPipeline(query, table);
  } else {
  ...
  if (const auto *mv = dynamic_cast<const StorageMaterializedView *>(table.get())) res.pipeline.addStorageHolder(mv->getTargetTable());

  return res;
}

StorageID InterpreterInsertQuery::getDatabaseTable() const { return query_ptr->as<ASTInsertQuery &>().table_id; }
...
void InterpreterInsertQuery::setInsertContextValues(StoragePtr table) {
  ...
  if (insert_query.columns) {
    const auto columns_ast =
        processColumnTransformers(getContext()->getCurrentDatabase(), table, table->getInMemoryMetadataPtr(), insert_query.columns);
    Names names;
    names.reserve(columns_ast->children.size());
    for (const auto &identifier : columns_ast->children) {
      std::string current_name = identifier->getColumnName();
  ...
  getContext()->setInsertionTable(insert_query.table_id, insert_columns,
                                  std::make_shared<ColumnsDescription>(table->getInMemoryMetadataPtr()->columns));
}
