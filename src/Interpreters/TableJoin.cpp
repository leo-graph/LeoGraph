...
bool forAllKeys(OnExpr& expressions, Func callback) {
  static_assert(std::is_same_v<SideTag, BothSidesTag> || std::is_same_v<SideTag, LeftSideTag> || std::is_same_v<SideTag, RightSideTag>);

  for (auto& expr : expressions) {
    if constexpr (std::is_same_v<SideTag, BothSidesTag>) assert(expr.key_names_left.size() == expr.key_names_right.size());

    size_t sz = !std::is_same_v<SideTag, RightSideTag> ? expr.key_names_left.size() : expr.key_names_right.size();
    for (size_t i = 0; i < sz; ++i) {
      bool cont;
      if constexpr (std::is_same_v<SideTag, BothSidesTag>) cont = callback(expr.key_names_left[i], expr.key_names_right[i]);
      if constexpr (std::is_same_v<SideTag, LeftSideTag>) cont = callback(expr.key_names_left[i]);
      if constexpr (std::is_same_v<SideTag, RightSideTag>) cont = callback(expr.key_names_right[i]);
...
std::string TableJoin::formatClauses(const TableJoin::Clauses& clauses, bool short_format) {
  ...
  for (const auto& clause : clauses) res.push_back("[" + clause.formatDebug(short_format) + "]");
...
TableJoin::TableJoin(SizeLimits limits, bool use_nulls, JoinKind kind, JoinStrictness strictness, const Names& key_names_right)
    : size_limits(limits), default_max_bytes(0), join_use_nulls(use_nulls), join_algorithms({JoinAlgorithm::DEFAULT}) {
...
void TableJoin::addDisjunct() {
  ...
  if (getStorageJoin() && clauses.size() > 1) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "StorageJoin with ORs is not supported");
}
...
size_t TableJoin::rightKeyInclusion(const String& name) const {
  if (hasUsing()) return 0;

  size_t count = 0;
  for (const auto& clause : clauses) count += std::count(clause.key_names_right.begin(), clause.key_names_right.end(), name);
  return count;
}
...
const NamesAndTypesList& TableJoin::getOutputColumns(JoinTableSide side) const {
  if (side == JoinTableSide::Left) return result_columns_from_left_table;
...
void TableJoin::deduplicateAndQualifyColumnNames(const NameSet& left_table_columns, const String& right_table_prefix) {
  ...
  for (auto& column : columns_from_joined_table) {
    if (joined_columns.contains(column.name)) continue;
    ...
    if (left_table_columns.contains(column.name)) inserted.name = right_table_prefix + column.name;

    original_names[inserted.name] = column.name;
    if (inserted.name != column.name) renames[column.name] = inserted.name;
  }
...
String TableJoin::getOriginalName(const String& column_name) const {
  ...
  if (it != original_names.end()) return it->second;
  return column_name;
}
...
ASTPtr TableJoin::leftKeysList() const {
  ...
  for (const auto& clause : clauses) {
    if (clause.on_filter_condition_left) keys_list->children.push_back(clause.on_filter_condition_left);
  }
  return keys_list;
}
...
ASTPtr TableJoin::rightKeysList() const {
  ...
  if (hasOn()) keys_list->children = key_asts_right;

  for (const auto& clause : clauses) {
    if (clause.on_filter_condition_right) keys_list->children.push_back(clause.on_filter_condition_right);
  }
  return keys_list;
}
...
Names TableJoin::requiredJoinedNames() const {
  ...
  for (const auto& joined_column : columns_added_by_join) required_columns_set.insert(joined_column.name);
   ...
   */
  if (required_columns_set.empty() && !columns_from_joined_table.empty()) return {columns_from_joined_table.begin()->name};
...
NameSet TableJoin::requiredRightKeys() const {
  ...
  forAllKeys<RightSideTag>(clauses, [this, &required](const auto& name) {
    ...
    for (const auto& column : columns_added_by_join)
      if (rename == column.name) required.insert(name);
    return true;
  });
  return required;
}
...
NamesWithAliases TableJoin::getRequiredColumns(const Block& sample, const Names& action_required_columns) const {
  ...
  for (auto& column : requiredJoinedNames())
    if (!sample.has(column)) required_columns.insert(column);
...
Block TableJoin::getRequiredRightKeys(const Block& right_table_keys, std::vector<String>& keys_sources) const {
  ...
  if (required_keys.empty()) return required_right_keys;
...
void TableJoin::setUsedColumns(const Names& column_names) {
  ...
  for (auto it = columns_from_left_table.begin(); it != columns_from_left_table.end(); ++it) left_columns_idx[it->name] = it;

  std::unordered_map<std::string_view, NamesAndTypesList::const_iterator> right_columns_idx;
  for (auto it = columns_from_joined_table.begin(); it != columns_from_joined_table.end(); ++it) right_columns_idx[it->name] = it;
  ...
  for (const auto& column_name : column_names) {
    if (auto lit = left_columns_idx.find(column_name); lit != left_columns_idx.end())
      ...
      throw Exception(ErrorCodes::NOT_FOUND_COLUMN_IN_BLOCK, "Column {} not found in JOIN, left columns: [{}], right columns: [{}]",
                      column_name,
...
void TableJoin::addJoinedColumn(const NameAndTypePair& joined_column) { setUsedColumn(joined_column, JoinTableSide::Right); }
...
NamesAndTypesList TableJoin::correctedColumnsAddedByJoin() const {
  ...
  for (const auto& col : columns_added_by_join) {
    ...
    if (hasUsing()) {
      if (auto it = right_type_map.find(col.name); it != right_type_map.end()) type = it->second;
    }

    if (rightBecomeNullable(type)) type = JoinCommon::convertTypeToNullable(type);
  ...
  for (auto& col : left_columns) {
    if (hasUsing()) {
      ...
      if (auto it = left_type_map.find(col.name); it != left_type_map.end()) {
        col.type = it->second;
        if constexpr (has_column) col.column = nullptr;
      }
    }
...
bool TableJoin::sameStrictnessAndKind(JoinStrictness strictness_, JoinKind kind_) const {
  if (strictness_ == strictness() && kind_ == kind()) return true;

  /// Compatibility: old ANY INNER == new SEMI LEFT
  if (strictness_ == JoinStrictness::Semi && isLeft(kind_) && strictness() == JoinStrictness::RightAny && isInner(kind())) return true;
  if (strictness() == JoinStrictness::Semi && isLeft(kind()) && strictness_ == JoinStrictness::RightAny && isInner(kind_)) return true;

  return false;
}

bool TableJoin::oneDisjunct() const { return clauses.size() == 1 && !clauses.front().isEmpty(); }

bool TableJoin::needStreamWithNonJoinedRows() const {
  if (strictness() == JoinStrictness::Asof || strictness() == JoinStrictness::Semi) return false;
  return isRightOrFull(kind());
}

static void renameIfNeeded(String& name, const NameToNameMap& renames) {
  if (const auto it = renames.find(name); it != renames.end()) name = it->second;
}
...
static void makeColumnNameUnique(const ColumnsWithTypeAndName& source_columns, String& name) {
  for (const auto& source_col : source_columns) {
    if (source_col.name != name) continue;

    /// Duplicate found, slow path
    NameSet names;
    for (const auto& col : source_columns) names.insert(col.name);
...
static std::optional<ActionsDAG> createWrapWithTupleActions(const ColumnsWithTypeAndName& source_columns,
                                                            std::unordered_set<std::string_view>&& column_names_to_wrap,
                                                            NameToNameMap& new_names) {
  if (column_names_to_wrap.empty()) return {};
  ...
  for (const auto* input_node : actions_dag.getInputs()) {
    ...
    if (it == column_names_to_wrap.end()) continue;
...
std::pair<NameSet, NameSet> TableJoin::getKeysForNullSafeComparion(const ColumnsWithTypeAndName& left_sample_columns,
                                                                   const ColumnsWithTypeAndName& right_sample_columns) {
  std::unordered_map<String, size_t> left_idx;
  for (size_t i = 0; i < left_sample_columns.size(); ++i) left_idx[left_sample_columns[i].name] = i;

  std::unordered_map<String, size_t> right_idx;
  for (size_t i = 0; i < right_sample_columns.size(); ++i) right_idx[right_sample_columns[i].name] = i;
  ...
  for (const auto& clause : clauses) {
    for (size_t i : clause.nullsafe_compare_key_indexes) {
      ...
      if (lit == left_idx.end())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't find key {} in left columns [{}]", left_key,
                        Block(left_sample_columns).dumpNames());
      auto rit = right_idx.find(right_key);
      if (rit == right_idx.end())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't find key {} in right columns [{}]", right_key,
                        Block(right_sample_columns).dumpNames());

      if (!left_sample_columns[lit->second].type->isNullable() || !right_sample_columns[rit->second].type->isNullable()) continue;
...
std::pair<std::optional<ActionsDAG>, std::optional<ActionsDAG>> TableJoin::createConvertingActions(
    const ColumnsWithTypeAndName& left_sample_columns, const ColumnsWithTypeAndName& right_sample_columns, const ContextPtr& context) {
  ...
  auto [left_keys_nullsafe_comparison, right_keys_nullsafe_comparison] = getKeysForNullSafeComparion(
      left_dag ? left_dag->getResultColumns() : left_sample_columns, right_dag ? right_dag->getResultColumns() : right_sample_columns);
  if (!left_keys_nullsafe_comparison.empty() || !right_keys_nullsafe_comparison.empty()) {
    auto new_left_dag = applyNullsafeWrapper(left_dag ? left_dag->getResultColumns() : left_sample_columns, left_keys_nullsafe_comparison,
                                             JoinTableSide::Left, left_column_rename);
  ...
  if (forceNullableLeft()) {
    auto new_left_dag =
        applyJoinUseNullsConversion(left_dag ? left_dag->getResultColumns() : left_sample_columns, left_column_rename, context);
    mergeDags(left_dag, std::move(new_left_dag));
  }

  if (forceNullableRight()) {
    auto new_right_dag =
        applyJoinUseNullsConversion(right_dag ? right_dag->getResultColumns() : right_sample_columns, right_column_rename, context);
    mergeDags(right_dag, std::move(new_right_dag));
  }
...
void TableJoin::inferJoinKeyCommonType(const LeftNamesAndTypes& left, const RightNamesAndTypes& right, bool allow_right,
                                       bool require_strict_keys_match) {
  if (!left_type_map.empty() || !right_type_map.empty()) return;

  NameToTypeMap left_types;
  for (const auto& col : left) left_types[col.name] = col.type;

  NameToTypeMap right_types;
  for (const auto& col : right) right_types[renamedRightColumnName(col.name)] = col.type;

  if (strictness() == JoinStrictness::Asof) {
    if (clauses.size() != 1) throw DB::Exception(ErrorCodes::NOT_IMPLEMENTED, "ASOF join over multiple keys is not supported");
  }
  ...
  forAllKeys(clauses, [&](const auto& left_key_name, const auto& right_key_name) {
    ...
    try {
      ...
      throw DB::Exception(ErrorCodes::TYPE_MISMATCH, "Can't infer common type for joined columns: {}: {} at left, {}: {} at right. {}",
                          left_key_name, ltype->getName(), right_key_name, rtype->getName(), ex.message());
    }

    if (!allow_right && !common_type->equals(*rtype)) {
      throw DB::Exception(ErrorCodes::TYPE_MISMATCH, "Can't change type for right table: {}: {} -> {}.", right_key_name, rtype->getName(),
                          common_type->getName());
    }
  ...
  if (!left_type_map.empty() || !right_type_map.empty()) {
    LOG_TRACE(getLogger("TableJoin"), "Infer supertype for joined columns. Left: [{}], Right: [{}]",
              formatTypeMap(left_type_map, left_types), formatTypeMap(right_type_map, right_types));
  }
}

static std::optional<ActionsDAG> changeKeyTypes(const ColumnsWithTypeAndName& cols_src, const TableJoin::NameToTypeMap& type_mapping,
                                                bool add_new_cols, NameToNameMap& key_column_rename, const ContextPtr& context) {
...
static std::optional<ActionsDAG> changeTypesToNullable(const ColumnsWithTypeAndName& cols_src, const NameSet& exception_cols,
                                                       const ContextPtr& context) {
  ColumnsWithTypeAndName cols_dst = cols_src;
  bool has_some_to_do = false;
  for (auto& col : cols_dst) {
    if (exception_cols.contains(col.name)) continue;
    col.type = JoinCommon::convertTypeToNullable(col.type);
    col.column = nullptr;
    has_some_to_do = true;
  }
...
std::optional<ActionsDAG> TableJoin::applyKeyConvertToTable(const ColumnsWithTypeAndName& cols_src, const NameToTypeMap& type_mapping,
                                                            JoinTableSide table_side, NameToNameMap& key_column_rename,
                                                            const ContextPtr& context) {
...
std::optional<ActionsDAG> TableJoin::applyNullsafeWrapper(const ColumnsWithTypeAndName& cols_src,
                                                          const NameSet& columns_for_nullsafe_comparison, JoinTableSide table_side,
                                                          NameToNameMap& key_column_rename) {
  if (columns_for_nullsafe_comparison.empty()) return {};
...
std::optional<ActionsDAG> TableJoin::applyJoinUseNullsConversion(const ColumnsWithTypeAndName& cols_src,
                                                                 const NameToNameMap& key_column_rename, const ContextPtr& context) {
  ...
  for (const auto& it : key_column_rename) exclude_columns.insert(it.second);

  /// Create DAG to make columns nullable if needed
  return changeTypesToNullable(cols_src, exclude_columns, context);
}

void TableJoin::setStorageJoin(std::shared_ptr<const IKeyValueEntity> storage) { right_kv_storage = storage; }

void TableJoin::setStorageJoin(std::shared_ptr<StorageJoin> storage) { right_storage_join = storage; }

String TableJoin::renamedRightColumnName(const String& name) const {
  if (const auto it = renames.find(name); it != renames.end()) return it->second;
  return name;
}

String TableJoin::renamedRightColumnNameWithAlias(const String& name) const {
  auto renamed = renamedRightColumnName(name);
  if (const auto it = right_key_aliases.find(renamed); it != right_key_aliases.end()) return it->second;
  return renamed;
}

void TableJoin::setRename(const String& from, const String& to) { renames[from] = to; }
...
void TableJoin::addJoinCondition(const ASTPtr& ast, bool is_left) {
  ...
  LOG_TRACE(getLogger("TableJoin"), "Adding join condition for {} table: {} -> {}", (is_left ? "left" : "right"),
            ast ? ast->formatForLogging() : "NULL", cond_ast ? cond_ast->formatForLogging() : "NULL");
...
std::unordered_map<String, String> TableJoin::leftToRightKeyRemap() const {
  ...
  if (hasUsing()) {
    ...
    forAllKeys(clauses, [&](const auto& left_key_name, const auto& right_key_name) {
      if (!required_right_keys.contains(right_key_name)) left_to_right_key_remap[left_key_name] = right_key_name;
      return true;
    });
  }
...
void TableJoin::assertHasOneOnExpr() const {
  if (!oneDisjunct()) {
    ...
    for (const auto& onexpr : clauses) text.push_back(onexpr.formatDebug());
    throw DB::Exception(ErrorCodes::LOGICAL_ERROR, "Expected to have only one join clause, got {}: [{}], query: '{}'", clauses.size(),
                        fmt::join(text, " | "), table_join.formatForErrorMessage());
  }
}
...
bool TableJoin::isEnabledAlgorithm(const std::vector<JoinAlgorithm>& join_algorithms, JoinAlgorithm val) {
  ...
  if (join_algorithm_is_default && std::ranges::find(default_algorithms, val) != default_algorithms.end()) return true;
  return std::ranges::find(join_algorithms, val) != join_algorithms.end();
}

size_t TableJoin::getMaxMemoryUsage() const { return max_memory_usage; }
...
void TableJoin::assertEnableAnalyzer() const {
  if (!enable_analyzer) throw DB::Exception(ErrorCodes::NOT_IMPLEMENTED, "TableJoin: analyzer is disabled");
}

TemporaryDataOnDiskScopePtr TableJoin::getTempDataOnDisk() {
  if (!tmp_data) return nullptr;
  return tmp_data->childScope({.current_metric = CurrentMetrics::TemporaryFilesForJoin,
                              ...
                              temporary_files_buffer_size, temporary_files_codec);
}

bool allowParallelHashJoin(const std::vector<JoinAlgorithm>& join_algorithms, JoinKind kind, JoinStrictness strictness,
                           bool is_special_storage, bool one_disjunct) {
  if (std::ranges::none_of(join_algorithms, [](auto algo) { return algo == JoinAlgorithm::PARALLEL_HASH; })) return false;
  if (kind != JoinKind::Left && kind != JoinKind::Inner && kind != JoinKind::Right && kind != JoinKind::Full) return false;
  if (strictness == JoinStrictness::Asof) return false;
  if (is_special_storage || !one_disjunct) return false;
  return true;
}
}  // namespace DB
