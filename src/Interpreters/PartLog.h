...
class PartLog : public SystemLog<PartLogElement> {
 ...
 public:
  struct PartLogEntry {
    ...
    PartLogEntry(std::shared_ptr<IMergeTreeDataPart> part_, UInt64 elapsed_ns_) : part(std::move(part_)), elapsed_ns(elapsed_ns_) {}

    PartLogEntry(std::shared_ptr<IMergeTreeDataPart> part_, UInt64 elapsed_ns_, ProfileCountersSnapshotPtr profile_counters_)
        : part(std::move(part_)), profile_counters(std::move(profile_counters_)), elapsed_ns(elapsed_ns_) {}
  };

  using PartLogEntries = std::vector<PartLogEntry>;

  static PartLogEntries createPartLogEntries(const MutableDataPartsVector& parts, UInt64 elapsed_ns,
                                             ProfileCountersSnapshotPtr profile_counters = {});

  /// Add a record about creation of a new part.
  static bool addNewPart(ContextPtr context, const PartLogEntry& part, Strings deduplication_block_ids,
                         const ExecutionStatus& execution_status = {});

  static bool addNewParts(ContextPtr context, const PartLogEntries& parts, const ExecutionStatus& execution_status = {});

 private:
  static bool addNewPartsImpl(ContextPtr context, const PartLogEntries& parts, std::vector<Strings> deduplication_block_ids,
                              const ExecutionStatus& execution_status);
};
