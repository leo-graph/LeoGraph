...
class Cluster {
 public:
  Cluster(const Poco::Util::AbstractConfiguration &config, const Settings &settings, const String &config_prefix_,
  ...
  Cluster(const Settings &settings, const std::vector<std::vector<String>> &names, const ClusterConnectionParameters &params);

  Cluster(const Settings &settings, const std::vector<std::vector<DatabaseReplicaInfo>> &infos, const ClusterConnectionParameters &params);
  ...
  struct Address {
    ...
    Address(const Poco::Util::AbstractConfiguration &config, const String &config_prefix, const String &cluster_,
            const String &cluster_secret_, UInt32 shard_index_ = 0, UInt32 replica_index_ = 0);

    Address(const DatabaseReplicaInfo &info, const ClusterConnectionParameters &params, UInt32 shard_index_, UInt32 replica_index_);
  ...
  const ShardInfo &getAnyShardInfo() const {
    if (shards_info.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cluster is empty");
 ...
 private:
  ...
  void addShard(const Settings &settings, Addresses addresses, bool treat_local_as_remote, UInt32 current_shard_num,
                String current_shard_name = "", UInt32 weight = 1, ShardInfoInsertPathForInternalReplication insert_paths = {},
...
class Clusters {
 public:
  Clusters(const Poco::Util::AbstractConfiguration &config, const Settings &settings, MultiVersion<Macros>::Version macros,
  ...
  void updateClusters(const Poco::Util::AbstractConfiguration &new_config, const Settings &settings, const String &config_prefix,
                      Poco::Util::AbstractConfiguration *old_config = nullptr);
