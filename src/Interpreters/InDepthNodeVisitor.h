...
class InDepthNodeVisitor {
 public:
  ...
  explicit InDepthNodeVisitor(Data &data_, WriteBuffer *ostr_ = nullptr) : data(data_), visit_depth(0), ostr(ostr_) {}
 ...
 private:
  ...
  void visitImplMain(T &ast) {
    if constexpr (!_top_to_bottom) visitChildren<with_dump>(ast);
...
class OneTypeMatcher {
 public:
  ...
  static void visit(T &ast, Data &data) {
    if (auto *t = typeid_cast<TypeToVisit *>(ast.get())) data.visit(*t, ast);
  }
};
