...
struct KeepAggregateFunctionMatcher {
  ...
  static bool needChildVisit(const ASTPtr &node, const ASTPtr &) { return !(node->as<ASTFunction>()); }
  ...
  static void visit(ASTIdentifier &ident, Data &data) {
    ...
    if (!data.group_by_keys.contains(ident.getColumnName())) data.keep_aggregator = true;
  }

  static void visit(const ASTPtr &ast, Data &data) {
    if (data.keep_aggregator) return;
...
class SelectAggregateFunctionOfGroupByKeysMatcher {
 public:
  ...
  static bool needChildVisit(const ASTPtr &node, const ASTPtr &) {
    ...
    return !node->as<ASTSubquery>() && !node->as<ASTTableExpression>() && !node->as<ASTSelectWithUnionQuery>() && !node->as<ASTArrayJoin>();
  }

  static void visit(ASTPtr &ast, Data &data) {
    /// Check if function is min/max/any
    auto *function_node = ast->as<ASTFunction>();
    if (function_node && (function_node->name == "min" || function_node->name == "max" || function_node->name == "any" ||
                          function_node->name == "anyLast")) {
