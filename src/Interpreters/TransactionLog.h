...
class SingletonHelper : private boost::noncopyable {
 public:
  ...
  static void shutdownIfAny() {
    ...
    if (instance_holder) instance_holder->shutdown();
  }
...
///   - start_csn is the newest CSN that existed when the transaction was started and also it's snapshot that is visible for this
///   transaction
...
///   - host_id is persistent UUID of host that has started the transaction, it's kind of tie-breaker that makes ID unique across all
///   servers
///
