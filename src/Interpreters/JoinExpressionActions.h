...
class BitSet {
 ...
 public:
  ...
  BitSet &set(size_t pos, bool val = true) {
    if (pos >= bitset.size()) bitset.resize(pos + 1);
    bitset.set(pos, val);
    return *this;
  }
...
template <>
struct std::hash<DB::JoinActionRef> {
  size_t operator()(const DB::JoinActionRef &ref) const { return std::hash<const DB::ActionsDAG::Node *>()(ref.node_ptr); }
};

template <>
struct std::hash<DB::BitSet> {
  size_t operator()(const DB::BitSet &bs) const { return bs.hashImpl(); }
};
