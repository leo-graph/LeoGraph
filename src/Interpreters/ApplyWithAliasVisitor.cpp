...
void ApplyWithAliasVisitor::visit(ASTPtr &ast, const Data &data) {
  ...
  if (auto *node_select = ast->as<ASTSelectQuery>()) {
    ...
    if (auto with = node_select->with()) {
      ...
      for (const auto &with_alias : data.exprs) with_expression_list->children.push_back(with_alias.second->clone());
    ...
    for (auto &child : node_select->children) {
      if (child != node_select->with()) visit(child, new_data ? *new_data : data);
    }
  } else
    for (auto &child : ast->children) visit(child, data);
}
