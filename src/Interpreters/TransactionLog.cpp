...
static void tryWriteEventToSystemLog(LoggerPtr log, ContextPtr context, TransactionsInfoLogElement::Type type, const TransactionID &tid,
                                     CSN csn = Tx::UnknownCSN) try {
...
TransactionLog::~TransactionLog() { shutdown(); }
...
String TransactionLog::serializeCSN(CSN csn) { return zkutil::getSequentialNodeName("csn-", csn); }

TransactionID TransactionLog::deserializeTID(const String &csn_node_content) {
  TransactionID tid = Tx::EmptyTID;
  if (csn_node_content.empty()) return tid;
...
void TransactionLog::loadEntries(Strings::const_iterator beg, Strings::const_iterator end) {
  ...
  for (auto it = beg; it != end; ++it) entry_paths.emplace_back(fs::path(zookeeper_path_log) / *it);
  ...
  NOEXCEPT_SCOPE_STRICT({
    ...
    for (const auto &entry : loaded) {
      if (entry.first == Tx::EmptyTID.getHash()) continue;
...
void TransactionLog::loadLogFromZooKeeper() {
  ...
  if (code != Coordination::Error::ZOK) {
    ...
    ops.emplace_back(
        zkutil::makeCreateRequest(zookeeper_path + "/tail_ptr", serializeCSN(Tx::MaxReservedCSN), zkutil::CreateMode::Persistent));
    ...
    if (code != Coordination::Error::ZNODEEXISTS) zkutil::KeeperMultiException::check(code, ops, res);
  }
...
void TransactionLog::runUpdatingThread() {
  ...
  while (true) {
    try {
      ...
      if (TSA_READ_ONE_THREAD(unknown_state_list_loaded).empty()) log_updated_event->wait();
...
void TransactionLog::removeOldEntries() {
  ...
  if (!global_context->isServerCompletelyStarted()) return;

  /// Because `loadTableFromMetadataAsync` is running asynchronously, it is possible that the outdated parts are loading while the
  /// `tail_ptr` is updated here. It might trigger assertion when the part `create_csn` is lower than `tail_ptr`. Refer:
  /// https://github.com/ClickHouse/ClickHouse/issues/60406 We keep track of `asyncTablesLoadingJobNumber`, and not update `tail_ptr` if
  ...
  if (new_tail_ptr == old_tail_ptr) return;
  ...
  for (const auto &elem : tids) {
    ...
    if (new_tail_ptr <= elem.second.tid.start_csn) continue;

    /// Keep at least one node (the latest one we fetched)
    if (elem.second.csn == latest_entry_csn) continue;
    ...
    if (code == Coordination::Error::ZOK || code == Coordination::Error::ZNONODE) removed_entries.push_back(elem.first);
  }

  std::lock_guard lock{mutex};
  for (const auto &tid_hash : removed_entries) tid_to_csn.erase(tid_hash);
}
...
CSN TransactionLog::getLatestSnapshot() const { return latest_snapshot.load(); }
...
MergeTreeTransactionPtr TransactionLog::beginTransaction() {
  MergeTreeTransactionPtr txn;
  {
    ...
    if (!inserted) throw Exception(ErrorCodes::LOGICAL_ERROR, "I's a bug: TID {} {} exists", txn->tid.getHash(), txn->tid);
  }
...
CSN TransactionLog::commitTransaction(const MergeTreeTransactionPtr &txn, bool throw_on_unknown_status) {
  ...
  if (txn->isReadOnly()) {
    ...
    try {
      ...
      requests.push_back(
          zkutil::makeCreateRequest(zookeeper_path_log + "/csn-", serializeTID(txn->tid), zkutil::CreateMode::PersistentSequential));
      ...
      if (throw_on_unknown_status)
        throw Exception(ErrorCodes::UNKNOWN_STATUS_OF_TRANSACTION,
                        "Connection lost on attempt to commit transaction {}, will finalize it later: {}", txn->tid, e.message());
...
MergeTreeTransactionPtr TransactionLog::tryGetRunningTransaction(const TIDHash &tid) {
  ...
  if (it == running_list.end()) return NO_TRANSACTION_PTR;
  return it->second;
}
...
CSN TransactionLog::getCSN(const TransactionID &tid, const std::atomic<CSN> *failback_with_strict_load_csn) {
  ...
  if (tid == Tx::PrehistoricTID) return Tx::PrehistoricCSN;
...
CSN TransactionLog::getCSN(const TIDHash &tid, const std::atomic<CSN> *failback_with_strict_load_csn) {
  /// Avoid creation of the instance if transactions are not actually involved
  if (tid == Tx::PrehistoricTID.getHash()) return Tx::PrehistoricCSN;
...
CSN TransactionLog::getCSNImpl(const TIDHash &tid_hash, const std::atomic<CSN> *failback_with_strict_load_csn) const {
...

  {
    ...
    if (it != tid_to_csn.end()) return it->second.csn;
  }
  ...
  /// There is a race possible, transaction could be committed concurrently. Right before getCSN has been called. In that case tid_to_csn
  /// has no tid_hash but commit csn is set. In order to be sure, commit csn has to be loaded with memory_order_seq_cst after lookup at
  /// tid_to_csn
  if (failback_with_strict_load_csn)
    if (CSN maybe_csn = failback_with_strict_load_csn->load()) return maybe_csn;
...
CSN TransactionLog::getCSNAndAssert(const TransactionID &tid, std::atomic<CSN> &failback_with_strict_load_csn) {
  ...
  if (CSN maybe_csn = getCSN(tid)) return maybe_csn;
...
void TransactionLog::assertTIDIsNotOutdated(const TransactionID &tid, const std::atomic<CSN> *failback_with_strict_load_csn) {
  ...
  if (failback_with_strict_load_csn)
    if (CSN _ = failback_with_strict_load_csn->load()) return;

  throw Exception(ErrorCodes::LOGICAL_ERROR, "Trying to get CSN for too old TID {}, current tail_ptr is {}, probably it's a bug", tid,
                  tail);
}

CSN TransactionLog::getOldestSnapshot() const {
  std::lock_guard lock{running_list_mutex};
  if (snapshots_in_use.empty()) return getLatestSnapshot();
...
void TransactionLog::increaseAsyncTablesLoadingJobNumber() { async_tables_loading_job_number.fetch_add(1); }
void TransactionLog::decreaseAsyncTablesLoadingJobNumber() { async_tables_loading_job_number.fetch_sub(1); }
Int64 TransactionLog::asyncTablesLoadingJobNumber() { return async_tables_loading_job_number.load(); }
}  // namespace DB
