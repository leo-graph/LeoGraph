...
namespace {
ASTPtr processColumnTransformersImpl(const ColumnsDescription& columns, const NamesAndTypesList& virtual_columns, ASTPtr query_columns,
                                     const String& current_database, const StorageID& storage_id) {
  ...
  for (const auto& identifier : query_columns->children) source_columns_set.insert(identifier->getColumnName());
...
ASTPtr processColumnTransformers(const String& current_database, const StoragePtr& table, const StorageMetadataPtr& metadata_snapshot,
                                 ASTPtr query_columns) {
  return processColumnTransformersImpl(metadata_snapshot->columns, table->getVirtualsList(), query_columns, current_database,
                                       table->getStorageID());
}

ASTPtr processColumnTransformers(const String& current_database, const StorageID& table_id, const ColumnsDescription& columns,
                                 ASTPtr query_columns) {
  if (table_id.empty()) return processColumnTransformersImpl(columns, {}, query_columns, current_database, StorageID("", "dummy"));
