...
AsynchronousInsertQueue::InsertQuery::InsertQuery(const ASTPtr &query_, const std::optional<UUID> &user_id_,
                                                  const std::vector<UUID> &current_roles_, const Settings &settings_,
...
AsynchronousInsertQueue::InsertQuery &AsynchronousInsertQueue::InsertQuery::operator=(const InsertQuery &other) {
  if (this != &other) {
    query = other.query->clone();
    query_str = other.query_str;
    user_id = other.user_id;
...
bool AsynchronousInsertQueue::InsertQuery::operator==(const InsertQuery &other) const { return toTupleCmp() == other.toTupleCmp(); }

StorageID AsynchronousInsertQueue::InsertQuery::getStorageID() const {
  if (!query) throw Exception(ErrorCodes::LOGICAL_ERROR, "InsertQuery AST is null");

  auto *insert_query = query->as<ASTInsertQuery>();
  if (!insert_query) throw Exception(ErrorCodes::LOGICAL_ERROR, "AST is not type of ASTInsertQuery");

  return insert_query->table_id;
}

AsynchronousInsertQueue::InsertData::Entry::Entry(DataChunk &&chunk_, String &&query_id_, const String &async_dedup_token_,
                                                  const String &format_, MemoryTracker *user_memory_tracker_)
...
AsynchronousInsertQueue::QueueShardFlushTimeHistory::TimePoints AsynchronousInsertQueue::QueueShardFlushTimeHistory::getRecentTimePoints()
    const {
...
AsynchronousInsertQueue::AsynchronousInsertQueue(ContextPtr context_, size_t pool_size_, bool flush_on_shutdown_)
    : WithContext(context_),
      ...
      pool(CurrentMetrics::AsynchronousInsertThreads, CurrentMetrics::AsynchronousInsertThreadsActive,
           CurrentMetrics::AsynchronousInsertThreadsScheduled, pool_size) {
  if (!pool_size) throw Exception(ErrorCodes::BAD_ARGUMENTS, "pool_size cannot be zero");

  const auto &settings = getContext()->getSettingsRef();

  for (size_t i = 0; i < pool_size; ++i)
    queue_shards[i].busy_timeout_ms = std::min(Milliseconds(settings[Setting::async_insert_busy_timeout_min_ms].totalMilliseconds()),
                                               Milliseconds(settings[Setting::async_insert_busy_timeout_max_ms].totalMilliseconds()));

  for (size_t i = 0; i < pool_size; ++i) dump_by_first_update_threads.emplace_back([this, i] { processBatchDeadlines(i); });
}
  ...
  try {
    ...
    for (size_t i = 0; i < pool_size; ++i) {
      ...
      if (flush_on_shutdown) {
        for (auto &[_, elem] : shard.queue) scheduleDataProcessingJob(elem.key, std::move(elem.data), getContext(), i);
      } else {
        for (auto &[_, elem] : shard.queue)
          for (const auto &entry : elem.data->entries)
            entry->finish(std::make_exception_ptr(Exception(ErrorCodes::TIMEOUT_EXCEEDED, "Wait for async insert timeout exceeded)")));
      }
    }
...
AsynchronousInsertQueue::~AsynchronousInsertQueue() {
  for (const auto &shard : queue_shards) {
    for (const auto &[first_update, elem] : shard.queue) {
      ...
      LOG_WARNING(log, "Has unprocessed async insert for {}.{}", backQuoteIfNeed(insert_query.getDatabase()),
                  backQuoteIfNeed(insert_query.getTable()));
    }
  }
}
...
void AsynchronousInsertQueue::scheduleDataProcessingJob(const InsertQuery &key, InsertDataPtr data, ContextPtr global_context,
                                                        size_t shard_num, ThreadGroupPtr current_query_thread_group) {
  ...
  try {
    pool.scheduleOrThrowOnError(
        [this, key, global_context, current_query_thread_group, shard_num, my_data = data_shared]() mutable {
          processData(key, std::move(*my_data), std::move(global_context), std::move(current_query_thread_group),
                      flush_time_history_per_queue_shard[shard_num]);
        },
        priority);
  } catch (...) {
    for (auto &entry : (**data_shared).entries) entry->finish(std::current_exception());
  }
}
...
void AsynchronousInsertQueue::preprocessInsertQuery(const ASTPtr &query, const ContextPtr &query_context) {
  ...
  InterpreterInsertQuery interpreter(query, insert_context, query_context->getSettingsRef()[Setting::insert_allow_materialized_columns],
                                     /* no_squash */ false,
  ...
  if (insert_query.table_id) query_context->checkAccess(AccessType::INSERT, insert_query.table_id, sample_block.getNames());

  insert_query.columns = make_intrusive<ASTExpressionList>();
  for (const auto &column : sample_block) insert_query.columns->children.push_back(make_intrusive<ASTIdentifier>(column.name));
}

AsynchronousInsertQueue::PushResult AsynchronousInsertQueue::pushQueryWithInlinedData(ASTPtr query, ContextPtr query_context) {
  {
    ...
    if (auto *insert_query = copy_ptr->as<ASTInsertQuery>()) {
      if (insert_query->tail) insert_query->tail.reset();
    }
  }
  ...
  StringWithMemoryTracking bytes;
  {
    ...
    LimitReadBuffer limit_buf(*read_buf, {.read_no_more = query_context->getSettingsRef()[Setting::async_insert_max_data_size]});

    if (const auto *insert_query = query->as<ASTInsertQuery>()) {
      size_t expected_data_size = 0;
      if (insert_query->data) expected_data_size += insert_query->end - insert_query->data;
      if (insert_query->tail) expected_data_size += insert_query->tail->buffer().size();
...
std::vector<std::string> AsynchronousInsertQueue::getInsertQueryIds(InsertData &data) {
  ...
  for (const auto &entry : data.entries) query_ids.push_back(entry->query_id);
  return query_ids;
}
...
AsynchronousInsertQueue::PushResult AsynchronousInsertQueue::pushDataChunk(ASTPtr query, DataChunk &&chunk, ContextPtr query_context) {
  ...
  auto entry = std::make_shared<InsertData::Entry>(std::move(chunk), query_context->getCurrentQueryId(),
                                                   settings[Setting::insert_deduplication_token], insert_query.format,
  ...
  if (data_kind == AsynchronousInsertQueueDataKind::Preprocessed) insert_query.format = "Native";

  /// Query parameters make sense only for format Values.
  if (insert_query.format == "Values") entry->query_parameters = query_context->getQueryParameters();
  ...
  const auto flush_time_points = flush_time_history_per_queue_shard[shard_num].getRecentTimePoints();
  {
    ...
    if (timeout_ms != shard.busy_timeout_ms) {
      LOG_TRACE(log, "Asynchronous timeout {} from {} to {} for queue shard {}.",
                timeout_ms < shard.busy_timeout_ms ? "decreased" : "increased", shard.busy_timeout_ms.count(), timeout_ms.count(),
                shard_num);
    }

    if (inserted) it->second = shard.queue.emplace(now + timeout_ms, Container{key, std::make_unique<InsertData>(timeout_ms)}).first;
    ...
    auto max_busy_timeout_exceeded = [&shard, &settings, &now, &flush_time_points]() -> bool {
      if (!settings[Setting::async_insert_use_adaptive_busy_timeout] || !shard.last_insert_time || !flush_time_points.first) return false;
    ...
    if (data_to_process) {
      if (!inserted) CurrentMetrics::sub(CurrentMetrics::AsynchronousInsertQueueSize);
      CurrentMetrics::sub(CurrentMetrics::AsynchronousInsertQueueBytes, size_in_bytes);
    } else {
      if (inserted) CurrentMetrics::add(CurrentMetrics::AsynchronousInsertQueueSize);
...
AsynchronousInsertQueue::Milliseconds AsynchronousInsertQueue::getBusyWaitTimeoutMs(
    const Settings &settings, const QueueShard &shard, const QueueShardFlushTimeHistory::TimePoints &flush_time_points,
  ...
  const auto min_ms =
      std::min(std::max(Milliseconds(settings[Setting::async_insert_busy_timeout_min_ms].totalMilliseconds()), Milliseconds(1)), max_ms);

  auto normalize = [&min_ms, &max_ms](const auto &t_ms) { return std::min(std::max(t_ms, min_ms), max_ms); };

  if (!shard.last_insert_time || !flush_time_points.first) return normalize(shard.busy_timeout_ms);
  ...
  const auto decreased_timeout_ms = std::min(std::chrono::duration_cast<Milliseconds>(shard.busy_timeout_ms / (1.0 + decrease_rate)),
                                             shard.busy_timeout_ms - Milliseconds(1));

  /// Increase the timeout for frequent inserts.
  if (last_insert_time + min_ms > now) {
    auto timeout_ms = std::max(std::chrono::duration_cast<Milliseconds>(shard.busy_timeout_ms * (1.0 + increase_rate)),
                               shard.busy_timeout_ms + Milliseconds(1));
  ...
  if (last_insert_time + decreased_timeout_ms < now && t1 + decreased_timeout_ms < t2) return normalize(decreased_timeout_ms);
...
void AsynchronousInsertQueue::validateSettings(const Settings &settings, LoggerPtr log) {
  ...
  if (!settings[Setting::async_insert_use_adaptive_busy_timeout]) return;
...
void AsynchronousInsertQueue::flush(const std::vector<StorageID> &tables) {
...

  {
    ...
    LOG_DEBUG(log, "Will wait for finishing of {} flushing jobs (about {} inserts, {} bytes, {} distinct queries) for tables: {}",
  ...
  for (auto &future : futures_to_wait) future.wait();
...
void AsynchronousInsertQueue::flushAll() {
  ...
  LOG_DEBUG(log, "Will wait for finishing of {} flushing jobs (about {} inserts, {} bytes, {} distinct queries)", pool.active(),
            total_entries, total_bytes, total_queries);
...
void AsynchronousInsertQueue::processBatchDeadlines(size_t shard_num) {
  ...
  while (!shutdown) {
    std::vector<Container> entries_to_flush;
    {
      ...
      const auto rel_time = std::min(
          shard.busy_timeout_ms, Milliseconds(getContext()->getSettingsRef()[Setting::async_insert_poll_timeout_ms].totalMilliseconds()));
      shard.are_tasks_available.wait_for(lock, rel_time, [&shard, this] {
        if (shutdown) return true;

        if (!shard.queue.empty() && shard.queue.begin()->first < std::chrono::steady_clock::now()) return true;
      ...
      if (flush_stopped) continue;

      const auto now = std::chrono::steady_clock::now();

      size_t size_in_bytes = 0;
      while (true) {
        if (shard.queue.empty() || shard.queue.begin()->first > now) break;
    ...
    for (auto &entry : entries_to_flush) scheduleDataProcessingJob(entry.key, std::move(entry.data), getContext(), shard_num);
  }
}
...
void appendElementsToLogSafe(AsynchronousInsertLog &log, std::vector<AsynchronousInsertLogElement> elements, TimePoint flush_time,
                             const String &flush_exception) try {
...
void convertBlockToHeader(Block &block, const Block &header, const ContextPtr &context) {
  auto converting_dag = ActionsDAG::makeConvertingActions(block.getColumnsWithTypeAndName(), header.getColumnsWithTypeAndName(),
...
String serializeQuery(const IAST &query, size_t max_length) {
  return query.hasSecretParts() ? query.formatForLogging(max_length)
...
void AsynchronousInsertQueue::processData(InsertQuery key, InsertDataPtr data, ContextPtr global_context,
                                          ThreadGroupPtr current_query_thread_group,
                                          QueueShardFlushTimeHistory &queue_shard_flush_time_history) try {
  if (!data) return;
  ...
  auto process_list_entry = insert_context->getProcessList().insert(query_for_logging, normalized_query_hash, key.query.get(),
                                                                    insert_context, start_watch.getStart(), internal);
  ...
  if (async_insert_log) log_elements.reserve(data->entries.size());

  auto add_entry_to_asynchronous_insert_log = [&, query_by_format = NameToNameMap{}](const InsertData::EntryPtr &entry,
                                                                                     const String &parsing_exception, size_t num_rows,
                                                                                     size_t num_bytes) mutable {
    if (!async_insert_log) return;
  ...
  try {
    interpreter = std::make_unique<InterpreterInsertQuery>(key.query, insert_context,
                                                           (*key.settings)[Setting::insert_allow_materialized_columns], false, false, true);

    pipeline = interpreter->execute().pipeline;
    chassert(pipeline.pushing());

    query_log_elem = logQueryStart(query_start_time, insert_context, query_for_logging, normalized_query_hash, key.query, pipeline,
                                   interpreter.get(), internal, query_database, query_table, async_insert);
  } catch (...) {
    logExceptionBeforeStart(query_for_logging, normalized_query_hash, insert_context, key.query, query_span,
                            start_watch.elapsedMilliseconds(), internal);
  ...
  auto finish_entries = [&](QueryPipeline &&pileline_, size_t num_rows, size_t num_bytes) {
    ...
    LOG_DEBUG(log, "Asynchronous insert query logQueryFinish query_kind '{}', 'query_id {}'", query_log_elem.query_kind,
              query_log_elem.client_info.current_query_id);
    logQueryFinish(query_log_elem, insert_context, key.query, std::move(pileline_), /*pulling_pipeline=*/false, query_span,
                   QueryResultCacheUsage::None, internal);
    ...
    for (const auto &entry : data->entries) {
      if (!entry->isFinished()) entry->finish();
    }
  };
...
Chunk AsynchronousInsertQueue::processEntriesWithParsing(const InsertQuery &key, const InsertDataPtr &data, const Block &header,
                                                         const ContextPtr &insert_context, LoggerPtr logger,
  ...
  auto on_error = [&](const MutableColumns &result_columns, const ColumnCheckpoints &checkpoints, Exception &e) {
    ...
    LOG_ERROR(logger, "Failed parsing for insert query id {}. {}", current_entry->query_id, current_exception);

    for (size_t i = 0; i < result_columns.size(); ++i) result_columns[i]->rollback(*checkpoints[i]);

    current_entry->finish(std::current_exception());
    return 0;
  };

  StreamingFormatExecutor executor(header, format, std::move(on_error), data->size_in_bytes, data->entries.size(),
                                   std::move(adding_defaults_transform));

  auto deduplication_info = DeduplicationInfo::create(
      /*async_insert=*/true, insert_context->getServerSettings()[ServerSetting::insert_deduplication_version].value);

  for (const auto &entry : data->entries) {
    current_entry = entry;

    const auto *bytes = entry->chunk.asString();
    if (!bytes) throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected entry with data kind Parsed. Got: {}", entry->chunk.getDataKind());
...
template <typename LogFunc>
Chunk AsynchronousInsertQueue::processPreprocessedEntries(const InsertDataPtr &data, const Block &header, const ContextPtr &context_,
                                                          LoggerPtr logger, LogFunc &&add_to_async_insert_log) {
  size_t total_rows = 0;
  auto deduplication_info = DeduplicationInfo::create(
      /*async_insert=*/true, context_->getServerSettings()[ServerSetting::insert_deduplication_version].value);
  auto result_columns = header.cloneEmptyColumns();

  for (const auto &entry : data->entries) {
    const auto *block = entry->chunk.asBlock();
    if (!block)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected entry with data kind Preprocessed. Got: {}", entry->chunk.getDataKind());
    ...
    if (!isCompatibleHeader(block_to_insert, header)) convertBlockToHeader(block_to_insert, header, context_);

    auto columns = block_to_insert.getColumns();
    for (size_t i = 0, s = columns.size(); i < s; ++i) result_columns[i]->insertRangeFrom(*columns[i], 0, columns[i]->size());
...
template <typename E>
void AsynchronousInsertQueue::finishWithException(const ASTPtr &query, const std::list<InsertData::EntryPtr> &entries, const E &exception) {
