...
Block InterpreterExistsQuery::getSampleBlock() { return Block{{ColumnUInt8::create(), std::make_shared<DataTypeUInt8>(), "result"}}; }
...
QueryPipeline InterpreterExistsQuery::executeImpl() {
  ...
  if ((exists_query = query_ptr->as<ASTExistsTableQuery>())) {
    if (exists_query->isTemporary()) {
      result = static_cast<bool>(getContext()->tryResolveStorageID({"", exists_query->getTable()}, Context::ResolveExternal));
    } else {
    ...
    if (exists_query->isTemporary()) {
      auto storage_id = getContext()->tryResolveStorageID({"", exists_query->getTable()}, Context::ResolveExternal);
      if (storage_id) {
    ...
    if (exists_query->isTemporary()) throw Exception(ErrorCodes::SYNTAX_ERROR, "Temporary dictionaries are not possible.");
    String database = getContext()->resolveDatabase(exists_query->getDatabase());
  ...
  return QueryPipeline(std::make_shared<SourceFromSingleChunk>(
      std::make_shared<const Block>(Block{{ColumnUInt8::create(1, result), std::make_shared<DataTypeUInt8>(), "result"}})));
}
