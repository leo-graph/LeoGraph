...
bool expressionEquals(const ASTPtr& lhs, const ASTPtr& rhs, const Aliases& alias) {
  if (lhs->getTreeHash(/*ignore_aliases=*/true) == rhs->getTreeHash(/*ignore_aliases=*/true)) return true;

  auto* lhs_idf = lhs->as<ASTIdentifier>();
  auto* rhs_idf = rhs->as<ASTIdentifier>();
  if (lhs_idf && rhs_idf) {
    /// compound identifiers, such as: <t.name, name>
    if (lhs_idf->shortName() == rhs_idf->shortName()) return true;

    /// translate alias
    if (alias.contains(lhs_idf->shortName())) lhs_idf = alias.find(lhs_idf->shortName())->second->as<ASTIdentifier>();

    if (alias.contains(rhs_idf->shortName())) rhs_idf = alias.find(rhs_idf->shortName())->second->as<ASTIdentifier>();

    if (lhs_idf && rhs_idf && lhs_idf->shortName() == rhs_idf->shortName()) return true;
  }
...
bool expressionListEquals(ASTExpressionList* lhs, ASTExpressionList* rhs, const Aliases& alias) {
  ...
  for (size_t i = 0; i < lhs->children.size(); i++) {
    if (!expressionEquals(lhs->children[i], rhs->children[i], alias)) return false;
  }
  return true;
}
...
bool expressionListContainsAll(ASTExpressionList* lhs, ASTExpressionList* rhs, const Aliases& alias) {
  ...
  for (const auto& re : rhs->children) {
    ...
    if (std::find_if(lhs->children.begin(), lhs->children.end(), predicate) == lhs->children.end()) return false;
  }
  return true;
}
...
void RewriteUniqToCountMatcher::visit(ASTPtr& ast, Data& /*data*/) {
  ...
  if (!selectq || !selectq->tables() || selectq->tables()->children.size() != 1) return;
  auto expr_list = selectq->select();
  if (!expr_list || expr_list->children.size() != 1) return;
  auto* func = expr_list->children[0]->as<ASTFunction>();
  if (!func || !matchFnUniq(func->name)) return;
  if (selectq->tables()->as<ASTTablesInSelectQuery>()->children[0]->as<ASTTablesInSelectQueryElement>()->children.size() != 1) return;
  ...
  if (!table_expr || table_expr->children.size() != 1 || !table_expr->subquery) return;
  auto* subquery = table_expr->subquery->as<ASTSubquery>();
  if (!subquery) return;
  auto* sub_selectq =
      subquery->children[0]->as<ASTSelectWithUnionQuery>()->children[0]->as<ASTExpressionList>()->children[0]->as<ASTSelectQuery>();
  if (!sub_selectq) return;
  auto sub_expr_list = sub_selectq->select();
  if (!sub_expr_list) return;

  /// collect subquery select expressions alias
  Aliases alias;
  for (const auto& expr : sub_expr_list->children) {
    if (!expr->tryGetAlias().empty()) alias.insert({expr->tryGetAlias(), expr});
  }

  /// Whether query matches 'SELECT uniq(x ...) FROM (SELECT DISTINCT x ...)'
  auto match_subquery_with_distinct = [&]() -> bool {
    if (!sub_selectq->distinct) return false;
    /// uniq expression list == subquery group by expression list
    if (!expressionListEquals(func->children[0]->as<ASTExpressionList>(), sub_expr_list->as<ASTExpressionList>(), alias)) return false;
    return true;
  };
  ...
  auto match_subquery_with_group_by = [&]() -> bool {
    ...
    /// uniq expression list == subquery group by expression list
    if (!expressionListEquals(func->children[0]->as<ASTExpressionList>(), group_by->as<ASTExpressionList>(), alias)) return false;
    /// subquery select expression list must contain all columns in uniq expression list
    if (!expressionListContainsAll(sub_expr_list->as<ASTExpressionList>(), func->children[0]->as<ASTExpressionList>(), alias)) return false;
    return true;
  };
