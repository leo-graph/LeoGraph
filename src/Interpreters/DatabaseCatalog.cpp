...
class DatabaseNameHints : public IHints<> {
 public:
  explicit DatabaseNameHints(const DatabaseCatalog &database_catalog_) : database_catalog(database_catalog_) {}
  ...
  Names getAllRegisteredNames() const override {
    ...
    for (const auto &database_name : databases_list | boost::adaptors::map_keys) {
      if (need_to_check_access_for_databases && !access->isGranted(AccessType::SHOW_DATABASES, database_name)) continue;

      if (database_name == DatabaseCatalog::TEMPORARY_DATABASE) continue;
...
TemporaryTableHolder::TemporaryTableHolder(ContextPtr context_, const ColumnsDescription &columns,
                                           const ConstraintsDescription &constraints, const ASTPtr &query, bool create_for_global_subquery)
    : TemporaryTableHolder(
          context_,
          [&](const StorageID &table_id) {
            auto storage = std::make_shared<StorageMemory>(table_id, ColumnsDescription{columns}, ConstraintsDescription{constraints},
                                                           String{}, MemorySettings{});

            if (create_for_global_subquery) storage->delayReadForGlobalSubqueries();
...
StoragePtr TemporaryTableHolder::getTable() const {
  ...
  if (!table) throw Exception(ErrorCodes::LOGICAL_ERROR, "Temporary table {} not found", getGlobalTableID().getNameForLogs());
  return table;
}
...
void DatabaseCatalog::createBackgroundTasks() {
  ...
  auto drop_task_holder = getContext()->getSchedulePool().createTask(StorageID::createEmpty(), "DatabaseCatalogDropTableTask",
                                                                     [this]() { this->dropTableDataTask(); });
  drop_task = std::make_unique<BackgroundSchedulePoolTaskHolder>(std::move(drop_task_holder));

  auto reload_disks_task_holder = getContext()->getSchedulePool().createTask(StorageID::createEmpty(), "DatabaseCatalogReloadDisksTask",
                                                                             [this]() { this->reloadDisksTask(); });
...
void DatabaseCatalog::startupBackgroundTasks() {
  ...
  if (!tables_marked_dropped.empty()) (*drop_task)->schedule();
}
...
void DatabaseCatalog::shutdownImpl(std::function<void()> shutdown_system_logs) {
  ...
  if (cleanup_task) (*cleanup_task)->deactivate();

  if (drop_task) (*drop_task)->deactivate();
  ...
  for (const auto &db : databases) {
    ...
    if (db_uuid != UUIDHelpers::Nil) removeUUIDMapping(db_uuid);
  }
...
DatabaseAndTable DatabaseCatalog::getTableImpl(const StorageID &table_id, ContextPtr context_, std::optional<Exception> *exception) const {
  ...
  if (!table_id) {
    if (exception) exception->emplace(Exception(ErrorCodes::UNKNOWN_TABLE, "Cannot find table: StorageID is empty"));
    return {};
  }
  ...
  if (table_id.hasUUID()) {
    ...
    if (!db_and_table.first || !db_and_table.second) {
      ...
      if (exception) {
        ...
        if (names.empty())
          ...
          exception->emplace(Exception(ErrorCodes::UNKNOWN_TABLE, "Table {} does not exist. Maybe you meant {}?", table_id.getNameForLogs(),
    ...
    /// In old analyzer resolving done in multiple places, so we ignore TABLE_UUID_MISMATCH error.
      ...
      if (db_and_table.first->getDatabaseName() != table_id.database_name ||
        ...
        if (exception)
          exception->emplace(Exception(ErrorCodes::TABLE_UUID_MISMATCH, "Table {} does not match {}", table_id.getNameForLogs(),
                                       table_storage_id.getNameForLogs()));
        return {};
      }
    }
    ...
    if (!context_->isInternalQuery() && (db_and_table.first->getEngineName() == "MaterializedPostgreSQL")) {
      db_and_table.second = std::make_shared<StorageMaterializedPostgreSQL>(
          std::move(db_and_table.second), getContext(),
  ...
  if (table_id.database_name == TEMPORARY_DATABASE) {
    ...
    if (exception)
      exception->emplace(
          Exception(ErrorCodes::DATABASE_ACCESS_DENIED, "Direct access to `{}` database is not allowed", TEMPORARY_DATABASE));
    return {};
  }
  ...
  DatabasePtr database;
  {
    ...
    if (!table_id.hasDatabase()) {
      if (exception) exception->emplace(Exception(ErrorCodes::UNKNOWN_DATABASE, "Empty database name"));
      return {};
    }

    std::lock_guard lock{databases_mutex};
    auto it = databases.find(table_id.getDatabaseName());
    if (databases.end() != it) database = it->second;
  }
  ...
  if (!database) {
    if (exception) {
      ...
      if (names.empty()) {
        exception->emplace(
            Exception(ErrorCodes::UNKNOWN_DATABASE, "Database {} does not exist", backQuoteIfNeed(table_id.getDatabaseName())));
      } else {
        exception->emplace(Exception(ErrorCodes::UNKNOWN_DATABASE, "Database {} does not exist. Maybe you meant {}?",
                                     backQuoteIfNeed(table_id.getDatabaseName()), backQuoteIfNeed(names[0])));
      }
    }
    return {};
  }
...
bool DatabaseCatalog::isPredefinedTable(const StorageID &table_id) const {
  ...
  if (table_id.hasUUID()) {
    if (auto storage = tryGetByUUID(table_id.uuid).second) {
      if (storage->isSystemStorage()) return true;
      auto res_id = storage->getStorageID();
      String database_name = res_id.getDatabaseName();
      if (database_name !=
          SYSTEM_DATABASE)  /// If (database_name == SYSTEM_DATABASE) then we have already checked it (see isSystemStorage() above).
...
void DatabaseCatalog::assertDatabaseExists(const String &database_name) const {
  if (database_name.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Database name cannot be empty");

  DatabasePtr db;
  {
    std::lock_guard lock{databases_mutex};
    if (auto it = databases.find(database_name); it != databases.end()) db = it->second;
  }
  if (!db) {
    DatabaseNameHints hints(*this);
    ...
    throw Exception(ErrorCodes::UNKNOWN_DATABASE, "Database {} does not exist. Maybe you meant {}?", backQuoteIfNeed(database_name),
                    backQuoteIfNeed(names[0]));
  }
}
...
void DatabaseCatalog::attachDatabase(const String &database_name, const DatabasePtr &database) {
  ...
  if (!database->isDatalakeCatalog()) databases_without_datalake_catalogs.emplace(database_name, database);

  NOEXCEPT_SCOPE({
    UUID db_uuid = database->getUUID();
    if (db_uuid != UUIDHelpers::Nil) addUUIDMapping(db_uuid, database, nullptr);
  });
}
...
DatabasePtr DatabaseCatalog::detachDatabase(ContextPtr local_context, const String &database_name, bool drop, bool check_empty) {
  ...
  if (check_empty) {
    try {
      ...
      if (!drop) db->assertCanBeDetached(false);
    } catch (...) {
  ...
  if (drop) {
    ...
    if (db_uuid != UUIDHelpers::Nil) removeUUIDMappingFinally(db_uuid);
  }
  return db;
}
...
void DatabaseCatalog::updateMetadataFile(const String &database_name, const ASTPtr &create_query) {
  ...
  writeMetadataFile(default_db_disk,
                    ...
                    /*content=*/statement, getContext()->getSettingsRef()[Setting::fsync_metadata]);
...
DatabasePtr DatabaseCatalog::tryGetDatabase(std::string_view database_name) const {
  ...
  if (it == databases.end()) return {};
  return it->second;
}

DatabasePtr DatabaseCatalog::getDatabase(const UUID &uuid) const {
  auto db_and_table = tryGetByUUID(uuid);
  if (!db_and_table.first || db_and_table.second) throw Exception(ErrorCodes::UNKNOWN_DATABASE, "Database UUID {} does not exist", uuid);
...
DatabasePtr DatabaseCatalog::tryGetDatabase(const UUID &uuid) const {
  ...
  auto db_and_table = tryGetByUUID(uuid);
  if (!db_and_table.first || db_and_table.second) return {};
  return db_and_table.first;
}
...
Databases DatabaseCatalog::getDatabases(GetDatabasesOptions options) const {
  std::lock_guard lock{databases_mutex};
  if (options.with_datalake_catalogs) return databases;
...
bool DatabaseCatalog::isTableExist(const DB::StorageID &table_id, ContextPtr context_) const {
  if (table_id.hasUUID()) return tryGetByUUID(table_id.uuid).second != nullptr;

  DatabasePtr db;
  {
    std::lock_guard lock{databases_mutex};
    auto iter = databases.find(table_id.database_name);
    if (iter != databases.end()) db = iter->second;
  }
...
void DatabaseCatalog::assertTableDoesntExist(const StorageID &table_id, ContextPtr context_) const {
  if (isTableExist(table_id, context_)) throw Exception(ErrorCodes::TABLE_ALREADY_EXISTS, "Table {} already exists.", table_id);
}

DatabasePtr DatabaseCatalog::getDatabaseForTemporaryTables() const { return getDatabase(TEMPORARY_DATABASE); }

DatabasePtr DatabaseCatalog::getSystemDatabase() const { return getDatabase(SYSTEM_DATABASE); }

void DatabaseCatalog::addUUIDMapping(const UUID &uuid) { addUUIDMapping(uuid, nullptr, nullptr); }
...
void DatabaseCatalog::addUUIDMapping(const UUID &uuid, const DatabasePtr &database, const StoragePtr &table) {
  ...
  if (database || table) throw Exception(ErrorCodes::LOGICAL_ERROR, "Mapping for table with UUID={} already exists", uuid);
  ...
  throw Exception(ErrorCodes::TABLE_ALREADY_EXISTS,
                  "Mapping for table with UUID={} already exists. It happened due to UUID collision, "
                  "most likely because some not random UUIDs were manually specified in CREATE queries.",
                  uuid);
}
...
void DatabaseCatalog::removeUUIDMapping(const UUID &uuid) {
  ...
  std::lock_guard lock{map_part.mutex};
  auto it = map_part.map.find(uuid);
  if (it == map_part.map.end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Mapping for table with UUID={} doesn't exist", uuid);
...
void DatabaseCatalog::removeUUIDMappingFinally(const UUID &uuid) {
  ...
  UUIDToStorageMapPart &map_part = uuid_map[getFirstLevelIdx(uuid)];
  std::lock_guard lock{map_part.mutex};
  if (!map_part.map.erase(uuid)) throw Exception(ErrorCodes::LOGICAL_ERROR, "Mapping for table with UUID={} doesn't exist", uuid);
}
...
void DatabaseCatalog::removeViewDependency(const StorageID &source_table_id, const StorageID &view_id) {
  std::lock_guard lock{databases_mutex};
  view_dependencies.removeDependency(StorageID{source_table_id.getQualifiedName()}, StorageID{view_id.getQualifiedName()},
...
DDLGuardPtr DatabaseCatalog::getDDLGuard(const String &database, const String &table, const IDatabase *expected_database) {
  if (database.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot obtain lock for empty database");
...
StoragePtr DatabaseCatalog::getTable(const StorageID &table_id, ContextPtr local_context) const {
  ...
  auto table = local_context->hasQueryContext()
                   ? local_context->getQueryContext()
                         ->getOrCacheStorage(table_id, [&]() { return getTableImpl(table_id, local_context, &exc); })
                         .second
                   : getTableImpl(table_id, local_context, &exc).second;
  if (!table) throw Exception(*exc);
  return table;
}
...
StoragePtr DatabaseCatalog::tryGetTable(const StorageID &table_id, ContextPtr local_context) const {
  return local_context->hasQueryContext()
             ? local_context->getQueryContext()
                   ->getOrCacheStorage(table_id, [&]() { return getTableImpl(table_id, local_context, nullptr); })
                   .second
...
DatabaseAndTable DatabaseCatalog::getDatabaseAndTable(const StorageID &table_id, ContextPtr local_context) const {
  std::optional<Exception> exc;
  auto res = local_context->hasQueryContext() ? local_context->getQueryContext()->getOrCacheStorage(
                                                    table_id, [&]() { return getTableImpl(table_id, local_context, &exc); })
                                              : getTableImpl(table_id, local_context, &exc);
  if (!res.second) throw Exception(*exc);
  return res;
}

DatabaseAndTable DatabaseCatalog::tryGetDatabaseAndTable(const StorageID &table_id, ContextPtr local_context) const {
  return local_context->hasQueryContext() ? local_context->getQueryContext()->getOrCacheStorage(
                                                table_id, [&]() { return getTableImpl(table_id, local_context, nullptr); })
...
void DatabaseCatalog::loadMarkedAsDroppedTables() {
  ...
  for (const auto &[_, db] : db_map) {
    ...
    if (!db_disk->existsDirectory(path)) continue;
  ...
  for (const auto &db_disk : metadata_disk_list) {
    for (const auto it = db_disk->iterateDirectory(path); it->isValid(); it->next()) {
      ...
      if (!sub_path_filename.ends_with(".sql")) continue;
  ...
  if (dropped_metadata.empty()) return;
  ...
  for (const auto &elem : dropped_metadata) {
    ...
    runner.enqueueAndKeepTrack(
        [this, full_path, storage_id, db_disk]() { this->enqueueDroppedTableCleanup(storage_id, nullptr, db_disk, full_path); });
  }
  runner.waitForAllToFinishAndRethrowFirstError();
}

String DatabaseCatalog::getPathForDroppedMetadata(const StorageID &table_id) const {
  return fs::path("metadata_dropped") / fmt::format("{}.{}.{}.sql", escapeForFileName(table_id.getDatabaseName()),
                                                    escapeForFileName(table_id.getTableName()), toString(table_id.uuid));
}
...
String DatabaseCatalog::getPathForMetadata(const StorageID &table_id) const {
  ...
  if (!database_ptr) throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed to get metadata path from database {}", table_id.getDatabaseName());

  auto metadata_path = database_ptr->getMetadataPath();
  if (metadata_path.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Empty metadata path in database {}", table_id.getDatabaseName());

  return metadata_path + escapeForFileName(table_id.getTableName()) + ".sql";
}

void DatabaseCatalog::enqueueDroppedTableCleanup(StorageID table_id, StoragePtr table, DiskPtr db_disk, String dropped_metadata_path,
                                                 bool ignore_delay) {
  ...
  if (table) {
    ...
    ASTPtr ast = DatabaseOnDisk::parseQueryFromMetadata(log, getContext(), db_disk, dropped_metadata_path, /*throw_on_error*/ false,
                                                        /*remove_empty*/ false);
    ...
    if (create) {
      ...
      try {
        table =
            createTableFromAST(*create, table_id.getDatabaseName(), data_path, getContext(), LoadingStrictnessLevel::FORCE_RESTORE).second;
        table->is_dropped = true;
      } catch (...) {
        tryLogCurrentException(log, "Cannot load partially dropped table " + table_id.getNameForLogs() + " from: " + dropped_metadata_path +
                                        ". Parsed query: " + create->formatForLogging() + ". Will remove metadata and " + data_path +
                                        ". Garbage may be left in ZooKeeper.");
      }
    } else {
      LOG_WARNING(log,
                  "Cannot parse metadata of partially dropped table {} from {}. Will remove metadata file and data directory. Garbage may "
                  "be left in /store directory and ZooKeeper.",
                  table_id.getNameForLogs(), dropped_metadata_path);
    }
  ...
  if (ignore_delay) {
    ...
    tables_marked_dropped.emplace(first_async_drop_in_queue,
                                  TableMarkedAsDropped{table_id, table, db_disk, dropped_metadata_path, drop_time});
  } else {
    tables_marked_dropped.push_back(
        {table_id, table, db_disk, dropped_metadata_path,
         drop_time + static_cast<time_t>(getContext()->getServerSettings()[ServerSetting::database_atomic_delay_before_drop_table_sec])});
    if (first_async_drop_in_queue == tables_marked_dropped.end()) --first_async_drop_in_queue;
  }
  ...
  if (drop_task && (tables_marked_dropped.size() == 1 || ignore_delay)) (*drop_task)->schedule();
}
...
void DatabaseCatalog::undropTable(StorageID table_id) {
  ...
  TableMarkedAsDropped dropped_table;
  {
    ...
    for (auto it = tables_marked_dropped.begin(); it != tables_marked_dropped.end(); ++it) {
      ...
      if (table_id.uuid != UUIDHelpers::Nil) continue;
    ...
    if (first_async_drop_in_queue == it_dropped_table) ++first_async_drop_in_queue;
  ...
  LOG_INFO(log, "Attaching undropped table {} (metadata moved from {})", dropped_table.table_id.getNameForLogs(),
  ...
  while (!isSharedPtrUnique(dropped_table.table)) std::this_thread::sleep_for(std::chrono::milliseconds(100));
...
std::vector<DatabaseCatalog::TablesMarkedAsDropped::iterator> DatabaseCatalog::getTablesToDrop() {
  ...
  for (auto it = tables_marked_dropped.begin(); it != tables_marked_dropped.end(); ++it) {
    ...
    if (!in_use && old_enough) result.emplace_back(it);
  }
...
void DatabaseCatalog::rescheduleDropTableTask() {
  ...
  if (tables_marked_dropped.empty()) return;
...
void DatabaseCatalog::dropTablesParallel(std::vector<DatabaseCatalog::TablesMarkedAsDropped::iterator> tables_to_drop) {
  if (tables_to_drop.empty()) return;
    ...
    auto job = [this, table_iterator = item]() {
      try {
        ...
        TableMarkedAsDropped table_to_delete_without_lock;
        {
          ...
          if (first_async_drop_in_queue == table_iterator) ++first_async_drop_in_queue;
        ...
        tryLogCurrentException(log, "Cannot drop table " + table_iterator->table_id.getNameForLogs() + ". Will retry later.");
        {
          std::lock_guard lock(tables_marked_dropped_mutex);
          ...
          table_iterator->drop_time = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()) +
                                      getContext()->getServerSettings()[ServerSetting::database_catalog_drop_error_cooldown_sec];
...
void DatabaseCatalog::dropTableDataTask() {
  ...
  if (!tables_to_drop.empty()) {
    LOG_INFO(log, "Have {} tables in drop queue ({} of them are in use), will try drop {} tables", drop_tables_count,
             drop_tables_in_use_count, tables_to_drop.size());
...
void DatabaseCatalog::dropTableFinally(const TableMarkedAsDropped &table) {
  ...
  for (const auto &[disk_name, disk] : getContext()->getDisksMap()) {
    ...
    if (!is_disk_eligible_for_search(disk, table_merge_tree) || !disk->existsDirectory(data_path)) continue;
...
void DatabaseCatalog::waitTableFinallyDropped(const UUID &uuid, std::function<void()> throw_if_cancelled) {
  if (uuid == UUIDHelpers::Nil) return;
  ...
  while (tables_marked_dropped_ids.contains(uuid) && !is_shutting_down) {
    if (throw_if_cancelled) throw_if_cancelled();  /// throws QUERY_WAS_CANCELLED or TIMEOUT_EXCEEDED if the query has been killed
  ...
  LOG_DEBUG(log, "Done waiting for the table {} to be dropped. The outcome: {}", toString(uuid),
            has_table ? "table still exists" : "table dropped successfully");

  if (has_table)
    throw Exception(ErrorCodes::UNFINISHED,
                    "Did not finish dropping the table with UUID {} because the server is shutting down, "
                    "will finish after restart",
                    uuid);
}

void DatabaseCatalog::addDependencies(const StorageID &table_id, const std::vector<StorageID> &new_referential_dependencies,
                                      const std::vector<StorageID> &new_loading_dependencies,
                                      const std::vector<StorageID> &new_view_dependencies) {
  if (new_referential_dependencies.empty() && new_loading_dependencies.empty()) return;
  std::lock_guard lock{databases_mutex};
  if (!new_referential_dependencies.empty()) referential_dependencies.addDependencies(table_id, new_referential_dependencies);
  if (!new_loading_dependencies.empty()) loading_dependencies.addDependencies(table_id, new_loading_dependencies);
  if (!new_view_dependencies.empty()) {
    for (const auto &new_view_dependency : new_view_dependencies) view_dependencies.addDependency(new_view_dependency, table_id);
  }
}

void DatabaseCatalog::addDependencies(const QualifiedTableName &table_name, const TableNamesSet &new_referential_dependencies,
                                      const TableNamesSet &new_loading_dependencies, const TableNamesSet &new_view_dependencies) {
  if (new_referential_dependencies.empty() && new_loading_dependencies.empty() && new_view_dependencies.empty()) return;
  std::lock_guard lock{databases_mutex};
  if (!new_referential_dependencies.empty()) referential_dependencies.addDependencies(table_name, new_referential_dependencies);
  if (!new_loading_dependencies.empty()) loading_dependencies.addDependencies(table_name, new_loading_dependencies);
...
void DatabaseCatalog::addDependencies(const TablesDependencyGraph &new_referential_dependencies,
  ...
  for (auto &table_id : new_view_dependencies.getTables()) {
    for (auto &dependency : new_view_dependencies.getDependencies(table_id)) view_dependencies.addDependency(table_id, dependency);
  }
}
...
std::tuple<std::vector<StorageID>, std::vector<StorageID>, std::vector<StorageID>> DatabaseCatalog::removeDependencies(
  ...
  return {referential_dependencies.removeDependencies(table_id, /* remove_isolated_tables= */ true),
          loading_dependencies.removeDependencies(table_id, /* remove_isolated_tables= */ true), old_view_dependencies};
}

void DatabaseCatalog::updateDependencies(const StorageID &table_id, const TableNamesSet &new_referential_dependencies,
                                         const TableNamesSet &new_loading_dependencies, const TableNamesSet &new_view_dependencies) {
  std::lock_guard lock{databases_mutex};
...
void DatabaseCatalog::checkTableCanBeRemovedOrRenamed(const StorageID &table_id, bool check_referential_dependencies,
                                                      bool check_loading_dependencies, bool is_drop_database) const {
  if (!check_referential_dependencies && !check_loading_dependencies) return;
  std::lock_guard lock{databases_mutex};
  checkTableCanBeRemovedOrRenamedUnlocked(table_id, check_referential_dependencies, check_loading_dependencies, is_drop_database);
}

void DatabaseCatalog::checkTableCanBeRemovedOrRenamedUnlocked(const StorageID &removing_table, bool check_referential_dependencies,
                                                              bool check_loading_dependencies, bool is_drop_database) const {
  ...
  if (!is_drop_database) {
    if (!dependents.empty())
      throw Exception(ErrorCodes::HAVE_DEPENDENT_OBJECTS, "Cannot drop or rename {}, because some tables depend on it: {}", removing_table,
                      fmt::join(dependents, ", "));
    return;
  }
  ...
  for (const auto &dependent : dependents)
    if (dependent.database_name != removing_table.database_name) from_other_databases.push_back(dependent);
...
void DatabaseCatalog::checkTableCanBeAddedWithNoCyclicDependencies(const QualifiedTableName &table_name,
  ...
  auto check = [&](TablesDependencyGraph &dependencies, const TableNamesSet &new_dependencies) {
    ...
    auto restore_dependencies = [&]() {
      ...
      if (!old_dependencies.empty()) dependencies.addDependencies(table_id, old_dependencies);
    };

    if (dependencies.hasCyclicDependencies()) {
      auto cyclic_dependencies_description = dependencies.describeCyclicDependencies();
      restore_dependencies();
      throw Exception(ErrorCodes::INFINITE_LOOP, "Cannot add dependencies for '{}', because it will lead to cyclic dependencies: {}",
                      table_name.getFullName(), cyclic_dependencies_description);
    }
...
void DatabaseCatalog::checkTableCanBeRenamedWithNoCyclicDependencies(const StorageID &from_table_id, const StorageID &to_table_id) {
  ...
  auto check = [&](TablesDependencyGraph &dependencies) {
    ...
    if (dependencies.hasCyclicDependencies()) {
      auto cyclic_dependencies_description = dependencies.describeCyclicDependencies();
      restore_dependencies();
      throw Exception(ErrorCodes::INFINITE_LOOP, "Cannot rename '{}' to '{}', because it will lead to cyclic dependencies: {}",
                      from_table_id.getFullTableName(), to_table_id.getFullTableName(), cyclic_dependencies_description);
    }
...
void DatabaseCatalog::checkTablesCanBeExchangedWithNoCyclicDependencies(const StorageID &table_id_1, const StorageID &table_id_2) {
  ...
  auto check = [&](TablesDependencyGraph &dependencies) {
    ...
    if (dependencies.hasCyclicDependencies()) {
      auto cyclic_dependencies_description = dependencies.describeCyclicDependencies();
      restore_dependencies();
      throw Exception(ErrorCodes::INFINITE_LOOP, "Cannot exchange '{}' and '{}', because it will lead to cyclic dependencies: {}",
                      table_id_1.getFullTableName(), table_id_2.getFullTableName(), cyclic_dependencies_description);
    }
...
void DatabaseCatalog::cleanupStoreDirectoryTask() {
  for (const auto &[disk_name, disk] : getContext()->getDisksMap()) {
    if (!disk->supportsStat() || !disk->supportsChmod()) continue;
    ...
    if (affected_dirs) LOG_INFO(log, "Cleaned up {} directories from store/ on disk {}", affected_dirs, disk_name);
    if (checked_dirs == 0) LOG_TEST(log, "Nothing to clean up from store/ on disk {}", disk_name);
  }
...
bool DatabaseCatalog::maybeRemoveDirectory(const String &disk_name, const DiskPtr &disk, const String &unused_dir) {
  ...
  try {
    ...
    if (st.st_mode & (S_IRWXU | S_IRWXG | S_IRWXO)) {
      ...
      LOG_INFO(log, "Removing access rights for unused directory {} from disk {} (will remove it when timeout exceed)", unused_dir,
               disk_name);
    ...
    if (!unused_dir_rm_timeout_sec) return false;

    if (current_time <= max_modification_time + unused_dir_rm_timeout_sec) return false;
...
void DatabaseCatalog::stopReplicatedDDLQueries() { replicated_ddl_queries_enabled = false; }

void DatabaseCatalog::startReplicatedDDLQueries() { replicated_ddl_queries_enabled = true; }

bool DatabaseCatalog::canPerformReplicatedDDLQueries() const { return replicated_ddl_queries_enabled; }
...
TemporaryLockForUUIDDirectory::TemporaryLockForUUIDDirectory(UUID uuid_) : uuid(uuid_) {
  if (uuid != UUIDHelpers::Nil) DatabaseCatalog::instance().addUUIDMapping(uuid);
}

TemporaryLockForUUIDDirectory::~TemporaryLockForUUIDDirectory() { maybeUnlockUUID(uuid); }

TemporaryLockForUUIDDirectory::TemporaryLockForUUIDDirectory(TemporaryLockForUUIDDirectory &&rhs) noexcept : uuid(rhs.uuid) {
...
DDLGuard::DDLGuard(Map &map_, SharedMutex &db_mutex_, std::unique_lock<std::mutex> guards_lock_, const String &elem,
  ...
  if (!is_database_guard) {
    ...
    for (int i = 0; i < MAX_TRY; ++i) {
      ...
      if (acquired_db_mutex_lock) break;
    ...
    if (!acquired_db_mutex_lock) {
      releaseTableLock();
      throw Exception(ErrorCodes::TIMEOUT_EXCEEDED, "Unable to acquire the database lock of {} after {} ms", database_name,
                      MAX_TRY * INTERVAL_MS);
    }
  }
}
...
void DDLGuard::releaseTableLock() noexcept {
  if (table_lock_removed) return;

  table_lock_removed = true;
  guards_lock.lock();
  UInt32 counter = --it->second.counter;
  table_lock.unlock();
  if (counter == 0) map.erase(it);
  guards_lock.unlock();
}

DDLGuard::~DDLGuard() {
  if (!is_database_guard) db_mutex.unlock_shared();
  releaseTableLock();
}
...
std::pair<String, String> TableNameHints::getHintForTable(const String &table_name) const {
  ...
  if (results.empty()) return getExtendedHintForTable(table_name);
...
std::pair<String, String> TableNameHints::getExtendedHintForTable(const String &table_name) const {
  ...
  for (const auto &[db_name, db] : all_databases) {
    ...
    if (db_name == database->getDatabaseName()) continue;
    ...
    if (!results.empty()) return std::make_pair(db_name, results[0]);
  }
  return {};
}

Names TableNameHints::getAllRegisteredNames() const {
  if (database) return database->getAllTableNames(context);
  return {};
}
