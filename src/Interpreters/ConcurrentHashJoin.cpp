...
void updateStatistics(const auto &hash_joins, const DB::StatsCollectingParams &params) {
  ...
  const auto source_rows = std::accumulate(hash_joins.begin(), hash_joins.end(), 0ull, [](auto acc, const auto &hash_join) {
    return acc + hash_join->data->getJoinedData()->rows_to_join;
  });
  if (ht_size) DB::getHashTablesStatistics<DB::HashJoinEntry>().update({.ht_size = ht_size, .source_rows = source_rows}, params);
}
...
void reserveSpaceInHashMaps(HashJoin &hash_join, size_t ind, const StatsCollectingParams &stats_collecting_params, size_t slots) {
  if (auto hint = getSizeHint(stats_collecting_params)) {
    ...
    auto reserve_space_in_buckets = [&](auto &maps, HashJoin::Type type, size_t idx) {
      APPLY_TO_MAP(INVOKE_WITH_MAP, type, maps, [&](auto &map) {
        for (size_t j = idx; j < map.NUM_BUCKETS; j += slots) map.impls[j].reserve(reserve_size / map.NUM_BUCKETS);
      })
    };
...
ConcurrentHashJoin::ConcurrentHashJoin(std::shared_ptr<TableJoin> table_join_, size_t slots_, SharedHeader right_sample_block,
                                       const StatsCollectingParams &stats_collecting_params_, bool any_take_last_row_)
    : table_join(table_join_),
      slots(toPowerOfTwo(std::min<UInt32>(static_cast<UInt32>(slots_), 256))),
      any_take_last_row(any_take_last_row_),
      pool(std::make_unique<ThreadPool>(CurrentMetrics::ConcurrentHashJoinPoolThreads, CurrentMetrics::ConcurrentHashJoinPoolThreadsActive,
  ...
  try {
    for (size_t i = 0; i < slots; ++i) {
      pool->scheduleOrThrow([&, i, thread_group = CurrentThread::getGroup()]() {
        ThreadGroupSwitcher switcher(thread_group, ThreadName::CONCURRENT_JOIN);

        /// reserve is not needed anyway - either we will use fixed-size hash map or shared two-level map (then reserve will be done in a
        /// special way below)
        const size_t reserve_size = 0;

        auto inner_hash_join = std::make_shared<InternalHashJoin>();
        inner_hash_join->data =
            std::make_unique<HashJoin>(table_join_, right_sample_block, any_take_last_row_, reserve_size, fmt::format("concurrent{}", i),
...
ConcurrentHashJoin::~ConcurrentHashJoin() {
  try {
    if (!hash_joins[0]->data->twoLevelMapIsUsed()) return;

    if (build_phase_finished) updateStatistics(hash_joins, stats_collecting_params);
    ...
    for (size_t i = 0; i < slots; ++i) {
      ...
      pool->scheduleOrThrow([join = hash_joins[0], i, this, thread_group = CurrentThread::getGroup()]() {
        ThreadGroupSwitcher switcher(thread_group, ThreadName::CONCURRENT_JOIN);

        auto clear_space_in_buckets = [&](auto &maps, HashJoin::Type type, size_t idx) {
          APPLY_TO_MAP(INVOKE_WITH_MAP, type, maps, [&](auto &map) {
            for (size_t j = idx; j < map.NUM_BUCKETS; j += slots) map.impls[j].clearAndShrink();
          })
        };
...
class ConcatStreams final : public IBlocksStream {
 public:
  ...
  explicit ConcatStreams(std::vector<IBlocksStreamPtr> children_) {
    children = Deque(std::make_move_iterator(children_.begin()), std::make_move_iterator(children_.end()), Deque::allocator_type{});
  }
...
class ConcurrentHashJoinResult : public IJoinResult {
 ...
 public:
  explicit ConcurrentHashJoinResult(const std::vector<std::shared_ptr<ConcurrentHashJoin::InternalHashJoin>> &hash_joins_,
                                    ScatteredBlocks &&dispatched_blocks_)
      : hash_joins(hash_joins_), dispatched_blocks(std::move(dispatched_blocks_)) {}

  JoinResultBlock next() override {
    if (!current_result) {
      if (next_block >= dispatched_blocks.size()) return {Block(), nullptr, true};
...
void ConcurrentHashJoin::checkTypesOfKeys(const Block &block) const { hash_joins[0]->data->checkTypesOfKeys(block); }
...
const Block &ConcurrentHashJoin::getTotals() const { return totals; }
...
bool ConcurrentHashJoin::alwaysReturnsEmptySet() const {
  for (const auto &hash_join : hash_joins) {
    std::lock_guard lock(hash_join->mutex);
    if (!hash_join->data->alwaysReturnsEmptySet()) return false;
  }
  return true;
}

IBlocksStreamPtr ConcurrentHashJoin::getNonJoinedBlocks(const Block &left_sample_block, const Block &result_sample_block,
...
IBlocksStreamPtr ConcurrentHashJoin::getNonJoinedBlocks(const Block &left_sample_block, const Block &result_sample_block,
                                                        UInt64 max_block_size, size_t stream_idx, size_t num_streams) const {
  if (!JoinCommon::hasNonJoinedBlocks(*table_join)) return {};

  if (!isRightOrFull(table_join->kind()))
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid join type. join kind: {}, strictness: {}", table_join->kind(),
...
BlockHashes calculateHashes(const HashTable &hash_table, const ColumnRawPtrs &key_columns, const Sizes &key_sizes) {
  ...
  for (size_t i = 0; i < num_rows; ++i) hash[i] = key_getter.getHash(hash_table, i, pool);
  return hash;
}
...
ScatteredBlocks scatterBlocksByCopying(size_t num_shards, const IColumn::Selector &selector, const Block &from_block) {
  ...
  for (size_t i = 0; i < num_shards; ++i) blocks[i] = from_block.cloneEmpty();
  ...
  for (size_t i = 0; i < num_shards; ++i) result.emplace_back(std::move(blocks[i]));
  return result;
}
...
ScatteredBlocks scatterBlocksWithSelector(size_t num_shards, const IColumn::Selector &selector, const Block &from_block) {
  ...
  ScatteredBlocks result;
  result.reserve(num_shards);
  for (size_t i = 0; i < num_shards; ++i) result.emplace_back(from_block, std::move(selectors[i]));
  return result;
}
...
ScatteredBlocks ConcurrentHashJoin::dispatchBlock(const Strings &key_columns_names, Block &&from_block) {
  ...
  const bool use_zero_copy_approach =
      std::accumulate(data_types.begin(), data_types.end(), 0u, [](size_t sum, const DataTypePtr &type) {
...
void ConcurrentHashJoin::onBuildPhaseFinish() {
  if (hash_joins[0]->data->twoLevelMapIsUsed()) {
    ...
    //     1. Merge hash maps into a single one. For that, we iterate over all sub-maps and move buckets from the current `HashJoin`
    //     instance to the common map.
    for (size_t i = 1; i < slots; ++i) {
      auto move_buckets = [&](auto &lhs_maps, HashJoin::Type type, auto &rhs_maps, size_t idx) {
        APPLY_TO_MAP(INVOKE_WITH_MAPS, type, lhs_maps, rhs_maps, [&](auto &lhs_map, auto &rhs_map) {
          for (size_t j = idx; j < lhs_map.NUM_BUCKETS; j += slots) {
            if (!lhs_map.impls[j].empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected non-empty map");
    ...
    if (std::any_of(hash_joins.begin(), hash_joins.end(), [&](const auto &hj) { return !getData(hj)->nullmaps.empty(); })) {
      ...
      auto filter_holder_by_selector = [&](const HashJoin::NullMapHolder &holder) {
        ...
        auto apply_range = [&](size_t start, size_t end) {
          if (holder.column) {
            ...
            for (size_t r = start; r < end; ++r) filtered->getData()[r] = src_mask[r];
          } else {
            for (size_t r = start; r < end; ++r) filtered->getData()[r] = 1;
          }
        };
        ...
        if (sc->selector.isContinuousRange()) {
          ...
          if (holder.column) {
            const auto &src_mask = assert_cast<const ColumnUInt8 &>(*holder.column).getData();
            for (size_t idx : idxs) {
              if (idx < src_mask.size()) filtered->getData()[idx] = src_mask[idx];
            }
          } else {
            for (size_t idx : idxs) filtered->getData()[idx] = 1;
          }
        }
      ...
      for (size_t i = 0; i < slots; ++i) {
        ...
        for (const auto &holder : src->nullmaps) filter_holder_by_selector(holder);
  ...
  for (const auto &hash_join : hash_joins) all_values_unique &= hash_join->data->all_values_unique;

  for (const auto &hash_join : hash_joins) hash_join->data->all_values_unique = all_values_unique;
  ...
  for (const auto &hash_join : hash_joins) hash_join->data->onBuildPhaseFinish();

  if (hash_joins[0]->data->twoLevelMapIsUsed()) {
    auto common_used_flags = hash_joins[0]->data->getUsedFlags();

    bool use_per_row_flags = std::ranges::any_of(hash_joins, [&](const auto &hash_join) {
