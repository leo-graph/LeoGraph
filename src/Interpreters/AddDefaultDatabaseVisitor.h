...
class AddDefaultDatabaseVisitor {
 public:
  explicit AddDefaultDatabaseVisitor(ContextPtr context_, const String &database_name_,
                                     bool only_replace_current_database_function_ = false, bool only_replace_in_join_ = false)
      : context(context_),
  ...
  void visitDDL(ASTPtr &ast) const { visitDDLWithParent(nullptr, ast); }

  /// TODO: Add `parent` to the IAST
  void visitDDLWithParent(ASTPtr parent, ASTPtr &ast) const {
    visitDDLChildren(ast);

    if (!tryVisitDynamicCast<ASTAlterQuery>(parent, ast) && !tryVisitDynamicCast<ASTQueryWithTableAndOutput>(parent, ast) &&
        !tryVisitDynamicCast<ASTRenameQuery>(parent, ast) && !tryVisitDynamicCast<ASTFunction>(parent, ast)) {
    }
  }

  void visit(ASTPtr &ast) const {
    if (!tryVisit<ASTSelectQuery>(ast) && !tryVisit<ASTSelectWithUnionQuery>(ast) && !tryVisit<ASTFunction>(ast)) visitChildren(*ast);
  }
  ...
  void visit(ASTColumns &columns) const {
    for (auto &child : columns.children) visit(child);
  }
 ...
 private:
  ...
  void visit(ASTSelectQuery &select, ASTPtr &) const {
    if (select.recursive_with)
      for (const auto &child : select.with()->children) {
        if (typeid_cast<ASTWithElement *>(child.get())) with_aliases.insert(child->as<ASTWithElement>()->name);
      }

    if (select.tables()) tryVisit<ASTTablesInSelectQuery>(select.refTables());
  ...
  void visit(ASTTablesInSelectQuery &tables, ASTPtr &) const {
    for (auto &child : tables.children) tryVisit<ASTTablesInSelectQueryElement>(child);
  }

  void visit(ASTTablesInSelectQueryElement &tables_element, ASTPtr &) const {
    if (only_replace_in_join && !tables_element.table_join) return;

    if (tables_element.table_expression) tryVisit<ASTTableExpression>(tables_element.table_expression);
  }

  void visit(ASTTableExpression &table_expression, ASTPtr &) const {
    if (table_expression.database_and_table_name) tryVisit<ASTTableIdentifier>(table_expression.database_and_table_name);
  }

  void visit(const ASTTableIdentifier &identifier, ASTPtr &ast) const {
    /// Already has database.
    if (identifier.compound()) return;
    /// There is temporary table with such name, should not be rewritten.
    if (external_tables.contains(identifier.shortName())) return;
    /// This is WITH RECURSIVE alias.
    if (with_aliases.contains(identifier.name())) return;

    auto qualified_identifier = make_intrusive<ASTTableIdentifier>(database_name, identifier.name());
    if (!identifier.alias.empty()) qualified_identifier->setAlias(identifier.alias);
        ...
        for (size_t i = 0; i < child->children.size(); ++i) {
          if (is_dict_get && i == 0) {
            if (auto *identifier = child->children[i]->as<ASTIdentifier>()) {
              ...
              if (identifier->compound()) continue;

              auto qualified_dictionary_name =
                  context->getExternalDictionariesLoader().qualifyDictionaryNameWithDatabase(identifier->name(), context);
              ...
              if (literal_value.getType() != Field::Types::String) continue;

              auto dictionary_name = literal_value.safeGet<String>();
              auto qualified_dictionary_name =
                  context->getExternalDictionariesLoader().qualifyDictionaryNameWithDatabase(dictionary_name, context);
            ...
            if (auto *identifier = child->children[i]->as<ASTIdentifier>()) {
              ...
              if (maybe_table_identifier) child->children[i] = maybe_table_identifier;
            }
            ...
            if (!tryVisit<ASTTableIdentifier>(child->children[i])) visit(child->children[i]);
          } else {
  ...
  void visit(ASTRefreshStrategy &refresh, ASTPtr &) const {
    if (refresh.dependencies)
      for (auto &table : refresh.dependencies->children) tryVisit<ASTTableIdentifier>(table);
  }

  void visitChildren(IAST &ast) const {
    for (auto &child : ast.children) visit(child);
  }
  ...
  void visitDDL(ASTPtr & /* parent */, ASTQueryWithTableAndOutput &node, ASTPtr &) const {
    if (only_replace_current_database_function) return;

    if (!node.database) node.setDatabase(database_name);
  }
  ...
  void visitDDL(ASTPtr & /* parent */, ASTAlterQuery &node, ASTPtr &) const {
    ...
    for (const auto &child : node.command_list->children) {
      ...
      if (command_ast->from_database.empty()) command_ast->from_database = database_name;
      if (command_ast->to_database.empty()) command_ast->to_database = database_name;
    }
  }

  void visitDDL(ASTPtr &parent, ASTFunction &function, ASTPtr &node) const {
    if (function.name == "currentDatabase") {
      /// The `updatePointerToChild` function replaces the old address with the new one without access, so it is safe to invalidate it in
      /// place. However, just for safety, let's store the old node for a little longer.
  ...
  void visitDDLChildren(ASTPtr &ast) const {
    for (auto &child : ast->children) visitDDLWithParent(ast, child);
  }
