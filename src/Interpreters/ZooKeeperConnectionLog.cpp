...
ColumnsDescription ZooKeeperConnectionLogElement::getColumnsDescription() {
  ...
  return ColumnsDescription{
      {"hostname", std::make_shared<DataTypeLowCardinality>(std::make_shared<DataTypeString>()),
       "Hostname of the server which is connected to or disconnected from ZooKeeper."},
      ...
      {"host", std::make_shared<DataTypeString>(),
       "The hostname/IP of the ZooKeeper node that ClickHouse connected to or disconnected from."},
      ...
      {"index", std::make_shared<DataTypeUInt8>(),
       "The index of the ZooKeeper node that ClickHouse connected to or disconnected from. The index is from ZooKeeper config."},
      ...
      {"reason", std::make_shared<DataTypeLowCardinality>(std::make_shared<DataTypeString>()),
       "Reason for the connection or disconnection."},  // Updated field
  };
}
...
void ZooKeeperConnectionLog::addDisconnected(const std::string_view name, const zkutil::ZooKeeper& zookeeper,
...
void ZooKeeperConnectionLog::addWithEventType(ZooKeeperConnectionLogElement::EventType type, const std::string_view name,
                                              const zkutil::ZooKeeper& zookeeper, const std::string_view reason) {
