...
Field convertNumericTypeImpl(const Field &from) {
  To result;
  if (!accurate::convertNumeric(from.safeGet<From>(), result)) return {};
  return result;
}

template <typename To>
Field convertNumericType(const Field &from, const IDataType &type) {
  if (from.getType() == Field::Types::UInt64 || from.getType() == Field::Types::Bool) return convertNumericTypeImpl<UInt64, To>(from);
  if (from.getType() == Field::Types::Int64) return convertNumericTypeImpl<Int64, To>(from);
  if (from.getType() == Field::Types::Float64) return convertNumericTypeImpl<Float64, To>(from);
  if (from.getType() == Field::Types::UInt128) return convertNumericTypeImpl<UInt128, To>(from);
  if (from.getType() == Field::Types::Int128) return convertNumericTypeImpl<Int128, To>(from);
  if (from.getType() == Field::Types::UInt256) return convertNumericTypeImpl<UInt256, To>(from);
  if (from.getType() == Field::Types::Int256) return convertNumericTypeImpl<Int256, To>(from);

  throw Exception(ErrorCodes::TYPE_MISMATCH, "Type mismatch in IN or VALUES section. Expected: {}. Got: {}", type.getName(),
                  from.getType());
}
...
Field convertIntToDecimalType(const Field &from, const DataTypeDecimal<T> &type) {
  ...
  if (!type.canStoreWhole(value)) throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "Number is too big to place in {}", type.getName());
...
Field convertDecimalType(const Field &from, const To &type) {
  if (from.getType() == Field::Types::UInt64) return convertIntToDecimalType<UInt64>(from, type);
  if (from.getType() == Field::Types::Int64) return convertIntToDecimalType<Int64>(from, type);
  if (from.getType() == Field::Types::UInt128) return convertIntToDecimalType<UInt128>(from, type);
  if (from.getType() == Field::Types::Int128) return convertIntToDecimalType<Int128>(from, type);
  if (from.getType() == Field::Types::UInt256) return convertIntToDecimalType<UInt256>(from, type);
  if (from.getType() == Field::Types::Int256) return convertIntToDecimalType<Int256>(from, type);

  if (from.getType() == Field::Types::String) return convertStringToDecimalType(from, type);

  if (from.getType() == Field::Types::Decimal32) return convertDecimalToDecimalType<Decimal32>(from, type);
  if (from.getType() == Field::Types::Decimal64) return convertDecimalToDecimalType<Decimal64>(from, type);
  if (from.getType() == Field::Types::Decimal128) return convertDecimalToDecimalType<Decimal128>(from, type);
  if (from.getType() == Field::Types::Decimal256) return convertDecimalToDecimalType<Decimal256>(from, type);

  if (from.getType() == Field::Types::Float64) return convertFloatToDecimalType<Float64>(from, type);
...
Field convertFieldToTypeImpl(const Field &src, const IDataType &type, const IDataType *from_type_hint,
  ...
  if (which_type.isTime() && which_from_type.isDate()) {
    return static_cast<const DataTypeTime &>(type).getTimeZone().fromDayNum(
        DayNum(static_cast<DayNum::UnderlyingType>(src.safeGet<UInt64>())));
  }
  if (which_type.isTime() && which_from_type.isDate32()) {
    return static_cast<const DataTypeTime &>(type).getTimeZone().fromDayNum(
        DayNum(static_cast<DayNum::UnderlyingType>(src.safeGet<Int32>())));
  }
  if (which_type.isTime64() && which_from_type.isDate()) {
    ...
    return DecimalField<Time64>(DecimalUtils::decimalFromComponentsWithMultiplier<Time64>(value, 0, time64_type.getScaleMultiplier()),
  ...
  if (type.isValueRepresentedByNumber() && src.getType() != Field::Types::String) {
    ...
    if (isInt64OrUInt64orBoolFieldType(src.getType()) && type.getName() == "Bool") return bool(src.safeGet<bool>());
    ...
    if (which_type.isTime64() && src.getType() == Field::Types::Decimal64) {
      ...
      const UInt64 value =
          scale_from > scale_to ? from_type.getValue().value / scale_multiplier_diff : from_type.getValue().value * scale_multiplier_diff;
    ...
    /// Already in needed type.
    if (src.getType() == Field::Types::IPv6) return src;
    ...
    auto convert_tuple_to_qbit = [&](const auto &src_container, size_t src_size) -> Field {
      ...
      for (size_t i = 0; i < src_container.size(); i++) {
        ...
        if (elem.size() != bytes_per_fixedstring)
          throw Exception(ErrorCodes::TYPE_MISMATCH, "To construct QBit, each string within tuple must consist of {} byte{}. Got {}",
                          bytes_per_fixedstring, bytes_per_fixedstring != 1 ? "s" : "", elem.size());
      }
    ...
    auto convert_array_to_qbit = [&](const auto &src_container, size_t src_size) -> Field {
      if (dst_dimension != src_size)
        throw Exception(ErrorCodes::TYPE_MISMATCH, "Bad size of QBit in IN or VALUES section. Expected size: {}, actual size: {}",
                        dst_dimension, src_size);

      for (const auto &elem : src_container)
        if (elem.getType() != Field::Types::Float64)
          throw Exception(ErrorCodes::TYPE_MISMATCH, "QBit can only be constructed from BFloat16, Float32 and Float64 values, got {}",
                          elem.getTypeName());
      ...
      auto transpose_bits = [&]<typename Word, typename FloatType>() {
        ...
        for (size_t i = 0; i < dst_element_size; ++i) plane[i] = reinterpret_cast<char *>(out[i].data());
        ...
        /// Move into Fields
        for (size_t i = 0; i < dst_element_size; ++i) res[i] = Field(std::move(out[i]));
      };
    ...
    if (src.getType() == Field::Types::Map) {
      ...
      for (size_t i = 0; i < map_size; ++i) {
        ...
        if (updated_entry[0].isNull() && !canContainNull(key_type)) have_unconvertible_element = true;

        updated_entry[1] = convertFieldToType(value, value_type, nullptr, format_settings);
        if (updated_entry[1].isNull() && !canContainNull(value_type)) have_unconvertible_element = true;
    ...
    if (from_type_hint && type_variant->tryGetVariantDiscriminator(from_type_hint->getName())) return src;
    ...
    if (col->tryInsert(src)) return src;
    ...
    if (src.getType() == Field::Types::Object) return src;  /// Already in needed type.
  ...
  throw Exception(ErrorCodes::TYPE_MISMATCH, "Type mismatch in IN or VALUES section. Expected: {}. Got: {}", type.getName(), src.getType());
}
...
Field tryConvertFieldToType(const Field &from_value, const IDataType &to_type, const IDataType *from_type_hint,
                            const FormatSettings &format_settings) {
  /// TODO: implement proper tryConvertFieldToType without try/catch by adding template flag to convertFieldToTypeImpl to not throw an
  /// exception.
  try {
...
Field convertFieldToType(const Field &from_value, const IDataType &to_type, const IDataType *from_type_hint,
                         const FormatSettings &format_settings) {
  if (from_value.isNull()) return from_value;

  if (from_type_hint && from_type_hint->equals(to_type)) return from_value;
  ...
  if (const auto *nullable_type = typeid_cast<const DataTypeNullable *>(&to_type)) {
    ...
    if (WhichDataType(nested_type).isNothing()) return {};

    if (from_type_hint && from_type_hint->equals(nested_type)) return from_value;
...
Field convertFieldToTypeOrThrow(const Field &from_value, const IDataType &to_type, const IDataType *from_type_hint,
  ...
  if (is_null && !canContainNull(to_type)) throw Exception(ErrorCodes::TYPE_MISMATCH, "Cannot convert NULL to {}", to_type.getName());
  ...
  if (!is_null && converted.isNull())
    throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "Cannot convert value '{}'{}: it cannot be represented as {}",
                    fieldToString(from_value), from_type_hint ? " from " + from_type_hint->getName() : "", to_type.getName());
...
std::optional<Field> convertFieldToTypeStrict(const Field &from_value, const IDataType &from_type, const IDataType &to_type,
  ...
  if (!from_value.isNull() && result_value.isNull()) return std::nullopt;
  ...
  if (from_value.getType() == Field::Types::Float64 && Field::isDecimal(result_value.getType())) {
    if (!decimalEqualsFloatByType(result_value, from_value.safeGet<Float64>())) return std::nullopt;
  }
