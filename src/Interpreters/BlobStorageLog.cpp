...
ColumnsDescription BlobStorageLogElement::getColumnsDescription() {
  auto event_enum_type = std::make_shared<DataTypeEnum8>(DataTypeEnum8::Values{
  ...
  return ColumnsDescription{
      ...
      {"event_type", event_enum_type,
       "Type of the event. Possible values: 'Upload', 'Delete', 'MultiPartUploadCreate', 'MultiPartUploadWrite', "
       "'MultiPartUploadComplete', 'MultiPartUploadAbort'"},
      ...
      {"local_path", std::make_shared<DataTypeString>(),
       "Path to the metadata file on the local system, which references the remote resource."},
...
void BlobStorageLog::addSettingsForQuery(ContextMutablePtr& mutable_context, IAST::QueryKind query_kind) const {
  ...
  if (query_kind == IAST::QueryKind::Insert) mutable_context->setSetting("enable_blob_storage_log", false);
}
