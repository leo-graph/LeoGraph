...
bool TranslateQualifiedNamesMatcher::Data::matchColumnName(std::string_view name, const String &column_name, DataTypePtr column_type) {
  if (name.size() < column_name.size()) return false;

  if (!name.starts_with(column_name)) return false;

  if (name.size() == column_name.size()) return true;
...
bool TranslateQualifiedNamesMatcher::Data::unknownColumn(size_t table_pos, const ASTIdentifier &identifier) const {
  ...
  for (const auto &column : columns) {
    if (matchColumnName(full_name, column.name, column.type)) return false;
  }
...
bool TranslateQualifiedNamesMatcher::needChildVisit(ASTPtr &node, const ASTPtr &child) {
  ...
  if (child->as<ASTTableExpression>() || child->as<ASTSelectWithUnionQuery>()) return false;

  /// Processed nodes. Do not go into children.
  if (node->as<ASTQualifiedAsterisk>() || node->as<ASTTableJoin>()) return false;
...
void TranslateQualifiedNamesMatcher::visit(ASTPtr &ast, Data &data) {
  if (auto *t = ast->as<ASTIdentifier>()) visit(*t, ast, data);
  if (auto *t = ast->as<ASTTableJoin>()) visit(*t, ast, data);
  if (auto *t = ast->as<ASTSelectQuery>()) visit(*t, ast, data);
  if (auto *node = ast->as<ASTExpressionList>()) visit(*node, ast, data);
  if (auto *node = ast->as<ASTFunction>()) visit(*node, ast, data);
}
...
void TranslateQualifiedNamesMatcher::visit(ASTFunction &node, const ASTPtr &, Data &) {
  ...
  if (safe_to_remove_asterisk && is_count_variant && is_not_count_distinct && !func_arguments->children.empty()) {
    ...
    func_arguments->children.erase(std::remove_if(func_arguments->children.begin(), func_arguments->children.end(),
...
void TranslateQualifiedNamesMatcher::visit(const ASTQualifiedAsterisk &node, const ASTPtr &, Data &data) {
  if (!node.qualifier) throw Exception(ErrorCodes::LOGICAL_ERROR, "Qualified asterisk must have a qualifier");
  ...
  for (const auto &known_table : data.tables)
    if (db_and_table.satisfies(known_table.table, true)) return;
...
void TranslateQualifiedNamesMatcher::visit(ASTSelectQuery &select, const ASTPtr &, Data &data) {
  if (const auto *join = select.join()) extractJoinUsingColumns(join->table_join, data);
  ...
  if (select.prewhere()) Visitor(data).visit(select.refPrewhere());
  if (select.where()) Visitor(data).visit(select.refWhere());
  if (select.having()) Visitor(data).visit(select.refHaving());
}
...
void TranslateQualifiedNamesMatcher::visit(ASTExpressionList &node, const ASTPtr &, Data &data) {
  ...
  if (data.processAsterisks()) {
    ...
    for (const auto &child : node.children) {
      if (child->as<ASTAsterisk>() || child->as<ASTColumnsListMatcher>() || child->as<ASTColumnsRegexpMatcher>()) {
        if (tables_with_columns.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "An asterisk cannot be replaced with empty columns.");
        has_asterisk = true;
  ...
  for (const auto &child : old_children) {
    ...
    if (const auto *asterisk = child->as<ASTAsterisk>()) {
      ...
      for (const auto &ident : asterisk_column_list->column_list->children) columns.emplace_back(ident->clone());
      ...
      for (const auto &table : tables_with_columns) {
        for (const auto &column : table.columns) {
          if (re2::RE2::PartialMatch(column.name, regexp) && (first_table || !data.join_using_columns.contains(column.name))) {
      ...
      for (const auto &table : tables_with_columns) {
        if (ident_db_and_name.satisfies(table.table, true)) {
          for (const auto &column : table.columns) addIdentifier(columns, table.table, column.name);
          break;
        }
      }

      if (qualified_asterisk->transformers) {
        for (const auto &transformer : qualified_asterisk->transformers->children) IASTColumnsTransformer::transform(transformer, columns);
      }
    } else
  ...
  if (table_join.using_expression_list) {
    ...
    for (const auto &key : keys.children)
      if (auto opt_column = tryGetIdentifierName(key))
        ...
        if (alias.empty())
          throw Exception(ErrorCodes::UNSUPPORTED_JOIN_KEYS, "Wrong key in USING. Expected identifier or alias, got: {}", key->getID());
...
bool RestoreQualifiedNamesMatcher::needChildVisit(ASTPtr &, const ASTPtr &child) {
  ...
  if (child->as<ASTSelectWithUnionQuery>()) return false;  // NOLINT
  return true;
}
