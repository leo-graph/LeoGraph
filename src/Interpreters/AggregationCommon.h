...
static inline T ALWAYS_INLINE packFixed(size_t i, size_t keys_size, const ColumnRawPtrs &key_columns, const Sizes &key_sizes,
  ...
  for (size_t j = 0; j < keys_size; ++j) {
    ...
    switch (key_sizes[j]) {
      ...
      case 8:
        ...
        break;
      default:
        memcpy(bytes + offset, static_cast<const ColumnFixedSizeHelper *>(column)->getRawDataBegin<1>() + index * key_sizes[j],
               key_sizes[j]);
        offset += key_sizes[j];
    }
  }
  ...
  for (size_t j = 0; j < keys_size; ++j) {
    ...
    switch (key_sizes[j]) {
      ...
      case 8:
        ...
        break;
      default:
        memcpy(bytes + offset, static_cast<const ColumnFixedSizeHelper *>(key_columns[j])->getRawDataBegin<1>() + i * key_sizes[j],
               key_sizes[j]);
        offset += key_sizes[j];
    }
  }
...
static inline std::string_view ALWAYS_INLINE serializeKeysToPoolContiguous(  /// NOLINT
  ...
  size_t sum_size = 0;
  for (size_t j = 0; j < keys_size; ++j) sum_size += key_columns[j]->serializeValueIntoArena(i, pool, begin, settings).size();
...
#if defined(__SSSE3__) && !defined(MEMORY_SANITIZER)
template <typename T>
static T inline packFixedShuffle(const char *__restrict *__restrict srcs, size_t num_srcs, const size_t *__restrict elem_sizes, size_t idx,
                                 const uint8_t *__restrict masks) {
  assert(num_srcs > 0);

  __m128i res = _mm_shuffle_epi8(_mm_loadu_si128(reinterpret_cast<const __m128i *>(srcs[0] + elem_sizes[0] * idx)),
                                 _mm_loadu_si128(reinterpret_cast<const __m128i *>(masks)));

  for (size_t i = 1; i < num_srcs; ++i) {
    res = _mm_xor_si128(res, _mm_shuffle_epi8(_mm_loadu_si128(reinterpret_cast<const __m128i *>(srcs[i] + elem_sizes[i] * idx)),
