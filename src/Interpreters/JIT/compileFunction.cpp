...
ColumnData getColumnData(const IColumn *column, size_t skip_rows) {
  ...
  if (is_const) throw Exception(ErrorCodes::LOGICAL_ERROR, "Input columns should not be constant");
...
static void compileFunction(llvm::Module &module, const IFunctionBase &function) {
  ...
  for (size_t i = 0; i < function_argument_types.size(); ++i) {
    ...
    auto *column_element_value =
        b.CreateLoad(column.data_element_type, b.CreateInBoundsGEP(column.data_element_type, column_data_ptr, counter_phi));
...
CompiledFunction compileFunction(CHJIT &jit, const IFunctionBase &function) {
  ...
  CompiledFunction result_compiled_function{.compiled_function = compiled_function_ptr, .compiled_module = compiled_module};

  return result_compiled_function;
}

static void compileCreateAggregateStatesFunctions(llvm::Module &module, const std::vector<AggregateFunctionWithOffset> &functions,
  ...
  auto *create_aggregate_states_function =
      llvm::Function::Create(create_aggregate_states_function_type, llvm::Function::ExternalLinkage, name, module);
...
static void compileAddIntoAggregateStatesFunctions(llvm::Module &module, const std::vector<AggregateFunctionWithOffset> &functions,
                                                   const std::string &name, AddIntoAggregateStatesPlacesArgumentType places_argument_type) {
  auto &context = module.getContext();
  llvm::IRBuilder<> b(context);
  ...
  auto *add_into_aggregate_states_func_declaration =
      llvm::FunctionType::get(b.getVoidTy(), {size_type, size_type, column_type->getPointerTo(), places_type}, false);
  auto *add_into_aggregate_states_func =
      llvm::Function::Create(add_into_aggregate_states_func_declaration, llvm::Function::ExternalLinkage, name, module);
  ...
  for (const auto &function : functions) {
    ...
    for (size_t column_argument_index = 0; column_argument_index < function_arguments_size; ++column_argument_index) {
      ...
      auto *data =
          b.CreateLoad(column_type, b.CreateConstInBoundsGEP1_64(column_type, columns_arg, previous_columns_size + column_argument_index));
      ...
      if (argument_type->isNullable()) data_placeholder.null_data_ptr = b.CreateExtractValue(data, {1});
  ...
  for (const auto &function : functions) {
    ...
    for (size_t column_argument_index = 0; column_argument_index < function_arguments_size; ++column_argument_index) {
      ...
      auto *column_data_element =
          b.CreateLoad(column.data_element_type, b.CreateGEP(column.data_element_type, column.data_ptr, counter_phi));
...
static void compileMergeAggregatesStates(llvm::Module &module, const std::vector<AggregateFunctionWithOffset> &functions,
                                         const std::string &name) {
  llvm::IRBuilder<> b(module.getContext());
  ...
  for (const auto &function_to_compile : functions) {
    auto *aggregate_data_place_dst =
        b.CreateLoad(aggregate_data_place_type,
                     b.CreateInBoundsGEP(aggregate_data_place_type->getPointerTo(), aggregate_data_places_dst_arg, counter_phi));
    auto *aggregate_data_place_src =
        b.CreateLoad(aggregate_data_place_type,
                     b.CreateInBoundsGEP(aggregate_data_place_type->getPointerTo(), aggregate_data_places_src_arg, counter_phi));

    size_t aggregate_function_offset = function_to_compile.aggregate_data_offset;

    auto *aggregate_data_place_merge_dst_with_offset =
        b.CreateConstInBoundsGEP1_64(b.getInt8Ty(), aggregate_data_place_dst, aggregate_function_offset);
    auto *aggregate_data_place_merge_src_with_offset =
        b.CreateConstInBoundsGEP1_64(b.getInt8Ty(), aggregate_data_place_src, aggregate_function_offset);
...
static void compileInsertAggregatesIntoResultColumns(llvm::Module &module, const std::vector<AggregateFunctionWithOffset> &functions,
                                                     const std::string &name) {
  auto &context = module.getContext();
  llvm::IRBuilder<> b(context);
  ...
  auto *insert_aggregates_into_result_func_declaration =
      llvm::FunctionType::get(b.getVoidTy(), {size_type, size_type, column_type->getPointerTo(), aggregate_data_places_type}, false);
  auto *insert_aggregates_into_result_func =
      llvm::Function::Create(insert_aggregates_into_result_func_declaration, llvm::Function::ExternalLinkage, name, module);
  ...
  for (size_t i = 0; i < functions.size(); ++i) {
    ...
    if (return_type->isNullable()) columns[i].null_data_ptr = b.CreateExtractValue(data, {1});
  }
  ...
  auto *aggregate_data_place =
      b.CreateLoad(b.getInt8Ty()->getPointerTo(), b.CreateGEP(b.getInt8Ty()->getPointerTo(), aggregate_data_places_arg, counter_phi));
...
CompiledAggregateFunctions compileAggregateFunctions(CHJIT &jit, const std::vector<AggregateFunctionWithOffset> &functions,
  ...
  auto compiled_module = jit.compileModule([&](llvm::Module &module) {
    ...
    compileAddIntoAggregateStatesFunctions(module, functions, add_aggregate_states_functions_name,
                                           AddIntoAggregateStatesPlacesArgumentType::MultiplePlaces);
    compileAddIntoAggregateStatesFunctions(module, functions, add_aggregate_states_functions_name_single_place,
                                           AddIntoAggregateStatesPlacesArgumentType::SinglePlace);
  ...
  auto create_aggregate_states_function =
      reinterpret_cast<JITCreateAggregateStatesFunction>(compiled_module.function_name_to_symbol[create_aggregate_states_functions_name]);
  auto add_into_aggregate_states_function =
      reinterpret_cast<JITAddIntoAggregateStatesFunction>(compiled_module.function_name_to_symbol[add_aggregate_states_functions_name]);
  auto add_into_aggregate_states_function_single_place = reinterpret_cast<JITAddIntoAggregateStatesFunctionSinglePlace>(
      compiled_module.function_name_to_symbol[add_aggregate_states_functions_name_single_place]);
  auto merge_aggregate_states_function =
      reinterpret_cast<JITMergeAggregateStatesFunction>(compiled_module.function_name_to_symbol[merge_aggregate_states_functions_name]);
  auto insert_aggregate_states_function = reinterpret_cast<JITInsertAggregateStatesIntoColumnsFunction>(
      compiled_module.function_name_to_symbol[insert_aggregate_states_functions_name]);
...
static void compileSortDescription(llvm::Module &module, SortDescription &description, const DataTypes &sort_description_types,
  ...
  std::vector<llvm::Type *> function_argument_types = {size_type, size_type, column_data_type->getPointerTo(),
                                                       column_data_type->getPointerTo()};
  auto *comparator_func_declaration = llvm::FunctionType::get(b.getInt8Ty(), function_argument_types, false);
  auto *comparator_func =
      llvm::Function::Create(comparator_func_declaration, llvm::Function::ExternalLinkage, sort_description_dump, module);
  ...
  for (size_t i = 0; i < columns_size; ++i) {
    ...
    if (!column_native_type) throw Exception(ErrorCodes::LOGICAL_ERROR, "No native type for column type {}", column_type->getName());
  ...
  for (const auto &[block, result_value] : comparator_steps_and_results) compare_result_phi->addIncoming(result_value, block);
...
CompiledSortDescriptionFunction compileSortDescription(CHJIT &jit, SortDescription &description, const DataTypes &sort_description_types,
  ...
  auto compiled_module = jit.compileModule(
      [&](llvm::Module &module) { compileSortDescription(module, description, sort_description_types, sort_description_dump); });
  ...
  CompiledSortDescriptionFunction compiled_sort_descriptor_function{.comparator_function = comparator_function,
                                                                    .compiled_module = std::move(compiled_module)};
