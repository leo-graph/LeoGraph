...
size_t getRatio(size_t total, double ratio, bool ceil = false) {
  if (ceil) return static_cast<size_t>(std::ceil(static_cast<double>(total) * std::clamp(ratio, 0.0, 1.0)));
  return std::lround(static_cast<double>(total) * std::clamp(ratio, 0.0, 1.0));
}
}  // namespace

SLRUFileCachePriority::SLRUFileCachePriority(size_t max_size_, size_t max_elements_, double size_ratio_, const std::string &description_,
  ...
  LOG_INFO(log,
           ...
           probationary_queue.max_size.load(), probationary_queue.max_elements.load(), protected_queue.max_size.load(),
           protected_queue.max_elements.load());
...
FileCachePriorityPtr SLRUFileCachePriority::copy() const {
  return std::make_unique<SLRUFileCachePriority>(max_size, max_elements, size_ratio, description, probationary_queue.state,
...
size_t SLRUFileCachePriority::getSizeApprox() const { return protected_queue.getSizeApprox() + probationary_queue.getSizeApprox(); }
...
bool SLRUFileCachePriority::canFit(  /// NOLINT
    size_t size, size_t elements, const CacheStateGuard::Lock &lock, IteratorPtr reservee, const OriginInfo &, bool best_effort) const {
  if (best_effort) return probationary_queue.canFit(size, elements, lock) || protected_queue.canFit(size, elements, lock);

  if (reservee) {
    const auto *slru_iterator = assert_cast<SLRUIterator *>(reservee->getNestedOrThis());
    if (slru_iterator->is_protected) return protected_queue.canFit(size, elements, lock);
...
IFileCachePriority::IteratorPtr SLRUFileCachePriority::add(  /// NOLINT
    KeyMetadataPtr key_metadata, size_t offset, size_t size, const CachePriorityGuard::WriteLock &lock,
    const CacheStateGuard::Lock *state_lock, bool is_startup) {
  bool is_protected = false;
  if (is_startup) {
    chassert(size);
    if (!state_lock) throw Exception(ErrorCodes::LOGICAL_ERROR, "Startup initialization requires state lock");
    ...
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    ...
                    "(key: {}, offset: {})",
                    key_metadata->key, offset);
  }

  auto entry = std::make_shared<Entry>(key_metadata->key, offset, size, key_metadata);
  return std::make_shared<SLRUIterator>(
      this,
      is_protected ? protected_queue.add(std::move(entry), lock, state_lock) : probationary_queue.add(std::move(entry), lock, state_lock),
      is_protected);
}

void SLRUFileCachePriority::iterate(IterateFunc func, FileCacheReserveStat &stat, const CachePriorityGuard::ReadLock &lock) {
...
EvictionInfoPtr SLRUFileCachePriority::collectEvictionInfo(size_t size, size_t elements, IFileCachePriority::Iterator *reservee,
                                                           bool is_total_space_cleanup, const OriginInfo &origin_info,
                                                           const CacheStateGuard::Lock &lock) {
  if (!size && !elements) return std::make_unique<EvictionInfo>();
  ...
  if (is_total_space_cleanup) {
    ...
    auto info = probationary_queue.collectEvictionInfo(evict_size_from_probationary, evict_elements_from_probationary, reservee,
    ...
    info->add(protected_queue.collectEvictionInfo(evict_size_from_protected, evict_elements_from_protected, reservee,
                                                  is_total_space_cleanup, origin_info, lock));
    return info;
  }
  ...
  if (reservee) {
    ...
    chassert(evict_in_protected ? slru_iterator->lru_iterator.cache_priority == &protected_queue
  ...
  if (evict_in_protected) {
    ...
    return protected_queue.collectEvictionInfo(size, elements, reservee, is_total_space_cleanup, origin_info, lock);
  }

  return probationary_queue.collectEvictionInfo(size, elements, reservee, is_total_space_cleanup, origin_info, lock);
}

bool SLRUFileCachePriority::collectCandidatesForEviction(const EvictionInfo &eviction_info, FileCacheReserveStat &stat,
                                                         EvictionCandidates &res, InvalidatedEntriesInfos &invalidated_entries,
                                                         IFileCachePriority::IteratorPtr reservee, bool continue_from_last_eviction_pos,
                                                         size_t max_candidates_size, bool is_total_space_cleanup,
                                                         const OriginInfo &origin_info, CachePriorityGuard &cache_guard,
                                                         CacheStateGuard &state_guard) {
  if (is_total_space_cleanup) {
    bool success_probationary = probationary_queue.collectCandidatesForEviction(
        eviction_info, stat, res, invalidated_entries, reservee, continue_from_last_eviction_pos, max_candidates_size,
        is_total_space_cleanup, origin_info, cache_guard, state_guard);
    ...
    bool success_protected = protected_queue.collectCandidatesForEviction(eviction_info, stat, res, invalidated_entries, reservee,
  ...
  if (!reservee) {
    return probationary_queue.collectCandidatesForEviction(eviction_info, stat, res, invalidated_entries, reservee,
                                                           continue_from_last_eviction_pos, max_candidates_size, is_total_space_cleanup,
                                                           origin_info, cache_guard, state_guard);
  }
  ...
  if (slru_iterator->is_protected) {
    ...
    success =
        collectCandidatesForEvictionInProtected(eviction_info, stat, res, invalidated_entries, reservee, continue_from_last_eviction_pos,
                                                max_candidates_size, is_total_space_cleanup, origin_info, cache_guard, state_guard);
  } else {
    chassert(slru_iterator->lru_iterator.cache_priority == &probationary_queue);
    success = probationary_queue.collectCandidatesForEviction(eviction_info, stat, res, invalidated_entries, reservee,
                                                              continue_from_last_eviction_pos, max_candidates_size, is_total_space_cleanup,
                                                              origin_info, cache_guard, state_guard);
  }
...
bool SLRUFileCachePriority::collectCandidatesForEvictionInProtected(const EvictionInfo &eviction_info, FileCacheReserveStat &stat,
                                                                    EvictionCandidates &res, InvalidatedEntriesInfos &invalidated_entries,
                                                                    IFileCachePriority::IteratorPtr reservee,
                                                                    bool continue_from_last_eviction_pos, size_t max_candidates_size,
                                                                    bool is_total_space_cleanup, const OriginInfo &origin_info,
                                                                    CachePriorityGuard &cache_guard, CacheStateGuard &state_guard) {
  auto downgrade_candidates = std::make_shared<EvictionCandidates>();
  FileCacheReserveStat downgrade_stat;
  if (!protected_queue.collectCandidatesForEviction(eviction_info, downgrade_stat, *downgrade_candidates, invalidated_entries, reservee,
                                                    continue_from_last_eviction_pos, max_candidates_size, is_total_space_cleanup,
                                                    origin_info, cache_guard, state_guard)) {
    return false;
  }
  ...
  /// So we do it now.
  auto probationary_eviction_info =
      probationary_queue.collectEvictionInfo(downgrade_stat.total_stat.releasable_size, downgrade_stat.total_stat.releasable_count,
                                             /* reservee */ nullptr,
                                             /* is_total_space_cleanup */ false, origin_info, state_guard.lock());
  ...
  if (requires_eviction) {
    ...
    if (!probationary_queue.collectCandidatesForEviction(eviction_info, stat, res, invalidated_entries, reservee,
                                                         continue_from_last_eviction_pos, max_candidates_size, is_total_space_cleanup,
                                                         origin_info, cache_guard, state_guard)) {
      return false;
    }
  ...
  struct DowngradedEntriesInfos : private std::vector<DowngradedEntryInfo> {
    ...
    ~DowngradedEntriesInfos() {
      ...
      for (auto &entry : *this) entry.new_nested_iterator.invalidate();
    }
  };
  ...
  res.setAfterEvictWriteFunc([=, this](const CachePriorityGuard::WriteLock &lk) mutable {
    for (auto &[key, key_candidates] : *downgrade_candidates) {
      while (!key_candidates.candidates.empty()) {
        ...
        downgraded_entries->add(DowngradedEntryInfo{.slru_iterator = iterator,
...
bool SLRUFileCachePriority::tryIncreasePriority(Iterator &iterator_, bool is_space_reservation_complete, CachePriorityGuard &queue_guard,
                                                CacheStateGuard &state_guard) {
  ...
  if (iterator.is_protected) {
    return protected_queue.tryIncreasePriority(iterator.lru_iterator, is_space_reservation_complete, queue_guard, state_guard);
    ...
    return probationary_queue.tryIncreasePriority(iterator.lru_iterator, is_space_reservation_complete, queue_guard, state_guard);
  }
...

  {
    ...
    if (entry_state != Entry::State::Active) return false;
  ...
  SCOPE_EXIT({
    if (reset_evicting_flag_for_prev_entry) prev_entry->resetFlag(/* from_state */ Entry::State::Moving);
  });
  ...
  std::unique_ptr<EvictionInfo> downgrade_info;
  {
    ...
    downgrade_info = protected_queue.collectEvictionInfo(prev_entry->size,
                                                         /* elements */ 1,
                                                         /* reservee */ nullptr,
                                                         /* is_total_space_cleanup */ false, FileCache::getInternalOrigin(), lock);

#ifdef DEBUG_OR_SANITIZER_BUILD
    LOG_TEST(log, "Entry: {}. Downgrade info: {} ({})", prev_entry->toString(), downgrade_info->toString(), getStateInfoForLog(lock));
#endif
  }
  ...
  if (!collectCandidatesForEvictionInProtected(*downgrade_info, downgrade_stat, downgrade_candidates, invalidated_entries,
                                               /* reservee */ nullptr,
                                               /* continue_from_last_eviction_pos */ false,
                                               /* max_candidates_size */ 0,
                                               /* is_total_space_cleanup */ false, FileCache::getInternalOrigin(), queue_guard,
                                               state_guard)) {
  ...
  ProfileEvents::increment(ProfileEvents::FilesystemCacheEvictedFileSegmentsDuringPriorityIncrease, downgrade_candidates.size());
  ...
  auto new_iterator = [&] {
    ...
    auto empty_entry = std::make_shared<Entry>(prev_entry->key, prev_entry->offset,
                                               /* size */ 0, prev_entry->key_metadata);
...
LRUFileCachePriority::LRUIterator SLRUFileCachePriority::addOrThrow(EntryPtr entry, LRUFileCachePriority &queue,
  ...
  try {
    ...
    try {
      ...
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected exception: {} (Initial exception: {}). Cache will become inconsistent",
                      getCurrentExceptionMessage(true), initial_exception);
    }

    throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed to create queue entry: {}", getCurrentExceptionMessage(true));
  }
}
...
bool SLRUFileCachePriority::modifySizeLimits(size_t max_size_, size_t max_elements_, double size_ratio_,
                                             const CacheStateGuard::Lock &lock) {
  if (max_size == max_size_ && max_elements == max_elements_ && size_ratio == size_ratio_) return false;  /// Nothing to change.
...
EvictionInfoPtr SLRUFileCachePriority::collectEvictionInfoForResize(size_t desired_max_size, size_t desired_max_elements,
  ...
  auto info = protected_queue.collectEvictionInfoForResize(getRatio(desired_max_size, size_ratio),
                                                           getRatio(desired_max_elements, size_ratio), origin_info, lock);

  info->add(probationary_queue.collectEvictionInfoForResize(getRatio(desired_max_size, 1 - size_ratio),
                                                            getRatio(desired_max_elements, 1 - size_ratio), origin_info, lock));

  return info;
}

SLRUFileCachePriority::SLRUIterator::SLRUIterator(SLRUFileCachePriority *cache_priority_, LRUFileCachePriority::LRUIterator &&lru_iterator_,
                                                  bool is_protected_)
    : cache_priority(cache_priority_), lru_iterator(lru_iterator_), entry(lru_iterator.getEntry()), is_protected(is_protected_) {}
...
SLRUFileCachePriority::EntryPtr SLRUFileCachePriority::SLRUIterator::getEntry() const {
  ...
  if (!entry_ptr) throw Exception(ErrorCodes::LOGICAL_ERROR, "Entry pointer expired");
  return entry_ptr;
}

void SLRUFileCachePriority::SLRUIterator::setIterator(LRUIterator &&iterator_, bool is_protected_, const CacheStateGuard::Lock &) {
...
bool SLRUFileCachePriority::SLRUIterator::isValid(const CachePriorityGuard::WriteLock &lock) const { return lru_iterator.isValid(lock); }
...
std::string SLRUFileCachePriority::getStateInfoForLog(const CacheStateGuard::Lock &lock) const {
  return fmt::format(
      ...
      getSize(lock), max_size.load(), getElementsCount(lock), max_elements.load(), probationary_queue.getSize(lock),
      probationary_queue.max_size.load(), probationary_queue.getElementsCount(lock), probationary_queue.max_elements.load(),
      protected_queue.getSize(lock), protected_queue.max_size.load(), protected_queue.getElementsCount(lock),
      protected_queue.max_elements.load());
}
