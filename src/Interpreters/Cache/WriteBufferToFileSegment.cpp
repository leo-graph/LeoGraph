...
size_t getCacheLockWaitTimeout() {
  ...
  if (query_context) return query_context->getSettingsRef()[Setting::temporary_data_in_cache_reserve_space_wait_lock_timeout_milliseconds];
...
void WriteBufferToFileSegment::nextImpl() {
  ...
  if (downloader != FileSegment::getCallerId()) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed to set a downloader (current downloader: {}, file segment info: {})", downloader,
  ...
  if (!ok) {
    ...
    for (const auto& [kind, stat] : reserve_stat.stat_by_kind)
      reserve_stat_msg += fmt::format("{} hold {}, can release {}; ", toString(kind), ReadableSize(stat.non_releasable_size),
                                      ReadableSize(stat.releasable_size));

    throw Exception(ErrorCodes::NOT_ENOUGH_SPACE, "Failed to reserve {} bytes for {}: reason {}, {}(segment info: {})", bytes_to_write,
                    file_segment->getKind() == FileSegmentKind::Ephemeral ? "temporary file" : "the file in cache", failure_reason,
                    reserve_stat_msg, file_segment->getInfoForLog());
  }
...
std::unique_ptr<ReadBuffer> WriteBufferToFileSegment::getReadBufferImpl() {
   ...
   */
  finalize();
  if (file_segment->getDownloadedSize() > 0) return std::make_unique<ReadBufferFromFile>(file_segment->getPath());
