...
struct CreateFileSegmentSettings {
  ...
  explicit CreateFileSegmentSettings(FileSegmentKind kind_) : kind(kind_), unbounded(kind == FileSegmentKind::Ephemeral) {}
};
...
class FileSegment : private boost::noncopyable {
 ...
 public:
  ...
  FileSegment(const Key &key_, size_t offset_, size_t size_, State download_state_, const CreateFileSegmentSettings &create_settings = {},
              bool background_download_enabled_ = false, FileCache *cache_ = nullptr,
              std::weak_ptr<KeyMetadata> key_metadata_ = std::weak_ptr<KeyMetadata>(), Priority::IteratorPtr queue_iterator_ = nullptr);
  ...
  bool reserve(size_t size_to_reserve, size_t lock_wait_timeout_milliseconds, std::string &failure_reason,
