...
String toString(FileSegmentKind kind) { return String(magic_enum::enum_name(kind)); }

FileSegment::FileSegment(const Key &key_, size_t offset_, size_t size_, State download_state_, const CreateFileSegmentSettings &settings,
                         bool background_download_enabled_, FileCache *cache_, std::weak_ptr<KeyMetadata> key_metadata_,
...
#endif
{
  ...
  switch (download_state) {
    ...
    default: {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Can only create file segment with either EMPTY, DOWNLOADED or DETACHED state");
    }
  }
}

FileSegment::Range::Range(size_t left_, size_t right_) : left(left_), right(right_) {
  if (left > right) throw Exception(ErrorCodes::LOGICAL_ERROR, "Attempt to create incorrect range: [{}, {}]", left, right);
}
...
String FileSegment::getPath() const { return getKeyMetadata()->getFileSegmentPath(*this); }
...
void FileSegment::setDownloadState(State state, const FileSegmentGuard::Lock &lock) {
  if (isCompleted(false) && state != State::DETACHED) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Updating state to {} of file segment is not allowed, because it is already completed ({})",
...
void FileSegment::setQueueIterator(Priority::IteratorPtr iterator) {
  auto lk = lock();
  if (queue_iterator) throw Exception(ErrorCodes::LOGICAL_ERROR, "Queue iterator cannot be set twice");
...
size_t FileSegment::getCurrentWriteOffset() const { return range().left + downloaded_size; }

size_t FileSegment::getDownloadedSize() const { return downloaded_size; }
...
String FileSegment::getDownloader() const { return getDownloaderUnlocked(lock()); }

String FileSegment::getDownloaderUnlocked(const FileSegmentGuard::Lock &) const { return downloader_id; }
...
String FileSegment::getOrSetDownloader() {
  ...
  if (current_downloader.empty()) {
    ...
    if (!allow_new_downloader) return "notAllowed:" + stateToString(download_state);
...
bool FileSegment::isDownloaderUnlocked(const FileSegmentGuard::Lock &lock) const { return getCallerId() == getDownloaderUnlocked(lock); }
...
FileSegment::LocalCacheWriterPtr FileSegment::getLocalCacheWriter() { return cache_writer; }
...
FileSegment::RemoteFileReaderPtr FileSegment::extractRemoteFileReader() {
  auto lk = lock();
  if (remote_file_reader && (download_state == State::DOWNLOADED || download_state == State::PARTIALLY_DOWNLOADED_NO_CONTINUATION)) {
...
void FileSegment::setRemoteFileReader(RemoteFileReaderPtr remote_file_reader_) {
  ...
  if (remote_file_reader) throw Exception(ErrorCodes::LOGICAL_ERROR, "Remote file reader already exists");
...
void FileSegment::write(char *from, size_t size, size_t offset_in_file) {
  ...
  {
    if (!size) throw Exception(ErrorCodes::LOGICAL_ERROR, "Writing zero size is not allowed");
    ...
    if (download_state != State::DOWNLOADING)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected DOWNLOADING state, got {}", stateToString(download_state));

    const size_t first_non_downloaded_offset = getCurrentWriteOffset();

    if (offset_in_file != first_non_downloaded_offset) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Attempt to write {} bytes to offset: {}, but current write offset is {}", size,
                      offset_in_file, first_non_downloaded_offset);
    }
    ...
    if (free_reserved_size < size)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Not enough space is reserved. Available: {}, expected: {}", free_reserved_size, size);

    if (!is_unbound) {
      if (current_downloaded_size == range().size()) throw Exception(ErrorCodes::LOGICAL_ERROR, "File segment is already fully downloaded");
  ...
  try {
    ...
    if (!cache_writer) {
      int flags = -1;
      if (downloaded_size > 0) flags = O_WRONLY | O_APPEND | O_CLOEXEC;
    ...
    if (fs::exists(file_segment_path)) {
      if (downloaded_size == 0) {
        ...
        LOG_TRACE(log,
                  "Failed to write to file: no space left on device "
        ...
        if (downloaded_size != file_size) downloaded_size = file_size;
      }
    }
...
FileSegment::State FileSegment::wait(size_t offset) {
  ...
  if (download_state == State::EMPTY) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot wait on a file segment with empty state");
  ...
  if (download_state == State::DOWNLOADING) {
    ...
    [[maybe_unused]] const auto ok = cv.wait_for(
        lk, std::chrono::seconds(60), [&, this]() { return download_state != State::DOWNLOADING || offset < getCurrentWriteOffset(); });
    /// chassert(ok);
  }
...
LockedKeyPtr FileSegment::lockKeyMetadata(bool assert_exists) const {
  if (assert_exists) return getKeyMetadata()->lock();
...
bool FileSegment::reserve(size_t size_to_reserve, size_t lock_wait_timeout_milliseconds, std::string &failure_reason,
                          FileCacheReserveStat *reserve_stat) {
  if (!size_to_reserve) throw Exception(ErrorCodes::LOGICAL_ERROR, "Zero space reservation is not allowed");
  ...
  bool is_file_segment_size_exceeded;
  {
    ...
    if (is_file_segment_size_exceeded && !is_unbound) {
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "Attempt to reserve space too much space ({}) for file segment with range: {} (downloaded size: {})", size_to_reserve,
                      range().toString(), downloaded_size.load());
    }
  ...
  if (already_reserved_size >= size_to_reserve) return true;
  ...
  if (!reserve_stat) reserve_stat = &dummy_stat;

  bool reserved =
      cache->tryReserve(*this, size_to_reserve, *reserve_stat, getKeyMetadata()->origin, lock_wait_timeout_milliseconds, failure_reason);

  if (!reserved) setDownloadFailedUnlocked(lock());

  return reserved;
}

void FileSegment::setDownloadedUnlocked(const FileSegmentGuard::Lock &) {
  if (download_state == State::DOWNLOADED) return;
...
void FileSegment::completePartAndResetDownloader() {
  ...
  chassert(download_state == State::DOWNLOADING || download_state == State::PARTIALLY_DOWNLOADED_NO_CONTINUATION);

  if (download_state == State::DOWNLOADING) resetDownloadingStateUnlocked(lk);
...
void FileSegment::shrinkFileSegmentToDownloadedSize(const LockedKey &locked_key, const FileSegmentGuard::Lock &lock,
  ...
  if (!locked_key.isLastOwnerOfFileSegment(offset())) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Shrinking of file segment can be done only by the last holder: {}", getInfoForLog());
  }
  ...
  LOG_TEST(log, "Shrinking file segment {} -> {} (downloaded size: {})", range().size(), result_size, downloaded_size.load());
...
size_t FileSegment::getSizeForBackgroundDownloadUnlocked(const FileSegmentGuard::Lock &) const {
  if (!background_download_enabled || !downloaded_size || !remote_file_reader) {
    return 0;
  }
...
void FileSegment::complete(const LockedKeyPtr &locked_key, bool allow_background_download, bool force_shrink_to_downloaded_size) {
  ...
  SCOPE_EXIT({
    if (is_downloader) cv.notify_all();
  });

  LOG_TEST(log, "Complete based on current state (is_last_holder: {}, force shrink: {}, {})", is_last_holder,
           force_shrink_to_downloaded_size, getInfoForLogUnlocked(segment_lock));

  if (is_downloader) {
    if (download_state == State::DOWNLOADING) resetDownloadingStateUnlocked(segment_lock);
  ...
  switch (download_state) {
    ...
    case State::EMPTY: {
      if (is_last_holder) locked_key->removeFileSegment(offset(), segment_lock);
      break;
    }
  ...
  if (download_state != State::DETACHED) chassert(assertCorrectnessUnlocked(segment_lock));
}
...
bool FileSegment::assertCorrectness() const { return assertCorrectnessUnlocked(lock()); }
...
bool FileSegment::assertCorrectnessUnlocked(const FileSegmentGuard::Lock &lock) const {
  ...
  auto check_iterator = [&](const Priority::IteratorPtr &it) {
    ...
    if (download_state != State::DOWNLOADING && entry_size != reserved_size)
      throw_logical(
          fmt::format("Expected entry.size == reserved_size ({} == {}, entry: {})", entry_size, reserved_size.load(), entry->toString()));
...

  {
    ...
    if (downloaded_size == 0) {
      if (download_state != State::DOWNLOADING && fs::exists(file_path)) throw_logical("Expected file " + file_path + " not to exist");
...
void FileSegment::assertNotDetachedUnlocked(const FileSegmentGuard::Lock &lock) const {
  if (download_state == State::DETACHED) {
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    ...
                    "Please, retry. File segment info: {}",
...
bool FileSegment::isCompleted(bool sync) const {
  auto is_completed_state = [this]() -> bool { return download_state == State::DOWNLOADED || download_state == State::DETACHED; };

  if (sync) {
    if (is_completed_state()) return true;
...
void FileSegment::setDetachedState(const FileSegmentGuard::Lock &lock) {
  ...
  if (cache_writer) cache_writer->cancel();
  cache_writer.reset();
  remote_file_reader.reset();
}

void FileSegment::detach(const FileSegmentGuard::Lock &lock, const LockedKey &) {
  if (download_state == State::DETACHED) return;

  if (!downloader_id.empty()) resetDownloaderUnlocked(lock);
...
void FileSegment::increasePriority() {
  ...
  if (lock.try_lock()) {
    ...
    if (it) {
      if (!cache->tryIncreasePriority(*this)) ProfileEvents::increment(ProfileEvents::FileSegmentFailToIncreasePriority);
...
FileSegment::~FileSegment() {
  try {
    ...
    if (cache_writer) cache_writer->finalize();
  } catch (...) {
    tryLogCurrentException(log);
  }
}

FileSegmentsHolder::FileSegmentsHolder(FileSegments &&file_segments_) : file_segments(std::move(file_segments_)) {
...
FileSegmentsHolder::~FileSegmentsHolder() { reset(); }
...
String FileSegmentsHolder::toString(bool with_state) const { return DB::toString(file_segments, with_state); }
...
String toString(const FileSegments &file_segments, bool with_state) {
  ...
  for (const auto &file_segment : file_segments) {
    ...
    if (file_segment->isUnbound()) ranges += "(unbound)";
    if (with_state) ranges += "(" + FileSegment::stateToString(file_segment->state()) + ")";
  }
  return ranges;
}
