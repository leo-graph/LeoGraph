...
FileSegmentMetadata::FileSegmentMetadata(FileSegmentPtr &&file_segment_) : file_segment(std::move(file_segment_)) {
  switch (file_segment->state()) {
    ...
    default:
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Can create file segment with either EMPTY, DOWNLOADED, DOWNLOADING state, got: {}",
                      FileSegment::stateToString(file_segment->state()));
  }
}

size_t FileSegmentMetadata::size() const { return file_segment->getReservedSize(); }

KeyMetadata::KeyMetadata(const Key &key_, const OriginInfo &origin_, const CacheMetadata *cache_metadata_, bool created_base_directory_)
    : key(key_), origin(origin_), cache_metadata(cache_metadata_), created_base_directory(created_base_directory_) {
  ...
  if (!origin_.weight.has_value()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot create key metadata without user weight");
...
void KeyMetadata::assertAccess(const UserID &user_id_) const {
  if (!checkAccess(user_id_)) {
    throw Exception(ErrorCodes::FILECACHE_ACCESS_DENIED, "Metadata for key {} belongs to another user", key.toString());
  }
}
...
LockedKeyPtr KeyMetadata::lock() {
  ...
  throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot lock key {} (state: {})", key, magic_enum::enum_name(key_state));
}

LockedKeyPtr KeyMetadata::tryLock() {
  auto locked = lockNoStateCheck();
  if (key_state == KeyMetadata::KeyState::ACTIVE) return locked;
...
bool KeyMetadata::createBaseDirectory(bool throw_if_failed) {
  if (created_base_directory.load()) return true;

  std::shared_lock lock(cache_metadata->key_prefix_directory_mutex);

  if (created_base_directory.load(std::memory_order_relaxed)) return true;
  ...
  try {
    ...
    if (!throw_if_failed && (e.code() == std::errc::no_space_on_device || e.code() == std::errc::read_only_file_system ||
                             e.code() == std::errc::permission_denied || e.code() == std::errc::too_many_files_open ||
                             e.code() == std::errc::operation_not_permitted)) {
      LOG_TRACE(cache_metadata->log,
                "Failed to create base directory for key {}, "
...
std::string KeyMetadata::getPath() const { return cache_metadata->getKeyPath(key, origin); }
...
LoggerPtr KeyMetadata::logger() const { return cache_metadata->log; }
...
String CacheMetadata::getFileSegmentPath(const Key &key, size_t offset, FileSegmentKind segment_kind, const OriginInfo &origin) const {
...
LockedKeyPtr CacheMetadata::lockKeyMetadata(const FileCacheKey &key, KeyNotFoundPolicy key_not_found_policy, const OriginInfo &origin,
                                            bool is_initial_load) {
  auto key_metadata = getKeyMetadata(key, key_not_found_policy, origin, is_initial_load);
  if (!key_metadata) return nullptr;

  {
    auto locked_metadata = key_metadata->lockNoStateCheck();
    const auto key_state = locked_metadata->getKeyState();

    if (key_state == KeyMetadata::KeyState::ACTIVE) return locked_metadata;

    if (key_not_found_policy == KeyNotFoundPolicy::THROW) throw Exception(ErrorCodes::BAD_ARGUMENTS, "No such key `{}` in cache", key);
    if (key_not_found_policy == KeyNotFoundPolicy::THROW_LOGICAL)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "No such key `{}` in cache", key);

    if (key_not_found_policy == KeyNotFoundPolicy::RETURN_NULL) return nullptr;
...
KeyMetadataPtr CacheMetadata::getKeyMetadata(const Key &key, KeyNotFoundPolicy key_not_found_policy, const OriginInfo &origin,
                                             bool is_initial_load) {
  ...
  if (it == bucket.end()) {
    ...
    it = bucket.emplace(key, std::make_shared<KeyMetadata>(key, origin, this, is_initial_load)).first;
...
void CacheMetadata::iterate(IterateFunc &&func, const KeyMetadata::UserID &user_id) {
  for (auto &bucket : metadata_buckets) {
    ...
    for (auto &[key, key_metadata] : bucket) {
      if (!key_metadata->checkAccess(user_id)) continue;
      ...
      if (key_state == KeyMetadata::KeyState::REMOVING) continue;

      throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot lock key {}: key does not exist", key_metadata->key);
    }
  }
}
...
class CacheMetadata::IteratorImpl {
 public:
  IteratorImpl(MetadataBuckets &metadata_buckets_, const UserID &user_id_)
      : user_id(user_id_), metadata_buckets(metadata_buckets_), bucket_it(metadata_buckets_.begin()) {}

  bool next(Iterator::OnFileSegmentFunc func) {
    while (true) {
      if (bucket_it == metadata_buckets.end()) return false;

      if (!bucket_lock) bucket_lock = bucket_it->lock();

      if (!key_it.has_value()) key_it = bucket_it->begin();
      ...
      if (!file_segment_it.has_value()) file_segment_it = key->begin();
  ...
  bool next(Iterator::OnFileSegmentFunc func) {
    ...
    while (bucket_it != metadata_buckets.end()) {
      ...
      for (const auto &[_, key_metadata] : *bucket_it) {
        ...
        if (!key_lock) continue;

        result |= key_metadata->size();
        for (const auto &[_, file_segment_metadata] : *key_metadata) func(FileSegment::getInfo(file_segment_metadata->file_segment));
      }
      ++bucket_it;
      if (result) break;
    }
...
bool CacheMetadata::Iterator::next(OnFileSegmentFunc func) {
  if (!impl.has_value()) impl = std::make_shared<IteratorImpl>(metadata_buckets, user_id);

  if (auto *iterator = std::get_if<CacheMetadata::IteratorImplPtr>(&impl.value()); iterator) return (*iterator)->next(func);
...
bool CacheMetadata::Iterator::nextBatch(OnFileSegmentFunc func) {
  if (!impl) impl = std::make_shared<BatchedIteratorImpl>(metadata_buckets, user_id);

  if (auto *iterator = std::get_if<CacheMetadata::BatchedIteratorImplPtr>(&impl.value()); iterator) return (*iterator)->next(func);
...
void CacheMetadata::removeKey(const Key &key, bool if_exists, bool if_releasable, const UserID &user_id) {
  ...
  bool removed_all = locked_key->removeAllFileSegments(if_releasable);
  if (removed_all) removeEmptyKey(bucket, it, *locked_key, lock);
}

CacheMetadata::MetadataBucket::iterator CacheMetadata::removeEmptyKey(MetadataBucket &bucket, MetadataBucket::iterator it,
                                                                      LockedKey &locked_key, const CacheMetadataGuard::Lock &) {
  const auto &key = locked_key.getKey();

  if (!locked_key.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot remove non-empty key: {}", key);
...
void CacheMetadata::cleanupThreadFunc() {
  while (true) {
    Key key;
    {
      ...
      if (cleanup_queue->cancelled) return;
...
class DownloadQueue {
 ...
 public:
  ...
  bool add(FileSegmentPtr file_segment) {
    {
      std::lock_guard lock(mutex);
      if (cancelled || (queue_size_limit && queue.size() >= queue_size_limit)) return false;
...
void CacheMetadata::downloadThreadFunc(const bool &stop_flag) {
  ...
  while (true) {
...

    {
      ...
      if (download_queue->cancelled || stop_flag) return;
      ...
      try {
        {
          ...
          if (!locked_key) continue;

          auto file_segment_metadata = locked_key->tryGetByOffset(offset);
          if (!file_segment_metadata || file_segment_metadata->isEvictingOrRemoved(*locked_key)) continue;

          auto file_segment = file_segment_weak.lock();

          if (!file_segment || file_segment != file_segment_metadata->file_segment ||
        ...
        if (file_segment.getOrSetDownloader() != FileSegment::getCallerId()) continue;
        ...
        if (holder) {
          ...
          LOG_ERROR(log, "Error during background download of {}:{} ({}): {}", file_segment.key(), file_segment.offset(),
...
bool CacheMetadata::setBackgroundDownloadQueueSizeLimit(size_t size) { return download_queue->setQueueLimit(size); }

void CacheMetadata::downloadImpl(FileSegment &file_segment, std::optional<Memory<>> &memory) const {
  LOG_TEST(log, "Downloading {} bytes for file segment {}", file_segment.range().size() - file_segment.getDownloadedSize(),
           file_segment.getInfoForLog());

  size_t size_to_download = file_segment.getSizeForBackgroundDownload();
  if (!size_to_download) return;

  auto buf = file_segment.getRemoteFileReader();
  if (!buf) {
    LOG_TEST(log, "No reader in {}:{} (state: {}, range: {}, downloaded size: {})", file_segment.key(), file_segment.offset(),
             file_segment.state(), file_segment.range().toString(), file_segment.getDownloadedSize());
    return;
  }

  chassert(buf->internalBuffer().empty(), fmt::format("Memory buffer for buffer must have been reset before "
                                                      "being put into background download ({})",
                                                      file_segment.getInfoForLog()));

  if (!memory) memory.emplace(std::min(size_t(DBMS_DEFAULT_BUFFER_SIZE), size_to_download));
  ...
  if (offset != static_cast<size_t>(buf->getPosition())) buf->seek(offset, SEEK_SET);
  ...
  while (size_to_download && !buf->eof()) {
    ...
    if (!file_segment.reserve(size, reserve_space_lock_wait_timeout_milliseconds, failure_reason)) {
      LOG_TEST(log,
               "Failed to reserve space during background download "
               "for {}:{} (downloaded size: {}/{})",
               file_segment.key(), file_segment.offset(), file_segment.getDownloadedSize(), file_segment.range().size());
      break;
    }
...
void CacheMetadata::startup() {
  ...
  for (size_t i = 0; i < download_threads_num; ++i) {
    ...
    download_threads.back()->thread =
        std::make_unique<ThreadFromGlobalPool>([this, thread = download_threads.back()] { downloadThreadFunc(thread->stop_flag); });
  }
...
void CacheMetadata::shutdown() {
  ...
  for (auto &download_thread : download_threads) {
    if (download_thread->thread && download_thread->thread->joinable()) download_thread->thread->join();
  }
  if (cleanup_thread && cleanup_thread->joinable()) cleanup_thread->join();
}

bool CacheMetadata::isBackgroundDownloadEnabled() { return download_threads_num; }

bool CacheMetadata::setBackgroundDownloadThreads(size_t threads_num) {
  if (threads_num == download_threads_num) return false;
  ...
  if (threads_num > download_threads_num) {
...

    {
      ...
      for (size_t i = 0; i < remove_threads; ++i) download_threads[download_threads.size() - 1 - i]->stop_flag = true;
    }
...
bool KeyMetadata::addToDownloadQueue(FileSegmentPtr file_segment) { return cache_metadata->download_queue->add(file_segment); }

void KeyMetadata::addToCleanupQueue() { cache_metadata->cleanup_queue->add(key); }

LockedKey::LockedKey(std::shared_ptr<KeyMetadata> key_metadata_) : key_metadata(key_metadata_), lock(key_metadata->guard.lock()) {}

LockedKey::~LockedKey() {
  if (!key_metadata->empty() || getKeyState() != KeyMetadata::KeyState::ACTIVE) return;
...
void LockedKey::removeFromCleanupQueue() {
  if (key_metadata->key_state != KeyMetadata::KeyState::REMOVING) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot remove non-removing");
...
void LockedKey::markAsRemoved() { key_metadata->key_state = KeyMetadata::KeyState::REMOVED; }
...
KeyMetadata::iterator LockedKey::removeFileSegment(size_t offset, bool can_be_broken, bool invalidate_queue_entry) {
  auto it = key_metadata->find(offset);
  if (it == key_metadata->end()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "There is no offset {}", offset);
...
KeyMetadata::iterator LockedKey::removeFileSegment(size_t offset, const FileSegmentGuard::Lock &segment_lock, bool can_be_broken,
  ...
  auto it = key_metadata->find(offset);
  if (it == key_metadata->end()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "There is no offset {} in key {}", offset, getKey());
...
KeyMetadata::iterator LockedKey::removeFileSegmentImpl(KeyMetadata::iterator it, const FileSegmentGuard::Lock &segment_lock,
                                                       bool can_be_broken, bool invalidate_queue_entry) {
  auto file_segment = it->second->file_segment;

  LOG_TEST(key_metadata->logger(), "Remove from cache. Key: {}, offset: {}, size: {}", getKey(), file_segment->offset(),
           file_segment->reserved_size.load());

  chassert(can_be_broken || file_segment->assertCorrectnessUnlocked(segment_lock));

  if (file_segment->queue_iterator && invalidate_queue_entry) file_segment->queue_iterator->invalidate();
...
#else
      LOG_WARNING(key_metadata->logger(), "Expected path {} to exist, while removing {}:{}", path, getKey(), file_segment->offset());
#endif
    }
  } catch (...) {
    tryLogCurrentException(__PRETTY_FUNCTION__);
...
bool LockedKey::addToDownloadQueue(size_t offset, const FileSegmentGuard::Lock &) {
  auto it = key_metadata->find(offset);
  if (it == key_metadata->end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "There is not offset {}", offset);
...
std::optional<FileSegment::Range> LockedKey::hasIntersectingRange(const FileSegment::Range &range) const {
  ...
  if (it != key_metadata->end())  /// has next range
  {
    ...
    if (!(range < next_range)) return next_range;

    if (it == key_metadata->begin()) return {};
  }

  auto prev_range = std::prev(it)->second->file_segment->range();
  if (!(prev_range < range)) return prev_range;
...
std::shared_ptr<const FileSegmentMetadata> LockedKey::tryGetByOffset(size_t offset) const {
  auto it = key_metadata->find(offset);
  if (it == key_metadata->end()) return nullptr;
  return it->second;
}
...
std::string LockedKey::toString() const {
  ...
  for (auto it = key_metadata->begin(); it != key_metadata->end(); ++it) {
    if (it != key_metadata->begin()) result += ", ";
...
std::vector<FileSegment::Info> LockedKey::sync() {
  ...
  for (auto it = key_metadata->begin(); it != key_metadata->end();) {
    ...
    if (file_segment->isDetached()) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "File segment has unexpected state: DETACHED ({})", file_segment->getInfoForLog());
    }
    ...
    const auto &path = key_metadata->getFileSegmentPath(*file_segment);
    if (!fs::exists(path)) {
      LOG_WARNING(key_metadata->logger(), "File segment has DOWNLOADED state, but file does not exist ({})", file_segment->getInfoForLog());
    ...
    LOG_WARNING(key_metadata->logger(), "File segment has unexpected size. Having {}, expected {} ({})", actual_size, expected_size,
                file_segment->getInfoForLog());
