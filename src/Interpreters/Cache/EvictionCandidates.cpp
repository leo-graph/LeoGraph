...
EvictionInfo::EvictionInfo(QueueID queue_id, QueueEvictionInfoPtr info) { addImpl(queue_id, std::move(info), /* merge_if_exists */ false); }
...
bool EvictionInfo::hasHoldSpace() const {
  for (const auto& [_, elem] : *this)
    if (elem->hasHoldSpace()) return true;
  return false;
}

void EvictionInfo::releaseHoldSpace(const CacheStateGuard::Lock& lock) {
  for (auto& [_, elem] : *this) elem->releaseHoldSpace(lock);
}

void EvictionInfo::add(EvictionInfoPtr&& info) {
  for (auto&& [queue_id, info_] : *info) addImpl(queue_id, std::move(info_), /* merge_if_exists */ false);
}

void EvictionInfo::addOrUpdate(EvictionInfoPtr&& info) {
  for (auto&& [queue_id, info_] : *info) addImpl(queue_id, std::move(info_), /* merge_if_exists */ true);
}

void EvictionInfo::addImpl(const QueueID& queue_id, QueueEvictionInfoPtr info, bool merge_if_exists) {
  size_to_evict += info->size_to_evict;
  elements_to_evict += info->elements_to_evict;
  auto [it, inserted] = emplace(queue_id, std::move(info));
  if (!inserted) {
    if (!merge_if_exists) throw Exception(ErrorCodes::LOGICAL_ERROR, "Queue with id {} already exists", queue_id);

    if (it->second) it->second->merge(std::move(info));
  }
}
...
const QueueEvictionInfo& EvictionInfo::get(const QueueID& queue_id) const {
  if (auto it = find(queue_id); it != end()) {
    ...
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Eviction info for queue  with id {} does not exist ({})", queue_id, toString());
  }
}

std::string EvictionCandidates::FailedCandidates::getFirstErrorMessage() const {
  if (failed_candidates_per_key.empty()) return "";

  const auto& first_failed = failed_candidates_per_key[0];
  if (!first_failed.error_messages.empty()) return first_failed.error_messages[0];

  chassert(false);
  return "";
}

EvictionCandidates::EvictionCandidates() : log(getLogger("EvictionCandidates")) {}
...
EvictionCandidates::~EvictionCandidates() {
  ...
  if (removed_queue_entries) return;
  ...
  for (const auto& [key, key_candidates] : candidates) {
    ...
    for (const auto& candidate : key_candidates.candidates) candidate->resetEvictingFlag();
  }
}
...
void EvictionCandidates::add(const FileSegmentMetadataPtr& candidate, LockedKey& locked_key) {
  ...
  if (inserted) it->second.key_metadata = locked_key.getKeyMetadata();
...
void EvictionCandidates::evict() {
  ...
  if (!removed_queue_entries) queue_entries_to_invalidate.reserve(candidates_size);
    ...
    while (!key_candidates.candidates.empty()) {
      ...
      try {
        if (!candidate->releasable()) {
          throw Exception(ErrorCodes::LOGICAL_ERROR, "Eviction candidate is not releasable: {} (evicting or removed flag: {})",
        ...
        locked_key->removeFileSegment(segment->offset(), segment->lock(), false /* can_be_broken */, false /* invalidate_queue_entry */);
        ...
        if (iterator) queue_entries_to_invalidate.push_back(iterator);
      } catch (...) {
        ...
        LOG_ERROR(log, "Failed to evict file segment ({}): {}", candidate->file_segment->getInfoForLog(), error_message);
      }

      key_candidates.candidates.pop_back();
    }

    if (!failed_key_candidates.candidates.empty()) failed_candidates.failed_candidates_per_key.push_back(failed_key_candidates);
  }
}
