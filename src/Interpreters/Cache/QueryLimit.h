...
class FileCacheQueryLimit {
 public:
  ...
  QueryContextPtr getOrSetQueryContext(const std::string &query_id, const ReadSettings &settings, const CachePriorityGuard::WriteLock &);
  ...
  class QueryContext {
   public:
    ...
    Priority::IteratorPtr tryGet(const Key &key, size_t offset, const CachePriorityGuard::WriteLock &);

    void add(KeyMetadataPtr key_metadata, size_t offset, size_t size, const CachePriorityGuard::WriteLock &);

    void remove(const Key &key, size_t offset, const CachePriorityGuard::WriteLock &);
