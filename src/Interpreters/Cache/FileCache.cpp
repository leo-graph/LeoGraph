...
FileCache::CheckCacheProbability::CheckCacheProbability(double probability, UInt64 seed)
    : rndgen(seed == 0 ? randomSeed() : seed), distribution(normalizeProbability(probability)) {}
...
FileCache::FileCache(const std::string &cache_name, const FileCacheSettings &settings)
    : max_file_segment_size(settings[FileCacheSetting::max_file_segment_size]),
      bypass_cache_threshold(
          settings[FileCacheSetting::enable_bypass_cache_with_threshold] ? settings[FileCacheSetting::bypass_cache_threshold] : 0),
      ...
      metadata(settings[FileCacheSetting::path], settings[FileCacheSetting::background_download_queue_size_limit],
               settings[FileCacheSetting::background_download_threads], write_cache_per_user_directory),
  ...
  switch (settings[FileCacheSetting::cache_policy].value) {
    case FileCachePolicy::LRU: {
      creator_function = [](size_t max_size, size_t max_elements, size_t /*size_ratio*/, size_t /*overcommit_eviction_evict_step*/,
                            String description) -> IFileCachePriorityPtr {
        return std::make_unique<LRUFileCachePriority>(max_size, max_elements, description);
      };
      break;
    }
    case FileCachePolicy::SLRU: {
      creator_function = [](size_t max_size, size_t max_elements, double size_ratio, size_t /*overcommit_eviction_evict_step*/,
                            String description) -> IFileCachePriorityPtr {
        return std::make_unique<SLRUFileCachePriority>(max_size, max_elements, size_ratio, description);
      };
      break;
    }
#if ENABLE_DISTRIBUTED_CACHE
    case FileCachePolicy::LRU_OVERCOMMIT: {
      creator_function = [](size_t max_size, size_t max_elements, double /*size_ratio*/, size_t overcommit_eviction_evict_step,
                            String /*description*/) -> IFileCachePriorityPtr {
        return std::make_unique<OvercommitFileCachePriority<LRUFileCachePriority>>(overcommit_eviction_evict_step, max_size, max_elements,
                                                                                   "overcommit");
      };
      break;
    }
    case FileCachePolicy::SLRU_OVERCOMMIT: {
      creator_function = [](size_t max_size, size_t max_elements, double size_ratio, size_t overcommit_eviction_evict_step,
                            String /*description*/) -> IFileCachePriorityPtr {
        return std::make_unique<OvercommitFileCachePriority<SLRUFileCachePriority>>(overcommit_eviction_evict_step, max_size, max_elements,
                                                                                    size_ratio, "overcommit");
      };
      break;
    }
#else
  ...
  if (use_split_cache) {
    main_priority = std::make_unique<SplitFileCachePriority>(
        creator_function, settings[FileCacheSetting::max_size], settings[FileCacheSetting::max_elements],
        settings[FileCacheSetting::slru_size_ratio], settings[FileCacheSetting::split_cache_ratio], cache_name);
  } else {
    main_priority = creator_function(settings[FileCacheSetting::max_size], settings[FileCacheSetting::max_elements],
                                     settings[FileCacheSetting::slru_size_ratio],
                                     settings[FileCacheSetting::overcommit_eviction_evict_step], cache_name);
  }
  LOG_DEBUG(log, "Using {} cache policy", settings[FileCacheSetting::cache_policy].value);

  if (settings[FileCacheSetting::enable_filesystem_query_cache_limit]) query_limit = std::make_unique<FileCacheQueryLimit>();
...
FileCache::OriginInfo FileCache::getCommonOriginWithSegmentKeyType(const fs::path &filename) const {
  ...
  if (!use_split_cache) return origin;
...
bool FileCache::isInitialized() const { return is_initialized; }

void FileCache::throwInitExceptionIfNeeded() {
  if (load_metadata_asynchronously) return;

  std::lock_guard lock(init_mutex);
  if (init_exception) std::rethrow_exception(init_exception);
}

const String &FileCache::getBasePath() const { return metadata.getBaseDirectory(); }
...
String FileCache::getKeyPath(const Key &key, const OriginInfo &origin) const { return metadata.getKeyPath(key, origin); }
...
void FileCache::assertInitialized() const {
  ...
  if (init_exception) std::rethrow_exception(init_exception);
  if (!is_initialized) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cache not initialized");
}

void FileCache::initialize() {
  // Prevent initialize() from running twice. This may be caused by two cache disks being created with the same path (see
  // integration/test_filesystem_cache).
  callOnce(initialize_called, [&] {
    bool need_to_load_metadata = fs::exists(getBasePath());
    try {
      if (!need_to_load_metadata) fs::create_directories(getBasePath());
      ...
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Failed to retrieve filesystem information for cache path {}. Error: {}", getBasePath(),
                      e.what());
    } catch (...) {
      init_exception = std::current_exception();
...
void FileCache::initializeImpl(bool load_metadata) {
  ...
  if (keep_current_size_to_max_ratio != 1 || keep_current_elements_to_max_ratio != 1) {
    keep_up_free_space_ratio_task = Context::getGlobalContextInstance()->getSchedulePool().createTask(
        StorageID::createEmpty(), log->name(), [this] { freeSpaceRatioKeepingThreadFunc(); });
...
CachePriorityGuard::WriteLock FileCache::lockCache() const { return cache_guard.writeLock(); }
...
FileSegments FileCache::getImpl(const LockedKey &locked_key, const FileSegment::Range &range, size_t file_segments_limit) const {
  ...
  if (bypass_cache_threshold && range.size() > bypass_cache_threshold) {
    auto file_segment = std::make_shared<FileSegment>(locked_key.getKey(), range.left, range.size(), FileSegment::State::DETACHED);
    return {file_segment};
  }
  ...
  auto add_to_result = [&](const FileSegmentMetadata &file_segment_metadata) {
    if (file_segments_limit && result.size() == file_segments_limit) return false;

    FileSegmentPtr file_segment;
    if (file_segment_metadata.isEvictingOrRemoved(locked_key)) {
      file_segment = std::make_shared<FileSegment>(locked_key.getKey(), file_segment_metadata.file_segment->offset(),
                                                   file_segment_metadata.file_segment->range().size(), FileSegment::State::DETACHED);
    } else {
  ...
  if (segment_it == file_segments.end()) {
    ...
    if (file_segment_metadata.file_segment->range().right < range.left) return {};

    if (!add_to_result(file_segment_metadata)) return result;
  } else  /// segment_it <-- segmment{k}
  {
    if (segment_it != file_segments.begin()) {
      ...
      if (range.left <= prev_range.right) {
        ...
        ///       ^
        ///       range.left
        if (!add_to_result(prev_file_segment_metadata)) return result;
      }
    }
    ...
    while (segment_it != file_segments.end()) {
      ...
      if (range.right < file_segment_metadata.file_segment->range().left) break;
...
FileSegments FileCache::createFileSegmentsFromRanges(LockedKey &locked_key, const std::vector<FileSegment::Range> &ranges,
                                                     size_t &file_segments_count, size_t file_segments_limit,
                                                     const CreateFileSegmentSettings &create_settings) {
  FileSegments result;
  for (const auto &r : ranges) {
    if (file_segments_limit && file_segments_count >= file_segments_limit) break;
...
void FileCache::fillHolesWithEmptyFileSegments(LockedKey &locked_key, FileSegments &file_segments, const FileSegment::Range &range,
                                               size_t non_aligned_right_offset, size_t file_segments_limit,
                                               bool fill_with_detached_file_segments, const CreateFileSegmentSettings &create_settings) {
  ...
  auto is_limit_reached = [&]() -> bool { return file_segments_limit && processed_count >= file_segments_limit; };
  ...
  while (current_pos <= range.right && it != file_segments.end() && !is_limit_reached()) {
    ...
    if (fill_with_detached_file_segments) {
      auto file_segment =
          std::make_shared<FileSegment>(locked_key.getKey(), current_pos, hole_size, FileSegment::State::DETACHED, create_settings);
  ...
  if (current_pos <= non_aligned_right_offset) {
    ...
    if (fill_with_detached_file_segments) {
      auto file_segment = std::make_shared<FileSegment>(locked_key.getKey(), current_pos, non_aligned_hole_size,
                                                        FileSegment::State::DETACHED, create_settings);
      ...
      if (is_limit_reached()) erase_unprocessed();
    }
  }
}

FileSegmentsHolderPtr FileCache::trySet(const Key &key, size_t offset, size_t size, const CreateFileSegmentSettings &create_settings,
  ...
  if (!file_segments.empty()) return nullptr;

  if (create_settings.unbounded) {
    /// If the file is unbounded, we can create a single file_segment_metadata for it.
    auto file_segment_metadata_it = addFileSegment(*locked_key, offset, size, FileSegment::State::EMPTY, create_settings);
...
FileSegmentsHolderPtr FileCache::set(const Key &key, size_t offset, size_t size, const CreateFileSegmentSettings &create_settings,
                                     const OriginInfo &origin) {
  if (auto holder = trySet(key, offset, size, create_settings, origin)) return holder;

  throw Exception(ErrorCodes::LOGICAL_ERROR, "Having intersection with already existing cache");
}

FileSegmentsHolderPtr FileCache::getOrSet(const Key &key, size_t offset, size_t size, size_t file_size,
                                          const CreateFileSegmentSettings &create_settings, size_t file_segments_limit,
                                          const OriginInfo &origin_info, std::optional<size_t> boundary_alignment_) {
  ...
  auto aligned_end_offset = (file_size ? std::min(FileCacheUtils::roundUpToMultiple(initial_range.right + 1, alignment), file_size)
                                       : FileCacheUtils::roundUpToMultiple(initial_range.right + 1, alignment)) -
                            1;

  chassert(aligned_offset <= initial_range.left);
  chassert(aligned_end_offset >= initial_range.right);

  auto locked_key = metadata.lockKeyMetadata(key, CacheMetadata::KeyNotFoundPolicy::CREATE_EMPTY, origin_info);
  ...
  if (file_segments_limit) {
    chassert(file_segments.size() <= file_segments_limit);
    if (file_segments.size() == file_segments_limit) result_range.right = aligned_end_offset = file_segments.back()->range().right;
  }
  ...
  if (aligned_offset < result_range.left && has_uncovered_prefix) {
    auto prefix_range =
        FileSegment::Range(aligned_offset, file_segments.empty() ? result_range.left - 1 : file_segments.front()->range().left - 1);
  ...
  if (file_segments.empty()) {
    ...
    fillHolesWithEmptyFileSegments(*locked_key, file_segments, result_range, offset + size - 1, file_segments_limit,
                                   /* fill_with_detached */ false, create_settings);
  ...
  chassert(file_segments_limit ? file_segments.back()->range().left <= initial_range.right
                               : file_segments.back()->range().contains(initial_range.right),
           fmt::format("Unexpected state. Back: {}, result range: {}, "
                       "limit: {}, initial offset: {}, initial size: {}, file size: {}",
                       file_segments.back()->range().toString(), result_range.toString(), file_segments_limit, offset, size, file_size));
...
FileSegmentsHolderPtr FileCache::get(const Key &key, size_t offset, size_t size, size_t file_segments_limit, const UserID &user_id) {
  ...
  if (auto locked_key = metadata.lockKeyMetadata(key, CacheMetadata::KeyNotFoundPolicy::RETURN_NULL, OriginInfo(user_id));
    ...
    if (!file_segments.empty()) {
      if (file_segments_limit) {
        chassert(file_segments.size() <= file_segments_limit);
        if (file_segments.size() == file_segments_limit) range.right = file_segments.back()->range().right;
      }

      fillHolesWithEmptyFileSegments(*locked_key, file_segments, range, offset + size - 1, file_segments_limit,
                                     /* fill_with_detached */ true, CreateFileSegmentSettings{});
  ...
  if (!holder)
    holder =
        std::make_unique<FileSegmentsHolder>(FileSegments{std::make_shared<FileSegment>(key, offset, size, FileSegment::State::DETACHED)});

  assertCacheCorrectnessWithProbability();
  return holder;
}

KeyMetadata::iterator FileCache::addFileSegment(LockedKey &locked_key, size_t offset, size_t size, FileSegment::State state,
                                                const CreateFileSegmentSettings &create_settings) {
  ...
  if (auto intersecting_range = locked_key.hasIntersectingRange(range)) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Attempt to add intersecting file segment in cache ({} intersects {})", range.toString(),
                    intersecting_range->toString());
  }

  FileSegment::State result_state = state;

  auto file_segment = std::make_shared<FileSegment>(key, offset, size, result_state, create_settings,
                                                    metadata.isBackgroundDownloadEnabled(), this, locked_key.getKeyMetadata());
  ...
  if (!inserted) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed to insert {}:{}: entry already exists", key, offset);
  }
...
bool FileCache::tryIncreasePriority(FileSegment &file_segment) {
  return main_priority->tryIncreasePriority(*file_segment.getQueueIterator(), file_segment.isCompleted(), cache_guard, cache_state_guard);
}

bool FileCache::tryReserve(FileSegment &file_segment, size_t size, FileCacheReserveStat &reserve_stat, const OriginInfo &origin_info,
                           size_t lock_wait_timeout_milliseconds, std::string &failure_reason) {
  ...
  SCOPE_EXIT({ cache_reserve_active_threads.fetch_sub(1, std::memory_order_relaxed); });

  LOG_TEST(log, "Trying to reserve space ({} bytes) for {}:{}", size, file_segment.key(), file_segment.offset());

  return doTryReserve(file_segment, size, reserve_stat, origin_info, lock_wait_timeout_milliseconds, failure_reason);
}

bool FileCache::doTryReserve(FileSegment &file_segment, size_t size, FileCacheReserveStat &reserve_stat, const OriginInfo &origin_info,
                             size_t /* lock_wait_timeout_milliseconds */, std::string &failure_reason) {
    ...
    if (query_limit) {
      ...
      if (query_context) {
        ...
        if (!query_priority->canFit(size, required_elements_num, lock, /* reservee */ nullptr, origin_info) &&
          ...
          LOG_TEST(log,
                   "Query limit exceeded, space reservation failed, "
        ...
        query_eviction_info = query_priority->collectEvictionInfo(size, required_elements_num, main_priority_iterator.get(),
                                                                  /* is_total_space_cleanup */ false, origin_info, lock);
      }
    }

    /// Check server-wide cache limits.
    main_eviction_info = main_priority->collectEvictionInfo(size, required_elements_num, main_priority_iterator.get(),
                                                            /* is_total_space_cleanup */ false, origin_info, lock);
  ...
  if (!doEviction(*main_eviction_info, query_eviction_info.get(), file_segment, origin_info, main_priority_iterator, reserve_stat,
                  eviction_candidates, invalidated_entries, query_priority, failure_reason)) {
  ...
  if (!main_priority_iterator || eviction_candidates.requiresAfterEvictWrite()) {
    ...
    if (!main_priority_iterator) {
      ...
      main_priority_iterator = main_priority->add(file_segment.getKeyMetadata(), file_segment.offset(),
                                                  /* size */ 0, lock, nullptr);

      if (query_context) {
        query_priority_iterator = query_context->tryGet(file_segment.key(), file_segment.offset(), lock);
        if (!query_priority_iterator) query_context->add(file_segment.getKeyMetadata(), file_segment.offset(), /* size */ 0, lock);
      }
    }
  } else if (!invalidated_entries.empty()) {
    if (auto lock = cache_guard.tryWriteLock(); lock.owns_lock()) IFileCachePriority::removeEntries(invalidated_entries, lock);
  }
  ...
  try {
    ...
    main_eviction_info->releaseHoldSpace(lock);
    if (query_eviction_info) query_eviction_info->releaseHoldSpace(lock);

    main_priority_iterator->check(lock);
    eviction_candidates.afterEvictState(lock);
    main_priority_iterator->incrementSize(size, lock);

    if (query_priority_iterator) query_priority_iterator->incrementSize(size, lock);
  } catch (...) {
    ...
    if (main_priority_iterator && added_new_main_entry) main_priority_iterator->invalidate();

    throw;
  }

  /// Mark that size was successfully updated.
  if (added_new_main_entry) file_segment.setQueueIterator(main_priority_iterator);
...
bool FileCache::doEviction(const EvictionInfo &main_eviction_info, const EvictionInfo *query_eviction_info, FileSegment &file_segment,
                           const OriginInfo &origin_info, const IFileCachePriority::IteratorPtr &main_priority_iterator,
                           FileCacheReserveStat &reserve_stat, EvictionCandidates &eviction_candidates,
                           IFileCachePriority::InvalidatedEntriesInfos &invalidated_entries, Priority *query_priority,
                           std::string &failure_reason) {
  LOG_TEST(log, "Main eviction info {}", main_eviction_info.toString());
  if (query_priority) LOG_TEST(log, "Query eviction info {}", query_eviction_info->toString());

  if (!main_eviction_info.requiresEviction() && (!query_eviction_info || !query_eviction_info->requiresEviction())) return true;
  ...
  /// This is done under "read (shared) lock" for query priority queue.
  {
    auto on_cannot_evict_enough_space_message = [&](const IFileCachePriority &priority) {
      ...
      return fmt::format(
          ...
          stat.toString(), priority.getSizeApprox(), priority.getSizeLimitApprox(), priority.getElementsCountApprox(),
          priority.getElementsLimitApprox(), CurrentMetrics::get(CurrentMetrics::FilesystemCacheDownloadQueueElements));
    };

    bool continue_from_last_eviction_pos = cache_reserve_active_threads.load(std::memory_order_relaxed) > 1;
    if (!continue_from_last_eviction_pos) main_priority->resetEvictionPos();

    if (query_eviction_info && query_eviction_info->requiresEviction()) {
      chassert(query_priority);
      if (!query_priority->collectCandidatesForEviction(*query_eviction_info, reserve_stat, eviction_candidates, invalidated_entries,
                                                        /* reservee */ {}, continue_from_last_eviction_pos,
                                                        /* max_candidates_size */ 0,
                                                        /* is_total_space_cleanup */ false, origin_info, cache_guard, cache_state_guard)) {
    ...
    if (!main_priority->collectCandidatesForEviction(main_eviction_info, reserve_stat, eviction_candidates, invalidated_entries,
                                                     main_priority_iterator, continue_from_last_eviction_pos,
                                                     /* max_candidates_size */ 0,
                                                     /* is_total_space_cleanup */ false, origin_info, cache_guard, cache_state_guard)) {
  ...
  if (eviction_candidates.size() > 0) {
    ...
    if (failed_candidates.size() > 0) {
      on_failed_evict();
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed to evict {} file segments (first error: {})", failed_candidates.size(),
                      failed_candidates.getFirstErrorMessage());
    }
  }
  return true;
}
...
void FileCache::freeSpaceRatioKeepingThreadFunc() {
  ...
  std::unique_ptr<EvictionInfo> eviction_info;
  {
    ...
    const size_t size_to_evict = current_size && (current_size > desired_size) ? current_size - desired_size : 0;
    const size_t elements_to_evict =
        current_elements_num && (current_elements_num > desired_elements_num) ? current_elements_num - desired_elements_num : 0;
    ...
    LOG_TRACE(log,
              "Starting an iteration to maintain desired size. "
              "Current size {}/{}, elements {}/{}. Desired size: {}, desired elements: {}",
              main_priority->getSize(lock), size_limit, main_priority->getElementsCount(lock), elements_limit, desired_size,
              desired_elements_num);

    eviction_info = main_priority->collectEvictionInfo(size_to_evict, elements_to_evict,
                                                       /* reservee */ nullptr,
  ...
  IFileCachePriority::InvalidatedEntriesInfos invalidated_entries;
  main_priority->collectCandidatesForEviction(*eviction_info, stat, eviction_candidates, invalidated_entries,
                                              /* reservee */ nullptr,
                                              /* continue_from_last_eviction_pos */ false,
                                              /* max_candidates_size */ keep_up_free_space_remove_batch,
                                              /* is_total_space_cleanup */ true, getInternalOrigin(), cache_guard, cache_state_guard);
  ...
  LOG_TRACE(log, "Free space ratio keeping thread finished with status `{}` in {} ms", desired_size_status, watch.elapsedMilliseconds());

  [[maybe_unused]] bool scheduled = false;
  switch (desired_size_status) {
    case IFileCachePriority::CollectStatus::SUCCESS:
      [[fallthrough]];
...
void FileCache::iterate(IterateFunc &&func, const UserID &user_id) {
  metadata.iterate(
      [&](const LockedKey &locked_key) {
        for (const auto &file_segment_metadata : locked_key) func(FileSegment::getInfo(file_segment_metadata.second->file_segment));
      },
      user_id);
}

FileCache::CacheIteratorPtr FileCache::getCacheIterator(const UserID &user_id) { return metadata.getIterator(user_id); }
...
void FileCache::removeFileSegmentIfExists(const Key &key, size_t offset, const UserID &user_id) {
  ...
  if (locked_key) locked_key->removeFileSegmentIfExists(offset);
}

void FileCache::removePathIfExists(const String &path, const UserID &user_id) { removeKeyIfExists(Key::fromPath(path), user_id); }
...
void FileCache::loadMetadataImpl() {
  ...
  auto get_keys_dir_to_process_with_user_dir = [&, initialized = false, user_it = fs::directory_iterator{}, origin = OriginInfo{},
                                                key_prefix_it = fs::directory_iterator{},
                                                get_key_mutex = std::mutex()]() mutable -> std::optional<std::pair<fs::path, OriginInfo>> {
    ...
    while (true) {
      if (key_prefix_it == fs::directory_iterator{}) {
        ...
        if (user_it->path().filename() == "status") continue;
  ...
  if (!use_split_cache) key_types_to_load.push_back(FileSegmentKeyType::General);

  auto get_keys_dir_to_process = [&, key_prefix_it = fs::directory_iterator{}, key_type_index = 0ull, origin = getCommonOrigin(),
                                  get_key_mutex = std::mutex()]() mutable -> std::optional<std::pair<fs::path, OriginInfo>> {
    std::lock_guard lk(get_key_mutex);

    while (true) {
      if (key_prefix_it == fs::directory_iterator()) {
        if (key_type_index == key_types_to_load.size()) return std::nullopt;
      ...
      if (key_prefix_it->is_directory() && key_dir != getKeyTypePrefix(FileSegmentKeyType::Data) &&
...
void FileCache::loadMetadataForKeys(const fs::path &keys_dir, const OriginInfo &origin_info) {
  ...
  for (; key_it != fs::directory_iterator(); key_it++) {
    ...
    if (!key_it->is_directory()) {
      LOG_DEBUG(log, "Unexpected file: {} (not a directory). Expected a directory", key_directory.string());
      continue;
    }
    ...
    auto key_metadata = metadata.getKeyMetadata(key, CacheMetadata::KeyNotFoundPolicy::CREATE_EMPTY, origin_info,
    ...
    for (fs::directory_iterator offset_it{key_directory}; offset_it != fs::directory_iterator(); ++offset_it) {
...

      {
        ...
        if (limits_satisfied) cache_it = main_priority->add(key_metadata, offset, size, lock, &state_lock, /* best_effort */ true);
      ...
      if (limits_satisfied) {
        bool inserted = false;
        try {
          auto file_segment = std::make_shared<FileSegment>(key, offset, size, FileSegment::State::DOWNLOADED,
                                                            CreateFileSegmentSettings(segment_kind), false, this, key_metadata, cache_it);
...
void FileCache::deactivateBackgroundOperations() {
  ...
  stop_loading_metadata = true;
  if (load_metadata_main_thread.joinable()) load_metadata_main_thread.join();

  metadata.shutdown();
  if (keep_up_free_space_ratio_task) keep_up_free_space_ratio_task->deactivate();
}
...
size_t FileCache::getMaxCacheSize() const { return main_priority->getSizeLimitApprox(); }
...
void FileCache::assertCacheCorrectness() {
  ...
  main_priority->iterate(
      [](LockedKey &, const FileSegmentMetadataPtr &file_segment_metadata) {
        chassert(file_segment_metadata->file_segment->assertCorrectness());
...
void FileCache::applySettingsIfPossible(const FileCacheSettings &new_settings, FileCacheSettings &actual_settings) {
  ...
  if (new_settings[FileCacheSetting::background_download_queue_size_limit] !=
  ...
  if (new_settings[FileCacheSetting::background_download_max_file_segment_size] !=
  ...
  {
    SizeLimits desired_limits{.max_size = new_settings[FileCacheSetting::max_size],
                              .max_elements = new_settings[FileCacheSetting::max_elements],
                              .slru_size_ratio = new_settings[FileCacheSetting::slru_size_ratio]};
    SizeLimits current_limits{.max_size = actual_settings[FileCacheSetting::max_size],
    ...
    if (allow_dynamic_cache_resize && do_dynamic_resize && !main_priority->isOvercommitEviction()) {
      ...
      LOG_WARNING(log,
                  "Filesystem cache size was modified, "
      ...
      LOG_DEBUG(log,
                "Nothing to resize in filesystem cache. "
  ...
  if (new_settings[FileCacheSetting::max_file_segment_size] != actual_settings[FileCacheSetting::max_file_segment_size]) {
    max_file_segment_size = actual_settings[FileCacheSetting::max_file_segment_size] =
        new_settings[FileCacheSetting::max_file_segment_size];
  }
}
...
FileCache::SizeLimits FileCache::doDynamicResize(const SizeLimits &prev_limits, const SizeLimits &desired_limits) {
  ...
  bool modified_size_limit = false;
  {
    ...
    try {
      ...
      if (!cache_lock.owns_lock()) cache_lock.lock();
      ...
      if (result_limits.max_size != max_size || result_limits.max_elements != max_elements) {
        LOG_DEBUG(log, "Resetting max size from {} to {}, max elements from {} to {}", result_limits.max_size, max_size,
  ...
  if (modified_size_limit) {
    LOG_INFO(log, "Changed max_size from {} to {}, max_elements from {} to {}", prev_limits.max_size, result_limits.max_size,
    ...
    LOG_WARNING(log,
                "Unable to modify size limit from {} to {}, elements limit from {} to {}. "
                ...
                prev_limits.max_size, desired_limits.max_size, prev_limits.max_elements, desired_limits.max_elements);
  }
...
bool FileCache::doDynamicResizeImpl(const SizeLimits &prev_limits, const SizeLimits &desired_limits, SizeLimits &result_limits,
  ...
  auto eviction_info =
      main_priority->collectEvictionInfoForResize(desired_limits.max_size, desired_limits.max_elements, getInternalOrigin(), state_lock);
  ...
  if (!eviction_info->requiresEviction()) {
    ...
    main_priority->modifySizeLimits(desired_limits.max_size, desired_limits.max_elements, desired_limits.slru_size_ratio, state_lock);

    result_limits = desired_limits;

    LOG_INFO(log, "Nothing needs to be evicted for new size limits ({})", main_priority->getStateInfoForLog(state_lock));
    return true;
  }
  ...
  FileCacheReserveStat stat;
  IFileCachePriority::InvalidatedEntriesInfos invalidated_entries;
  if (!main_priority->collectCandidatesForEviction(*eviction_info, stat, eviction_candidates, invalidated_entries,
                                                   /* reservee */ nullptr,
                                                   /* continue_from_last_eviction_pos */ false,
                                                   /* max_candidates_size */ 0,
                                                   /* is_total_space_cleanup */ true, getInternalOrigin(), cache_guard,
                                                   cache_state_guard)) {
  ...
  if (failed_candidates.size() == 0) {
    LOG_INFO(log, "Successfully evicted {} cache elements needed for resize", eviction_candidates.size());

    result_limits = desired_limits;
    return true;
  }

  result_limits.max_size = std::min(prev_limits.max_size, desired_limits.max_size + failed_candidates.total_cache_size);

  result_limits.max_elements = std::min(prev_limits.max_elements, desired_limits.max_elements + failed_candidates.total_cache_elements);

  LOG_INFO(log,
           "Having {} failed candidates with total size {}. "
           "Will set current limits as {} in size and {} in elements number",
           failed_candidates.total_cache_elements, failed_candidates.total_cache_size, result_limits.max_size, result_limits.max_elements);
  ...
  main_priority->modifySizeLimits(result_limits.max_size, result_limits.max_elements, result_limits.slru_size_ratio, state_lock);
  ...
  for (const auto &[key_metadata, key_candidates, _] : failed_candidates.failed_candidates_per_key) {
    ...
    for (const auto &candidate : key_candidates) {
      ...
      LOG_DEBUG(log, "Adding back file segment after failed eviction: {}:{}, size: {}", file_segment->key(), file_segment->offset(),
                file_segment->getDownloadedSize());

      auto main_priority_iterator =
          main_priority->add(key_metadata, file_segment->offset(), file_segment->getDownloadedSize(), cache_write_lock, &state_lock, false);
...
FileCache::QueryContextHolderPtr FileCache::getQueryContextHolder(const String &query_id, const ReadSettings &read_settings) {
  if (!query_limit || read_settings.filesystem_cache_max_download_size == 0) return {};
