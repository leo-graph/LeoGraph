...
FileCacheFactory::FileCacheData::FileCacheData(FileCachePtr cache_, const FileCacheSettings& settings_, const std::string& config_path_)
    : cache(cache_), config_path(config_path_), settings(settings_) {}
...
FileCacheFactory::Caches FileCacheFactory::getUniqueInstances() {
  std::lock_guard lock(mutex);
  Caches caches;
  for (const auto& [_, cache_data] : caches_by_name) caches.insert(cache_data);
  return caches;
}
...
FileCachePtr FileCacheFactory::get(const std::string& cache_name) {
  ...
  if (it == caches_by_name.end()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "There is no cache by name `{}`", cache_name);
...
FileCachePtr FileCacheFactory::getOrCreate(const std::string& cache_name, const FileCacheSettings& file_cache_settings,
  ...
  if (it == caches_by_name.end()) {
    ...
    bool inserted;
    std::tie(it, inserted) = caches_by_name.emplace(cache_name, std::make_unique<FileCacheData>(cache, file_cache_settings, config_path));

    if (!inserted) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cache with name {} exists, but it has a different path", cache_name);
    }
...
FileCachePtr FileCacheFactory::create(const std::string& cache_name, const FileCacheSettings& file_cache_settings,
  ...
  if (it != caches_by_name.end()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cache with name {} already exists", cache_name);
  ...
  if (it == caches_by_name.end()) {
    auto cache = std::make_shared<FileCache>(cache_name, file_cache_settings);
    it = caches_by_name.emplace(cache_name, std::make_unique<FileCacheData>(cache, file_cache_settings, config_path)).first;
  } else if (it->second->getSettings() != file_cache_settings) {
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    "Found more than one cache configuration with the same path, "
                    "but with different cache settings ({} and {})",
...
void FileCacheFactory::updateSettingsFromConfig(const Poco::Util::AbstractConfiguration& config) {
  ...
  for (const auto& [_, cache_info] : caches_by_name_copy) {
    if (cache_info->config_path.empty() || checked_paths.contains(cache_info->config_path)) continue;
...
void FileCacheFactory::loadDefaultCaches(const Poco::Util::AbstractConfiguration& config, ContextPtr context) {
  ...
  for (const auto& name : cache_names) {
    ...
    settings.loadFromConfig(config, config_path, getPathPrefixForRelativeCachePath(context),
                            /* default_cache_path */ "");
...
std::string getPathPrefixForRelativeCachePath(ContextPtr context) {
  ...
  if (!config_fs_caches_dir.empty()) return config_fs_caches_dir;
