 ...
 public:
  ...
  struct Entry {
    ...
    void setEvictingFlag(const LockedKey &) {
      ...
      chassert(prev == State::Active, printUnexpectedState(prev, "Active", "Evicting"));
    }

    void setMovingFlag(const LockedKey &) {
      [[maybe_unused]] auto prev = state.exchange(State::Moving, std::memory_order_relaxed);
      chassert(prev == State::Active, printUnexpectedState(prev, "Active", "Moving"));
    }
    ...
    void setRemoved(const CachePriorityGuard::WriteLock &) {
      ...
      chassert(prev == State::Active || prev == State::Evicting || prev == State::Invalidated,
    ...
    void setInvalidatedFlag() {
      ...
      chassert(prev == State::Active || prev == State::Evicting || prev == State::Moving,
    ...
    void resetFlag(State from_state, State to_state = State::Active) {
      ...
      chassert(prev == from_state,
               printUnexpectedState(prev, magic_enum::enum_name(from_state), fmt::format("{}", magic_enum::enum_name(to_state))));
    }

   private:
    std::string printUnexpectedState(State prev_state, std::string_view expected_state, std::string type) const {
      return fmt::format("Previous state is {}, but expected state to be {} while setting {} flag for {}",
  ...
  virtual EvictionInfoPtr collectEvictionInfo(size_t size, size_t elements, IFileCachePriority::Iterator *reservee,
                                              bool is_total_space_cleanup, const IFileCachePriority::OriginInfo &origin,
  ...
  virtual void iterate(IterateFunc func, FileCacheReserveStat &stat, const CachePriorityGuard::ReadLock &) = 0;
  ...
  virtual bool tryIncreasePriority(Iterator &iterator, bool is_space_reservation_complete, CachePriorityGuard &queue_guard,
  ...
  virtual bool collectCandidatesForEviction(const EvictionInfo &eviction_info, FileCacheReserveStat &stat, EvictionCandidates &res,
                                            InvalidatedEntriesInfos &invalidated_entries, IteratorPtr reservee,
                                            bool continue_from_last_eviction_pos, size_t max_candidates_size, bool is_total_space_cleanup,
                                            const OriginInfo &origin_info, CachePriorityGuard &, CacheStateGuard &) = 0;
  ...
  virtual bool modifySizeLimits(size_t max_size_, size_t max_elements_, double size_ratio_, const CacheStateGuard::Lock &) = 0;
  ...
  virtual EvictionInfoPtr collectEvictionInfoForResize(size_t desired_max_size, size_t desired_max_elements, const OriginInfo &origin_info,
  ...
  class HoldSpace : private boost::noncopyable {
   public:
    HoldSpace(size_t size_, size_t elements_, IFileCachePriority &priority_, const CacheStateGuard::Lock &lock)
   ...
   private:
    ...
    void releaseUnlocked() {
      if (released || (!size && !elements)) return;
      released = true;
