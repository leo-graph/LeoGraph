...
struct FileSegmentMetadata : private boost::noncopyable {
  ...
  void setEvictingFlag(const LockedKey &lock) const {
    auto iterator = getQueueIterator();
    if (!iterator) throw Exception(ErrorCodes::LOGICAL_ERROR, "Iterator is not set");
...
struct KeyMetadata : private std::map<size_t, FileSegmentMetadataPtr>,
  ...
  KeyMetadata(const Key &key_, const OriginInfo &origin_, const CacheMetadata *cache_metadata_, bool created_base_directory_ = false);
...
class CacheMetadata : private boost::noncopyable {
 ...
 public:
  ...
  KeyMetadataPtr getKeyMetadata(const Key &key, KeyNotFoundPolicy key_not_found_policy, const OriginInfo &origin,
                                bool is_initial_load = false);

  LockedKeyPtr lockKeyMetadata(const Key &key, KeyNotFoundPolicy key_not_found_policy, const OriginInfo &origin,
                               bool is_initial_load = false);
 ...
 private:
  ...
  MetadataBucket::iterator removeEmptyKey(MetadataBucket &bucket, MetadataBucket::iterator it, LockedKey &,
...
struct LockedKey : private boost::noncopyable {
  ...
  auto emplace(Args &&...args) {
    return key_metadata->emplace(std::forward<Args>(args)...);
  }
  ...
  KeyMetadata::iterator removeFileSegment(size_t offset, bool can_be_broken = false, bool invalidate_queue_entry = true);

  KeyMetadata::iterator removeFileSegmentIfExists(size_t offset, bool can_be_broken = false, bool invalidate_queue_entry = true);
