...
LRUFileCachePriority::LRUFileCachePriority(size_t max_size_, size_t max_elements_, const std::string &description_, StatePtr state_)
...
IFileCachePriority::IteratorPtr LRUFileCachePriority::add(  /// NOLINT
    ...
    const CacheStateGuard::Lock *state_lock, bool) {
  return std::make_shared<LRUIterator>(add(std::make_shared<Entry>(key_metadata->key, offset, size, key_metadata), lock, state_lock));
}

LRUFileCachePriority::LRUIterator LRUFileCachePriority::add(EntryPtr entry, const CachePriorityGuard::WriteLock &,
  ...
  for (const auto &queue_entry : queue) {
    if (queue_entry->getState() == Entry::State::Active && queue_entry->key == entry->key && queue_entry->offset == entry->offset) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Attempt to add duplicate queue entry to queue: {}", entry->toString());
    }
  }
#endif

  if (entry->size && !canFit(entry->size, /* elements */ 1, *state_lock)) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Not enough space to add a new entry {}. Current state: {}", entry->toString(),
                    getStateInfoForLog(*state_lock));
  }

  auto iterator = queue.insert(queue.end(), entry);

  if (entry->size) state->add(entry->size, /* elements */ 1, *state_lock);

  LOG_TEST(log, "Added entry into LRU queue, key: {}, offset: {}, size: {}", entry->key, entry->offset, entry->size.load());

  return LRUIterator(this, iterator);
}

LRUFileCachePriority::LRUQueue::iterator LRUFileCachePriority::remove(LRUQueue::iterator it, const CachePriorityGuard::WriteLock &lock) {
  /// If size is 0, entry is invalidated, current_elements_num was already updated.
  auto &entry = **it;
  if (entry.size) state->sub(entry.size, /* elements */ 1);

  entry.setRemoved(lock);

  LOG_TEST(log, "Removed entry from LRU queue, key: {}, offset: {}, size: {}", entry.key, entry.offset, entry.size.load());

  moveEvictionPosIfEqual(it, lock);
  return queue.erase(it);
}

LRUFileCachePriority::LRUIterator::LRUIterator(LRUFileCachePriority *cache_priority_, LRUQueue::iterator iterator_)
    : cache_priority(cache_priority_), iterator(iterator_), entry(*iterator) {
  assertValid();
}

LRUFileCachePriority::LRUIterator::LRUIterator(const LRUIterator &other) { *this = other; }
...
void LRUFileCachePriority::iterate(IterateFunc func, FileCacheReserveStat &stat, const CachePriorityGuard::ReadLock &lock) {
  InvalidatedEntriesInfos invalidated_entries;
  iterateImpl(queue.begin(), func, stat, invalidated_entries, lock);
}

LRUFileCachePriority::LRUQueue::iterator LRUFileCachePriority::iterateImpl(LRUQueue::iterator start_pos, IterateFunc func,
...
bool LRUFileCachePriority::canFit(  /// NOLINT
    size_t size, size_t elements, const CacheStateGuard::Lock &lock, IteratorPtr, const OriginInfo &, bool) const {
  return canFit(size, elements, 0, 0, lock);
}

bool LRUFileCachePriority::canFit(size_t size, size_t elements, size_t released_size_assumption, size_t released_elements_assumption,
                                  const CacheStateGuard::Lock &lock, const size_t *max_size_, const size_t *max_elements_) const {
  const size_t current_size = state->getSize(lock);
  const size_t current_elements_num = state->getElementsCount(lock);
  return (max_size == 0 || (current_size + size - released_size_assumption <= (max_size_ ? *max_size_ : max_size.load()))) &&
         (max_elements == 0 ||
          current_elements_num + elements - released_elements_assumption <= (max_elements_ ? *max_elements_ : max_elements.load()));
}

EvictionInfoPtr LRUFileCachePriority::collectEvictionInfo(size_t size, size_t elements, IFileCachePriority::Iterator *,
                                                          bool is_total_space_cleanup, const IFileCachePriority::OriginInfo &origin_info,
                                                          const CacheStateGuard::Lock &lock) {
  auto info = std::make_unique<QueueEvictionInfo>(description, origin_info.user_id);
  if (!size && !elements) return std::make_unique<EvictionInfo>(queue_id, std::move(info));
  ...
  if (available_size < size) info->size_to_evict = size - available_size;
  ...
  if (available_elements < elements) info->elements_to_evict = elements - available_elements;
  ...
  if (size_to_hold || elements_to_hold) {
    info->hold_space = std::make_unique<IFileCachePriority::HoldSpace>(size_to_hold, elements_to_hold, *this, lock);
  }
  return std::make_unique<EvictionInfo>(queue_id, std::move(info));
}

bool LRUFileCachePriority::collectCandidatesForEviction(const EvictionInfo &eviction_info, FileCacheReserveStat &stat,
                                                        ...
                                                        bool /* is_total_space_cleanup */, const OriginInfo &,
                                                        CachePriorityGuard &cache_guard, CacheStateGuard &) {
  ...
  auto get_iteration_result = [&]() {
    ...
    if (max_candidates_size && res.size() >= max_candidates_size) return IterationResult::BREAK;
  ...
  if (continue_from_last_eviction_pos && current_eviction_pos != LRUQueue::iterator{} && current_eviction_pos != queue.end() &&
  ...
  auto iteration_pos = iterateImpl(
      ...
      [&](LockedKey &locked_key, const FileSegmentMetadataPtr &segment_metadata) {
        if (get_iteration_result() == IterationResult::BREAK) return IterationResult::BREAK;
  ...
  if (continue_from_last_eviction_pos) setEvictionPos(iteration_pos, lock);

  lock.unlock();

  const bool success = (max_candidates_size && res.size() >= max_candidates_size) ||
                       ((!size || stat.total_stat.releasable_size >= size) && (!elements || stat.total_stat.releasable_count >= elements));
...
LRUFileCachePriority::LRUIterator LRUFileCachePriority::move(LRUIterator &it, LRUFileCachePriority &other,
...
IFileCachePriority::PriorityDumpPtr LRUFileCachePriority::dump(const CachePriorityGuard::ReadLock &lock) {
  ...
  FileCacheReserveStat stat{};
  iterate(
      [&](LockedKey &, const FileSegmentMetadataPtr &segment_metadata) {
...
bool LRUFileCachePriority::modifySizeLimits(size_t max_size_, size_t max_elements_, double /* size_ratio_ */,
                                            const CacheStateGuard::Lock &lock) {
  if (max_size == max_size_ && max_elements == max_elements_) return false;  /// Nothing to change.
  ...
  if (state->getSize(lock) > max_size_ || state->getElementsCount(lock) > max_elements_) {
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    ...
                    max_size_, max_elements_, state->getSize(lock), max_size.load(), state->getElementsCount(lock), max_elements.load(),
                    description);
  }

  LOG_INFO(log,
           "Modifying size limits from {} to {} in size, "
...
EvictionInfoPtr LRUFileCachePriority::collectEvictionInfoForResize(size_t desired_max_size, size_t desired_max_elements,
                                                                   const OriginInfo &origin_info, const CacheStateGuard::Lock &lock) {
  ...
  return collectEvictionInfo(size_to_evict, elements_to_evict,
                             /* reservee */ nullptr,
                             /* is_total_space_cleanup */ true, origin_info, lock);
}

bool LRUFileCachePriority::tryIncreasePriority(Iterator &iterator, bool /* is_space_reservation_complete */,
                                               CachePriorityGuard &queue_guard, CacheStateGuard &) {
...
void LRUFileCachePriority::LRUIterator::invalidate() {
  ...
  LOG_TEST(cache_priority->log, "Invalidating entry in LRU queue {}: {}", entry_ptr->toString(),
           cache_priority->getApproxStateInfoForLog());

  size_t entry_size = entry_ptr->size;
  entry_ptr->size = 0;
  entry_ptr->setInvalidatedFlag();

  if (entry_size) cache_priority->state->sub(entry_size, 1);
}
...
void LRUFileCachePriority::LRUIterator::incrementSize(size_t size, const CacheStateGuard::Lock &lock) {
  ...
  if (!cache_priority->canFit(size, elements, lock)) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot increment size by {} for entry {}. Current state: {}", size, entry_ptr->toString(),
                    cache_priority->getStateInfoForLog(lock));
  }

  LOG_TEST(cache_priority->log, "Incrementing size with {} in LRU queue for entry {}", size, entry_ptr->toString());
...
void LRUFileCachePriority::LRUIterator::decrementSize(size_t size) {
  ...
  LOG_TEST(cache_priority->log, "Decrement size with {} in LRU queue entry {}", size, entry_ptr->toString());
...
bool LRUFileCachePriority::LRUIterator::assertValid() const {
  ...
  if (!entry_ptr || !is_iterator_valid) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Attempt to use invalid iterator (entry: {}, iterator: {})", bool(entry_ptr),
                    is_iterator_valid);
  }
...
void LRUFileCachePriority::shuffle(const CachePriorityGuard::WriteLock &) {
  ...
  for (auto it = queue.begin(); it != queue.end(); ++it) its.push_back(it);
  pcg64 generator(randomSeed());
  std::shuffle(its.begin(), its.end(), generator);
  for (auto &it : its) queue.splice(queue.end(), queue, it);
}

std::string LRUFileCachePriority::getStateInfoForLog(const CacheStateGuard::Lock &lock) const {
  return fmt::format("size: {}/{}, elements: {}/{}, hold size: {}, hold elements: {}, description: {}", getSize(lock), max_size.load(),
                     getElementsCount(lock), total_hold_size.load(), total_hold_elements.load(), max_elements.load(), description);
}

std::string LRUFileCachePriority::getApproxStateInfoForLog() const {
  return fmt::format("size: {}/{}, elements: {}/{} (description: {})", getSizeApprox(), max_size.load(), getElementsCountApprox(),
                     max_elements.load(), description);
}

void LRUFileCachePriority::holdImpl(size_t size, size_t elements, const CacheStateGuard::Lock &lock) {
...
void LRUFileCachePriority::releaseImpl(size_t size, size_t elements) {
  auto lock = cache_usage_stat_guard ? std::optional<CacheUsageStatGuard::Lock>(cache_usage_stat_guard->lock()) : std::nullopt;
...
void LRUFileCachePriority::moveEvictionPosIfEqual(LRUQueue::iterator it, const CachePriorityGuard::WriteLock &) {
  std::lock_guard lk(eviction_pos_mutex);
  if (eviction_pos != LRUQueue::iterator{} && eviction_pos == it) eviction_pos = std::next(it);
}
}  // namespace DB
