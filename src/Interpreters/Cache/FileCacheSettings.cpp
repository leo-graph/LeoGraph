...
#define LIST_OF_FILE_CACHE_SETTINGS(DECLARE, ALIAS)                                                                                        \
  ...
  DECLARE(UInt64, max_elements, FILECACHE_DEFAULT_MAX_ELEMENTS,                                                                            \
          "Maximum number of cache elements, e.g. file segments (limits number of files on filesystem)", 0)                                \
  ...
  DECLARE(UInt64, background_download_threads, FILECACHE_DEFAULT_BACKGROUND_DOWNLOAD_THREADS,                                              \
          "Number of background download threads. Value 0 disables background download", 0)                                                \
  DECLARE(UInt64, background_download_queue_size_limit, FILECACHE_DEFAULT_BACKGROUND_DOWNLOAD_QUEUE_SIZE_LIMIT,                            \
          "Size of background download queue. Value 0 disables background download", 0)                                                    \
  DECLARE(UInt64, background_download_max_file_segment_size, FILECACHE_DEFAULT_MAX_FILE_SEGMENT_SIZE_WITH_BACKGROUND_DOWLOAD,              \
          "Maximum size which can be downloaded in background download", 0)                                                                \
  DECLARE(UInt64, load_metadata_threads, FILECACHE_DEFAULT_LOAD_METADATA_THREADS,                                                          \
          "Number of threads to load cache metadata at server startup. Value 0 disables asynchronous loading of metadata", 0)              \
  DECLARE(Bool, load_metadata_asynchronously, false, "Enables asynchronous loading of metadata on server startup", 0)                      \
  DECLARE(Double, keep_free_space_size_ratio, FILECACHE_DEFAULT_FREE_SPACE_SIZE_RATIO,                                                     \
          "A ratio of free space which cache would try to uphold in the background", 0)                                                    \
  DECLARE(Double, keep_free_space_elements_ratio, FILECACHE_DEFAULT_FREE_SPACE_ELEMENTS_RATIO,                                             \
          "A ratio of free elements which cache would try to uphold in the background", 0)                                                 \
  DECLARE(UInt64, keep_free_space_remove_batch, FILECACHE_DEFAULT_FREE_SPACE_REMOVE_BATCH,                                                 \
          "A remove batch size of cache elements made by background thread which upholds free space/elements ratio", 0)                    \
  DECLARE(Bool, enable_filesystem_query_cache_limit, false, "Enable limiting maximum size of cache which can be written within a query",   \
          0)                                                                                                                               \
  ...
  DECLARE(UInt64, overcommit_eviction_evict_step, 10 * 1_MiB,                                                                              \
          "Eviction step in bytes for overcommit eviction policy. Used for keep_free_space_*_ratio settings", 0)                           \
  DECLARE(Double, check_cache_probability, 0.001,                                                                                          \
          "Works only for debug or sanitizer build. Checks cache correctness by going through all cache and checking state of each cache " \
          "element",                                                                                                                       \
          0)
...
FileCacheSettings::FileCacheSettings(const FileCacheSettings& settings) : impl(std::make_unique<FileCacheSettingsImpl>(*settings.impl)) {}
...
bool FileCacheSettings::operator==(const FileCacheSettings& settings) const noexcept { return *impl == *settings.impl; }

ColumnsDescription FileCacheSettings::getColumnsDescription() {
  FileCacheSettingsImpl impl;
  ColumnsDescription result;

  result.add(ColumnDescription("cache_name", std::make_shared<DataTypeString>(), "Cache name"));
  ...
  result.add(
      ColumnDescription("is_initialized", std::make_shared<DataTypeUInt8>(), "Indicates whether cache was successfully initialized"));

  result.add(ColumnDescription("current_size", std::make_shared<DataTypeUInt64>(), "Current cache size"));
  result.add(
      ColumnDescription("current_elements_num", std::make_shared<DataTypeUInt64>(), "Current cache elements (file segments) number"));

  return result;
}

void FileCacheSettings::dumpToSystemSettingsColumns(MutableColumnsAndConstraints& params, const std::string& cache_name,
                                                    const FileCachePtr& cache) const {
  MutableColumns& res_columns = params.res_columns;
  size_t i = 0;
  res_columns[i++]->insert(cache_name);

  for (const auto& setting : impl->all()) res_columns[i++]->insert(setting.getValue());
...
void FileCacheSettings::loadFromConfig(const Poco::Util::AbstractConfiguration& config, const std::string& config_prefix,
                                       const std::string& cache_path_prefix_if_relative, const std::string& default_cache_path) {
...
void FileCacheSettings::loadFromCollection(const NamedCollection& collection, const std::string& cache_path_prefix_if_relative) {
...
void FileCacheSettings::validate() {
  ...
  if (settings[FileCacheSetting::max_size_ratio_to_total_space].changed) {
    ...
    LOG_INFO(getLogger("FileCacheSettings"), "Using max_size as ratio {} to total disk space on path {}: {} (total space: {})",
             settings[FileCacheSetting::max_size_ratio_to_total_space].value, settings[FileCacheSetting::path].value,
