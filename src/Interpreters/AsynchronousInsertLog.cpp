...
ColumnsDescription AsynchronousInsertLogElement::getColumnsDescription() {
  auto type_status = std::make_shared<DataTypeEnum8>(DataTypeEnum8::Values{
  ...
  auto type_data_kind = std::make_shared<DataTypeEnum8>(DataTypeEnum8::Values{
  ...
  return ColumnsDescription{
      ...
      {"event_time_microseconds", std::make_shared<DataTypeDateTime64>(6),
       "The date and time when the async insert finished execution with microseconds precision."},
      ...
      {"status", type_status,
       "Status of the view. Values: 'Ok' = 1 — Successful insert, 'ParsingError' = 2 — Exception when parsing the data, 'FlushError' = 3 — "
       "Exception when flushing the data"},
      ...
      {"flush_time_microseconds", std::make_shared<DataTypeDateTime64>(6),
       "The date and time when the flush happened with microseconds precision."},
