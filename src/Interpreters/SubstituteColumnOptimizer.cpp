...
class ComponentMatcher {
 public:
  ...
  struct Data {
    ...
    Data(const ComparisonGraph<ASTPtr> &graph_, std::set<UInt64> &components_, std::unordered_map<String, String> &old_name_,
         std::unordered_map<String, UInt64> &component_, UInt64 &current_id_)
        : graph(graph_), components(components_), old_name(old_name_), component(component_), current_id(current_id_) {}
  };
 ...
 public:
  ...
  static void visit(ASTPtr &ast, Data &data) {
    ...
    if (identifier && data.name_to_component_id.contains(identifier->name())) {
      ...
      if (data.is_select) new_ast->setAlias(data.old_name.at(name));
...
ColumnPrice calculatePrice(const ColumnPriceByName &column_prices, const IdentifierNameSet &identifiers) {
  ColumnPrice result(0, 0);
  for (const auto &ident : identifiers) {
    auto it = column_prices.find(ident);
    if (it != column_prices.end()) result += it->second;
  }
...
void bruteforce(const ComparisonGraph<ASTPtr> &graph, const std::vector<UInt64> &components, size_t current_component,
                const ColumnPriceByName &column_prices, ColumnPrice current_price, std::vector<ASTPtr> &expressions_stack,
                ColumnPrice &min_price, std::vector<ASTPtr> &min_expressions) {
  if (current_component == components.size()) {
    ...
    for (const auto &ast : graph.getComponent(components[current_component])) {
      ...
      for (const auto &identifier : identifiers) new_prices[identifier] = ColumnPrice(0, 0);
...
SubstituteColumnOptimizer::SubstituteColumnOptimizer(ASTSelectQuery *select_query_, const StorageMetadataPtr &metadata_snapshot_,
                                                     const ConstStoragePtr &storage_)
    : select_query(select_query_), metadata_snapshot(metadata_snapshot_), storage(storage_) {}
...
void SubstituteColumnOptimizer::perform() {
  ...
  if (compare_graph.getNumOfComponents() == 0) return;

  auto run_for_all = [&](const auto func) {
    if (select_query->where()) func(select_query->refWhere(), false);
    if (select_query->prewhere()) func(select_query->refPrewhere(), false);
    if (select_query->select()) func(select_query->refSelect(), true);
    if (select_query->having()) func(select_query->refHaving(), false);
  };
  ...
  ComponentVisitor::Data component_data(compare_graph, components, old_name, name_to_component, counter_id);
  ...
  for (const auto &column_name : primary_key) column_prices[column_name] = ColumnPrice(INDEX_PRICE, INDEX_PRICE);

  for (const auto &column_name : identifiers) column_prices[column_name] = ColumnPrice(0, 0);
  ...
  bruteforce(compare_graph, components_list, 0, column_prices, ColumnPrice(0, 0), expressions_stack, min_price, min_expressions);

  for (size_t i = 0; i < components_list.size(); ++i) id_to_expression_map[components_list[i]] = min_expressions[i];
