...
class TableJoin {
 public:
  ...
  struct JoinOnClause {
    ...
    void addKey(const String& left_name, const String& right_name, bool null_safe_comparison) {
      ...
      if (null_safe_comparison) nullsafe_compare_key_indexes.insert(key_names_left.size() - 1);
    }

    std::pair<String, String> condColumnNames() const {
      std::pair<String, String> res;

      if (!analyzer_left_filter_condition_column_name.empty()) res.first = analyzer_left_filter_condition_column_name;

      if (!analyzer_right_filter_condition_column_name.empty()) res.second = analyzer_right_filter_condition_column_name;

      if (on_filter_condition_left) res.first = on_filter_condition_left->getColumnName();
      if (on_filter_condition_right) res.second = on_filter_condition_right->getColumnName();
    ...
    String formatDebug(bool short_format = false) const {
      ...
      return fmt::format("Left keys: [{}] Right keys [{}] Condition columns: '{}', '{}'", fmt::join(key_names_left, ", "),
                         fmt::join(key_names_right, ", "), left_cond, right_cond);
    }
 ...
 private:
  ...
  std::optional<ActionsDAG> applyKeyConvertToTable(const ColumnsWithTypeAndName& cols_src, const NameToTypeMap& type_mapping,
                                                   JoinTableSide table_side, NameToNameMap& key_column_rename, const ContextPtr& context);

  std::optional<ActionsDAG> applyNullsafeWrapper(const ColumnsWithTypeAndName& cols_src, const NameSet& columns_for_nullsafe_comparison,
                                                 JoinTableSide table_side, NameToNameMap& key_column_rename);

  std::optional<ActionsDAG> applyJoinUseNullsConversion(const ColumnsWithTypeAndName& cols_src, const NameToNameMap& key_column_rename,
                                                        const ContextPtr& context);

  void applyRename(JoinTableSide side, const NameToNameMap& name_map);

  void addKey(const String& left_name, const String& right_name, const ASTPtr& left_ast, const ASTPtr& right_ast,
  ...
  std::pair<NameSet, NameSet> getKeysForNullSafeComparion(const ColumnsWithTypeAndName& left_sample_columns,
 ...
 public:
  ...
  /// for StorageJoin
  TableJoin(SizeLimits limits, bool use_nulls, JoinKind kind, JoinStrictness strictness, const Names& key_names_right);
  ...
  bool isEnabledAlgorithm(JoinAlgorithm val) const { return isEnabledAlgorithm(join_algorithms, val); }
  ...
  const JoinOnClause& getOnlyClause() const {
    assertHasOneOnExpr();
    return clauses[0];
  }
  ...
  void setIsJoinWithConstant(bool is_join_with_constant_value) { is_join_with_constant = is_join_with_constant_value; }
  ...
  void setColumnsAddedByJoin(const NamesAndTypesList& columns_added_by_join_value) { columns_added_by_join = columns_added_by_join_value; }
  ...
  std::pair<std::optional<ActionsDAG>, std::optional<ActionsDAG>> createConvertingActions(
  ...
  void setColumnsFromJoinedTable(NamesAndTypesList columns_from_joined_table_value, const NameSet& left_table_columns,
                                 const String& right_table_prefix, const NamesAndTypesList& columns_from_left_table_) {
