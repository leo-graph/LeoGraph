...
static MutationCommand createMutationCommand(const ASTUpdateQuery &update_query) {
  ...
  if (update_query.partition) alter_query->set(alter_query->partition, update_query.partition);

  auto mutation_command = MutationCommand::parse(*alter_query);
  if (!mutation_command)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed to convert query '{}' to mutation command. It's a bug",
                    update_query.formatForErrorMessage());
...
BlockIO InterpreterUpdateQuery::execute() {
  ...
  if (!settings[Setting::enable_lightweight_update])
    throw Exception(ErrorCodes::SUPPORT_IS_DISABLED,
                    "Lightweight updates are not allowed. Set 'enable_lightweight_update = 1' to allow them");
