...
void TableStatus::write(WriteBuffer& out, UInt64 client_protocol_revision) const {
  ...
  if (is_replicated) {
    ...
    if (client_protocol_revision >= DBMS_MIN_REVISION_WITH_TABLE_READ_ONLY_CHECK) writeVarUInt(is_readonly, out);
  }
}
...
void TableStatus::read(ReadBuffer& in, UInt64 server_protocol_revision) {
  ...
  if (is_replicated) {
    ...
    if (server_protocol_revision >= DBMS_MIN_REVISION_WITH_TABLE_READ_ONLY_CHECK) readVarUInt(is_readonly, in);
  }
}
...
void TablesStatusRequest::read(ReadBuffer& in, UInt64 client_protocol_revision) {
  ...
  if (size > DEFAULT_MAX_STRING_SIZE) throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE, "Too large collection size.");
