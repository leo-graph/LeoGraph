...
struct ContextSharedPart : boost::noncopyable {
  ...
  /// Separate mutex for re-initialization of zookeeper session. This operation could take a long time and must not interfere with another
  /// operations.
  ...
  mutable std::map<String, zkutil::ZooKeeperPtr> auxiliary_zookeepers
      TSA_GUARDED_BY(auxiliary_zookeepers_mutex);                                           /// Map for auxiliary ZooKeeper clients.
  ...
  mutable std::unique_ptr<AsyncLoader>
      async_loader;  /// Thread pool for asynchronous initialization of arbitrary DAG of `LoadJob`s (used for tables loading)

  mutable std::unique_ptr<EmbeddedDictionaries> embedded_dictionaries
      TSA_GUARDED_BY(embedded_dictionaries_mutex);  /// Metrica's dictionaries. Have lazy initialization.
  ...
  mutable std::unique_ptr<ExternalUserDefinedExecutableFunctionsLoader> external_user_defined_executable_functions_loader
      TSA_GUARDED_BY(external_user_defined_executable_functions_mutex);
  ExternalLoaderXMLConfigRepository *user_defined_executable_functions_config_repository
      TSA_GUARDED_BY(external_user_defined_executable_functions_mutex) = nullptr;
  ...
  mutable VectorSimilarityIndexCachePtr vector_similarity_index_cache
      TSA_GUARDED_BY(mutex);                                                          /// Cache of deserialized secondary index granules.
...
#if USE_AVRO
  mutable IcebergMetadataFilesCachePtr iceberg_metadata_files_cache
      TSA_GUARDED_BY(mutex);  /// Cache of deserialized iceberg metadata files.
#endif
#if USE_PARQUET
  ...
  mutable BackgroundSchedulePoolPtr message_broker_schedule_pool;  /// A thread pool that can run different jobs in background (used for
  ...
  std::shared_ptr<std::unordered_set<std::string>> users_to_ignore_early_memory_limit_check
      TSA_GUARDED_BY(users_to_ignore_early_memory_limit_check_mutex);
  ...
  void setConfig(const ConfigurationPtr &config_value) {
    if (!config_value) throw Exception(ErrorCodes::LOGICAL_ERROR, "Set nullptr config is invalid");
  ...
  void shutdown() TSA_NO_THREAD_SAFETY_ANALYSIS {
    ...
    if (delete_async_insert_queue) delete_async_insert_queue->flushAndShutdown();
    ...
    DatabaseCatalog::shutdown([this]() { SHUTDOWN(log, "system logs", TSA_SUPPRESS_WARNING_FOR_READ(system_logs), flushAndShutdown()); });
    ...
    for (const auto &cache_data : FileCacheFactory::instance().getUniqueInstances()) cache_data->cache->deactivateBackgroundOperations();
...

    {
      ...
      if (temporary_volume_legacy) {
        ...
        for (auto &disk : disks) disk->shutdown();
      }
    }
...

    {
      ...
      if (auto *cache = CompiledExpressionCacheFactory::instance().tryGetCache()) cache->clear();
#endif
    ...
    if (delete_zookeeper) delete_zookeeper->finalize("shutdown");
    ...
    for (auto &[name, zk] : delete_auxiliary_zookeepers) zk->finalize("shutdown");
  ...
  void removeAllWarnings() TSA_REQUIRES(mutex) { warnings.clear(); }
  ...
  void configureServerWideThrottling() {
    ...
    if (auto bandwidth = server_settings[ServerSetting::max_remote_read_network_bandwidth_for_server])
      remote_read_throttler = std::make_shared<Throttler>(bandwidth, ProfileEvents::RemoteReadThrottlerBytes,
    ...
    if (auto bandwidth = server_settings[ServerSetting::max_remote_write_network_bandwidth_for_server])
      remote_write_throttler = std::make_shared<Throttler>(bandwidth, ProfileEvents::RemoteWriteThrottlerBytes,
                                                           ProfileEvents::RemoteWriteThrottlerSleepMicroseconds);

    if (auto bandwidth = server_settings[ServerSetting::max_local_read_bandwidth_for_server])
      local_read_throttler = std::make_shared<Throttler>(bandwidth, ProfileEvents::LocalReadThrottlerBytes,
                                                         ProfileEvents::LocalReadThrottlerSleepMicroseconds);

    if (auto bandwidth = server_settings[ServerSetting::max_local_write_bandwidth_for_server])
      local_write_throttler = std::make_shared<Throttler>(bandwidth, ProfileEvents::LocalWriteThrottlerBytes,
                                                          ProfileEvents::LocalWriteThrottlerSleepMicroseconds);

    if (auto bandwidth = server_settings[ServerSetting::max_backup_bandwidth_for_server])
      backups_server_throttler =
          std::make_shared<Throttler>(bandwidth, ProfileEvents::BackupThrottlerBytes, ProfileEvents::BackupThrottlerSleepMicroseconds);

    if (auto bandwidth = server_settings[ServerSetting::max_mutations_bandwidth_for_server])
      mutations_throttler = std::make_shared<Throttler>(bandwidth, ProfileEvents::MutationsThrottlerBytes,
                                                        ProfileEvents::MutationsThrottlerSleepMicroseconds);

    if (auto bandwidth = server_settings[ServerSetting::max_merges_bandwidth_for_server])
      merges_throttler =
          std::make_shared<Throttler>(bandwidth, ProfileEvents::MergesThrottlerBytes, ProfileEvents::MergesThrottlerSleepMicroseconds);
  }
};
...
ContextData::ContextData() { settings = std::make_unique<Settings>(); }
...
SharedContextHolder Context::createShared() { return SharedContextHolder(std::make_unique<ContextSharedPart>()); }
...
ContextMutablePtr Context::createCopy(const ContextMutablePtr &other) { return createCopy(std::const_pointer_cast<const Context>(other)); }
...
String Context::resolveDatabase(const String &database_name) const {
  ...
  if (res.empty()) throw Exception(ErrorCodes::UNKNOWN_DATABASE, "Default database is not selected");
  return res;
}
...
std::shared_ptr<IDisk> Context::getDatabaseDisk() const {
  {
    SharedLockGuard lock(shared->mutex);
    if (shared->default_db_disk) return shared->default_db_disk;
  }

  // This is called first time early during the initialization.
  // Even if multiple threads try to get target_db_disk, only the first one will initialize the disks as there is another mutex in
  // `getDiskMap()` It is not necessary to introduce a mutex here.
  auto target_db_disk = [&]() -> std::shared_ptr<IDisk> {
    ...
    if (it == disk_map.end()) throw Exception(ErrorCodes::UNKNOWN_DISK, "No disk {}", backQuote(disk_name));
...
DatabaseAndTable Context::getOrCacheStorage(const StorageID &id, std::function<DatabaseAndTable()> storage_getter) const {
  ...
  if (auto it = shard.set.find(id); it != shard.set.end()) {
    ...
    if (storage.second) return storage;
  ...
  std::unordered_map<Context::WarningType, PreformattedMessage> common_warnings;
  {
    ...
    if (attached_tables > static_cast<Int64>(shared->max_table_num_to_warn)) {
      if (auto limit = shared->server_settings[ServerSetting::max_table_num_to_throw]; limit > shared->max_table_num_to_warn.load())
        common_warnings[Context::WarningType::MAX_ATTACHED_TABLES] = PreformattedMessage::create(
            "The number of attached tables ({}) exceeds the warning limit of {}. You will not be able to create new tables once the limit "
            "of {} is reached.",
            attached_tables, shared->max_table_num_to_warn.load(), limit.value);
      else
        common_warnings[Context::WarningType::MAX_ATTACHED_TABLES] = PreformattedMessage::create(
            "The number of attached tables ({}) exceeds the warning limit of {}.", attached_tables, shared->max_table_num_to_warn.load());
    }
    ...
    if (attached_views > static_cast<Int64>(shared->max_view_num_to_warn)) {
      if (auto limit = shared->server_settings[ServerSetting::max_view_num_to_throw]; limit > shared->max_view_num_to_warn.load())
        common_warnings[Context::WarningType::MAX_ATTACHED_VIEWS] = PreformattedMessage::create(
            "The number of attached views ({}) exceeds the warning limit of {}. You will not be able to create new views once the limit of "
            "{} is reached.",
            attached_views, shared->max_view_num_to_warn.load(), limit.value);
      else
        common_warnings[Context::WarningType::MAX_ATTACHED_VIEWS] = PreformattedMessage::create(
            "The number of attached views ({}) exceeds the warning limit of {}.", attached_views, shared->max_view_num_to_warn.load());
    }

    if (attached_dictionaries > static_cast<Int64>(shared->max_dictionary_num_to_warn)) {
      if (auto limit = shared->server_settings[ServerSetting::max_dictionary_num_to_throw];
          limit > shared->max_dictionary_num_to_warn.load())
        common_warnings[Context::WarningType::MAX_ATTACHED_DICTIONARIES] = PreformattedMessage::create(
            "The number of attached dictionaries ({}) exceeds the warning limit of {}. You will not be able to create new dictionaries "
            "once the limit of {} is reached.",
        ...
        common_warnings[Context::WarningType::MAX_ATTACHED_DICTIONARIES] =
            PreformattedMessage::create("The number of attached dictionaries ({}) exceeds the warning limit of {}.", attached_dictionaries,
                                        shared->max_dictionary_num_to_warn.load());
    }
    ...
    if (attached_databases > static_cast<Int64>(shared->max_database_num_to_warn)) {
      if (auto limit = shared->server_settings[ServerSetting::max_database_num_to_throw]; limit > shared->max_database_num_to_warn.load())
        common_warnings[Context::WarningType::MAX_ATTACHED_DATABASES] = PreformattedMessage::create(
            "The number of attached databases ({}) exceeds the warning limit of {}. You will not be able to create new databases once the "
            "limit of {} is reached.",
        ...
        common_warnings[Context::WarningType::MAX_ATTACHED_DATABASES] =
            PreformattedMessage::create("The number of attached databases ({}) exceeds the warning limit of {}.", attached_databases,
                                        shared->max_database_num_to_warn.load());
    }

    if (attached_named_collections > static_cast<Int64>(shared->max_named_collection_num_to_warn)) {
      if (auto limit = shared->server_settings[ServerSetting::max_named_collection_num_to_throw];
          limit > shared->max_named_collection_num_to_warn.load())
        common_warnings[Context::WarningType::MAX_NAMED_COLLECTIONS] = PreformattedMessage::create(
            "The number of named collections ({}) exceeds the warning limit of {}. You will not be able to create new named collections "
            "once the limit of {} is reached.",
            attached_named_collections, shared->max_named_collection_num_to_warn.load(), limit.value);
      else
        common_warnings[Context::WarningType::MAX_NAMED_COLLECTIONS] =
            PreformattedMessage::create("The number of named collections ({}) exceeds the warning limit of {}.", attached_named_collections,
                                        shared->max_named_collection_num_to_warn.load());
    }
    ...
    if (active_parts > static_cast<Int64>(shared->max_part_num_to_warn))
      common_warnings[Context::WarningType::MAX_ACTIVE_PARTS] = PreformattedMessage::create(
          "The number of active parts ({}) exceeds the warning limit of {}.", active_parts, shared->max_part_num_to_warn.load());
  }
...
VolumePtr Context::getGlobalTemporaryVolume() const {
  ...
  if (shared->temporary_volume_legacy) return shared->temporary_volume_legacy;
  return nullptr;
}

TemporaryDataOnDiskScopePtr Context::getTempDataOnDisk() const {
  if (temp_data_on_disk) return temp_data_on_disk;
...
void Context::setPath(const String &path) {
  ...
  if (shared->tmp_path.empty() && !shared->root_temp_data_on_disk) shared->tmp_path = shared->path + "tmp/";

  if (shared->flags_path.empty()) shared->flags_path = shared->path + "flags/";

  if (shared->user_files_path.empty()) shared->user_files_path = shared->path + "user_files/";

  if (shared->dictionaries_lib_path.empty()) shared->dictionaries_lib_path = shared->path + "dictionaries_lib/";

  if (shared->user_scripts_path.empty()) shared->user_scripts_path = shared->path + "user_scripts/";
}
...
static void setupTmpPath(LoggerPtr log, const DiskPtr &disk) try {
  ...
  DB::tryLogCurrentException(log, fmt::format("Caught exception while setting up temporary disk {}:{}. "
...
void Context::setTemporaryStoragePath(const String &path, size_t max_size) {
  ...
  if (shared->root_temp_data_on_disk) throw Exception(ErrorCodes::LOGICAL_ERROR, "Temporary storage is already set");

  shared->tmp_path = path;
  if (!shared->tmp_path.ends_with('/')) shared->tmp_path += '/';

  VolumePtr volume = createLocalSingleDiskVolume(shared->tmp_path, shared->getConfigRefWithLock(lock));

  for (const auto &disk : volume->getDisks()) setupTmpPath(shared->log, disk);
...
void Context::setTemporaryStoragePolicy(const String &policy_name, size_t max_size) {
  ...
  if (tmp_policy->getVolumes().size() != 1)
    throw Exception(ErrorCodes::NO_ELEMENTS_IN_CONFIG, "Policy '{}' is used temporary files, such policy should have exactly one volume",
                    policy_name);

  VolumePtr volume = tmp_policy->getVolume(0);

  if (volume->getDisks().empty()) throw Exception(ErrorCodes::NO_ELEMENTS_IN_CONFIG, "No disks volume for temporary files");

  for (const auto &disk : volume->getDisks()) {
    if (!disk) throw Exception(ErrorCodes::NO_ELEMENTS_IN_CONFIG, "Temporary disk is null");
...
void Context::setTemporaryStorageInCache(const String &cache_disk_name, size_t max_size) {
  ...
  if (!disk_ptr) throw Exception(ErrorCodes::NO_ELEMENTS_IN_CONFIG, "Disk '{}' is not found", cache_disk_name);

  std::lock_guard lock(shared->mutex);
  if (shared->root_temp_data_on_disk) throw Exception(ErrorCodes::LOGICAL_ERROR, "Temporary storage is already set");

  auto file_cache = FileCacheFactory::instance().getByName(disk_ptr->getCacheName())->cache;
  if (!file_cache) throw Exception(ErrorCodes::NO_ELEMENTS_IN_CONFIG, "Cache '{}' is not found", disk_ptr->getCacheName());
...
void Context::addOrUpdateWarningMessage(WarningType warning, const PreformattedMessage &message) const {
  ...
  if (!is_supressed) shared->addOrUpdateWarningMessage(warning, message);
}
...
void Context::addWarningMessageAboutDatabaseOrdinary(const String &database_name) const {
  ...
  if (is_called.exchange(true)) return;
  ...
  shared->addOrUpdateWarningMessage(Context::WarningType::DB_ORDINARY_DEPRECATED,
                                    PreformattedMessage::create(message_format_string, database_name, convert_databases_flag.string(),
                                                                convert_databases_flag.string(), convert_databases_flag.string()));
}
...
void Context::setConfig(const ConfigurationPtr &config) { shared->setConfig(config); }

const Poco::Util::AbstractConfiguration &Context::getConfigRef() const { return shared->getConfigRef(); }
...
void Context::setUser(const UUID &user_id_, const std::vector<UUID> &external_roles_) {
  ...
  auto enabled_profiles = access_control.getEnabledSettingsInfo(user_id_, user->settings, enabled_roles->enabled_roles,
                                                                enabled_roles->settings_from_enabled_roles);
  ...
  if (!database.empty()) setCurrentDatabaseWithLock(database, lock);
}

std::shared_ptr<const User> Context::getUser() const { return getAccess()->getUser(); }

String Context::getUserName() const { return getAccess()->getUserName(); }
...
void Context::setCurrentRolesImpl(const std::vector<UUID> &new_current_roles, bool throw_if_not_granted, bool skip_if_not_granted,
    ...
    for (const auto &role_id : new_current_roles) {
      if (!user->granted_roles.isGranted(role_id)) {
        ...
        throw Exception(ErrorCodes::SET_NON_GRANTED_ROLE, "Role {} should be granted to set as a current",
                        role_name.value_or(toString(role_id)));
      }
    }
  }
...
void Context::setCurrentRoles(const RolesOrUsersSet &new_current_roles, bool check_grants) {
  if (new_current_roles.all) {
    auto user = getUser();
    setCurrentRolesImpl(user->granted_roles.findGranted(new_current_roles), /* throw_if_not_granted= */ false,
                        /* skip_if_not_granted= */ false, user);
  } else {
...
void Context::setCurrentRolesDefault() {
  auto user = getUser();
  setCurrentRolesImpl(user->granted_roles.findGranted(user->default_roles), /* throw_if_not_granted= */ false,
                      /* skip_if_not_granted= */ false, user);
}

std::vector<UUID> Context::getCurrentRoles() const { return getRolesInfo()->getCurrentRoles(); }

std::vector<UUID> Context::getEnabledRoles() const { return getRolesInfo()->getEnabledRoles(); }

std::shared_ptr<const EnabledRolesInfo> Context::getRolesInfo() const { return getAccess()->getRolesInfo(); }

namespace {
ALWAYS_INLINE inline void contextSanityClampSettingsWithLock(const Context &context, Settings &settings,
                                                             const std::lock_guard<ContextSharedMutex> &) {
...
void Context::checkAccess(const AccessFlags &flags, std::string_view database, std::string_view table) const {
  checkAccessImpl(flags, database, table);
}
void Context::checkAccess(const AccessFlags &flags, std::string_view database, std::string_view table, std::string_view column) const {
  checkAccessImpl(flags, database, table, column);
}
void Context::checkAccess(const AccessFlags &flags, std::string_view database, std::string_view table,
                          const std::vector<std::string_view> &columns) const {
  checkAccessImpl(flags, database, table, columns);
}
...
std::shared_ptr<const ContextAccessWrapper> Context::getAccess() const {
  ...
  auto get_params = [this]() {
    ...
    if (client_info.initial_user != client_info.current_user) initial_user_id = getAccessControl().find<User>(client_info.initial_user);

    return ContextAccessParams{user_id,          full_access,    /* use_default_roles= */ false,
                               current_roles,    external_roles, *settings,
                               current_database, client_info,    initial_user_id};
  };
...
std::shared_ptr<const EnabledQuota> Context::getQuota() const { return getAccess()->getQuota(); }

std::optional<QuotaUsage> Context::getQuotaUsage() const { return getAccess()->getQuotaUsage(); }

void Context::setCurrentProfileWithLock(const String &profile_name, bool check_constraints,
                                        const std::lock_guard<ContextSharedMutex> &lock) {
  try {
...
void Context::setCurrentProfilesWithLock(const SettingsProfilesInfo &profiles_info, bool check_constraints,
                                         const std::lock_guard<ContextSharedMutex> &lock) {
  if (check_constraints) checkSettingsConstraintsWithLock(profiles_info.settings, SettingSource::PROFILE);
...
std::vector<UUID> Context::getCurrentProfiles() const {
  SharedLockGuard lock(mutex);
  if (!settings_constraints_and_current_profiles) return {};
...
ResourceManagerPtr Context::getResourceManager() const {
  callOnce(shared->resource_manager_initialized, [&] { shared->resource_manager = createResourceManager(getGlobalContext()); });
...
ClassifierPtr Context::getWorkloadClassifier() const {
  ...
  if (!classifier) classifier = getResourceManager()->acquire(getSettingsRef()[Setting::workload], settings);
  return classifier;
}

void Context::releaseQuerySlot() const {
  if (auto elem = getProcessListElementSafe()) elem->releaseQuerySlot();
}
...
std::pair<UInt64, String> Context::setConcurrentThreadsSoftLimit(UInt64 num, UInt64 ratio_to_cores, const String &scheduler) {
  ...
  if (ok)
    ...
    LOG_ERROR(shared->log, "Invalid value '{}' is set for the server setting 'concurrent_threads_scheduler'. Scheduler was not changed.",
              scheduler);

  // Set the limit
  SlotCount concurrent_threads_soft_limit = UnlimitedSlots;
  if (num > 0 && num < concurrent_threads_soft_limit) concurrent_threads_soft_limit = num;
  if (ratio_to_cores > 0) {
    auto value = ratio_to_cores * getNumberOfCPUCoresToUse();
    if (value > 0 && value < concurrent_threads_soft_limit) concurrent_threads_soft_limit = value;
  }
...
std::optional<Block> Context::tryGetSpecialScalar(const String &name) const {
  ...
  if (special_scalars.end() == it) return std::nullopt;
  return it->second;
}

Tables Context::getExternalTables() const {
  if (isGlobalContext()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Global context cannot have external tables");

  SharedLockGuard lock(mutex);

  Tables res;
  for (const auto &table : external_tables_mapping) res[table.first] = table.second->getTable();
...
void Context::addOrUpdateExternalTable(const String &table_name, std::shared_ptr<TemporaryTableHolder> temporary_table) {
  ...
  if (!inserted) it->second = std::move(temporary_table);
}
...
std::shared_ptr<TemporaryTableHolder> Context::findExternalTable(const String &table_name) const {
  ...
  std::shared_ptr<TemporaryTableHolder> holder;
  {
    ...
    if (iter == external_tables_mapping.end()) return {};
    holder = iter->second;
  }
  return holder;
}
...
void Context::addScalar(const String &name, const Block &block) {
  if (isGlobalContext()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Global context cannot have scalars");
...
void Context::addSpecialScalar(const String &name, const Block &block) {
  if (isGlobalContext()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Global context cannot have local scalars");
...
void Context::addQueryAccessInfo(const StorageID &table_id, const Names &column_names) {
...
std::shared_ptr<ContextTimeSeriesTagsCollector> Context::getTimeSeriesTagsCollector() {
  {
    SharedLockGuard lock(mutex);
    if (time_series_tags_collector) return time_series_tags_collector;
  }
  std::lock_guard lock(mutex);
  if (!time_series_tags_collector) time_series_tags_collector = std::make_shared<ContextTimeSeriesTagsCollector>();
...
void Context::addQueryAccessInfo(const String &quoted_database_name, const String &full_quoted_table_name, const Names &column_names) {
  if (isGlobalContext()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Global context cannot have query access info");
...
void Context::addQueryAccessInfo(const Names &partition_names) {
  ...
  for (const auto &partition_name : partition_names) query_access_info->partitions.emplace(partition_name);
}
...
void Context::addQueryAccessInfo(const QualifiedProjectionName &qualified_projection_name) {
  if (!qualified_projection_name) return;

  if (isGlobalContext()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Global context cannot have query access info");

  std::lock_guard<std::mutex> lock(query_access_info->mutex);
  query_access_info->projections.emplace(fmt::format("{}.{}", qualified_projection_name.storage_id.getFullTableName(),
                                                     backQuoteIfNeed(qualified_projection_name.projection_name)));
}

Context::QueryFactoriesInfo Context::getQueryFactoriesInfo() const { return query_factories_info; }

void Context::addQueryFactoriesInfo(QueryLogFactories factory_type, const String &created_object) const {
  if (isGlobalContext()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Global context cannot have query factories info");
...
static bool findIdentifier(const ASTFunction *function) {
  if (!function || !function->arguments) return false;
  if (const auto *arguments = function->arguments->as<ASTExpressionList>()) {
    for (const auto &argument : arguments->children) {
      if (argument->as<ASTIdentifier>()) return true;
      if (const auto *f = argument->as<ASTFunction>(); f && findIdentifier(f)) return true;
    }
  }
  return false;
}
...
StoragePtr Context::executeTableFunction(const ASTPtr &table_expression, const ASTSelectQuery *select_query_hint) {
  ...
  if (table) {
    if (table.get()->isView() && table->as<StorageView>() && table->as<StorageView>()->isParameterizedView()) {
      ...
      auto res = std::make_shared<StorageView>(StorageID(database_name, table_name), create,
    ...
    if (select_query_hint && use_structure_from_insertion_table_in_table_functions && table_function_ptr->needStructureHint() &&
      ...
      if (use_columns_from_insert_query) {
        if (expression == expression_list.end()) {
          ...
          if (!structure_hint.empty()) table_function_ptr->setStructureHint(structure_hint);
...
StoragePtr Context::executeTableFunction(const ASTPtr &table_expression, const TableFunctionPtr &table_function_ptr,
  ...
  StoragePtr res;
  {
    ...
    if (!res && key != bare_key) res = table_function_results[bare_key];
  }
...
StoragePtr Context::buildParameterizedViewStorage(const String &database_name, const String &table_name,
                                                  const NameToNameMap &param_values) const {
  if (table_name.empty()) return nullptr;

  StoragePtr original_view = DatabaseCatalog::instance().tryGetTable({database_name, table_name}, getQueryContext());
  if (!original_view || !original_view->isView()) return nullptr;
  auto *storage_view = original_view->as<StorageView>();
  if (!storage_view || !storage_view->isParameterizedView()) return nullptr;
  ...
  if (original_view_metadata->definer) sql_security->definer = make_intrusive<ASTUserNameWithHost>(*original_view_metadata->definer);
  ...
  auto res =
      std::make_shared<StorageView>(StorageID(database_name, table_name), create, ColumnsDescription(sample_block->getNamesAndTypesList()),
                                    /* comment */ "",
                                    /* is_parameterized_view */ true);
  res->startup();
  return res;
...
StoragePtr Context::getViewSource() const { return view_source; }

bool Context::displaySecretsInShowAndSelect() const { return shared->server_settings[ServerSetting::display_secrets_in_show_and_select]; }
...
void Context::setSettingWithLock(std::string_view name, const String &value, const std::lock_guard<ContextSharedMutex> &lock) {
  ...
  if (ContextAccessParams::dependsOnSettingName(name)) need_recalculate_access = true;
  contextSanityClampSettingsWithLock(*this, *settings, lock);
}
...
void Context::applySettingChangeWithLock(const SettingChange &change, const std::lock_guard<ContextSharedMutex> &lock) {
  try {
    ...
    e.addMessage(
        fmt::format("in attempt to set the value of setting '{}' to {}", change.name, applyVisitor(FieldVisitorToString(), change.value)));
    throw;
  }
}

void Context::applySettingsChangesWithLock(const SettingsChanges &changes, const std::lock_guard<ContextSharedMutex> &lock) {
  for (const SettingChange &change : changes) applySettingChangeWithLock(change, lock);
...
void Context::checkMergeTreeSettingsConstraintsWithLock(const MergeTreeSettings &merge_tree_settings,
                                                        const SettingsChanges &changes) const {
...
void Context::resetSettingsToDefaultValue(const std::vector<String> &names) {
  std::lock_guard lock(mutex);
  for (const String &name : names) settings->setDefaultValue(name);
}

std::shared_ptr<const SettingsConstraintsAndProfileIDs> Context::getSettingsConstraintsAndCurrentProfilesWithLock() const {
  if (settings_constraints_and_current_profiles) return settings_constraints_and_current_profiles;
...
String Context::getInitialQueryId() const { return client_info.initial_query_id; }

void Context::setCurrentDatabaseNameInGlobalContext(const String &name) {
  if (name.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Database name cannot be empty");
...
void Context::setCurrentQueryId(const String &query_id) {
  ...
  if (client_info.query_kind == ClientInfo::QueryKind::INITIAL_QUERY) client_info.initial_query_id = client_info.current_query_id;
}

void Context::killCurrentQuery() const {
  if (auto elem = getProcessListElement()) elem->cancelQuery(CancelReason::CANCELLED_BY_USER);
}
...
bool Context::isCurrentQueryKilled() const {
  ...
  if (auto elem = getProcessListElementSafe()) return elem->isKilled();

  return false;
}

void Context::setInsertionTable(StorageID db_and_table, std::optional<Names> column_names,
...
String Context::getDefaultFormat() const { return default_format.empty() ? "TabSeparated" : default_format; }

void Context::setDefaultFormat(const String &name) { default_format = name; }

String Context::getInsertFormat() const { return insert_format; }

void Context::setInsertFormat(const String &name) { insert_format = name; }

MultiVersion<Macros>::Version Context::getMacros() const { return shared->macros.get(); }

void Context::setMacros(std::unique_ptr<Macros> &&macros) { shared->macros.set(std::move(macros)); }
...
ContextMutablePtr Context::getBackgroundContext() const {
  ...
  if (!ptr) throw Exception(ErrorCodes::LOGICAL_ERROR, "There is no background context or background context has not been initialized");
  return ptr;
}
...
void Context::makeQueryContextForMerge(const MergeTreeSettings &merge_tree_settings) {
  ...
  (*settings)[Setting::workload] = merge_tree_settings[MergeTreeSetting::merge_workload].value.empty()
                                       ? getMergeWorkload()
                                       : merge_tree_settings[MergeTreeSetting::merge_workload];
}

void Context::makeQueryContextForMutate(const MergeTreeSettings &merge_tree_settings) {
  makeQueryContext();
  classifier.reset();  // It is assumed that there are no active queries running using this classifier, otherwise this will lead to crashes
  (*settings)[Setting::workload] = merge_tree_settings[MergeTreeSetting::mutation_workload].value.empty()
                                       ? getMutationWorkload()
                                       : merge_tree_settings[MergeTreeSetting::mutation_workload];
}

void Context::makeSessionContext() { session_context = shared_from_this(); }
...
const EmbeddedDictionaries &Context::getEmbeddedDictionaries() const { return getEmbeddedDictionariesImpl(false); }

EmbeddedDictionaries &Context::getEmbeddedDictionaries() { return getEmbeddedDictionariesImpl(false); }
...
AsyncLoader &Context::getAsyncLoader() const {
  callOnce(shared->async_loader_initialized, [&] {
    shared->async_loader = std::make_unique<AsyncLoader>(
        std::vector<AsyncLoader::PoolInitializer>{
            // IMPORTANT: Pool declaration order should match the order in `PoolId.h` to get the indices right.
            {// TablesLoaderForegroundPoolId
             "ForegroundLoad", CurrentMetrics::TablesLoaderForegroundThreads, CurrentMetrics::TablesLoaderForegroundThreadsActive,
             CurrentMetrics::TablesLoaderForegroundThreadsScheduled,
             shared->server_settings[ServerSetting::tables_loader_foreground_pool_size], TablesLoaderForegroundPriority},
            {// TablesLoaderBackgroundLoadPoolId
             "BackgroundLoad", CurrentMetrics::TablesLoaderBackgroundThreads, CurrentMetrics::TablesLoaderBackgroundThreadsActive,
             CurrentMetrics::TablesLoaderBackgroundThreadsScheduled,
             shared->server_settings[ServerSetting::tables_loader_background_pool_size], TablesLoaderBackgroundLoadPriority},
            {// TablesLoaderBackgroundStartupPoolId
             "BackgrndStartup", CurrentMetrics::TablesLoaderBackgroundThreads, CurrentMetrics::TablesLoaderBackgroundThreadsActive,
             CurrentMetrics::TablesLoaderBackgroundThreadsScheduled,
             shared->server_settings[ServerSetting::tables_loader_background_pool_size], TablesLoaderBackgroundStartupPriority}},
        /* log_failures = */ true,
        /* log_progress = */ true,
        /* log_events = */ true);
  });
...
ExternalUserDefinedExecutableFunctionsLoader &Context::getExternalUserDefinedExecutableFunctionsLoaderWithLock(
    const std::lock_guard<std::mutex> &) TSA_REQUIRES(shared->external_user_defined_executable_functions_mutex) {
...
void Context::tryCreateEmbeddedDictionaries() const {
  if (!shared->server_settings[ServerSetting::dictionaries_lazy_load]) static_cast<void>(getEmbeddedDictionariesImpl(true));
}
...
WasmModuleManager *Context::initWasmModuleManager() {
  ...
  if (shared->wasm_module_manager) return shared->wasm_module_manager.get();

  if (!shared->server_settings[ServerSetting::allow_experimental_webassembly_udf]) return nullptr;
  ...
  shared->wasm_module_manager =
      std::make_unique<WasmModuleManager>(std::move(user_scripts_disk), /* user_scripts_path_ */ "wasm", engine_name);
...
WasmModuleManager &Context::getWasmModuleManager() const {
  SharedLockGuard lock(shared->mutex);
  if (!shared->wasm_module_manager) throw Exception(ErrorCodes::SUPPORT_IS_DISABLED, "WebAssembly support is not enabled");
...
void Context::waitAllBackupsAndRestores() const {
  if (shared->backups_worker) shared->backups_worker->waitAll();
}

void Context::cancelAllBackupsAndRestores() const {
  if (shared->backups_worker) shared->backups_worker->cancelAll();
}

std::shared_ptr<BackupsInMemoryHolder> Context::getBackupsInMemory() {
  std::lock_guard lock(mutex);
  if (!backups_in_memory) backups_in_memory = std::make_shared<BackupsInMemoryHolder>();
...
ProgressCallback Context::getProgressCallback() const { return progress_callback; }

void Context::setProcessListElement(QueryStatusPtr elem) {
  if (isGlobalContext()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Global context cannot have process list element");
...
QueryStatusPtr Context::getProcessListElement() const {
  if (!has_process_list_elem) return {};
  if (auto res = process_list_elem.lock()) return res;
...
void Context::setUncompressedCache(const String &cache_policy, size_t max_size_in_bytes, double size_ratio) {
  ...
  if (shared->uncompressed_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Uncompressed cache has been already created.");

  shared->uncompressed_cache = std::make_shared<UncompressedCache>(cache_policy, CurrentMetrics::UncompressedCacheBytes,
                                                                   CurrentMetrics::UncompressedCacheCells, max_size_in_bytes, size_ratio);
}

void Context::updateUncompressedCacheConfiguration(const Poco::Util::AbstractConfiguration &config) {
  std::lock_guard lock(shared->mutex);

  if (!shared->uncompressed_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Uncompressed cache was not created yet.");
...
void Context::setPageCache(std::chrono::milliseconds history_window, const String &cache_policy, double size_ratio,
                           size_t min_size_in_bytes, size_t max_size_in_bytes, double free_memory_ratio, size_t num_shards) {
  std::lock_guard lock(shared->mutex);

  if (shared->page_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Page cache has been already created.");

  shared->page_cache = std::make_shared<PageCache>(history_window, cache_policy, size_ratio, min_size_in_bytes, max_size_in_bytes,
                                                   free_memory_ratio, num_shards);
}
...
void Context::setMarkCache(const String &cache_policy, size_t max_cache_size_in_bytes, double size_ratio) {
  ...
  if (shared->mark_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Mark cache has been already created.");

  shared->mark_cache = std::make_shared<MarkCache>(cache_policy, CurrentMetrics::MarkCacheBytes, CurrentMetrics::MarkCacheFiles,
...
void Context::updateMarkCacheConfiguration(const Poco::Util::AbstractConfiguration &config) {
  ...
  if (!shared->mark_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Mark cache was not created yet.");
...
ThreadPool &Context::getLoadMarksThreadpool() const {
  callOnce(shared->load_marks_threadpool_initialized, [&] {
    ...
    shared->load_marks_threadpool =
        std::make_unique<ThreadPool>(CurrentMetrics::MarksLoaderThreads, CurrentMetrics::MarksLoaderThreadsActive,
                                     CurrentMetrics::MarksLoaderThreadsScheduled, pool_size, pool_size, queue_size);
  });
...
ThreadPool &Context::getIcebergCatalogThreadpool() const {
  callOnce(shared->iceberg_catalog_threadpool_initialized, [&] {
    ...
    shared->iceberg_catalog_threadpool =
        std::make_unique<ThreadPool>(CurrentMetrics::IcebergCatalogThreads, CurrentMetrics::IcebergCatalogThreadsActive,
                                     CurrentMetrics::IcebergCatalogThreadsScheduled, pool_size, pool_size, queue_size);
  });
...
void Context::setPrimaryIndexCache(const String &cache_policy, size_t max_cache_size_in_bytes, double size_ratio) {
  ...
  if (shared->primary_index_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Primary index cache has been already created.");
...
void Context::updatePrimaryIndexCacheConfiguration(const Poco::Util::AbstractConfiguration &config) {
  ...
  if (!shared->primary_index_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Primary index cache was not created yet.");
...
void Context::setUsersToIgnoreEarlyMemoryLimitCheck(std::string users) {
  ...
  if (users == shared->users_to_ignore_early_memory_limit_check_source) return;
...
void Context::setIndexUncompressedCache(const String &cache_policy, size_t max_size_in_bytes, double size_ratio) {
  ...
  if (shared->index_uncompressed_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Index uncompressed cache has been already created.");

  shared->index_uncompressed_cache =
      std::make_shared<UncompressedCache>(cache_policy, CurrentMetrics::IndexUncompressedCacheBytes,
                                          CurrentMetrics::IndexUncompressedCacheCells, max_size_in_bytes, size_ratio);
}

void Context::updateIndexUncompressedCacheConfiguration(const Poco::Util::AbstractConfiguration &config) {
  std::lock_guard lock(shared->mutex);

  if (!shared->index_uncompressed_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Index uncompressed cache was not created yet.");
...
void Context::setIndexMarkCache(const String &cache_policy, size_t max_cache_size_in_bytes, double size_ratio) {
  ...
  if (shared->index_mark_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Index mark cache has been already created.");

  shared->index_mark_cache = std::make_shared<MarkCache>(cache_policy, CurrentMetrics::IndexMarkCacheBytes,
                                                         CurrentMetrics::IndexMarkCacheFiles, max_cache_size_in_bytes, size_ratio);
}

void Context::updateIndexMarkCacheConfiguration(const Poco::Util::AbstractConfiguration &config) {
  std::lock_guard lock(shared->mutex);

  if (!shared->index_mark_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Index mark cache was not created yet.");
...
void Context::setVectorSimilarityIndexCache(const String &cache_policy, size_t max_size_in_bytes, size_t max_entries, double size_ratio) {
  ...
  shared->vector_similarity_index_cache =
      std::make_shared<VectorSimilarityIndexCache>(cache_policy, max_size_in_bytes, max_entries, size_ratio);
}
...
void Context::clearVectorSimilarityIndexCache() const {
  ...
  if (shared->vector_similarity_index_cache) shared->vector_similarity_index_cache->clear();
}
...
void Context::setTextIndexTokensCache(const String &cache_policy, size_t max_size_in_bytes, size_t max_entries, double size_ratio) {
  ...
  if (shared->text_index_tokens_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Text index tokens cache has been already created.");
...
void Context::updateTextIndexTokensCacheConfiguration(const Poco::Util::AbstractConfiguration &config) {
  ...
  if (!shared->text_index_tokens_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Text index tokens cache was not created yet.");
...
void Context::setTextIndexHeaderCache(const String &cache_policy, size_t max_size_in_bytes, size_t max_entries, double size_ratio) {
  ...
  if (shared->text_index_header_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Text index header cache has been already created.");
...
void Context::updateTextIndexHeaderCacheConfiguration(const Poco::Util::AbstractConfiguration &config) {
  ...
  if (!shared->text_index_header_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Text index header cache was not created yet.");
...
void Context::updateTextIndexPostingsCacheConfiguration(const Poco::Util::AbstractConfiguration &config) {
  ...
  if (!shared->text_index_postings_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Text index posting list cache was not created yet.");
...
void Context::setMMappedFileCache(size_t max_cache_size_in_num_entries) {
  ...
  if (shared->mmap_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Mapped file cache has been already created.");
...
void Context::updateMMappedFileCacheConfiguration(const Poco::Util::AbstractConfiguration &config) {
  ...
  if (!shared->mmap_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Mapped file cache was not created yet.");
...
void Context::setIcebergMetadataFilesCache(const String &cache_policy, size_t max_size_in_bytes, size_t max_entries, double size_ratio) {
  ...
  if (shared->iceberg_metadata_files_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Iceberg metadata cache has been already created.");

  shared->iceberg_metadata_files_cache =
      std::make_shared<IcebergMetadataFilesCache>(cache_policy, max_size_in_bytes, max_entries, size_ratio);
}

void Context::updateIcebergMetadataFilesCacheConfiguration(const Poco::Util::AbstractConfiguration &config) {
  std::lock_guard lock(shared->mutex);

  if (!shared->iceberg_metadata_files_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Iceberg metadata cache was not created yet.");
...
void Context::setParquetMetadataCache(const String &cache_policy, size_t max_size_in_bytes, size_t max_entries, double size_ratio) {
  ...
  if (shared->parquet_metadata_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Parquet metadata cache has been already created.");
...
void Context::updateParquetMetadataCacheConfiguration(const Poco::Util::AbstractConfiguration &config) {
  ...
  if (!shared->parquet_metadata_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Parquet metadata cache was not created yet.");
...
void Context::setQueryConditionCache(const String &cache_policy, size_t max_size_in_bytes, double size_ratio) {
  ...
  if (shared->query_condition_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Mark filter cache has been already create.");
...
void Context::updateQueryConditionCacheConfiguration(const Poco::Util::AbstractConfiguration &config) {
  ...
  if (!shared->query_condition_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Query condition cache was not created yet.");
...
void Context::clearQueryConditionCache() const {
  ...
  if (shared->query_condition_cache) shared->query_condition_cache->clear();
}

void Context::setQueryResultCache(size_t max_size_in_bytes, size_t max_entries, size_t max_entry_size_in_bytes,
                                  size_t max_entry_size_in_rows) {
  std::lock_guard lock(shared->mutex);

  if (shared->query_result_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Query cache has been already created.");

  shared->query_result_cache =
      std::make_shared<QueryResultCache>(max_size_in_bytes, max_entries, max_entry_size_in_bytes, max_entry_size_in_rows);
}

void Context::updateQueryResultCacheConfiguration(const Poco::Util::AbstractConfiguration &config) {
  std::lock_guard lock(shared->mutex);

  if (!shared->query_result_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Query cache was not created yet.");
  ...
  size_t max_entry_size_in_bytes =
      config.getUInt64("query_cache.max_entry_size_in_bytes", DEFAULT_QUERY_RESULT_CACHE_MAX_ENTRY_SIZE_IN_BYTES);
...
void Context::clearCaches() const {
  ...
  if (!shared->text_index_postings_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Text index postings cache was not created yet.");
  shared->text_index_postings_cache->clear();

  if (!shared->mmap_cache) throw Exception(ErrorCodes::LOGICAL_ERROR, "Mmapped file cache was not created yet.");
...
ThreadPool &Context::getPrefetchThreadpool() const {
  callOnce(shared->prefetch_threadpool_initialized, [&] {
    ...
    shared->prefetch_threadpool =
        std::make_unique<ThreadPool>(CurrentMetrics::IOPrefetchThreads, CurrentMetrics::IOPrefetchThreadsActive,
                                     CurrentMetrics::IOPrefetchThreadsScheduled, pool_size, pool_size, queue_size);
  });

  return *shared->prefetch_threadpool;
}

size_t Context::getPrefetchThreadpoolSize() const { return shared->server_settings[ServerSetting::prefetch_threadpool_pool_size]; }
...
ThreadPool &Context::getBuildVectorSimilarityIndexThreadPool() const {
  callOnce(shared->build_vector_similarity_index_threadpool_initialized, [&] {
    ...
    shared->build_vector_similarity_index_threadpool = std::make_unique<ThreadPool>(
        CurrentMetrics::BuildVectorSimilarityIndexThreads, CurrentMetrics::BuildVectorSimilarityIndexThreadsActive,
...
BackgroundSchedulePool &Context::getBufferFlushSchedulePool() const {
  callOnce(shared->buffer_flush_schedule_pool_initialized, [&] {
    shared->buffer_flush_schedule_pool = BackgroundSchedulePool::create(
        ...
        /*max_parallel_tasks_per_type*/ 0, CurrentMetrics::BackgroundBufferFlushSchedulePoolTask,
        CurrentMetrics::BackgroundBufferFlushSchedulePoolSize, ThreadName::BACKGROUND_BUFFER_FLUSH_SCHEDULE_POOL);
  });
...
BackgroundSchedulePool &Context::getSchedulePool() const {
  size_t max_parallel_tasks_per_type =
      static_cast<size_t>(static_cast<double>(shared->server_settings[ServerSetting::background_schedule_pool_size]) *
  ...
  callOnce(shared->schedule_pool_initialized, [&] {
    shared->schedule_pool = BackgroundSchedulePool::create(
        shared->server_settings[ServerSetting::background_schedule_pool_size], max_parallel_tasks_per_type,
        CurrentMetrics::BackgroundSchedulePoolTask, CurrentMetrics::BackgroundSchedulePoolSize, DB::ThreadName::BACKGROUND_SCHEDULE_POOL);
  });
...
BackgroundSchedulePool &Context::getDistributedSchedulePool() const {
  callOnce(shared->distributed_schedule_pool_initialized, [&] {
    shared->distributed_schedule_pool =
        BackgroundSchedulePool::create(shared->server_settings[ServerSetting::background_distributed_schedule_pool_size],
                                       /*max_parallel_tasks_per_type*/ 0, CurrentMetrics::BackgroundDistributedSchedulePoolTask,
                                       CurrentMetrics::BackgroundDistributedSchedulePoolSize, DB::ThreadName::DISTRIBUTED_SCHEDULE_POOL);
  });
...
BackgroundSchedulePool &Context::getMessageBrokerSchedulePool() const {
  callOnce(shared->message_broker_schedule_pool_initialized, [&] {
    shared->message_broker_schedule_pool =
        BackgroundSchedulePool::create(shared->server_settings[ServerSetting::background_message_broker_schedule_pool_size],
                                       /*max_parallel_tasks_per_type*/ 0, CurrentMetrics::BackgroundMessageBrokerSchedulePoolTask,
                                       CurrentMetrics::BackgroundMessageBrokerSchedulePoolSize, DB::ThreadName::MSG_BROKER_SCHEDULE_POOL);
  });
...
void Context::configureServerWideThrottling() {
  if (shared->application_type == ApplicationType::LOCAL || shared->application_type == ApplicationType::SERVER ||
      shared->application_type == ApplicationType::DISKS)
    shared->server_settings.loadSettingsFromConfig(Poco::Util::Application::instance().config());
  if (shared->application_type == ApplicationType::SERVER) shared->configureServerWideThrottling();
}

ThrottlerPtr Context::getReplicatedFetchesThrottler() const { return shared->replicated_fetches_throttler; }

ThrottlerPtr Context::getReplicatedSendsThrottler() const { return shared->replicated_sends_throttler; }
...
ThrottlerPtr Context::getRemoteReadThrottler() const {
  ...
  if (auto bandwidth = getSettingsRef()[Setting::max_remote_read_network_bandwidth]) {
    ...
    if (!remote_read_query_throttler)
      remote_read_query_throttler = std::make_shared<Throttler>(bandwidth, throttler, ProfileEvents::QueryRemoteReadThrottlerBytes,
                                                                ProfileEvents::QueryRemoteReadThrottlerSleepMicroseconds);
...
ThrottlerPtr Context::getRemoteWriteThrottler() const {
  ...
  if (auto bandwidth = getSettingsRef()[Setting::max_remote_write_network_bandwidth]) {
    ...
    if (!remote_write_query_throttler)
      remote_write_query_throttler = std::make_shared<Throttler>(bandwidth, throttler, ProfileEvents::QueryRemoteWriteThrottlerBytes,
                                                                 ProfileEvents::QueryRemoteWriteThrottlerSleepMicroseconds);
...
ThrottlerPtr Context::getLocalReadThrottler() const {
  ...
  if (auto bandwidth = getSettingsRef()[Setting::max_local_read_bandwidth]) {
    ...
    if (!local_read_query_throttler)
      local_read_query_throttler = std::make_shared<Throttler>(bandwidth, throttler, ProfileEvents::QueryLocalReadThrottlerBytes,
                                                               ProfileEvents::QueryLocalReadThrottlerSleepMicroseconds);
...
ThrottlerPtr Context::getLocalWriteThrottler() const {
  ...
  if (auto bandwidth = getSettingsRef()[Setting::max_local_write_bandwidth]) {
    ...
    if (!local_write_query_throttler)
      local_write_query_throttler = std::make_shared<Throttler>(bandwidth, throttler, ProfileEvents::QueryLocalWriteThrottlerBytes,
                                                                ProfileEvents::QueryLocalWriteThrottlerSleepMicroseconds);
...
ThrottlerPtr Context::getBackupsThrottler() const {
  ...
  if (auto bandwidth = getSettingsRef()[Setting::max_backup_bandwidth]) {
    ...
    if (!backups_query_throttler)
      backups_query_throttler = std::make_shared<Throttler>(bandwidth, throttler, ProfileEvents::QueryBackupThrottlerBytes,
                                                            ProfileEvents::QueryBackupThrottlerSleepMicroseconds);
    throttler = backups_query_throttler;
  }
  return throttler;
}

ThrottlerPtr Context::getMutationsThrottler() const { return shared->mutations_throttler; }

ThrottlerPtr Context::getMergesThrottler() const { return shared->merges_throttler; }

ThrottlerPtr Context::getDistributedCacheReadThrottler() const { return shared->distributed_cache_read_throttler; }
...
void Context::reloadRemoteThrottlerConfig(size_t read_bandwidth, size_t write_bandwidth) const {
  if (read_bandwidth) {
    ...
    if (!shared->remote_read_throttler)
      shared->remote_read_throttler = std::make_shared<Throttler>(read_bandwidth, ProfileEvents::RemoteReadThrottlerBytes,
                                                                  ProfileEvents::RemoteReadThrottlerSleepMicroseconds);
  }

  if (shared->remote_read_throttler) std::static_pointer_cast<Throttler>(shared->remote_read_throttler)->setMaxSpeed(read_bandwidth);

  if (write_bandwidth) {
    std::lock_guard lock(shared->mutex);
    if (!shared->remote_write_throttler)
      shared->remote_write_throttler = std::make_shared<Throttler>(write_bandwidth, ProfileEvents::RemoteWriteThrottlerBytes,
                                                                   ProfileEvents::RemoteWriteThrottlerSleepMicroseconds);
  }

  if (shared->remote_write_throttler) std::static_pointer_cast<Throttler>(shared->remote_write_throttler)->setMaxSpeed(write_bandwidth);
}

void Context::reloadLocalThrottlerConfig(size_t read_bandwidth, size_t write_bandwidth) const {
  if (read_bandwidth) {
    std::lock_guard lock(shared->mutex);
    if (!shared->local_read_throttler) shared->local_read_throttler = std::make_shared<Throttler>(read_bandwidth);
  }

  if (shared->local_read_throttler) std::static_pointer_cast<Throttler>(shared->local_read_throttler)->setMaxSpeed(read_bandwidth);

  if (write_bandwidth) {
    std::lock_guard lock(shared->mutex);
    if (!shared->local_write_throttler) shared->local_write_throttler = std::make_shared<Throttler>(write_bandwidth);
  }

  if (shared->local_write_throttler) std::static_pointer_cast<Throttler>(shared->local_write_throttler)->setMaxSpeed(write_bandwidth);
}

bool Context::hasDistributedDDL() const { return getConfigRef().has("distributed_ddl"); }

void Context::setDDLWorker(std::unique_ptr<DDLWorker> ddl_worker, const LoadTaskPtrs &startup_after) {
  std::lock_guard lock(shared->mutex);
  if (shared->ddl_worker) throw Exception(ErrorCodes::LOGICAL_ERROR, "DDL background thread has already been initialized");

  shared->ddl_worker = std::move(ddl_worker);

  auto job = makeLoadJob(getGoals(startup_after), TablesLoaderBackgroundStartupPoolId, "startup ddl worker",
...
DDLWorker &Context::getDDLWorker() const {
...

  {
    ...
    SharedLockGuard lock(shared->mutex);
    if (shared->ddl_worker) return *shared->ddl_worker;
  }

  if (!hasZooKeeper()) throw Exception(ErrorCodes::NO_ELEMENTS_IN_CONFIG, "There is no Zookeeper configuration in server config");

  if (!hasDistributedDDL()) throw Exception(ErrorCodes::NO_ELEMENTS_IN_CONFIG, "There is no DistributedDDL configuration in server config");
...
void recordZooKeeperConnectionLoss() {
  ...
  CurrentMetrics::cas(CurrentMetrics::ZooKeeperConnectionLossStartedTimestampSeconds, 0,
...
zkutil::ZooKeeperPtr Context::getZooKeeper() const {
  ...
  if (!shared->zookeeper) {
    ...
    if (auto zookeeper_connection_log = getZooKeeperConnectionLog(); zookeeper_connection_log)
      zookeeper_connection_log->addConnected(ZooKeeperConnectionLog::default_zookeeper_name, *shared->zookeeper,
                                             ZooKeeperConnectionLog::keeper_init_reason);
  }
  ...
  if (shared->zookeeper->expired()) {
    ...
    if (auto zookeeper_connection_log = getZooKeeperConnectionLog(); zookeeper_connection_log) {
      zookeeper_connection_log->addDisconnected(ZooKeeperConnectionLog::default_zookeeper_name, *old_zookeeper,
                                                ZooKeeperConnectionLog::keeper_expired_reason);
      zookeeper_connection_log->addConnected(ZooKeeperConnectionLog::default_zookeeper_name, *shared->zookeeper,
                                             ZooKeeperConnectionLog::keeper_expired_reason);
    }

    if (isServerCompletelyStarted()) shared->zookeeper->setServerCompletelyStarted();
...
bool checkZooKeeperConfigIsLocal(const Poco::Util::AbstractConfiguration &config, const std::string &config_name) {
  ...
  for (const auto &key : keys) {
    if (startsWith(key, "node")) {
      ...
      if (isLocalAddress(DNSResolver::instance().resolveHostAllInOriginOrder(host).front())) return true;
    }
  }
  return false;
}
  ...
  try {
    ...
    for (const auto &aux_zk_name : keys) {
      if (checkZooKeeperConfigIsLocal(getConfigRef(), "auxiliary_zookeepers." + aux_zk_name)) {
        LOG_DEBUG(shared->log, "Our Keeper server is participant of the auxiliary zookeeper cluster ({}), will try to connect to it",
                  aux_zk_name);
...
UInt32 Context::getZooKeeperSessionUptime() const {
  std::lock_guard lock(shared->zookeeper_mutex);
  if (!shared->zookeeper || shared->zookeeper->expired()) return 0;
...
void Context::handleSystemZooKeeperConnectionLogAfterInitializationIfNeeded() {
...

  {
    ...
    for (auto &zk : shared->auxiliary_zookeepers) {
      if (zookeeper_connection_log)
        zookeeper_connection_log->addConnected(zk.first, *zk.second, ZooKeeperConnectionLog::keeper_init_reason);
    }
  }
}
...
std::shared_ptr<KeeperDispatcher> Context::getKeeperDispatcher() const {
  ...
  if (!dispatcher) throw Exception(ErrorCodes::LOGICAL_ERROR, "Keeper must be initialized before requests");
...
#if USE_NURAFT
  if (auto dispatcher = tryGetKeeperDispatcher()) dispatcher->signalShutdown();
#endif
}
...
zkutil::ZooKeeperPtr Context::getAuxiliaryZooKeeper(const String &name) const {
  ...
  if (zookeeper == shared->auxiliary_zookeepers.end()) {
    ...
    zookeeper = shared->auxiliary_zookeepers
                    .emplace(name, zkutil::ZooKeeper::create(std::move(args), getZooKeeperLog(), getAggregatedZooKeeperLog()))
                    .first;
...
static void reloadZooKeeperIfChangedImpl(const ConfigurationPtr &config, const std::string_view keeper_name, const std::string &config_name,
                                         zkutil::ZooKeeperPtr &zk, std::shared_ptr<ZooKeeperLog> zk_log,
                                         std::shared_ptr<ZooKeeperConnectionLog> zk_concection_log,
                                         std::shared_ptr<AggregatedZooKeeperLog> aggregated_zookeeper_log, bool server_started,
                                         const Int32 send_receive_os_threads_nice_value, bool enforce_component_tracking) {
  ...
  if (!zk || zk->configChanged(*config, config_name)) {
    ...
    if (zk_concection_log) {
      if (old_zk) zk_concection_log->addDisconnected(keeper_name, *old_zk, reason);
      zk_concection_log->addConnected(keeper_name, *zk, reason);
    }

    if (server_started) zk->setServerCompletelyStarted();
  }
}
...
void Context::reloadZooKeeperIfChanged(const ConfigurationPtr &config) const {
  ...
  reloadZooKeeperIfChangedImpl(config, ZooKeeperConnectionLog::default_zookeeper_name, zkutil::getZooKeeperConfigName(*config),
                               shared->zookeeper, getZooKeeperLog(), getZooKeeperConnectionLog(), getAggregatedZooKeeperLog(),
                               server_started, getServerSettings()[ServerSetting::os_threads_nice_value_zookeeper_client_send_receive],
...
void Context::reloadAuxiliaryZooKeepersConfigIfChanged(const ConfigurationPtr &config) {
  ...
  for (auto it = shared->auxiliary_zookeepers.begin(); it != shared->auxiliary_zookeepers.end();) {
    ...
    if (!config->has(config_name)) {
      ...
      reloadZooKeeperIfChangedImpl(config, it->first, config_name, it->second, getZooKeeperLog(), zookeeper_connection_log,
                                   getAggregatedZooKeeperLog(), server_started,
...
bool Context::hasZooKeeper() const { return zkutil::hasZooKeeperConfig(getConfigRef()); }

bool Context::hasAuxiliaryZooKeeper(const String &name) const { return getConfigRef().has("auxiliary_zookeepers." + name); }
...
void Context::reloadQueryMaskingRulesIfChanged(const ConfigurationPtr &config) const {
  ...
  if (old_config && isSameConfiguration(*config, *old_config, "query_masking_rules")) return;

  SensitiveDataMasker::setInstance(std::make_unique<SensitiveDataMasker>(*config, "query_masking_rules"));
  shared->sensitive_data_masker_config = config;
}

InterserverCredentialsPtr Context::getInterserverCredentials() const { return shared->interserver_io_credentials.get(); }
...
void Context::setInterserverScheme(const String &scheme) { shared->interserver_scheme = scheme; }

String Context::getInterserverScheme() const { return shared->interserver_scheme; }

void Context::setRemoteHostFilter(const Poco::Util::AbstractConfiguration &config) {
  shared->remote_host_filter.setValuesFromConfig(config);
}

const RemoteHostFilter &Context::getRemoteHostFilter() const { return shared->remote_host_filter; }
...
const HTTPHeaderFilter &Context::getHTTPHeaderFilter() const { return shared->http_header_filter; }
...
std::optional<UInt16> Context::getTCPPortSecure() const {
  const auto &config = getConfigRef();
  if (config.has("tcp_port_secure")) return config.getInt("tcp_port_secure");
  return {};
}

void Context::registerServerPort(String port_name, UInt16 port) { shared->server_ports.emplace(std::move(port_name), port); }

UInt16 Context::getServerPort(const String &port_name) const {
  auto it = shared->server_ports.find(port_name);
  if (it == shared->server_ports.end()) throw Exception(ErrorCodes::CLUSTER_DOESNT_EXIST, "There is no port named {}", port_name);
  return it->second;
}
...
void Context::setOSCPUOverloadSettings(double min_os_cpu_wait_time_ratio_to_drop_connection,
...
std::shared_ptr<Cluster> Context::getCluster(const std::string &cluster_name) const {
  if (auto res = tryGetCluster(cluster_name)) return res;
...
std::shared_ptr<Cluster> Context::tryGetCluster(const std::string &cluster_name) const {
...

  {
    ...
    if (res == nullptr && shared->cluster_discovery) res = shared->cluster_discovery->getCluster(cluster_name);
  }

  if (res == nullptr && !cluster_name.empty()) res = tryGetReplicatedDatabaseCluster(cluster_name);
...
std::map<String, ClusterPtr> Context::getClusters() const {
  ...
  if (shared->cluster_discovery) {
    ...
    for (const auto &[name, cluster] : cluster_discovery_map) clusters.emplace(name, cluster);
  }
  return clusters;
}
...
void Context::startClusterDiscovery() {
  std::lock_guard lock(shared->clusters_mutex);
  if (!shared->cluster_discovery) return;
...
void Context::setClustersConfig(const ConfigurationPtr &config, bool enable_discovery, const String &config_name) {
  {
    ...
    if (shared->clusters && isSameConfiguration(*config, *shared->clusters_config, config_name)) return;
  ...
  {
    SharedLockGuard lock(shared->mutex);
    if (shared->ddl_worker) shared->ddl_worker->notifyHostIDsUpdated();
  }
}
...
void Context::setCluster(const String &cluster_name, const std::shared_ptr<Cluster> &cluster) {
  ...
  if (!shared->clusters) throw Exception(ErrorCodes::LOGICAL_ERROR, "Clusters are not set");
...
void Context::createTraceCollector() { shared->createTraceCollector(); }

void Context::initializeTraceCollector() { shared->initializeTraceCollector(getTraceLog()); }
...
bool Context::hasTraceCollector() const { return shared->hasTraceCollector(); }
...
std::shared_ptr<BlobStorageLog> Context::getBlobStorageLog() const {
  ...
  if (hasQueryContext()) enable_blob_storage_log = getQueryContext()->getSettingsRef()[Setting::enable_blob_storage_log];

  if (!enable_blob_storage_log) return {};
...
DisksMap Context::getDisksMap(std::lock_guard<std::mutex> &lock) const { return getDiskSelector(lock)->getDisksMap(); }
...
void Context::updateStorageConfiguration(const Poco::Util::AbstractConfiguration &config) {
  {
    ...
    if (shared->merge_tree_storage_policy_selector) {
      try {
        ...
        LOG_ERROR(shared->log, "An error has occurred while reloading storage policies, storage policies were not applied: {}",
                  e.message());
      }
    }
  ...
  {
    std::lock_guard lock(shared->mutex);
    if (shared->storage_s3_settings) shared->storage_s3_settings->loadFromConfig(config, /* config_prefix */ "s3", getSettingsRef());
  }
...
void Context::checkCanBeDropped(const String &database, const String &table, const size_t &size, const size_t &max_size_to_drop) const {
  if (!max_size_to_drop || size <= max_size_to_drop) return;
  ...
  throw Exception(ErrorCodes::TABLE_SIZE_EXCEEDS_MAX_DROP_SIZE_LIMIT,
                  ...
                  backQuoteIfNeed(database), backQuoteIfNeed(table), size_str, max_size_to_drop_str, force_file.string(),
                  force_file_exists ? "exists but not writeable (could not be removed)" : "doesn't exist", force_file.string(),
...
size_t Context::getMaxTableSizeToDrop() const { return shared->max_table_size_to_drop.load(); }
...
void Context::checkTableCanBeDropped(const String &database, const String &table, const size_t &table_size,
                                     const size_t &max_table_size_to_drop) const {
  checkCanBeDropped(database, table, table_size, max_table_size_to_drop);
}
...
size_t Context::getMaxPartitionSizeToDrop() const { return shared->max_partition_size_to_drop.load(); }
...
void Context::checkPartitionCanBeDropped(const String &database, const String &table, const size_t &partition_size,
                                         const size_t &max_partition_size_to_drop) const {
  checkCanBeDropped(database, table, partition_size, max_partition_size_to_drop);
}

void Context::setConfigReloaderInterval(size_t value_ms) { shared->config_reload_interval_ms.store(value_ms, std::memory_order_relaxed); }

size_t Context::getConfigReloaderInterval() const { return shared->config_reload_interval_ms.load(std::memory_order_relaxed); }

InputFormatPtr Context::getInputFormat(const String &name, ReadBuffer &buf, const Block &sample, UInt64 max_block_size,
                                       const std::optional<FormatSettings> &format_settings,
                                       const std::optional<UInt64> &max_block_size_bytes, const std::optional<UInt64> &min_block_size_rows,
                                       const std::optional<UInt64> &min_block_size_bytes) const {
  return FormatFactory::instance().getInput(name, buf, sample, shared_from_this(), max_block_size, format_settings, nullptr, nullptr, false,
                                            CompressionMethod::None, false, max_block_size_bytes, min_block_size_rows,
                                            min_block_size_bytes);
}

OutputFormatPtr Context::getOutputFormat(const String &name, WriteBuffer &buf, const Block &sample,
...
OutputFormatPtr Context::getOutputFormatParallelIfPossible(const String &name, WriteBuffer &buf, const Block &sample,
                                                           const std::optional<FormatSettings> &format_settings) const {
...
void Context::shutdown() TSA_NO_THREAD_SAFETY_ANALYSIS { shared->shutdown(); }

Context::ApplicationType Context::getApplicationType() const { return shared->application_type; }
...
String Context::getDefaultProfileName() const { return shared->default_profile_name; }

String Context::getSystemProfileName() const { return shared->system_profile_name; }

String Context::getFormatSchemaPath() const { return shared->format_schema_path; }

void Context::setFormatSchemaPath(const String &path) { shared->format_schema_path = path; }

String Context::getGoogleProtosPath() const { return shared->google_protos_path; }

void Context::setGoogleProtosPath(const String &path) { shared->google_protos_path = path; }
...
void Context::setQueryMetadataCache(const QueryMetadataCachePtr &query_metadata_cache_) { query_metadata_cache = query_metadata_cache_; }

bool Context::hasQueryParameters() const { return !query_parameters.empty(); }

const NameToNameMap &Context::getQueryParameters() const { return query_parameters; }
...
void Context::addQueryParameters(const NameToNameMap &parameters) {
  for (const auto &[name, value] : parameters) query_parameters.insert_or_assign(name, value);
}

IHostContextPtr &Context::getHostContext() { return host_context; }
...
void Context::setExternalTablesInitializer(ExternalTablesInitializer &&initializer) {
  if (external_tables_initializer_callback) throw Exception(ErrorCodes::LOGICAL_ERROR, "External tables initializer is already set");
...
std::shared_ptr<QueryPlanAndSets> Context::getDeserializedQueryPlan() {
  if (!query_plan_deserialization_callback) throw Exception(ErrorCodes::LOGICAL_ERROR, "Query plan deserialization callback is not set");

  return query_plan_deserialization_callback();
}

void Context::setInputInitializer(InputInitializer &&initializer) {
  if (input_initializer_callback) throw Exception(ErrorCodes::LOGICAL_ERROR, "Input initializer is already set");

  input_initializer_callback = std::move(initializer);
}

void Context::initializeInput(const StoragePtr &input_storage) {
  if (!input_initializer_callback) throw Exception(ErrorCodes::LOGICAL_ERROR, "Input initializer is not set");
...
void Context::setInputBlocksReaderCallback(InputBlocksReader &&reader) {
  if (input_blocks_reader) throw Exception(ErrorCodes::LOGICAL_ERROR, "Input blocks reader is already set");

  input_blocks_reader = std::move(reader);
}

InputBlocksReader Context::getInputBlocksReaderCallback() const { return input_blocks_reader; }

void Context::resetInputCallbacks() {
  if (input_initializer_callback) input_initializer_callback = {};

  if (input_blocks_reader) input_blocks_reader = {};
}
...
void Context::setClientName(const String &client_name) { client_info.client_name = client_name; }
...
void Context::setClientVersion(UInt64 client_version_major, UInt64 client_version_minor, UInt64 client_version_patch,
                               unsigned client_tcp_protocol_version) {
...
void Context::setClientConnectionId(uint32_t connection_id_) { client_info.connection_id = connection_id_; }
...
void Context::setQueryKind(ClientInfo::QueryKind query_kind) { client_info.query_kind = query_kind; }
...
void Context::setInitialQueryId(const String &initial_query_id) { client_info.initial_query_id = initial_query_id; }
...
void Context::setConnectionClientVersion(UInt64 client_version_major, UInt64 client_version_minor, UInt64 client_version_patch,
                                         unsigned client_tcp_protocol_version) {
...
void Context::increaseDistributedDepth() { ++client_info.distributed_depth; }

StorageID Context::resolveStorageID(StorageID storage_id, StorageNamespace where) const {
  if (storage_id.uuid != UUIDHelpers::Nil) return storage_id;
...
StorageID Context::tryResolveStorageID(StorageID storage_id, StorageNamespace where) const {
  ...
  if (resolved && !resolved.hasUUID() && resolved.database_name != DatabaseCatalog::TEMPORARY_DATABASE) {
    ...
    if (db) resolved.uuid = db->tryGetTableUUID(resolved.table_name);
  }
  return resolved;
}
...
StorageID Context::resolveStorageIDImpl(StorageID storage_id, StorageNamespace where, std::optional<Exception> *exception) const {
  ...
  if (!storage_id) {
    if (exception) exception->emplace(Exception(ErrorCodes::UNKNOWN_TABLE, "Both table name and UUID are empty"));
    return storage_id;
  }

  bool look_for_external_table = where & StorageNamespace::ResolveExternal;
  /// Global context should not contain temporary tables
  if (isGlobalContext()) look_for_external_table = false;
  ...
  if (!storage_id.database_name.empty()) {
    if (in_specified_database) return storage_id;  /// NOTE There is no guarantees that table actually exists in database.
    if (exception)
  ...
  if (look_for_external_table) {
    ...
    if (try_resolve(shared_from_this())) return resolved_id;
    ...
    if (is_local_context && try_resolve(query_context_ptr)) return resolved_id;
    ...
    if (is_local_or_query_context && try_resolve(session_context_ptr)) return resolved_id;
  }
  ...
  if (in_current_database) {
    if (current_database.empty()) {
      if (exception) exception->emplace(Exception(ErrorCodes::UNKNOWN_DATABASE, "Default database is not selected"));
      return StorageID::createEmpty();
    }
...
std::optional<UUID> Context::getParentTable() const { return parent_table_uuid; }
...
StopToken Context::getDDLQueryCancellation() const { return ddl_query_cancellation; }

void Context::setDDLAdditionalChecksOnEnqueue(Coordination::Requests requests) { ddl_additional_checks_on_enqueue = requests; }

Coordination::Requests Context::getDDLAdditionalChecksOnEnqueue() const { return ddl_additional_checks_on_enqueue; }

void Context::checkTransactionsAreAllowed(bool explicit_tcl_query /* = false */) const {
  if (getConfigRef().getInt("allow_experimental_transactions", 0)) return;

  if (explicit_tcl_query) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Transactions are not supported");
...
void Context::setCurrentTransaction(MergeTreeTransactionPtr txn) {
  ...
  if (!merge_tree_transaction) merge_tree_transaction_holder = {};
}

MergeTreeTransactionPtr Context::getCurrentTransaction() const { return merge_tree_transaction; }
...
void Context::setServerCompletelyStarted() {
  {
    {
      std::lock_guard lock(shared->zookeeper_mutex);
      if (shared->zookeeper) shared->zookeeper->setServerCompletelyStarted();
    }

    {
      std::lock_guard lock(shared->auxiliary_zookeepers_mutex);
      for (auto &zk : shared->auxiliary_zookeepers) zk.second->setServerCompletelyStarted();
    }
  }
...
void Context::setClusterFunctionReadTaskCallback(ClusterFunctionReadTaskCallback &&callback) { next_task_callback = callback; }

bool Context::hasClusterFunctionReadTaskCallback() const { return next_task_callback.has_value(); }
...
void Context::setMergeTreeReadTaskCallback(MergeTreeReadTaskCallback &&callback) { merge_tree_read_task_callback = callback; }

bool Context::hasMergeTreeAllRangesCallback() const { return merge_tree_all_ranges_callback.has_value(); }
...
void Context::setMergeTreeAllRangesCallback(MergeTreeAllRangesCallback &&callback) { merge_tree_all_ranges_callback = callback; }

BlockMarshallingCallback Context::getBlockMarshallingCallback() const { return block_marshalling_callback; }

void Context::setBlockMarshallingCallback(BlockMarshallingCallback &&callback) { block_marshalling_callback = std::move(callback); }

void Context::setParallelReplicasGroupUUID(UUID uuid) { parallel_replicas_group_uuid = uuid; }

UUID Context::getParallelReplicasGroupUUID() const { return parallel_replicas_group_uuid; }

PartUUIDsPtr Context::getIgnoredPartUUIDs() const {
  std::lock_guard lock(mutex);
  if (!ignored_part_uuids) const_cast<PartUUIDsPtr &>(ignored_part_uuids) = std::make_shared<PartUUIDs>();
...
void Context::setAsynchronousInsertQueue(const std::shared_ptr<AsynchronousInsertQueue> &ptr) {
  ...
  if (std::chrono::milliseconds(getSettingsRef()[Setting::async_insert_poll_timeout_ms].totalMilliseconds()) ==
...
void Context::initializeBackgroundExecutorsIfNeeded() {
  ...
  if (shared->are_background_executors_initialized) return;
  ...
  size_t background_pool_max_tasks_count =
      static_cast<size_t>(static_cast<double>(background_pool_size) * background_merges_mutations_concurrency_ratio);
  ...
  shared->merge_mutate_executor = std::make_shared<MergeMutateBackgroundExecutor>(
      ...
      /*max_tasks_count*/ background_pool_max_tasks_count, CurrentMetrics::BackgroundMergesAndMutationsPoolTask,
      CurrentMetrics::BackgroundMergesAndMutationsPoolSize, ProfileEvents::MergeMutateBackgroundExecutorTaskExecuteStepMicroseconds,
      ProfileEvents::MergeMutateBackgroundExecutorTaskCancelMicroseconds, ProfileEvents::MergeMutateBackgroundExecutorTaskResetMicroseconds,
      ProfileEvents::MergeMutateBackgroundExecutorWaitMicroseconds, background_merges_mutations_scheduling_policy);
  ...
  shared->moves_executor = std::make_shared<OrdinaryBackgroundExecutor>(
      ThreadName::MERGETREE_MOVE, background_move_pool_size, background_move_pool_size, CurrentMetrics::BackgroundMovePoolTask,
      CurrentMetrics::BackgroundMovePoolSize, ProfileEvents::MoveBackgroundExecutorTaskExecuteStepMicroseconds,
      ProfileEvents::MoveBackgroundExecutorTaskCancelMicroseconds, ProfileEvents::MoveBackgroundExecutorTaskResetMicroseconds,
      ProfileEvents::MoveBackgroundExecutorWaitMicroseconds);
  LOG_INFO(shared->log, "Initialized background executor for move operations with num_threads={}, num_tasks={}", background_move_pool_size,
           background_move_pool_size);

  shared->fetch_executor = std::make_shared<OrdinaryBackgroundExecutor>(
      ThreadName::MERGETREE_FETCH, background_fetches_pool_size, background_fetches_pool_size, CurrentMetrics::BackgroundFetchesPoolTask,
      CurrentMetrics::BackgroundFetchesPoolSize, ProfileEvents::FetchBackgroundExecutorTaskExecuteStepMicroseconds,
      ProfileEvents::FetchBackgroundExecutorTaskCancelMicroseconds, ProfileEvents::FetchBackgroundExecutorTaskResetMicroseconds,
      ProfileEvents::FetchBackgroundExecutorWaitMicroseconds);
  LOG_INFO(shared->log, "Initialized background executor for fetches with num_threads={}, num_tasks={}", background_fetches_pool_size,
           background_fetches_pool_size);

  shared->common_executor = std::make_shared<OrdinaryBackgroundExecutor>(
      ThreadName::MERGETREE_COMMON, background_common_pool_size, background_common_pool_size, CurrentMetrics::BackgroundCommonPoolTask,
      CurrentMetrics::BackgroundCommonPoolSize, ProfileEvents::CommonBackgroundExecutorTaskExecuteStepMicroseconds,
      ProfileEvents::CommonBackgroundExecutorTaskCancelMicroseconds, ProfileEvents::CommonBackgroundExecutorTaskResetMicroseconds,
...
IAsynchronousReader &Context::getThreadPoolReader(FilesystemReaderType type) const {
  callOnce(shared->readers_initialized, [&] {
    ...
    shared->asynchronous_remote_fs_reader = createThreadPoolReader(FilesystemReaderType::ASYNCHRONOUS_REMOTE_FS_READER, server_settings);
    shared->asynchronous_local_fs_reader = createThreadPoolReader(FilesystemReaderType::ASYNCHRONOUS_LOCAL_FS_READER, server_settings);
    shared->synchronous_local_fs_reader = createThreadPoolReader(FilesystemReaderType::SYNCHRONOUS_LOCAL_FS_READER, server_settings);
  });
...
IOUringReader &Context::getIOUringReader() const {
  callOnce(shared->io_uring_reader_initialized, [&] { shared->io_uring_reader = createIOUringReader(); });
...
ThreadPool &Context::getThreadPoolWriter() const {
  callOnce(shared->threadpool_writer_initialized, [&] {
    ...
    shared->threadpool_writer = std::make_unique<ThreadPool>(CurrentMetrics::IOWriterThreads, CurrentMetrics::IOWriterThreadsActive,
                                                             CurrentMetrics::IOWriterThreadsScheduled, pool_size, pool_size, queue_size);
  });
...
ReadSettings Context::getReadSettings() const {
  ...
  res.filesystem_cache_allow_background_download_during_fetch =
  ...
  res.filesystem_cache_skip_download_if_exceeds_per_query_cache_write_limit =
  ...
  if (!getSettingsRef()[Setting::max_read_buffer_size]) {
    throw Exception(ErrorCodes::INVALID_SETTING_VALUE, "Invalid value '{}' for max_read_buffer_size",
                    getSettingsRef()[Setting::max_read_buffer_size].value);
  }

  res.local_fs_buffer_size = settings_ref[Setting::max_read_buffer_size_local_fs] ? settings_ref[Setting::max_read_buffer_size_local_fs]
                                                                                  : settings_ref[Setting::max_read_buffer_size];
  res.remote_fs_buffer_size = settings_ref[Setting::max_read_buffer_size_remote_fs] ? settings_ref[Setting::max_read_buffer_size_remote_fs]
                                                                                    : settings_ref[Setting::max_read_buffer_size];
...
std::shared_ptr<AsyncReadCounters> Context::getAsyncReadCounters() const { return async_read_counters; }

bool Context::canUseTaskBasedParallelReplicas() const {
  const auto &settings_ref = getSettingsRef();

  if (!settings_ref[Setting::allow_experimental_analyzer] && settings_ref[Setting::parallel_replicas_only_with_analyzer]) return false;
...
bool Context::canUseOffsetParallelReplicas() const {
  ...
  const bool is_parallel_replicas_with_custom_key_or_native_sampling_key =
      ...
      settings_ref[Setting::parallel_replicas_mode] == ParallelReplicasMode::CUSTOM_KEY_SAMPLING ||
      settings_ref[Setting::parallel_replicas_mode] == ParallelReplicasMode::CUSTOM_KEY_RANGE;
  return offset_parallel_replicas_enabled && has_enough_servers && parallel_replicas_enabled &&
         is_parallel_replicas_with_custom_key_or_native_sampling_key;
}

void Context::disableOffsetParallelReplicas() { offset_parallel_replicas_enabled = false; }
...
ClusterPtr Context::getClusterForParallelReplicas() const {
  ...
  if (settings_ref[Setting::cluster_for_parallel_replicas].value.empty())
    throw Exception(ErrorCodes::CLUSTER_DOESNT_EXIST,
...
void Context::setPreparedSetsCache(const PreparedSetsCachePtr &cache) { prepared_sets_cache = cache; }

PreparedSetsCachePtr Context::getPreparedSetsCache() const { return prepared_sets_cache; }
...
ReverseLookupCache &Context::getReverseLookupCache() const {
  ...
  if (!query_context->reverse_lookup_cache) {
    query_context->reverse_lookup_cache = std::make_shared<ReverseLookupCache>(
        "LRU", CurrentMetrics::end(), CurrentMetrics::end(), settings_ref[Setting::max_reverse_dictionary_lookup_cache_size_bytes],
        ReverseLookupCache::NO_MAX_COUNT, ReverseLookupCache::DEFAULT_SIZE_RATIO);
  }
  return *query_context->reverse_lookup_cache;
}

void Context::setRuntimeFilterLookup(const RuntimeFilterLookupPtr &filter_lookup) { runtime_filter_lookup = filter_lookup; }

RuntimeFilterLookupPtr Context::getRuntimeFilterLookup() const { return runtime_filter_lookup; }

UInt64 Context::getClientProtocolVersion() const { return client_protocol_version; }

void Context::setClientProtocolVersion(UInt64 version) { client_protocol_version = version; }
...
const ServerSettings &Context::getServerSettings() const { return shared->server_settings; }

uint64_t HTTPContext::getMaxHstsAge() const { return context->getSettingsRef()[Setting::hsts_max_age]; }

uint64_t HTTPContext::getMaxUriSize() const { return context->getSettingsRef()[Setting::http_max_uri_size]; }

uint64_t HTTPContext::getMaxFields() const { return context->getSettingsRef()[Setting::http_max_fields]; }

uint64_t HTTPContext::getMaxFieldNameSize() const { return context->getSettingsRef()[Setting::http_max_field_name_size]; }

uint64_t HTTPContext::getMaxFieldValueSize() const { return context->getSettingsRef()[Setting::http_max_field_value_size]; }

Poco::Timespan HTTPContext::getReceiveTimeout() const { return context->getSettingsRef()[Setting::http_receive_timeout]; }

Poco::Timespan HTTPContext::getSendTimeout() const { return context->getSettingsRef()[Setting::http_send_timeout]; }
