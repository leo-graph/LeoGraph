...
void FunctionNameNormalizer::visit(IAST* ast) {
  ...
  for (auto& child : ast->children) visit(child.get());

  if (auto* ttl_elem = ast->as<ASTTTLElement>()) {
    for (const auto& a : ttl_elem->group_by_key) visit(a.get());
    for (const auto& a : ttl_elem->group_by_assignments) visit(a.get());
  }
}
