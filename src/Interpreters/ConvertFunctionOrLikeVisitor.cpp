    ...
    if (auto *expr_list_fn = child->as<ASTExpressionList>()) {
      ...
      for (const auto &child_expr_fn : expr_list_fn->children) {
        ...
        if (const auto *child_fn = child_expr_fn->as<ASTFunction>()) {
          ...
          if (!identifier || !literal || literal->value.getType() != Field::Types::String) continue;
      ...
      if (unique_elems.size() == 1) unique_elems.push_back(make_intrusive<ASTLiteral>(Field(false)));
