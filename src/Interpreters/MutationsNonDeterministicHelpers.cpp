...
class FirstNonDeterministicFunctionMatcher {
 public:
  ...
  static void visit(const ASTPtr &node, Data &data) {
    if (data.result.nondeterministic_function_name || data.result.subquery) return;
    ...
    if (node->as<ASTSelectQuery>()) {
      ...
      if (function->name != "lambda") {
        ...
        if (!func->isDeterministic()) data.result.nondeterministic_function_name = func->getName();
      }
    }
  }
};
...
class ExecuteNonDeterministicConstFunctionsMatcher {
 public:
  ...
  static void visit(ASTPtr &ast, const Data &data) {
    if (auto *function = ast->as<ASTFunction>()) visit(*function, ast, data);
  }

  static void visit(ASTFunction &function, ASTPtr &ast, const Data &data) {
    if (!FunctionFactory::instance().has(function.name)) return;
    ...
    if (builder->isDeterministic() || !builder->isDeterministicInScopeOfQuery()) return;
...
FirstNonDeterministicFunctionResult findFirstNonDeterministicFunction(const MutationCommand &command, ContextPtr context) {
  ...
  switch (command.type) {
    case MutationCommand::UPDATE: {
      ...
      if (finder_data.result.nondeterministic_function_name) return finder_data.result;
...
ASTPtr replaceNonDeterministicToScalars(const ASTAlterCommand &alter_command, ContextPtr context) {
  ...
  if (settings[Setting::mutations_execute_subqueries_on_initiator]) {
    ...
    ExecuteScalarSubqueriesVisitor::Data data{WithContext{context},
                                              /*subquery_depth=*/0,
                                              scalars,
                                              local_scalars,
                                              /*only_analyze=*/false,
