...
void RewriteOrderBy::visit(ASTPtr &ast, Data &) {
  ...
  if (const auto *inner_list = func->children.front()->as<ASTExpressionList>()) {
    ...
    if (!new_order_by->children.empty()) query->setExpression(ASTSelectQuery::Expression::ORDER_BY, std::move(new_order_by));
  }
}
