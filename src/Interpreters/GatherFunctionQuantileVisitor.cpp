...
String GatherFunctionQuantileData::toFusedNameOrSelf(const String &func_name) {
  if (auto it = quantile_fuse_name_mapping.find(func_name); it != quantile_fuse_name_mapping.end()) return it->second;
  return func_name;
}
...
void GatherFunctionQuantileData::visit(ASTFunction &function, ASTPtr &ast) {
  if (!quantile_fuse_name_mapping.contains(function.name)) return;
...
void GatherFunctionQuantileData::FuseQuantileAggregatesData::addFuncNode(ASTPtr &ast) {
  ...
  if (!func || func->parameters == nullptr) return;
  ...
  if (arguments[0]->getColumnName().contains(',')) return;
  String arg_name = arguments[0]->getColumnName();
  if (need_two_args) {
    if (arguments[1]->getColumnName().contains(',')) return;
...
bool GatherFunctionQuantileData::needChild(const ASTPtr &node, const ASTPtr &) {
  ...
  if (const auto *func = node ? node->as<ASTFunction>() : nullptr) return !quantile_fuse_name_mapping.contains(func->name);
  return true;
}
