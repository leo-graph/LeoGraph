...
SessionLogElement::SessionLogElement(const UUID &auth_id_, Type type_) : auth_id(auth_id_), type(type_) {
  std::tie(event_time, event_time_microseconds) = eventTime();
}

ColumnsDescription SessionLogElement::getColumnsDescription() {
  auto event_type = std::make_shared<DataTypeEnum8>(DataTypeEnum8::Values{{"LoginFailure", static_cast<Int8>(SESSION_LOGIN_FAILURE)},
...
#define AUTH_TYPE_NAME_AND_VALUE(v) std::make_pair(toString(v), static_cast<Int8>(v))
  auto identified_with_column = std::make_shared<DataTypeEnum8>(DataTypeEnum8::Values{
...
#undef AUTH_TYPE_NAME_AND_VALUE
  ...
  auto interface_type_column = std::make_shared<DataTypeEnum8>(DataTypeEnum8::Values{
  ...
  auto settings_type_column =
      std::make_shared<DataTypeArray>(std::make_shared<DataTypeTuple>(DataTypes({// setting name
                                                                                 lc_string_datatype,
                                                                                 // value
  ...
  return ColumnsDescription{
      ...
      {"type", std::move(event_type),
       "Login/logout result. Possible values: "
       "LoginFailure — Login error. "
       "LoginSuccess — Successful login. "
       "Logout — Logout from the system."},
      {"auth_id", std::make_shared<DataTypeUUID>(),
       "Authentication ID, which is a UUID that is automatically generated each time user logins."},
      ...
      {"client_hostname", std::make_shared<DataTypeString>(),
       "The hostname of the client machine where the clickhouse-client or another TCP client is run."},
...
void SessionLog::addLoginSuccess(const UUID &auth_id, const String &session_id, const Settings &settings, const ContextAccessPtr &access,
                                 const ClientInfo &client_info, const UserPtr &login_user,
  ...
  if (const auto roles_info = access->getRolesInfo()) log_entry.roles = roles_info->getCurrentRolesNames();

  if (const auto profile_info = access->getDefaultProfileInfo()) log_entry.profiles = profile_info->getProfileNames();

  SettingsChanges changes = settings.changes();
  for (const auto &change : changes) log_entry.settings.emplace_back(change.name, Settings::valueToStringUtil(change.name, change.value));

  add(std::move(log_entry));
}

void SessionLog::addLoginFailure(const UUID &auth_id, const ClientInfo &info, const std::optional<String> &user, const Exception &reason) {
...
void SessionLog::addLogOut(const UUID &auth_id, const UserPtr &login_user, const AuthenticationData &user_authenticated_with,
