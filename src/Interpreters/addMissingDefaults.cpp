...
ActionsDAG addMissingDefaults(const Block &header, const NamesAndTypesList &required_columns, const ColumnsDescription &columns,
                              ContextPtr context, bool null_as_default) {
  ...
  for (size_t i = 0, size = header.columns(); i < size; ++i) {
    ...
    if (typeid_cast<const ColumnArray *>(&*elem.column)) {
      ...
      if (group.empty()) group.push_back(nullptr);
  ...
  for (const auto &column : required_columns) {
    if (header.has(column.name)) continue;

    if (columns.hasDefault(column.name)) continue;
