...
Block getSingleValueBlock(UInt8 value) { return Block{{ColumnUInt8::create(1, value), std::make_shared<DataTypeUInt8>(), "result"}}; }
...
class TableCheckTask : public ChunkInfoCloneable<TableCheckTask> {
 public:
  ...
  TableCheckTask(StoragePtr table_, ContextPtr context) : table(table_), check_data_tasks(table->getCheckTaskList({}, context)) {
    chassert(context);
    context->checkAccess(AccessType::CHECK, table_->getStorageID());
  }

  TableCheckTask(const TableCheckTask &other)
      : table(other.table), check_data_tasks(other.check_data_tasks), is_finished(other.is_finished.load()) {}
...
class TableCheckSource : public ISource {
 public:
  TableCheckSource(Strings databases_, ContextPtr context_, LoggerPtr log_)
      : ISource(std::make_shared<const Block>(getSingleValueBlock(0))), databases(databases_), context(context_), log(log_) {}

  TableCheckSource(std::shared_ptr<TableCheckTask> table_check_task_, LoggerPtr log_)
      : ISource(std::make_shared<const Block>(getSingleValueBlock(0))), table_check_task(table_check_task_), log(log_) {}
 ...
 private:
  std::shared_ptr<TableCheckTask> getTableCheckTask() {
    if (table_check_task && !table_check_task->isFinished()) return table_check_task;

    /// Advance iterator
    if (current_table_it && current_table_it->isValid()) current_table_it->next();
  ...
  StoragePtr getValidTable() {
    while (current_table_it && current_table_it->isValid()) {
      ...
      if (table && table->isMergeTree()) return table;

      LOG_TRACE(log, "Skip checking table '{}.{}' because it is not {}", current_table_it->databaseName(), current_table_it->name(),
                table ? "MergeTree" : "found");
 ...
 protected:
  void transform(Chunk &chunk) override {
    ...
    if (!check_result->success) {
      LOG_WARNING(log, "Check query for table {} failed, path {}, reason: {}", table_check_task->getNameForLogs(), check_result->fs_path,
...
class TableCheckResultEmitter : public IAccumulatingTransform {
 public:
  ...
  Chunk generate() override {
    if (is_value_emitted.exchange(true)) return {};
...
static Strings getAllDatabases(const ContextPtr &context) {
  ...
  for (const auto &[database_name, _] : databases) {
    if (DatabaseCatalog::isPredefinedDatabase(database_name)) continue;
...
BlockIO InterpreterCheckQuery::execute() {
  ...
  OutputPort *resize_outport;
  {
    ...
    for (size_t i = 0; i < worker_ports.size(); ++i, ++resize_inport_it) connect(*worker_ports[i], *resize_inport_it);
    processors->emplace_back(resize_processor);
