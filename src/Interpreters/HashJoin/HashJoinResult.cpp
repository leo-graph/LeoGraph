...
static ColumnWithTypeAndName copyLeftKeyColumnToRight(const DataTypePtr &right_key_type, const String &renamed_right_column,
                                                      const ColumnWithTypeAndName &left_column, const IColumn::Filter *null_map_filter) {
  ...
  if (null_map_filter) right_column.column = JoinCommon::filterWithBlanks(right_column.column, *null_map_filter);
...
static void replicateColumnLazily(ColumnPtr &column, const IColumn::Offsets &offsets, ColumnPtr &indexes) {
  if (isLazyReplicationUseful(column)) {
    if (!indexes) indexes = convertOffsetsToIndexes(offsets);
...
static void appendRightColumns(Block &block, MutableColumns columns, const IColumn::Offsets &offsets, const IColumn::Filter &filter,
                               const NamesAndTypes &type_name, const HashJoinResult::Properties &properties) {
  ...
  if (HashJoin::canRemoveColumnsFromLeftBlock(table_join)) {
    ...
    for (const auto &out_column : table_join.getOutputColumns(JoinTableSide::Left)) left_output_columns.insert(out_column.name);
    for (size_t i = 0; i < block.columns(); ++i) {
      if (!left_output_columns.contains(block.getByPosition(i).name)) block_columns_to_erase.insert(i);
    }
  }
  ...
  for (size_t i = 0; i < properties.required_right_keys.columns(); ++i) {
    ...
    if (is_asof_join && right_key.name == table_join.getOnlyClause().key_names_right.back()) continue;
    ...
    if (!offsets.empty()) right_keys_to_replicate.push_back(block.columns() - 1);
  }
  ...
  if (!offsets.empty()) {
    ...
    if (properties.enable_lazy_columns_replication) {
      ColumnPtr indexes;
      for (size_t i = 0; i < existing_columns; ++i) replicateColumnLazily(columns_to_replicate[i], offsets, indexes);
      for (size_t pos : right_keys_to_replicate) replicateColumnLazily(columns_to_replicate[pos], offsets, indexes);
    } else {
      for (size_t i = 0; i < existing_columns; ++i) columns_to_replicate[i] = columns_to_replicate[i]->replicate(offsets);
      for (size_t pos : right_keys_to_replicate) columns_to_replicate[pos] = columns_to_replicate[pos]->replicate(offsets);
    }
...
MutableColumns copyEmptyColumns(const MutableColumns &columns) {
  ...
  for (const auto &column : columns) res_columns.push_back(column->cloneEmpty());
  return res_columns;
}
...
Block HashJoinResult::generateBlock(std::optional<HashJoinResult::GenerateCurrentRowState> &state, const LazyOutput &lazy_output,
  ...
  if (properties.is_join_get) {
    ...
    rows_added =
        lazy_output.buildOutput(state->rows_to_reserve, state->block, state->offsets, columns, off_data + state->row_ref_begin,
                                off_data + state->row_ref_end, state->state_row_offset, state->state_row_limit, state->state_bytes_limit);
  }
  ...
  if (is_state_finished) state.reset();

  return block;
}

static size_t numLeftRowsForNextBlock(size_t next_row, const IColumn::Offsets &offsets, size_t max_joined_block_rows,
                                      size_t max_joined_block_bytes, size_t avg_bytes_per_row) {
  /// If rows are not replicated, do not split block.
  if (offsets.empty() || (max_joined_block_rows == 0 && max_joined_block_bytes == 0)) return 0;

  /// If offsets does not increase block size, do not split block.
  if (offsets.back() <= offsets.size()) return 0;

  size_t max_rows = max_joined_block_rows;
  if (max_joined_block_bytes) max_rows = std::min<size_t>(max_rows, max_joined_block_bytes / std::max<size_t>(avg_bytes_per_row, 1));

  const size_t prev_offset = next_row ? offsets[next_row - 1] : 0;
  const size_t next_allowed_offset = prev_offset + max_rows;

  if (offsets.back() <= next_allowed_offset) return offsets.size() - next_row;
...
HashJoinResult::HashJoinResult(LazyOutput &&lazy_output_, MutableColumns columns_, IColumn::Offsets offsets_, IColumn::Filter filter_,
                               IColumn::Offsets &&matched_rows_, ScatteredBlock &&block_, Properties properties_)
...
static size_t getAvgBytesPerRow(const Block &block) { return block.allocatedBytes() / std::max<size_t>(1, block.rows()); }

void HashJoinResult::setNextBlock(ScatteredBlock &&block) { next_scattered_block.emplace(std::move(block)); }
...
IJoinResult::JoinResultBlock HashJoinResult::next() {
  ...
  if (!scattered_block) return {Block(), next_block_ptr, true};
  ...
  auto num_lhs_rows =
      numLeftRowsForNextBlock(next_row, offsets, properties.max_joined_block_rows, properties.max_joined_block_bytes, avg_bytes_per_row);
  ...
  if (num_lhs_rows == 0 || (next_row == 0 && num_lhs_rows >= scattered_block->rows())) {
    ...
    if (properties.need_filter) scattered_block->filter(std::span<UInt64>{matched_rows});
  ...
  if (lazy_output.output_by_row_list && !add_missing && !lazy_output.row_refs.empty()) {
    ...
    for (size_t row = 0; row < num_lhs_rows; ++row) {
      ...
      if (offset == last_offset) ++num_skipped_not_matched_rows_in_row_ref_list;
      last_offset = offset;
    }
  } else {
    for (size_t row = 0; row < num_lhs_rows; ++row) partial_offsets[row] = offsets[row + next_row] - prev_offset;
  }

  size_t num_refs = 0;
  if (!lazy_output.row_refs.empty()) {
    num_refs = num_rhs_rows;
    if (lazy_output.output_by_row_list) num_refs = num_lhs_rows - num_skipped_not_matched_rows_in_row_ref_list;
  }
  ...
  if (!lazy_output.row_refs.empty()) {
    if (next_row_ref > lazy_output.row_refs.size())
      throw Exception(ErrorCodes::LOGICAL_ERROR, "The number of joined row_refs {} is more than expected number of row_refs {}",
                      lazy_output.row_refs.size(), next_row_ref);

    if (num_joined_rows > lazy_output.row_count)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "The number or joined rows {} is more than expected number of rows {}", num_joined_rows,
  ...
  if (!lazy_output.row_refs.empty()) {
    ...
    if (start_row == 0 && is_last) {
      ...
      for (auto &column : columns) rhs_columns.push_back(column->cut(start_row, num_rhs_rows)->assumeMutable());

      if (is_last) columns.clear();
    }
  }

  /// Note: need_filter flag cannot be replaced with !added_columns.need_filter.empty()
  /// This is because e.g. for ALL LEFT JOIN filter is used to replace non-matched right keys to defaults.
  if (properties.need_filter) current_scattered_block.filter(partial_matched_rows);
  ...
  auto block = generateBlock(current_row_state, lazy_output, properties);
  if (is_last) scattered_block.reset();
