...
class HashJoin : public IJoin {
 public:
  HashJoin(std::shared_ptr<TableJoin> table_join_, SharedHeader right_sample_block, bool any_take_last_row_ = false,
           size_t reserve_num_ = 0, const String& instance_id_ = "", bool use_two_level_maps_ = false);
  ...
  JoinPipelineType pipelineType() const override {
    ...
    if (from_storage_join) return JoinPipelineType::FilledRight;
...
#undef M
  ...
  struct NullMapHolder {
    ...
    explicit NullMapHolder(const ScatteredColumns* columns_, ColumnPtr column_) : columns(columns_), column(column_) {
