...
class Selector {
 public:
  ...
  class Iterator {
   public:
    ...
    size_t ALWAYS_INLINE operator*() const {
      ...
      if (idx >= selector.size()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Index {} out of range size {}", idx, selector.size());
      return selector[idx];
    }
  ...
  std::pair<Selector, Selector> split(size_t num_rows) {
    ...
    if (std::holds_alternative<Range>(data)) {
      ...
      if (num_rows == 0) return {Selector(), Selector{range.first, range.second}};

      if (num_rows == size()) return {Selector{range.first, range.second}, Selector()};
 ...
 private:
  ...
  Data initializeFromSelector(IndexesPtr &&selector_) {
    ...
    if (selector.empty()) return Range{0, 0};

    /// selector represents continuous range
    if (selector.back() == selector.front() + selector.size() - 1) return Range{selector.front(), selector.front() + selector.size()};
...
struct ScatteredBlock : private boost::noncopyable {
  ...
  ScatteredBlock compress() const {
    if (wasScattered()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot compress ScatteredBlock");
  ...
  bool wasScattered() const { return selector.size() != block.rows(); }
  ...
  void filterBySelector() {
    ...
    for (auto &col : columns) col = col->index(selector.getIndexes(), /*limit*/ 0);
