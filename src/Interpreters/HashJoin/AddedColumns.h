...
struct JoinOnKeyColumns {
  ...
  JoinOnKeyColumns(const ScatteredBlock &block, const Names &key_names_, const String &cond_column_name, const Sizes &key_sizes_);
...
struct LazyOutput {
  ...
  [[nodiscard]] size_t buildOutput(size_t size_to_reserve, const Block &left_block, const IColumn::Offsets &left_offsets,
                                   MutableColumns &columns, const UInt64 *row_refs_begin, const UInt64 *row_refs_end, size_t rows_offset,
                                   size_t rows_limit, size_t bytes_limit) const;
  ...
  /** Build output from the blocks that extract from `RowRef` or `RowRefList`, to avoid block cache miss which may cause performance slow
   * down. And This problem would happen it we directly build output from `RowRef` or `RowRefList`.
   */
  template <bool from_row_list>
  void buildOutputFromBlocks(size_t size_to_reserve, MutableColumns &columns, const UInt64 *row_refs_begin,
                             const UInt64 *row_refs_end) const;

  void buildOutputFromRowRefLists(size_t size_to_reserve, MutableColumns &columns, const UInt64 *row_refs_begin,
                                  const UInt64 *row_refs_end) const;

  [[nodiscard]] size_t buildOutputFromBlocksLimitAndOffset(MutableColumns &columns, const UInt64 *row_refs_begin,
                                                           const UInt64 *row_refs_end, const PaddedPODArray<UInt64> &left_sizes,
                                                           const IColumn::Offsets &left_offsets, size_t rows_offset, size_t rows_limit,
...
class AddedColumns {
 public:
  AddedColumns(const ScatteredBlock &left_block_, const Block &block_with_columns_to_add, const Block &saved_block_sample,
               const HashJoin &join, std::vector<JoinOnKeyColumns> &&join_on_keys_, ExpressionActionsPtr additional_filter_expression_,
               const std::vector<std::pair<size_t, size_t>> &additional_filter_required_rhs_pos_, bool is_asof_join, bool is_join_get_)
    ...
    if (is_asof_join) ++num_columns_to_add;
    ...
    for (const auto &src_column : block_with_columns_to_add) {
      ...
      if (!left_block.has(qualified_name)) addColumn(src_column);
    }
    ...
    for (auto &tn : lazy_output.type_name) lazy_output.right_indexes.push_back(saved_block_sample.getPositionByName(tn.name));
  ...
  void appendDefaultRow() {
    if constexpr (!lazy) {
      ...
      if (has_columns_to_add) lazy_output.addDefault();
    }
  }
  ...
  void reserve(bool need_replicate) {
    /// If lazy, we will reserve right after actual insertion into columns, because at that moment we will know the exact number of rows to
    /// add.
    if constexpr (lazy) return;

    if (!max_joined_block_rows) return;
 ...
 private:
  void checkColumns(const Columns &to_check) {
    for (size_t j = 0; j < lazy_output.right_indexes.size(); ++j) {
      ...
      if (auto *nullable_col = nullable_column_ptrs[j]) {
        if (!is_join_get)
          throw Exception(ErrorCodes::LOGICAL_ERROR, "Columns {} and {} can have different nullability only in joinGetOrNull",
      ...
      if (typeid(*dest_column) != typeid(*column_from_block))
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Columns {} and {} have different types {} and {}", dest_column->getName(),
                        column_from_block->getName(), demangle(typeid(*dest_column).name()), demangle(typeid(*column_from_block).name()));
    }
  }
  ...
  void addColumn(const ColumnWithTypeAndName &src_column) {
    ...
    /// If lazy, we will reserve right after actual insertion into columns, because at that moment we will know the exact number of rows to
    /// add.
    if constexpr (!lazy) columns.back()->reserve(rows_to_add);
