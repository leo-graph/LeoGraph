...
void HashJoinMethods<KIND, STRICTNESS, MapsTemplate>::insertFromBlockImpl(HashJoin &join, HashJoin::Type type, MapsTemplate &maps,
                                                                          const ColumnRawPtrs &key_columns, const Sizes &key_sizes,
                                                                          const ColumnsInfo *stored_columns_info,
                                                                          const ScatteredBlock::Selector &selector,
                                                                          ConstNullMapPtr null_map, const JoinCommon::JoinMask &join_mask,
                                                                          Arena &pool, bool &is_inserted, bool &all_values_unique) {
  switch (type) {
...
#define M(TYPE)                                                                                                                          \
  case HashJoin::Type::TYPE:                                                                                                             \
    if (selector.isContinuousRange())                                                                                                    \
      insertFromBlockImplTypeCase<typename KeyGetterForType<HashJoin::Type::TYPE, std::remove_reference_t<decltype(*maps.TYPE)>>::Type>( \
          join, *maps.TYPE, key_columns, key_sizes, stored_columns_info, selector.getRange(), null_map, join_mask, pool, is_inserted,    \
          all_values_unique);                                                                                                            \
    else                                                                                                                                 \
      insertFromBlockImplTypeCase<typename KeyGetterForType<HashJoin::Type::TYPE, std::remove_reference_t<decltype(*maps.TYPE)>>::Type>( \
          join, *maps.TYPE, key_columns, key_sizes, stored_columns_info, selector.getIndexes(), null_map, join_mask, pool, is_inserted,  \
          all_values_unique);                                                                                                            \
    break;
...
template <JoinKind KIND, JoinStrictness STRICTNESS, typename MapsTemplate>
JoinResultPtr HashJoinMethods<KIND, STRICTNESS, MapsTemplate>::joinBlockImpl(const HashJoin &join, Block block,
                                                                             const Block &block_with_columns_to_add,
                                                                             const MapsTemplateVector &maps_, bool is_join_get) {
...
template <JoinKind KIND, JoinStrictness STRICTNESS, typename MapsTemplate>
JoinResultPtr HashJoinMethods<KIND, STRICTNESS, MapsTemplate>::joinBlockImpl(const HashJoin &join, ScatteredBlock block,
                                                                             const Block &block_with_columns_to_add,
                                                                             const MapsTemplateVector &maps_, bool is_join_get) {
   ...
   */
  AddedColumns<!join_features.is_any_join> added_columns(block, block_with_columns_to_add, join.savedBlockSample(), join,
                                                         std::move(join_on_keys), join.table_join->getMixedJoinExpression(),
                                                         join.additional_filter_required_rhs_pos, join_features.is_asof_join, is_join_get);
  ...
  auto join_result = std::make_unique<HashJoinResult>(std::move(added_columns.lazy_output), std::move(added_columns.columns),
                                                      std::move(added_columns.offsets_to_replicate), std::move(added_columns.filter),
                                                      std::move(added_columns.matched_rows), std::move(block),
  ...
  if (next_scattered_block) join_result->setNextBlock(std::move(next_scattered_block.value()));
  return join_result;
}
...
void HashJoinMethods<KIND, STRICTNESS, MapsTemplate>::insertFromBlockImplTypeCase(HashJoin &join, HashMap &map,
                                                                                  const ColumnRawPtrs &key_columns, const Sizes &key_sizes,
                                                                                  const ColumnsInfo *stored_columns_info,
                                                                                  const Selector &selector, ConstNullMapPtr null_map,
                                                                                  const JoinCommon::JoinMask &join_mask, Arena &pool,
                                                                                  bool &is_inserted, bool &all_values_unique) {
  [[maybe_unused]] constexpr bool mapped_one = std::is_same_v<typename HashMap::mapped_type, RowRef>;
  constexpr bool is_asof_join = STRICTNESS == JoinStrictness::Asof;

  const IColumn *asof_column [[maybe_unused]] = nullptr;
  if constexpr (is_asof_join) asof_column = key_columns.back();
  ...
  for (size_t i = 0; i < rows; ++i) {
    ...
    if (join_mask.isRowFiltered(ind)) continue;
...
size_t HashJoinMethods<KIND, STRICTNESS, MapsTemplate>::switchJoinRightColumns(const std::vector<const MapsTemplate *> &mapv,
                                                                               AddedColumns &added_columns,
                                                                               const ScatteredBlock::Selector &selector,
                                                                               HashJoin::Type type, JoinStuff::JoinUsedFlags &used_flags) {
  constexpr bool is_asof_join = STRICTNESS == JoinStrictness::Asof;
  switch (type) {
    case HashJoin::Type::EMPTY: {
      if constexpr (!is_asof_join) {
        ...
        return joinRightColumnsSwitchNullability<KeyGetter>(std::move(key_getter_vector), a_map_type_vector, added_columns, selector,
                                                            used_flags);
      }
...
size_t HashJoinMethods<KIND, STRICTNESS, MapsTemplate>::joinRightColumnsSwitchNullability(std::vector<KeyGetter> &&key_getter_vector,
                                                                                          const std::vector<const Map *> &mapv,
                                                                                          AddedColumns &added_columns,
                                                                                          const ScatteredBlock::Selector &selector,
                                                                                          JoinStuff::JoinUsedFlags &used_flags) {
  if (added_columns.need_filter)
    return joinRightColumnsSwitchMultipleDisjuncts<KeyGetter, Map, true>(std::forward<std::vector<KeyGetter>>(key_getter_vector), mapv,
                                                                         added_columns, selector, used_flags);
  else
    return joinRightColumnsSwitchMultipleDisjuncts<KeyGetter, Map, false>(std::forward<std::vector<KeyGetter>>(key_getter_vector), mapv,
                                                                          added_columns, selector, used_flags);
}

template <JoinKind KIND, JoinStrictness STRICTNESS, typename MapsTemplate>
template <typename KeyGetter, typename Map, bool need_filter, typename AddedColumns>
size_t HashJoinMethods<KIND, STRICTNESS, MapsTemplate>::joinRightColumnsSwitchMultipleDisjuncts(std::vector<KeyGetter> &&key_getter_vector,
                                                                                                const std::vector<const Map *> &mapv,
                                                                                                AddedColumns &added_columns,
                                                                                                const ScatteredBlock::Selector &selector,
                                                                                                JoinStuff::JoinUsedFlags &used_flags) {
  ...
  if constexpr (join_features.is_maps_all) {
    if (added_columns.additional_filter_expression) {
      ...
      return joinRightColumnsWithAdditionalFilter<KeyGetter, Map>(std::forward<std::vector<KeyGetter>>(key_getter_vector), mapv,
                                                                  added_columns, used_flags, selector, need_filter, mark_per_row_used);
    }
  }
...
template <JoinKind KIND, JoinStrictness STRICTNESS, bool need_filter, bool flag_per_row, typename MapsTemplate, typename Map,
          typename KeyGetter, typename AddedColumns>
void processMatch(const typename KeyGetter::FindResult &find_result, AddedColumns &added_columns, JoinStuff::JoinUsedFlags &used_flags,
                  size_t i, size_t ind, IColumn::Offset &current_offset, KnownRowsHolder<flag_per_row> &known_rows) {
  constexpr JoinFeatures<KIND, STRICTNESS, MapsTemplate> join_features;
...
template <JoinKind KIND, JoinStrictness STRICTNESS, typename MapsTemplate>
template <typename KeyGetter, typename Map, bool need_filter, bool check_null_map, JoinCommon::JoinMask::Kind join_mask_kind,
          typename AddedColumns, typename Selector>
size_t HashJoinMethods<KIND, STRICTNESS, MapsTemplate>::joinRightColumns(KeyGetter &key_getter, const Map *map, AddedColumns &added_columns,
                                                                         JoinStuff::JoinUsedFlags &used_flags, const Selector &selector) {
  ...
  if constexpr (join_features.need_replication) added_columns.offsets_to_replicate = IColumn::Offsets(rows);
  ...
  for (size_t i = 0; i < rows; ++i) {
    ...
    bool skip_row = false;
    if constexpr (check_null_map) skip_row = join_keys.null_map && (*join_keys.null_map)[ind];
    ...
    if (!skip_row) {
      ...
      if (find_result.isFound()) {
        ...
        processMatch<KIND, STRICTNESS, need_filter, flag_per_row, MapsTemplate, Map, KeyGetter>(find_result, added_columns, used_flags, i,
                                                                                                ind, current_offset, dummy_known_rows);
      }
    }
...
size_t HashJoinMethods<KIND, STRICTNESS, MapsTemplate>::joinRightColumnsSwitchJoinMaskKind(KeyGetter &key_getter, const Map *map,
                                                                                           AddedColumns &added_columns,
                                                                                           JoinStuff::JoinUsedFlags &used_flags,
...
template <typename KeyGetter, typename Map, bool need_filter, bool check_null_map, JoinCommon::JoinMask::Kind join_mask_kind,
          typename AddedColumns, typename Selector>
size_t HashJoinMethods<KIND, STRICTNESS, MapsTemplate>::joinRightColumns(std::vector<KeyGetter> &&key_getter_vector,
                                                                         const std::vector<const Map *> &mapv, AddedColumns &added_columns,
                                                                         JoinStuff::JoinUsedFlags &used_flags, const Selector &selector) {
  ...
  for (; i < rows && current_offset < max_joined_rows; ++i) {
    ...
    for (size_t onexpr_idx = 0; onexpr_idx < added_columns.join_on_keys.size(); ++onexpr_idx) {
      ...
      if (!skip_row) {
        ...
        if (find_result.isFound()) {
          right_row_found = true;
          processMatch<KIND, STRICTNESS, need_filter, flag_per_row, MapsTemplate, Map, KeyGetter>(find_result, added_columns, used_flags, i,
                                                                                                  ind, current_offset, known_rows);

          if constexpr ((join_features.is_any_join && join_features.inner) || (join_features.is_any_or_semi_join)) break;
        }
      }
    }
    ...
    if constexpr (join_features.need_replication) added_columns.offsets_to_replicate.push_back(current_offset);
  }
...
template <JoinKind KIND, JoinStrictness STRICTNESS, typename MapsTemplate>
template <typename KeyGetter, typename Map, bool need_filter, bool check_null_map, typename AddedColumns, typename Selector>
size_t HashJoinMethods<KIND, STRICTNESS, MapsTemplate>::joinRightColumnsSwitchJoinMaskKind(std::vector<KeyGetter> &&key_getter_vector,
                                                                                           const std::vector<const Map *> &mapv,
                                                                                           AddedColumns &added_columns,
                                                                                           JoinStuff::JoinUsedFlags &used_flags,
                                                                                           const Selector &selector) {
  ...
  if (std::ranges::all_of(added_columns.join_on_keys, [](const auto &elem) { return elem.join_mask_column.getKind() == Kind::AllTrue; })) {
    return joinRightColumns<KeyGetter, Map, need_filter, check_null_map, Kind::AllTrue>(std::move(key_getter_vector), mapv, added_columns,
                                                                                        used_flags, selector);
  } else if (std::ranges::all_of(added_columns.join_on_keys,
                                 [](const auto &elem) { return elem.join_mask_column.getKind() == Kind::AllFalse; })) {
    return joinRightColumns<KeyGetter, Map, need_filter, check_null_map, Kind::AllFalse>(std::move(key_getter_vector), mapv, added_columns,
                                                                                         used_flags, selector);
  } else {
    return joinRightColumns<KeyGetter, Map, need_filter, check_null_map, Kind::Unknown>(std::move(key_getter_vector), mapv, added_columns,
                                                                                        used_flags, selector);
  }
}

template <typename AddedColumns, typename Selector>
static ColumnPtr buildAdditionalFilter(const Selector &selector, const PODArray<const RowRef *> &selected_rows,
                                       const IColumn::Offsets &row_replicate_offset, const AddedColumns &added_columns) {
      ...
      if (rhs_pos_it != added_columns.additional_filter_required_rhs_pos.end() && pos == rhs_pos_it->first) {
        ...
        if (!src_col)
          throw Exception(ErrorCodes::LOGICAL_ERROR,
                          ...
                          required_cols.toString(), added_columns.left_block.dumpNames(), col_name);
  ...
  if (result_column->isNullable()) {
    ...
    for (size_t i = 0; i < column_data.size(); ++i) {
      if (mask_column_data[i]) column_data[i] = 0;
    }
...
size_t HashJoinMethods<KIND, STRICTNESS, MapsTemplate>::joinRightColumnsWithAdditionalFilter(
    ...
    JoinStuff::JoinUsedFlags &used_flags [[maybe_unused]], const ScatteredBlock::Selector &selector, bool need_filter [[maybe_unused]],
  ...
  if constexpr (join_features.need_replication) added_columns.offsets_to_replicate = IColumn::Offsets(left_block_rows);
  ...
  if (max_joined_rows == 0) max_joined_rows = std::numeric_limits<size_t>::max();

  {
    ...
    for (auto ind : selector) {
      ...
      for (size_t join_clause_idx = 0; join_clause_idx < added_columns.join_on_keys.size(); ++join_clause_idx) {
        ...
        if (join_keys.null_map && (*join_keys.null_map)[ind]) continue;

        bool row_acceptable = !join_keys.isRowFiltered(ind);
        auto find_result = row_acceptable ? key_getter_vector[join_clause_idx].findKey(*(mapv[join_clause_idx]), ind, *pool) : FindResult();
      ...
      if (current_added_rows >= max_joined_rows) break;
    }
    ...
    if (selected_rows.size() != current_added_rows)
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      ...
                      selected_rows.size(), current_added_rows, row_replicate_offset.size());
  }
        ...
        for (size_t replicated_row = prev_replicated_row; replicated_row < row_replicate_offset[i]; ++replicated_row) {
          if (filter_flags[replicated_row]) {
            if constexpr (join_features.is_semi_join || join_features.is_any_join) {
              ...
              any_matched = true;
              if constexpr (join_features.right && join_features.need_flags)
                used_flags.template setUsed<true, true>(&(*selected_right_row_it)->columns_info->columns, (*selected_right_row_it)->row_num,
                                                        0);
            } else {
              any_matched = true;
              total_added_rows += 1;
              added_columns.appendFromBlock(*selected_right_row_it, join_features.add_missing);
              used_flags.template setUsed<join_features.need_flags, true>(&(*selected_right_row_it)->columns_info->columns,
                                                                          (*selected_right_row_it)->row_num, 0);
            }
          }
      ...
      if constexpr (join_features.is_anti_join) {
        if (!any_matched) {
          if constexpr (join_features.left)
            if (need_filter) setUsed<true>(added_columns.filter, i, added_columns.matched_rows);
        ...
        if (!any_matched) {
          ...
          if (!flag_per_row) used_flags.template setUsed<join_features.need_flags, false>(find_results[find_result_index]);
          if (need_filter) setUsed<true>(added_columns.filter, i, added_columns.matched_rows);
          if constexpr (join_features.add_missing) added_columns.applyLazyDefaults();
        }
      }
