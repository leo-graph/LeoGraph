 ...
 public:
  ...
  void reinit(const Columns *columns, const ScatteredBlock::Selector &selector) {
    if constexpr (MapGetter<KIND, STRICTNESS, prefer_use_maps_all>::flagged) {
      ...
      for (auto &flag : per_row_flags[columns]) flag.store(true);
      for (size_t index : selector) per_row_flags[columns][index].store(false);
    }
  }
  ...
  bool getUsedSafe(const Columns *columns, size_t row_idx) const {
    if (auto it = per_row_flags.find(columns); it != per_row_flags.end()) return it->second[row_idx].load();
    return !need_flags;
  }

  template <bool use_flags, bool flag_per_row, typename FindResult>
  void setUsed(const FindResult &f) {
    if constexpr (!use_flags) return;
  ...
  bool getUsed(const FindResult &f) {
    if constexpr (!use_flags) return true;
  ...
  bool setUsedOnce(const FindResult &f) {
    ...
    if constexpr (flag_per_row) {
      ...
      if (per_row_flags[&mapped.columns_info->columns][mapped.row_num].load(std::memory_order_relaxed)) return false;
      ...
      /// fast check to prevent heavy CAS with seq_cst order
      if (per_offset_flags[off].load(std::memory_order_relaxed)) return false;
  ...
  bool setUsedOnce(const Columns *columns, size_t row_num, size_t offset) {
    ...
    if constexpr (flag_per_row) {
      /// fast check to prevent heavy CAS with seq_cst order
      if (per_row_flags[columns][row_num].load(std::memory_order_relaxed)) return false;

      bool expected = false;
      return per_row_flags[columns][row_num].compare_exchange_strong(expected, true);
    } else {
      /// fast check to prevent heavy CAS with seq_cst order
      if (per_offset_flags[offset].load(std::memory_order_relaxed)) return false;
  ...
  bool allOffsetFlagsSet() const noexcept {
    for (const auto &per_offset_flag : per_offset_flags)
      if (!per_offset_flag.load(std::memory_order_relaxed)) return false;
    return true;
  }
};
