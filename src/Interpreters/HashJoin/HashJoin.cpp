...
Int64 getCurrentQueryMemoryUsage() {
  ...
  if (auto *memory_tracker_child = CurrentThread::getMemoryTracker())
    if (auto *memory_tracker = memory_tracker_child->getParent()) return memory_tracker->get();
  return 0;
}

Block filterColumnsPresentInSampleBlock(const Block &block, const Block &sample_block) {
  Block filtered_block;
  for (const auto &sample_column : sample_block.getColumnsWithTypeAndName()) filtered_block.insert(block.getByName(sample_column.name));
...
Block materializeColumnsFromRightBlock(Block block, const Block &sample_block, const Names &) {
  ...
  for (auto &column : block) block_index[column.name].push_back(&column);
  ...
  for (const auto &sample_column : sample_block.getColumnsWithTypeAndName()) {
    for (auto *column_ptr : block_index[sample_column.name]) {
      ...
      if (replicated_column) actual_column = replicated_column->getNestedColumn();
      ...
      if (sample_column.column->isNullable()) JoinCommon::convertColumnToNullable(column);

      if (replicated_column) column.column = ColumnReplicated::create(column.column, replicated_column->getIndexesColumn());
    }
  }
...
HashJoin::HashJoin(std::shared_ptr<TableJoin> table_join_, SharedHeader right_sample_block_, bool any_take_last_row_, size_t reserve_num_,
                   const String &instance_id_, bool use_two_level_maps)
  ...
  if (joined_block_split_single_row && max_joined_block_rows == 0) {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                    ...
                    "Set max_joined_block_rows > 0 or use `max_joined_block_bytes` with default `max_joined_block_rows` (by default equals "
                    "to block size).");
  }
  ...
  LOG_TEST(log, "{}Keys: {}, datatype: {}, kind: {}, strictness: {}, right header: {}", instance_log_id,
           TableJoin::formatClauses(table_join->getClauses(), true), data->type, kind, strictness, right_sample_block.dumpStructure());
  ...
  for (auto &maps : data->maps) dataMapInit(maps);
...
size_t HashJoin::ScatteredColumns::allocatedBytes() const {
  if (columns_info.columns.empty()) return 0;

  size_t rows = columns_info.columns.front()->size();
  if (rows == 0) return 0;

  size_t res = 0;
  for (const auto &column : columns_info.columns) res += column->allocatedBytes();
...
size_t HashJoin::NullMapHolder::allocatedBytes() const {
  ...
  if (rows == 0) return 0;
  if (rows < selector_rows) throw Exception(ErrorCodes::LOGICAL_ERROR, "The column size is smaller than the cached size");
...
static HashJoin::Type chooseMethod(JoinKind kind, const ColumnRawPtrs &key_columns, Sizes &key_sizes) {
  ...
  if (keys_size == 0) {
    if (isCrossOrComma(kind)) return Type::CROSS;
    return Type::EMPTY;
  }
  ...
  if (keys_size == 1 && key_columns[0]->isNumeric()) {
    ...
    if (size_of_field == 16) return Type::keys128;
    if (size_of_field == 32) return Type::keys256;
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Numeric column has sizeOfField not in 1, 2, 4, 8, 16, 32.");
  }

  /// If the keys fit in N bits, we will use a hash table for N-bit-packed keys
  if (all_fixed && keys_bytes <= 16) return Type::keys128;
  if (all_fixed && keys_bytes <= 32) return Type::keys256;
  ...
  if (keys_size == 1 && typeid_cast<const ColumnFixedString *>(key_columns[0])) return Type::key_fixed_string;
...
static HashJoin::Type chooseMethod(JoinKind kind, const ColumnRawPtrs &key_columns, Sizes &key_sizes, bool use_two_level_maps) {
  ...
  if (!use_two_level_maps) return chooseMethod(kind, key_columns, key_sizes);
...
void HashJoin::dataMapInit(MapsVariant &map) {
  ...
  if (!data) throw Exception(ErrorCodes::LOGICAL_ERROR, "HashJoin::dataMapInit called with empty data");
}

bool HashJoin::preferUseMapsAll() const {
  return all_join_was_promoted_to_right_any  // It means that we built hash tables for ALL strictness, but upon finishing found out that we
                                             // can switch to RIGHT ANY. In this case we still have to use ALL maps.
         || table_join->getMixedJoinExpression() != nullptr;
}

bool HashJoin::empty() const { return data->type == Type::EMPTY; }

bool HashJoin::alwaysReturnsEmptySet() const { return (isInnerOrRight(getKind()) || isCrossOrComma(getKind())) && data->rows_to_join == 0; }
...
void HashJoin::doDebugAsserts() const {
  ...
  for (const auto &columns : data->columns) debug_allocated_size += columns.allocatedBytes();

  if (data->allocated_size != debug_allocated_size)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "data->allocated_size != debug_allocated_size ({} != {})", data->allocated_size,
                    debug_allocated_size);

  size_t debug_nullmaps_allocated_size = 0;
  for (const auto &nullmap : data->nullmaps) debug_nullmaps_allocated_size += nullmap.allocatedBytes();

  if (data->nullmaps_allocated_size != debug_nullmaps_allocated_size)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "data->nullmaps_allocated_size != debug_nullmaps_allocated_size ({} != {})",
...
size_t HashJoin::getTotalByteCount() const {
  ...
  if (data->type != Type::CROSS) {
    const bool prefer_use_maps_all = preferUseMapsAll();
    for (const auto &map : data->maps) {
      joinDispatch(kind, strictness, map, prefer_use_maps_all,
...
bool HashJoin::isUsedByAnotherAlgorithm() const { return isUsedByAnotherAlgorithm(*table_join); }

bool HashJoin::canRemoveColumnsFromLeftBlock() const { return canRemoveColumnsFromLeftBlock(*table_join); }
...
Block HashJoin::prepareRightBlock(const Block &block) const { return prepareRightBlock(block, savedBlockSample()); }
...
bool HashJoin::addBlockToJoin(const Block &source_block, size_t num_rows, bool check_limits) {
  ...
  if (rows == 0 && num_rows != 0 && !materialized.columns()) rows = num_rows;
...
bool HashJoin::addBlockToJoin(const Block &block, ScatteredBlock::Selector selector, bool check_limits) {
  if (!data) throw Exception(ErrorCodes::LOGICAL_ERROR, "Join data was released");
   ...
   */
  if (!memory_usage_before_adding_blocks) memory_usage_before_adding_blocks = getCurrentQueryMemoryUsage();
  ...
  if (strictness == JoinStrictness::Asof) {
    ...
    if (asof_column.type->isNullable()) {
      ...
      if (const auto *asof_const_column = typeid_cast<const ColumnConst *>(asof_column.column.get())) {
        if (asof_const_column->isNullAt(0)) return false;
      } else {
        ...
        for (size_t i = 0; i < asof_column_nullable.size(); ++i)
          if (!asof_column_nullable[i]) new_selector_data.push_back(i);
  ...
  for (const auto &column_name : right_key_names) {
    ...
    all_key_columns[column_name] =
        removeSpecialRepresentations(column->convertToFullColumnIfConst())->convertToFullColumnIfLowCardinality();
  }

  Block block_to_save = filterColumnsPresentInSampleBlock(block, savedBlockSample());
  if (shrink_blocks) block_to_save = block_to_save.shrinkToFit();
  ...
  if (kind == JoinKind::Cross && tmp_data &&
    ...
    if (!tmp_stream) tmp_stream.emplace(std::make_shared<const Block>(right_sample_block), tmp_data);
  ...
  {
    if (storage_join_lock) throw DB::Exception(ErrorCodes::LOGICAL_ERROR, "addBlockToJoin called when HashJoin locked to prevent updates");
    ...
    if (kind == JoinKind::Cross && ((min_bytes_to_compress && getTotalByteCount() >= min_bytes_to_compress) ||
    ...
    for (size_t onexpr_idx = 0; onexpr_idx < onexprs.size(); ++onexpr_idx) {
      ...
      for (const auto &name : onexprs[onexpr_idx].key_names_right) key_columns.push_back(all_key_columns[name].get());
      ...
      if (isRightOrFull(kind) && null_map) {
        ...
        for (size_t i = 0; !save_nullmap && i < null_map->size(); ++i) save_nullmap |= (*null_map)[i];
      }
      ...
      if (!flag_per_row && isRightOrFull(kind) && join_mask_col.hasData()) {
        ...
        auto mark_if_needed = [&](size_t row) {
          if (!join_mask_col.isRowFiltered(row)) return;  // ON condition passed -> not "non-joined"
          if (save_nullmap && (*null_map)[row]) return;   // already covered by null-keys map
          not_joined_map->getData()[row] = 1;
          has_right_not_joined = true;
        };

        for (size_t r : sel) mark_if_needed(r);
      }
      ...
      if (kind != JoinKind::Cross) {
        joinDispatch(kind, strictness, data->maps[onexpr_idx], prefer_use_maps_all, [&](auto kind_, auto strictness_, auto &map) {
          HashJoinMethods<kind_, strictness_, std::decay_t<decltype(map)>>::insertFromBlockImpl(
              *this, data->type, map, key_columns, key_sizes[onexpr_idx], &stored_columns->columns_info, stored_columns->selector, null_map,
              join_mask_col, data->pool, is_inserted, all_values_unique);

          if (flag_per_row)
            used_flags->reinit<kind_, strictness_, std::is_same_v<std::decay_t<decltype(map)>, MapsAll>>(
                &stored_columns->columns_info.columns, stored_columns->selector);
        });
      }
...
void HashJoin::shrinkStoredBlocksToFit(size_t &total_bytes_in_join, bool force_optimize) {
  ...
  if (!force_optimize) {
    if (shrink_blocks) return;  /// Already shrunk
  ...
  for (auto &stored_columns : data->columns) {
    ...
    try {
      for (auto &column : stored_columns.columns_info.columns) column = column->cloneResized(column->size());
    } catch (...) {
...
class CrossJoinResult final : public IJoinResult {
 ...
 public:
  CrossJoinResult(const HashJoin &join_, Block block_) : block(std::move(block_)), join(join_) {}
...
IJoinResult::JoinResultBlock CrossJoinResult::next() {
...

  {
    ...
    size_t to_reserve = 0;
    if (common::mulOverflow(block.rows(), join.data->rows_to_join, to_reserve)) to_reserve = join.max_joined_block_rows;

    to_reserve = std::min(join.max_joined_block_rows, to_reserve);

    for (auto &dst : dst_columns) dst->reserve(to_reserve);
  }
  ...
  for (; left_row < rows_total; ++left_row) {
    ...
    auto process_right_block = [&](const ColumnsInfo &columns_info, size_t rows_right) {
      ...
      for (size_t col_num = 0; col_num < num_columns_to_add; ++col_num) {
        if (const auto *replicated_column_right = columns_info.replicated_columns[col_num]) {
          for (size_t row = 0; row != rows_right; ++row)
            dst_columns[num_existing_columns + col_num]->insertFrom(*replicated_column_right->getNestedColumn(),
      ...
      if (join.max_joined_block_bytes) {
        ...
        for (const auto &dst : dst_columns) bytes_added += dst->byteSize();
      }
    };

    if (!right_block_it.has_value()) right_block_it = join.data->columns.begin();

    for (; *right_block_it != join.data->columns.end(); ++*right_block_it) {
      if (enough_data()) break;

      const auto &scattered_columns = **right_block_it;
      /// The following statement cannot be substituted with `process_right_block(!have_compressed ? block_right :
      /// block_right.decompress())` because it will lead to copying of `block_right` even if its branch is taken (because common type of
      /// `block_right` and `block_right.decompress()` is `Block`).
      if (!join.have_compressed)
        ...
        chassert(scattered_columns.columns_info.columns.empty() ||
                 scattered_columns.selector.size() ==
                     scattered_columns.columns_info.columns.at(0)
        ...
        for (const auto &column : scattered_columns.columns_info.columns) new_columns.emplace_back(column->decompress());

        process_right_block(ColumnsInfo(std::move(new_columns)), scattered_columns.selector.size());
      }
    }

    if (*right_block_it != join.data->columns.end()) break;

    if (join.tmp_stream) {
      if (!reader) reader = join.tmp_stream->getReadStream();
  ...
  for (const ColumnWithTypeAndName &src_column : join.sample_block_with_columns_to_add) res.insert(src_column);
...
JoinResultPtr HashJoin::joinBlockImplCross(Block block) const { return std::make_unique<CrossJoinResult>(*this, std::move(block)); }
...
DataTypePtr HashJoin::joinGetCheckAndGetReturnType(const DataTypes &data_types, const String &column_name, bool or_null) const {
  ...
  if (right_table_keys.columns() != num_keys)
    throw Exception(
        ...
        toString(or_null ? "OrNull" : ""), toString(num_keys), toString(right_table_keys.columns()));
  ...
  for (size_t i = 0; i < num_keys; ++i) {
    ...
    if (!left_type->equals(*right_type))
      throw Exception(ErrorCodes::TYPE_MISMATCH,
                      ...
                      i, left_type->getName(), right_type->getName());
  }
  ...
  if (or_null && JoinCommon::canBecomeNullable(elem.type)) elem.type = makeNullable(elem.type);
  return elem.type;
}
...
ColumnWithTypeAndName HashJoin::joinGet(const Block &block, const Block &block_with_columns_to_add) const {
  ...
  if (!is_valid) throw Exception(ErrorCodes::INCOMPATIBLE_TYPE_OF_JOIN, "joinGet only supports StorageJoin of type Left Any");
  const auto &key_names_right = table_join->getOnlyClause().key_names_right;
  ...
  auto res = HashJoinMethods<JoinKind::Left, JoinStrictness::Any, MapsOne>::joinBlockImpl(*this, std::move(keys), block_with_columns_to_add,
                                                                                          maps_vector, /* is_join_get = */ true)
                 ->next();
...
JoinResultPtr HashJoin::joinBlock(Block block) {
  if (!data) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot join after data has been released");

  for (const auto &onexpr : table_join->getClauses()) {
    auto cond_column_name = onexpr.condColumnNames();
    JoinCommon::checkTypesOfKeys(block, onexpr.key_names_left, cond_column_name.first, right_sample_block, onexpr.key_names_right,
                                 cond_column_name.second);
  }

  if (kind == JoinKind::Cross || kind == JoinKind::Comma) return joinBlockImplCross(std::move(block));
  ...
  const bool prefer_use_maps_all = preferUseMapsAll();
  {
    ...
    for (size_t i = 0; i < table_join->getClauses().size(); ++i) maps_vector.push_back(&data->maps[i]);
    ...
    if (joinDispatch(kind, strictness, maps_vector, prefer_use_maps_all, [&](auto kind_, auto strictness_, auto &maps_vector_) {
          if constexpr (std::is_same_v<std::decay_t<decltype(maps_vector_)>, std::vector<const MapsAll *>>) {
            res = HashJoinMethods<kind_, strictness_, MapsAll>::joinBlockImpl(*this, std::move(block), sample_block_with_columns_to_add,
                                                                              maps_vector_);
          } else if constexpr (std::is_same_v<std::decay_t<decltype(maps_vector_)>, std::vector<const MapsOne *>>) {
            res = HashJoinMethods<kind_, strictness_, MapsOne>::joinBlockImpl(*this, std::move(block), sample_block_with_columns_to_add,
                                                                              maps_vector_);
          } else if constexpr (std::is_same_v<std::decay_t<decltype(maps_vector_)>, std::vector<const MapsAsof *>>) {
            res = HashJoinMethods<kind_, strictness_, MapsAsof>::joinBlockImpl(*this, std::move(block), sample_block_with_columns_to_add,
                                                                               maps_vector_);
          } else {
...
JoinResultPtr HashJoin::joinScatteredBlock(ScatteredBlock block) {
  ...
  for (const auto &onexpr : table_join->getClauses()) {
    auto cond_column_name = onexpr.condColumnNames();
    JoinCommon::checkTypesOfKeys(block.getSourceBlock(), onexpr.key_names_left, cond_column_name.first, right_sample_block,
                                 onexpr.key_names_right, cond_column_name.second);
  }
  ...
  const bool prefer_use_maps_all = preferUseMapsAll();
  JoinResultPtr res;
  [[maybe_unused]] const bool joined =
      joinDispatch(kind, strictness, maps_vector, prefer_use_maps_all, [&](auto kind_, auto strictness_, auto &maps_vector_) {
        if constexpr (std::is_same_v<std::decay_t<decltype(maps_vector_)>, std::vector<const MapsAll *>>) {
          res = HashJoinMethods<kind_, strictness_, MapsAll>::joinBlockImpl(*this, std::move(block), sample_block_with_columns_to_add,
                                                                            maps_vector_);
        } else if constexpr (std::is_same_v<std::decay_t<decltype(maps_vector_)>, std::vector<const MapsOne *>>) {
...
HashJoin::~HashJoin() {
  ...
  LOG_TEST(log, "{}Join data is being destroyed, {} bytes and {} rows in hash table", instance_log_id, getTotalByteCount(),
           getTotalRowCount());
}

bool HashJoin::hasNonJoinedRows() {
  if (has_non_joined_rows_checked) return has_non_joined_rows;

  if (!isRightOrFull(kind)) return false;

  if (!needUsedFlagsForPerRightTableRow(table_join)) return false;
...
class NotJoinedHash final : public NotJoinedBlocks::RightColumnsFiller {
 public:
  ...
  NotJoinedHash(const HashJoin &parent_, UInt64 max_block_size_, bool flag_per_row_, size_t bucket_idx_, size_t num_buckets_)
    ...
    if (parent.data == nullptr) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot join after data has been released");
  }
 ...
 private:
  ...
  size_t fillColumnsFromData(const HashJoin::ScatteredColumnsList &columns, MutableColumns &columns_right) {
    if (!position.has_value()) position = std::make_any<HashJoin::ScatteredColumnsList::const_iterator>(columns.begin());
    ...
    for (; block_it != end; ++block_it) {
      size_t rows_from_block =
          std::min<size_t>(max_block_size - rows_added, block_it->columns_info.columns.at(0)->size() - current_block_start);
      for (size_t j = 0; j < columns_right.size(); ++j) {
...
#undef M
  ...
  size_t fillColumns(const Map &map, MutableColumns &columns_keys_and_right) {
    ...
    if (flag_per_row) {
      ...
      if (bucket_idx != 0) return rows_added;

      if (!used_position.has_value()) used_position = parent.data->columns.begin();
          ...
          if (!parent.isUsed(&mapped_block.columns_info.columns, row)) {
            for (size_t column = 0; column < columns_keys_and_right.size(); ++column) {
              if (const auto *replicated_column = mapped_block.columns_info.replicated_columns[column])
                columns_keys_and_right[column]->insertFrom(*replicated_column->getNestedColumn(),
      ...
      if (!position.has_value()) position = std::make_any<Iterator>(map.begin());
      ...
      if constexpr (requires {
        ...
        if (!skipToNextOwnedBucket()) return rows_added;
        ...
        while (it != end && rows_added < max_block_size) {
          ...
          if (it != end && !isBucketInRange(it.getBucket()) && !skipToNextOwnedBucket()) break;
        }
      } else {
        /// Single-level hash tables - no bucket filtering
        for (; it != end; ++it) {
          size_t offset = map.offsetInternal(it.getPtr());
          if (parent.isUsed(offset)) continue;
  ...
  void fillNullsFromBlocks(MutableColumns &columns_keys_and_right, size_t &rows_added) {
    ...
    if (!nulls_position.has_value()) nulls_position = parent.data->nullmaps.begin();
    ...
    for (auto &it = *nulls_position; it != end && rows_added < max_block_size; ++it) {
      ...
      if (it->column) nullmap = &assert_cast<const ColumnUInt8 &>(*it->column).getData();
        ...
        if (nullmap && (*nullmap)[row]) {
          for (size_t col = 0; col < columns_keys_and_right.size(); ++col) {
            if (const auto *replicated_column = columns->columns_info.replicated_columns[col])
              columns_keys_and_right[col]->insertFrom(*replicated_column->getNestedColumn(),
                                                      replicated_column->getIndexes().getIndexAt(row));
            else
...
IBlocksStreamPtr HashJoin::getNonJoinedBlocks(const Block &left_sample_block, const Block &result_sample_block,
                                              UInt64 max_block_size) const {
  return getNonJoinedBlocks(left_sample_block, result_sample_block, max_block_size, 0, 1);
}

IBlocksStreamPtr HashJoin::getNonJoinedBlocks(const Block &left_sample_block, const Block &result_sample_block, UInt64 max_block_size,
                                              size_t bucket_idx, size_t num_buckets) const {
  if (!JoinCommon::hasNonJoinedBlocks(*table_join)) return {};

  size_t left_columns_count = left_sample_block.columns();
  if (canRemoveColumnsFromLeftBlock()) left_columns_count = table_join->getOutputColumns(JoinTableSide::Left).size();
  ...
  if (!flag_per_row) {
    ...
    if (expected_columns_count != result_sample_block.columns()) {
      ...
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      ...
                      result_sample_block.columns(), expected_columns_count, result_sample_block.dumpNames(),
                      fmt::join(left_block_names, ", "), required_right_keys.dumpNames(), sample_block_with_columns_to_add.dumpNames());
    }
  }
...
void HashJoin::reuseJoinedData(const HashJoin &join) {
  ...
  bool flag_per_row = needUsedFlagsForPerRightTableRow(table_join);
  if (flag_per_row) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "StorageJoin with ORs is not supported");

  const bool prefer_use_maps_all = preferUseMapsAll();
  for (auto &map : data->maps) {
    joinDispatch(kind, strictness, map, prefer_use_maps_all, [this](auto kind_, auto strictness_, auto &map_) {
      used_flags->reinit<kind_, strictness_, std::is_same_v<std::decay_t<decltype(map_)>, MapsAll>>(map_.getBufferSizeInCells(data->type) +
                                                                                                    1);
    });
  }
}
...
BlocksList HashJoin::releaseJoinedBlocks(bool restructure [[maybe_unused]]) {
  LOG_TRACE(log, "{}Join data is being released, {} bytes and {} rows in hash table", instance_log_id, getTotalByteCount(),
            getTotalRowCount());

  auto extract_source_blocks = [](ScatteredColumnsList &&columns_list, const Block &sample_block) {
    BlocksList result;
    for (auto &columns : columns_list) result.emplace_back(sample_block.cloneWithColumns(columns.columns_info.columns));
    return result;
  };
...
void HashJoin::validateAdditionalFilterExpression(ExpressionActionsPtr additional_filter_expression) {
  if (!additional_filter_expression) return;
  ...
  if (expression_sample_block.columns() != 1) {
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    ...
                    expression_sample_block.dumpStructure(), additional_filter_expression->dumpActions());
  }
  ...
  if (!type->equals(*std::make_shared<DataTypeUInt8>())) {
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    ...
                    expression_sample_block.getByPosition(0).type->getName(), additional_filter_expression->dumpActions());
  }
...
bool HashJoin::isUsed(size_t off) const { return used_flags->getUsedSafe(off); }

bool HashJoin::isUsed(const Columns *columns_ptr, size_t row_idx) const { return used_flags->getUsedSafe(columns_ptr, row_idx); }
...
bool HashJoin::needUsedFlagsForPerRightTableRow(std::shared_ptr<TableJoin> table_join_) const {
  ...
  if (table_join_->kind() == JoinKind::Cross) return false;

  if (!table_join_->oneDisjunct()) return true;
  /// If it'a a all right join with inequal conditions, we need to mark each row
  if (table_join_->getMixedJoinExpression() && isRightOrFull(table_join_->kind())) return true;
  return false;
}
  ...
  if constexpr (!join_features.is_all_join || (!join_features.left && !join_features.inner))
    ...
    auto merge_rows_into_one_block = [&](ScatteredColumnsList &columns_list, RowRefList &rows_ref) {
      ...
      if (columns_list.empty() || columns_list.back().columns_info.columns.at(0)->size() >= DEFAULT_BLOCK_SIZE) {
        ...
        for (const auto &col : (*it)->columns_info->columns) columns.push_back(col->cloneEmpty());
...
size_t HashJoin::getAndSetRightTableKeys() const {
  ...
  if (data) data->keys_to_join = total_rows;
  return total_rows;
}

void HashJoin::tryRerangeRightTableData() {
  if (!rightTableCanBeReranged()) return;
  ...
  if (!data || data->sorted || data->columns.empty() || data->maps.size() != 1 ||
      data->rows_to_join > table_join->sortRightMaximumTableRows() || data->avgPerKeyRows() < table_join->sortRightMinimumPerkeyRows()) {
    return;
  }

  if (data->keys_to_join == 0) data->keys_to_join = getTotalRowCount();

  /// If the there is no columns to add, means no columns to output, then the rerange would not improve performance by using column's
  /// `insertRangeFrom` to replace column's `insertFrom` to make the output.
  ...
  [[maybe_unused]] bool result = joinDispatch(
      kind, strictness, data->maps.front(), preferUseMapsAll(),
...
void HashJoin::onBuildPhaseFinish() {
  ...
  LOG_TRACE(log, "{}Join data is built, {} and {} rows in hash table", instance_log_id, ReadableSize(getTotalByteCount()),
            getTotalRowCount());
}
}  // namespace DB
