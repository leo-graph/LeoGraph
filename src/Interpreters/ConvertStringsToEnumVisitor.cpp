...
String makeStringsEnum(const std::set<String> &values) {
  ...
  if (values.size() >= 255) enum_string = "Enum16(";

  size_t number = 1;
  for (const auto &item : values) {
    enum_string += "\'" + item + "\' = " + std::to_string(number++);

    if (number <= values.size()) enum_string += ", ";
  }
...
void changeTransformArguments(ASTPtr &array_to, ASTPtr &other) {
  ...
  for (const auto &item : array_to->as<ASTLiteral>()->value.safeGet<Array>()) values.insert(item.safeGet<String>());
...
bool checkSameType(const Array &array, const String &type) {
  for (const auto &item : array)
    if (item.getTypeName() != type) return false;
  return true;
}

}  // namespace

bool FindUsedFunctionsMatcher::needChildVisit(const ASTPtr &node, const ASTPtr &) { return !(node->as<ASTFunction>()); }

void FindUsedFunctionsMatcher::visit(const ASTPtr &ast, Data &data) {
  if (auto *func = ast->as<ASTFunction>()) visit(*func, data);
}
...
bool ConvertStringsToEnumMatcher::needChildVisit(const ASTPtr &node, const ASTPtr &) { return !(node->as<ASTFunction>()); }
...
void ConvertStringsToEnumMatcher::visit(ASTFunction &function_node, Data &data) {
  if (!function_node.arguments) return;

  /// We are not sure we could change the type of function result
  /// cause it is present in other function as argument
  if (data.used_functions.contains(function_node.tryGetAlias())) return;

  if (function_node.name == "if") {
    if (function_node.arguments->children.size() != 3) return;
    ...
    if (literal1->value.getTypeName() != "String" || literal2->value.getTypeName() != "String") return;

    changeIfArguments(function_node.arguments->children[1], function_node.arguments->children[2]);
  } else if (function_node.name == "transform") {
    if (function_node.arguments->children.size() != 4) return;
    ...
    if (!literal_to || !literal_other) return;

    if (literal_to->value.getTypeName() != "Array" || literal_other->value.getTypeName() != "String") return;

    Array array_to = literal_to->value.safeGet<Array>();
    if (array_to.empty()) return;

    bool to_strings = checkSameType(array_to, "String");
    if (!to_strings) return;
