...
EntryType fromXRayEntryType(XRayEntryType entry_type) {
  switch (entry_type) {
    ...
    default:
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Wrong XRay entry_type to convert: {}", entry_type);
  }
}
...
String InstrumentationManager::InstrumentedPointInfo::toString() const {
  ...
  for (size_t i = 0; i < parameters.size(); ++i) {
    ...
    if (i < parameters.size() - 1) parameters_str += ", ";
  }
  parameters_str += "]";

  return fmt::format("id {}, function_id {}, function_name '{}', handler_name {}, entry_type {}, symbol {}{}", id, function_id,
                     function_name, handler_name, entry_type_str, symbol, parameters_str);
}
...
static void handleXRayPatchingStatus(XRayPatchingStatus status, Int32 function_id, const String &method) {
  switch (status) {
    ...
    case XRayPatchingStatus::SUCCESS:
      [[fallthrough]];
...
void InstrumentationManager::patchFunctionIfNeeded(Int32 function_id) {
  if (instrumented_points.get<FunctionId>().contains(function_id)) return;
...
void InstrumentationManager::unpatchFunctionIfNeeded(Int32 function_id) {
  ...
  if (count == 0) throw Exception(ErrorCodes::LOGICAL_ERROR, "Function id {} to unpatch not previously patched", function_id);
...
bool InstrumentationManager::shouldPatchFunction(String function_to_patch, String full_qualified_function) {
  ...
  if (found_pos != std::string::npos) {
    ...
    LOG_INFO(logger, "Not instrumenting function '{}' because the match is within a template argument: '{}'", function_to_patch,
             full_qualified_function);
  }
  return false;
}

void InstrumentationManager::patchFunction(ContextPtr context, const String &function_name, const String &handler_name,
                                           Instrumentation::EntryType entry_type, const std::vector<InstrumentedParameter> &parameters) {
  ...
  if (fn_it != functions_container.get<FunctionName>().end()) {
    ...
    for (const auto &[id, function] : functions_container) {
      if (shouldPatchFunction(function_name, function)) functions_to_patch.emplace_back(id, function);
    }
  }

  if (functions_to_patch.empty())
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    "Unknown function to instrument: '{}'. XRay instruments by default only functions of at least 200 instructions. "
                    "You can change that threshold with '-fxray-instruction-threshold=1'. You can also force the instrumentation of "
                    "specific functions decorating them with '[[clang::xray_always_instrument]]' "
                    "and making sure they are not decorated with '[[clang::xray_never_instrument]]'",
                    function_name);
  ...
  for (const auto &[function_id, symbol] : functions_to_patch) {
    ...
    InstrumentedPointInfo info{context,   instrumented_point_ids, function_id, function_name, handler_name_lower, entry_type, symbol,
                               parameters};
...
void InstrumentationManager::unpatchFunction(std::variant<UInt64, Instrumentation::All, String> id) {
  ...
  if (std::holds_alternative<Instrumentation::All>(id)) {
    ...
    if (std::holds_alternative<String>(id)) {
      ...
      for (const auto &info : instrumented_points) {
        if (info.function_name == name) functions_to_unpatch.push_back(info);
      }
    ...
    if (functions_to_unpatch.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Not found any instrumentation point that matches");
...
InstrumentationManager::InstrumentedPoints InstrumentationManager::getInstrumentedPoints() const {
  ...
  for (const auto &info : instrumented_points) points.emplace_back(info);
...
void InstrumentationManager::dispatchHandlerImpl(Int32 func_id, XRayEntryType entry_type) {
  ...
  if (entry_type == XRayEntryType::TAIL) entry_type = XRayEntryType::EXIT;

  if (entry_type != XRayEntryType::ENTRY && entry_type != XRayEntryType::EXIT) return;
  ...
  for (const auto &info : func_ips) {
    ...
    try {
      ...
      if (handler_function == handler_name_to_function.end())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Handler {} not found when trying to run instrumentation point {}", info.handler_name,
                        info.toString());
...
void InstrumentationManager::parseInstrumentationMap() {
  ...
  LOG_DEBUG(logger, "Finished parsing the XRay instrumentation map: {} symbols parsed successfully, {} with errors",
            functions_container.size(), errors);
}

TraceLogElement InstrumentationManager::createTraceLogElement(const InstrumentedPointInfo &instrumented_point, XRayEntryType entry_type,
                                                              std::chrono::system_clock::time_point event_time) const {
...
void InstrumentationManager::sleep([[maybe_unused]] XRayEntryType entry_type, const InstrumentedPointInfo &instrumented_point) {
  ...
  if (params.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Missing parameters for sleep instrumentation");
  ...
  if (duration_ms < 0) throw DB::Exception(ErrorCodes::BAD_ARGUMENTS, "Sleep duration must be non-negative");

  LOG_TRACE(logger, "Sleep ({}, function_id {}): sleeping for {} ms", instrumented_point.function_name, instrumented_point.function_id,
            duration_ms);
  ...
  if (instrumented_point.context) {
    if (auto log = instrumented_point.context->getTraceLog()) log->add(std::move(element));
  }
}

void InstrumentationManager::log(XRayEntryType entry_type, const InstrumentedPointInfo &instrumented_point) {
  const auto &params = instrumented_point.parameters;
  if (params.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Missing parameters for log instrumentation");
  ...
  if (std::holds_alternative<String>(param)) {
    ...
    String stack_trace_str = StackTrace::toString(stack_trace.getFramePointers().data(), stack_trace.getOffset(),
                                                  stack_trace.getSize() - stack_trace.getOffset());
...
void InstrumentationManager::profile(XRayEntryType entry_type, const InstrumentedPointInfo &instrumented_point) {
  ...
  if (entry_type == XRayEntryType::ENTRY) {
    auto element = createTraceLogElement(instrumented_point, entry_type, std::chrono::system_clock::now());
    if (instrumented_point.context) {
      if (auto log = instrumented_point.context->getTraceLog()) log->add(element);
    }
    ...
    if (it != profile_elements.end()) {
      ...
      if (element.instrumented_point_id != instrumented_point.id) {
        LOG_TRACE(logger,
                  "Profile exit called for a different ID than the one set up. "
      ...
      if (it->second.empty()) profile_elements.erase(it);
    }
  }
}
