...
struct ITokenizer {
 public:
  ...
  virtual bool nextInString(const char *data, size_t length, size_t &__restrict pos, size_t &__restrict token_start,
  ...
  virtual void substringToBloomFilter(const char *data, size_t length, BloomFilter &bloom_filter, bool /*is_prefix*/,
  ...
  /// tokens depending on whether the substring is a prefix or a suffix.
  /// This method is inefficient and should be used only for constants.
  virtual void substringToTokens(const char *data, size_t length, std::vector<String> &tokens, bool /*is_prefix*/,
                                 bool /*is_suffix*/) const {
...
class ITokenizerHelper : public ITokenizer {
 ...
 private:
  std::unique_ptr<ITokenizer> clone() const override { return std::make_unique<Derived>(*static_cast<const Derived *>(this)); }
  ...
  void stringLikeToTokens(const char *data, size_t length, std::vector<String> &tokens) const override {
    ...
    while (cur < length && static_cast<const Derived *>(this)->nextInStringLike(data, length, cur, token)) tokens.push_back(token);
  }
...
struct NgramsTokenizer final : public ITokenizerHelper<NgramsTokenizer> {
  ...
  bool nextInString(const char *data, size_t length, size_t &__restrict pos, size_t &__restrict token_start,
...
#  else
      ...
      unsigned result_bitmask = _mm_movemask_epi8(_mm_or_si128(
          _mm_or_si128(_mm_or_si128(_mm_cmplt_epi8(haystack, zero),
...
#  endif
      ...
      while (pos < next_pos) {
        if (result_bitmask == 0) {
          ...
          if (pos > begin) {
            if (callback(begin, pos - begin)) return;
          }
...
struct SplitByStringTokenizer final : public ITokenizerHelper<SplitByStringTokenizer> {
  explicit SplitByStringTokenizer(const std::vector<String> &separators_)
      : ITokenizerHelper(Type::SplitByString), separators(separators_) {}
...
void forEachTokenImpl(const Tokenizer &tokenizer, const char *__restrict data, size_t length, Callback &&callback) {
  ...
  while (cur < length && tokenizer.nextInString(data, length, cur, token_start, token_len)) {
    if (callback(data + token_start, token_len)) return;
  }
}
...
void forEachToken(const ITokenizer &tokenizer, const char *__restrict data, size_t length, Callback &&callback) {
  ...
  switch (tokenizer.getType()) {
    ...
    case ITokenizer::Type::Ngrams: {
      ...
      if (length < ngrams_tokenizer.getN()) return;
