...
std::shared_ptr<TSystemLog> createSystemLog(ContextPtr context, const String &default_database_name, const String &default_table_name,
                                            const Poco::Util::AbstractConfiguration &config, const String &config_prefix,
                                            const String &comment) {
  if (!config.has(config_prefix)) {
    LOG_DEBUG(getLogger("SystemLog"), "Not creating {}.{} since corresponding section '{}' is missing from config", default_database_name,
              default_table_name, config_prefix);

    return {};
  }
  LOG_DEBUG(getLogger("SystemLog"), "Creating {}.{} from {}", default_database_name, default_table_name, config_prefix);
  ...
  if (config.has(config_prefix + ".engine")) {
    ...
    if (!partition_by.empty()) log_settings.engine += " PARTITION BY (" + partition_by + ")";

    /// TTL expr is not necessary.
    String ttl = config.getString(config_prefix + ".ttl", "");
    if (!ttl.empty()) log_settings.engine += " TTL " + ttl;
    ...
    if (!storage_policy.empty() || !settings.empty()) {
      ...
      if (!storage_policy.empty()) log_settings.engine += " storage_policy = " + quoteString(storage_policy);
      if (!settings.empty()) log_settings.engine += (storage_policy.empty() ? " " : ", ") + settings;
    }
  }
  ...
  log_settings.queue_settings.flush_interval_milliseconds =
      config.getUInt64(config_prefix + ".flush_interval_milliseconds", TSystemLog::getDefaultFlushIntervalMilliseconds());

  log_settings.queue_settings.max_size_rows = config.getUInt64(config_prefix + ".max_size_rows", TSystemLog::getDefaultMaxSize());

  if (log_settings.queue_settings.max_size_rows < 1)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "{0}.max_size_rows {1} should be 1 at least", config_prefix,
                    log_settings.queue_settings.max_size_rows);

  log_settings.queue_settings.reserved_size_rows =
      config.getUInt64(config_prefix + ".reserved_size_rows", TSystemLog::getDefaultReservedSize());

  if (log_settings.queue_settings.max_size_rows < log_settings.queue_settings.reserved_size_rows) {
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "{0}.max_size_rows {1} should be greater or equal to {0}.reserved_size_rows {2}",
                    config_prefix, log_settings.queue_settings.max_size_rows, log_settings.queue_settings.reserved_size_rows);
  }

  log_settings.queue_settings.buffer_size_rows_flush_threshold =
      config.getUInt64(config_prefix + ".buffer_size_rows_flush_threshold", log_settings.queue_settings.max_size_rows / 2);

  log_settings.queue_settings.notify_flush_on_crash =
      config.getBool(config_prefix + ".flush_on_crash", TSystemLog::shouldNotifyFlushOnCrash());

  if constexpr (std::is_same_v<TSystemLog, TraceLog>) log_settings.symbolize_traces = config.getBool(config_prefix + ".symbolize", true);
  ...
  if constexpr (std::is_same_v<TSystemLog, MetricLog>) {
    ...
    if (schema == "wide") return std::make_shared<TSystemLog>(context, log_settings);
...
#undef CREATE_PUBLIC_MEMBERS
  ...
  if (metric_log == nullptr && config.has("metric_log") &&
      (config.getString("metric_log.schema_type", "wide") == "transposed" ||
       config.getString("metric_log.schema_type", "wide") == "transposed_with_wide_view")) {
  ...
  try {
    for (auto &log : getAllLogs()) {
      log->startup();
      if (should_prepare || log->mustBePreparedAtStartup()) log->prepareTable();
    }
  } catch (...) {
    /// join threads
    shutdown();
  ...
  if (metric_log) {
    size_t collect_interval_milliseconds =
        config.getUInt64("metric_log.collect_interval_milliseconds", DEFAULT_METRIC_LOG_COLLECT_INTERVAL_MILLISECONDS);
  ...
  if (error_log) {
    size_t collect_interval_milliseconds =
        config.getUInt64("error_log.collect_interval_milliseconds", DEFAULT_ERROR_LOG_COLLECT_INTERVAL_MILLISECONDS);
...
#define GET_RAW_POINTERS(log_type, member, descr) (member).get(),

  std::vector<ISystemLog *> result = {LIST_OF_ALL_SYSTEM_LOGS(GET_RAW_POINTERS)
#if CLICKHOUSE_CLOUD
...
void SystemLogs::flushImpl(const std::vector<std::pair<String, String>> &names, bool should_prepare_tables_anyway, bool ignore_errors) {
  ...
  if (names.empty()) {
    if (text_log) BaseDaemon::instance().flushTextLogs();
...
#define GET_MAP_VALUES(log_type, member, descr)                \
  {getLowerCaseAndRemoveUnderscores(#member), (member).get()}, \
    ...
    std::unordered_map<String, ISystemLog *> logs_map{LIST_OF_ALL_SYSTEM_LOGS(GET_MAP_VALUES)
#if CLICKHOUSE_CLOUD
...
#undef GET_MAP_VALUES
    ...
    for (const auto &name : names) {
      ...
      if (it == logs_map.end()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Log name '{}' does not exist", name);
      ...
      if (log == text_log.get()) BaseDaemon::instance().flushTextLogs();
...
void SystemLogs::shutdown() {
  ...
  for (auto &log : logs) log->shutdown();
}

void SystemLogs::handleCrash() {
  auto logs = getAllLogs();
  for (auto &log : logs) log->handleCrash();
}

template <typename LogElement>
SystemLog<LogElement>::SystemLog(ContextPtr context_, const SystemLogSettings &settings_,
...
void SystemLog<LogElement>::shutdown() {
  ...
  if (table) table->flushAndShutdown();
}
...
void SystemLog<LogElement>::flushImpl(const std::vector<LogElement> &to_flush, uint64_t to_flush_end) {
  ...
  try {
    LOG_TRACE(log, "Flushing system log, {} entries to flush up to offset {}", to_flush.size(), to_flush_end);

    fiu_do_on(FailPoints::sleep_in_logs_flush, { sleepForSeconds(30); });
    ...
    for (auto &column : columns) column->reserve(to_flush.size());

    for (const auto &elem : to_flush) elem.appendToBlock(columns);
  ...
  if (total_time > 100)
    LOG_TRACE(log, "Flushed system log up to offset {}. Entries {}. Timings: Prepare={}ms, Block={}ms, Insert={}ms, Confirm={}ms",
              to_flush_end, flush_size, prepare_table_time, prepare_insert_data_to_block, execute_insert_time, confirm_time);
  else
...
void SystemLog<LogElement>::prepareTable() {
  ...
  if (table) {
    ...
    if (old_create_query != create_query) {
      ...
      int suffix = 0;
      while (DatabaseCatalog::instance().isTableExist({table_id.database_name, table_id.table_name + "_" + toString(suffix)}, getContext()))
        ++suffix;

      ASTRenameQuery::Element elem{
          ASTRenameQuery::Table{table_id.database_name.empty() ? nullptr : make_intrusive<ASTIdentifier>(table_id.database_name),
                                make_intrusive<ASTIdentifier>(table_id.table_name)},
          ASTRenameQuery::Table{table_id.database_name.empty() ? nullptr : make_intrusive<ASTIdentifier>(table_id.database_name),
                                make_intrusive<ASTIdentifier>(table_id.table_name + "_" + toString(suffix))}};

      LOG_DEBUG(log, "Existing table {} for system log has obsolete or different structure. Renaming it to {}.\nOld: {}\nNew: {}\n.",
                description, backQuoteIfNeed(elem.to.getTable()), old_create_query, create_query);
...
ASTPtr SystemLog<LogElement>::getCreateTableQuery() {
  ...
  ASTPtr storage_with_comment_ast =
      parseQuery(storage_parser, storage_def.data(), storage_def.data() + storage_def.size(),
  ...
  if (endsWith(engine.name, "MergeTree")) {
    ...
    auto add_index = [&](const char *definition) {
      ...
      ASTPtr ast = parseQuery(parser, definition, definition + strlen(definition), "index declaration for " + LogElement::name(), 0,
                              DBMS_DEFAULT_MAX_PARSER_DEPTH, DBMS_DEFAULT_MAX_PARSER_BACKTRACKS);
      indices->children.push_back(ast);
    };

    if (ordinary_columns.has("event_time")) add_index("event_time_index event_time TYPE minmax GRANULARITY 1");
    ...
    if (ordinary_columns.has("query_id")) add_index("query_id_index query_id TYPE bloom_filter(0.001) GRANULARITY 1");
    ...
    if (!indices->children.empty()) new_columns_list->set(new_columns_list->indices, indices);
  }
