...
static void executeCreateQuery(const String &query, ContextMutablePtr context, const String &database, const String &file_name, bool create,
                               bool has_force_restore_data_flag) {
  const Settings &settings = context->getSettingsRef();
  ParserCreateQuery parser;
  ASTPtr ast = parseQuery(parser, query.data(), query.data() + query.size(), "in file " + file_name, 0, settings[Setting::max_parser_depth],
...
static void loadDatabase(ContextMutablePtr context, const String &database, const fs::path &metadata_file_path, bool force_restore_data) {
  /// If it is already loaded.
  if (DatabaseCatalog::instance().isDatabaseExist(database)) return;
...
static void checkIncompleteOrdinaryToAtomicConversion(ContextPtr context, const std::map<String, fs::path> &databases) {
  ...
  for (const auto &[name, metadata_file] : databases) {
    if (!name.starts_with(ORDINARY_TO_ATOMIC_PREFIX)) continue;
    size_t last_dot = name.rfind('.');
    if (last_dot <= strlen(ORDINARY_TO_ATOMIC_PREFIX)) continue;
    ...
    throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                    ...
                    backQuote(name), backQuote(actual_name), backQuote(actual_name), backQuote(name), backQuote(actual_name),
                    backQuote(name), backQuote(actual_name));
  }
}
void dropRestoringDatabasesForTableDropping(ContextMutablePtr context, const std::unordered_set<String> &restoring_database_names) {
  for (const auto &restoring_database_name : restoring_database_names) {
    ...
    if (!CurrentThread::getGroup()) query_scope = CurrentThread::QueryScope::create(drop_context);
...
LoadTaskPtrs loadMetadata(ContextMutablePtr context, const String &default_database_name, bool async_load_databases) {
  ...
  for (const auto it = default_db_disk->iterateDirectory(metadata_dir_path); it->isValid(); it->next()) {
    ...
    if (sub_path.filename().empty()) sub_path = sub_path.parent_path();
    ...
    if (default_db_disk->existsDirectory(sub_path)) continue;

    if (sub_path.extension() == ".sql") {
      String db_name = sub_path.stem();
      orphan_directories_and_symlinks.erase(unescapeForFileName(db_name));
      if (!isSystemOrInformationSchema(db_name)) databases.emplace(unescapeForFileName(db_name), sub_path);
    }
...
static void loadSystemDatabaseImpl(ContextMutablePtr context, const String &database_name, const String &default_engine) {
  /// If it is already loaded.
  if (DatabaseCatalog::instance().isDatabaseExist(database_name)) return;
  ...
  LOG_TEST(getLogger("loadSystemDatabase"), "metadata_file_path {}, existsFile {}", metadata_file,
...
static void convertOrdinaryDatabaseToAtomic(LoggerPtr log, ContextMutablePtr context, const DatabasePtr &database, const String &name,
...

  {
    ...
    CurrentThread::QueryScope query_scope;
    if (!CurrentThread::getGroup()) query_scope = CurrentThread::QueryScope::create(create_database_query_context);
  ...
  for (auto iterator = database->getTablesIterator(context); iterator->isValid(); iterator->next()) {
    ...
    if (const auto *mv = dynamic_cast<const StorageMaterializedView *>(iterator->table().get())) {
      ...
      if (mv->hasInnerTable()) inner_mv_tables.emplace(mv->getTargetTable()->getStorageID().table_name);
    }
  }
  ...
  for (auto iterator = database->getTablesIterator(context); iterator->isValid(); iterator->next()) {
    ...
    CurrentThread::QueryScope query_scope;
    if (!CurrentThread::getGroup()) query_scope = CurrentThread::QueryScope::create(move_table_query_context);
  ...
  if (!database->empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Database {} is not empty after moving tables", name_quoted);

  {
    ...
    CurrentThread::QueryScope query_scope;
    if (!CurrentThread::getGroup()) query_scope = CurrentThread::QueryScope::create(drop_query_context);
  ...
  CurrentThread::QueryScope query_scope;
  if (!CurrentThread::getGroup()) query_scope = CurrentThread::QueryScope::create(rename_query_context);
...
static void maybeConvertOrdinaryDatabaseToAtomic(ContextMutablePtr context, const String &database_name,
                                                 const LoadTaskPtrs &load_system_metadata_tasks = {}) {
  LoggerPtr log = getLogger("loadMetadata");
  ...
  if (database->getEngineName() != "Ordinary") return;
  ...
  if (!permanently_detached_tables.empty()) {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                    "Cannot automatically convert database {} from Ordinary to Atomic, "
  ...
  try {
    ...
    static const auto actions_to_stop = {ActionLocks::PartsMerge, ActionLocks::PartsFetch, ActionLocks::PartsSend,
                                         ActionLocks::DistributedSend};
    ...
    CurrentThread::QueryScope query_scope;
    if (!CurrentThread::getGroup()) query_scope = CurrentThread::QueryScope::create(detach_query_context);
    ...
    for (const auto &uuid : tables_uuids) DatabaseCatalog::instance().removeUUIDMappingFinally(uuid);
    ...
    e.addMessage(
        "Exception while trying to convert database {} from Ordinary to Atomic. It may be in some intermediate state."
...
void maybeConvertSystemDatabase(ContextMutablePtr context, LoadTaskPtrs &load_system_metadata_tasks) {
  ...
  if (context->getSettingsRef()[Setting::allow_deprecated_database_ordinary]) return;
...
void convertDatabasesEnginesIfNeed(const LoadTaskPtrs &load_metadata, ContextMutablePtr context) {
  ...
  LOG_INFO(getLogger("loadMetadata"),
  ...
  for (const auto &[name, _] : DatabaseCatalog::instance().getDatabases(GetDatabasesOptions{.with_datalake_catalogs = false}))
    if (name != DatabaseCatalog::SYSTEM_DATABASE) maybeConvertOrdinaryDatabaseToAtomic(context, name);
...
LoadTaskPtrs loadMetadataSystem(ContextMutablePtr context, bool async_load_system_database) {
  ...
  TablesLoader::Databases databases = {
      ...
      {DatabaseCatalog::INFORMATION_SCHEMA_UPPERCASE,
       DatabaseCatalog::instance().getDatabase(DatabaseCatalog::INFORMATION_SCHEMA_UPPERCASE)},
  };
  TablesLoader loader{context, databases, LoadingStrictnessLevel::FORCE_RESTORE};
