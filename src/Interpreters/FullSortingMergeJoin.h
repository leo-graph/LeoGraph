...
class FullSortingMergeJoin : public IJoin {
 public:
  explicit FullSortingMergeJoin(std::shared_ptr<TableJoin> table_join_, SharedHeader& right_sample_block_, int null_direction_ = 1)
      : table_join(table_join_), right_sample_block(right_sample_block_), null_direction(null_direction_) {
  ...
  static bool isSupported(const std::shared_ptr<TableJoin>& table_join) {
    if (!table_join->oneDisjunct()) return false;
  ...
  void checkTypesOfKeys(const Block& left_block) const override {
    if (!isSupported(table_join)) throw DB::Exception(ErrorCodes::NOT_IMPLEMENTED, "FullSortingMergeJoin doesn't support specified query");
    ...
    for (size_t i = 0; i < onexpr.key_names_left.size(); ++i) {
      ...
      if (!type_equals) {
        throw DB::Exception(ErrorCodes::TYPE_MISMATCH, "Type mismatch of columns to JOIN by: {} :: {} at left, {} :: {} at right",
                            onexpr.key_names_left[i], left_type->getName(), onexpr.key_names_right[i], right_type->getName());
      }
    }
  }
  ...
  JoinResultPtr joinBlock(Block block) override {
    for (const auto& col : *right_sample_block) block.insert(col);
