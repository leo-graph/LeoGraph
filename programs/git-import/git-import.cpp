...
static void writeText(FileChangeType type, WriteBuffer& out) {
  switch (type) {
    ...
    case FileChangeType::Delete:
      writeString("Delete", out);
      break;
    case FileChangeType::Modify:
      writeString("Modify", out);
      break;
    case FileChangeType::Rename:
      writeString("Rename", out);
      break;
    case FileChangeType::Copy:
      writeString("Copy", out);
      break;
    case FileChangeType::Type:
...
static void writeText(LineType type, WriteBuffer& out) {
  switch (type) {
    case LineType::Empty:
      writeString("Empty", out);
      break;
    case LineType::Comment:
      writeString("Comment", out);
      break;
    case LineType::Punct:
      writeString("Punct", out);
      break;
    case LineType::Code:
...
struct LineChange {
  ...
  void setLineInfo(std::string full_line) {
    ...
    if (pos == end) {
      ...
      if (pos == end) line_type = LineType::Punct;
    }
  }
...
struct FileBlame {
  ...
  FileBlame(const FileBlame& rhs) { *this = rhs; }
  ...
  const Commit* find(uint32_t num) {
    ...
    if (current_idx == num && it != lines.end()) return &*it;
    return {};
  }
  ...
  void removeLine(uint32_t num) {
    ...
    if (current_idx == num && it != lines.end()) it = lines.erase(it);
  }
};
...
static void updateSnapshot(Snapshot& snapshot, const Commit& commit, CommitDiff& file_changes) {
  ...
  for (auto& elem : file_changes) {
    ...
    if (!file.old_path.empty() && file.path != file.old_path) snapshot[file.path] = snapshot[file.old_path];
  }
  ...
  for (auto& elem : file_changes) {
    ...
    for (auto& line_change : elem.second.line_changes) {
      if (line_change.sign == -1) {
        if (const Commit* prev_commit = file_snapshot.find(line_change.line_number_old); prev_commit && prev_commit->time <= commit.time) {
        ...
        uint32_t this_line_in_prev_commit =
            line_change.hunk_start_line_number_old + (line_change.line_number_new - line_change.hunk_start_line_number_new);
 ...
 */
static void processFileChanges(ReadBuffer& in, const Options& options, Commit& commit, CommitDiff& file_changes) {
  while (checkChar(':', in)) {
    FileChange file_change;
    ...
    if (!(options.skip_paths && re2::RE2::PartialMatch(file_change.path, *options.skip_paths))) {
      file_changes.emplace(file_change.path, FileDiff(file_change));
    }
  }
}
 ...
 */
static void processDiffs(ReadBuffer& in, std::optional<size_t> size_limit, Commit& commit, CommitDiff& file_changes) {
  ...
  while (!in.eof()) {
    if (checkString("@@ ", in)) {
      if (!file_change_and_line_changes) {
        ...
        if (file_changes.end() != it) file_change_and_line_changes = &it->second;
      }
      ...
      if (file_change_and_line_changes) {
        ...
        if (checkChar(',', in)) readText(old_lines, in);

        assertString(" +", in);
        readText(line_change.hunk_start_line_number_new, in);
        if (checkChar(',', in)) readText(new_lines, in);
        ...
        if (line_change.hunk_start_line_number_new == 0) line_change.hunk_start_line_number_new = 1;
    ...
    if (size_limit && diff_size > *size_limit) return;
  }
}

/** Process the "git show" result for a single commit. Append the result to tables.
 */
static void processCommit(ReadBuffer& in, const Options& options, size_t commit_num, size_t total_commits, std::string hash,
                          Snapshot& snapshot, DiffHashes& diff_hashes, ResultWriter& result) {
  Commit commit;
  commit.hash = hash;
  ...
  if (options.skip_commits_with_messages && re2::RE2::PartialMatch(commit.message, *options.skip_commits_with_messages)) return;
  ...
  if (options.diff_size_limit && commit_num != 0 && commit.lines_added + commit.lines_deleted > *options.diff_size_limit) return;

  /// Calculate hash of diff and skip duplicates
  if (options.skip_commits_with_duplicate_diffs && !diff_hashes.insert(diffHash(file_changes)).second) return;
...
static auto gitShow(const std::string& hash) {
  std::string command = fmt::format("git show --raw --pretty='format:%ct%x00%aN%x00%P%x00%s%x00' --patch --unified=0 {}", hash);
...
static void processLog(const Options& options) {
  ...
  while (!in.eof()) {
    ...
    if (!options.skip_commits.contains(hash)) hashes.emplace_back(std::move(hash));
  }
  ...
  if (num_threads == 0) throw Exception(ErrorCodes::INCORRECT_DATA, "num-threads cannot be zero");

  std::vector<std::unique_ptr<ShellCommand>> show_commands(num_threads);
  for (size_t i = 0; i < num_commits && i < num_threads; ++i) show_commands[i] = gitShow(hashes[i]);
  ...
  for (size_t i = 0; i < num_commits; ++i) {
    ...
    if (!options.stop_after_commit.empty() && hashes[i] == options.stop_after_commit) break;

    if (i + num_threads < num_commits) show_commands[i % num_threads] = gitShow(hashes[i + num_threads]);
  }
...
int mainEntryClickHouseGitImport(int argc, char** argv) try {
  using namespace DB;

  po::options_description desc("Allowed options", getTerminalWidth());
  desc.add_options()("help,h", "produce help message")("skip-commits-without-parents", po::value<bool>()->default_value(true),
      ...
      "skip-commit", po::value<std::vector<std::string>>(), "Skip commit with specified hash. The option can be specified multiple times.")(
      "skip-paths", po::value<std::string>(), "Skip paths that matches regular expression (re2 syntax).")(
      "skip-commits-with-messages", po::value<std::string>(), "Skip commits whose messages matches regular expression (re2 syntax).")(
      "diff-size-limit", po::value<size_t>()->default_value(100000),
      "Skip commits whose diff size (number of added + removed lines) is larger than specified threshold. Does not apply for initial "
      "commit.")("stop-after-commit", po::value<std::string>(), "Stop processing after specified commit hash.")(
      "threads", po::value<size_t>()->default_value(std::thread::hardware_concurrency()), "Number of concurrent git subprocesses to spawn");
  ...
  if (options.contains("help")) {
    std::cout << documentation << '\n'
              ...
              << desc << '\n'
              << "\nExample:\n"
              << "\nclickhouse git-import --skip-paths "
                 "'generated\\.cpp|^(contrib|docs?|website|libs/"
                 "(libcityhash|liblz4|libdivide|libvectorclass|libdouble-conversion|libcpuid|libzstd|libfarmhash|libmetrohash|libpoco|"
                 "libwidechar_width))/' --skip-commits-with-messages '^Merge branch '\n";
    return 1;
  }
