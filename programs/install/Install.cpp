...
static bool filesEqual(std::string path1, std::string path2) {
  ...
  return in1.buffer().size() == in2.buffer().size() && 0 == memcmp(in1.buffer().begin(), in2.buffer().begin(), in1.buffer().size());
}
...
#elif defined(OS_FREEBSD)
    std::string command = group_name.empty() ? fmt::format("pw useradd -s /bin/false -d /nonexistent -n {}", user_name)
...
int mainEntryClickHouseInstall(int argc, char **argv) {
  try {
    ...
    desc.add_options()("help,h", "produce help message")("prefix", po::value<std::string>()->default_value("/"), "prefix for all paths")
#if defined(OS_DARWIN)
        ...
        ("binary-path", po::value<std::string>()->default_value("usr/bin"), "where to install binaries")
            ...
            ("config-path", po::value<std::string>()->default_value("etc/clickhouse-server"), "where to install configs")(
                ...
                "noninteractive,y", "run non-interactively")("link", "create symlink to the binary instead of copying to binary-path");
    ...
    if (path_length <= 1) throw Exception(ErrorCodes::FILE_DOESNT_EXIST, "Cannot obtain path to the binary");

    std::string path(path_length, std::string::value_type());
    auto res = _NSGetExecutablePath(&path[0], &path_length);
    if (res != 0) throw Exception(ErrorCodes::FILE_DOESNT_EXIST, "Cannot obtain path to the binary");
    ...
    if (options.contains("link")) {
      if (old_binary_exists) {
        ...
        fs::path points_to;
        if (is_symlink) points_to = fs::weakly_canonical(FS::readSymlink(main_bin_path));
        ...
        if (is_symlink && points_to == binary_self_canonical_path) {
          ...
          if (!is_symlink) {
            ...
            fmt::print("Symlink {} already exists but it points to {}. Will replace the old symlink to {}.\n", main_bin_path.string(),
                       points_to.string(), binary_self_canonical_path.string());
        ...
        /// Check if binary has the same content.
          ...
          if (filesEqual(binary_self_path.string(), main_bin_path.string())) {
            ...
            fmt::print("ClickHouse binary is already located at {} and it has the same content as {}\n", main_bin_path.string(),
                       binary_self_canonical_path.string());
          }
        }
      }
    ...
    for (const auto &tool : tools) {
      ...
      if (fs::exists(symlink_path)) {
        ...
        if (!tool.starts_with("clickhouse")) {
          ...
          fs::path points_to;
          if (is_symlink) points_to = fs::weakly_canonical(FS::readSymlink(symlink_path));
          ...
          if (is_symlink && (points_to == main_bin_path || (options.contains("link") && points_to == binary_self_canonical_path))) {
            ...
            if (!is_symlink) {
              ...
              fmt::print("Symlink {} already exists but it points to {}. Will replace the old symlink to {}.\n", symlink_path.string(),
                         points_to.string(), main_bin_path.string());
    ...
    if (log_path.is_relative()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Log path is relative: {}", log_path.string());
    if (data_path.is_relative()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Data path is relative: {}", data_path.string());
    if (pid_path.is_relative()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Pid path is relative: {}", pid_path.string());
    ...
    if (fs::exists(odbc_bridge_path) || fs::exists(library_bridge_path)) {
      ...
      if (fs::exists(odbc_bridge_path)) changeOwnership(odbc_bridge_path, DEFAULT_CLICKHOUSE_BRIDGE_USER, DEFAULT_CLICKHOUSE_BRIDGE_GROUP);
    ...
    bool noninteractive =
        (debian_frontend_var && debian_frontend_var == std::string_view("noninteractive")) || options.contains("noninteractive");
    ...
    if (has_password_for_default_user) {
      fmt::print("{}Password for the default user is already specified. To remind or reset, see {} and {}.{}\n", start_hilite,
                 users_config_file.string(), users_d.string(), end_hilite);
      ...
      char buf[1000] = {};
      std::string password;
      if (auto *result = readpassphrase("Set up the password for the default user: ", buf, sizeof(buf), 0)) password = result;
      ...
      if (!password.empty()) {
        ...
        std::string hash_hex;
        hash_hex.resize(64);
        for (size_t i = 0; i < 32; ++i) writeHexByteLowercase(hash[i], &hash_hex[2 * i]);
        out << "<clickhouse>\n"
               "    <users>\n"
               "        <default>\n"
               "            <password remove='1' />\n"
    ...
    if (fs::exists(config_d)) fs::permissions(config_d, fs::perms::owner_read | fs::perms::owner_exec, fs::perm_options::replace);
    if (fs::exists(users_d)) fs::permissions(users_d, fs::perms::owner_read | fs::perms::owner_exec, fs::perm_options::replace);

    /// Readonly.
    if (fs::exists(main_config_file)) fs::permissions(main_config_file, fs::perms::owner_read, fs::perm_options::replace);
    if (fs::exists(users_config_file)) fs::permissions(users_config_file, fs::perms::owner_read, fs::perm_options::replace);

    std::string maybe_password;
    if (has_password_for_default_user) maybe_password = " --password";
    ...
    if (fs::exists(pid_file)) {
      fmt::print(
          ...
          " clickhouse-client{}\n\n",
          formatWithSudo("clickhouse restart"), maybe_password);
    } else {
      fmt::print(
          "\nClickHouse has been successfully installed.\n"
          ...
          "\nStart clickhouse-client with:\n"
          " clickhouse-client{}\n\n",
          formatWithSudo("clickhouse start"), maybe_password);
    }
...
int start(const std::string &user, const fs::path &executable, const fs::path &config, const fs::path &pid_file, unsigned max_tries) {
  ...
  std::string command = fmt::format("{} --config-file {} --pid-file {} --daemon", executable.string(), config.string(), pid_file.string());
  ...
  if (try_num == max_tries) {
...

    {
      ...
      if (configuration->has("logger.log")) log_path = fs::path(configuration->getString("logger.log")).remove_filename();
    }
...
int isRunning(const fs::path &pid_file, bool ignore_file_does_not_exist) {
  ...
  try {
    ...
    if (!ignore_file_does_not_exist || e.code() != ErrorCodes::FILE_DOESNT_EXIST) throw;
  }
...
int stop(const fs::path &pid_file, bool force, bool do_not_kill, unsigned max_tries) {
  if (force && do_not_kill) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Specified flags are incompatible");

  int signal = force ? SIGKILL : SIGTERM;
  const char *signal_name = force ? "kill" : "terminate";

  if (sendSignalAndWaitForStop(pid_file, signal, max_tries, 1000, signal_name)) return 0;
  ...
  if (sendSignalAndWaitForStop(pid_file, signal, std::min(10U, max_tries), 1000, signal_name)) return 0;
  ...
  if (sendSignalAndWaitForStop(pid_file, SIGKILL, num_kill_check_tries, kill_check_delay_ms, "kill")) return 0;

  if (!isRunning(pid_file, /*ignore_file_does_not_exist=*/true)) return 0;

  throw Exception(ErrorCodes::CANNOT_KILL, "The server process still exists after {} tries (delay: {} ms)", num_kill_check_tries,
                  kill_check_delay_ms);
}
}  // namespace
...
int mainEntryClickHouseStop(int argc, char **argv) {
  try {
    ...
    desc.add_options()("help,h", "produce help message")("prefix", po::value<std::string>()->default_value("/"), "prefix for all paths")(
        "pid-path", po::value<std::string>()->default_value("var/run/clickhouse-server"), "directory for pid file")(
        "force", po::bool_switch(), "Stop with KILL signal instead of TERM")("do-not-kill", po::bool_switch(),
                                                                             "Do not send KILL even if TERM did not help")(
        "max-tries", po::value<unsigned>()->default_value(60),
        "Max number of tries for waiting the server to finish after sending TERM (with 1 second delay)");
...
int mainEntryClickHouseRestart(int argc, char **argv) {
  try {
    ...
    if (int res = stop(pid_file, force, do_not_kill, max_tries)) return res;
    return start(user, executable, config, pid_file, max_tries);
  } catch (...) {
    std::cerr << getCurrentExceptionMessage(false) << '\n';
    return getCurrentExceptionCode();
