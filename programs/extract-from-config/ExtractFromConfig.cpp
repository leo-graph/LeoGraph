...
static std::vector<std::string> extactFromConfigAccordingToGlobs(DB::ConfigurationPtr configuration, const std::string& pattern,
                                                                 bool ignore_errors) {
  ...
  while (!working_queue.empty()) {
    ...
    /// This is a leave
    if (keys.empty()) {
      if (!re2::RE2::FullMatch(node, *matcher)) continue;
    ...
    for (const auto& key : keys) working_queue.emplace(fmt::format("{}.{}", node, key));
  }
...
static std::vector<std::string> extractFromConfig(const std::string& config_path, const std::string& key, bool process_zk_includes,
                                                  bool ignore_errors, bool get_users) {
  ...
  if (get_users) {
    ...
    if (!has_user_directories && !ignore_errors) throw DB::Exception(DB::ErrorCodes::CANNOT_LOAD_CONFIG, "Can't load config for users");
  ...
  if (key.find_first_of("*?{") != std::string::npos) return extactFromConfigAccordingToGlobs(configuration, key, ignore_errors);

  /// Do not throw exception if not found.
  if (ignore_errors) return {configuration->getString(key, "")};
...
int mainEntryClickHouseExtractFromConfig(int argc, char** argv) {
  ...
  options_desc.add_options()("help", "produce this help message")("stacktrace", po::bool_switch(&print_stacktrace),
      ...
      "try", po::bool_switch(&ignore_errors),
      "Do not warn about missing keys, missing users configurations or non existing file from include_from tag")(
  ...
  try {
    ...
    if (options.contains("help")) {
      std::cerr << "Preprocess config file and extract value of the given key." << std::endl << std::endl;
      std::cerr << "Usage: clickhouse extract-from-config [options]" << std::endl << std::endl;
