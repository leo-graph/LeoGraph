...
int mainEntryClickHouseZooKeeperDumpTree(int argc, char** argv) {
  try {
    ...
    desc.add_options()("help,h", "produce help message")(
        ...
        "path,p", boost::program_options::value<std::string>()->default_value("/"), "where to start")("ctime,c", "print node ctime");
    ...
    auto ensure_session = [&] {
      if (zookeeper->expired()) {
        if (num_reconnects == max_reconnects) return false;
        ++num_reconnects;
    ...
    for (auto it = list_futures.begin(); it != list_futures.end(); ++it) {
      ...
      if (dump_ctime) std::cout << '\t' << response.stat.ctime;
      std::cout << '\n';
