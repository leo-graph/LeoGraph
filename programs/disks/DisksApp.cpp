...
CommandPtr DisksApp::getCommandByName(const String &command) const {
  try {
    if (auto it = aliases.find(command); it != aliases.end()) return command_descriptions.at(it->second);
...
std::vector<String> DisksApp::getCompletions(const String &prefix) const {
  ...
  std::vector<String> answer = {};
  if (command->command_name == "help") return getCommandsToComplete(last_token);

  answer = [&]() -> std::vector<String> {
    if (multidisk_commands.contains(command->command_name)) return client->getAllFilesByPrefixFromInitializedDisks(last_token, true);
...
void DisksApp::runInteractiveReplxx() {
  ...
  while (true) {
    ...
    String prompt =
        "\x1b[1;34m" + disk_with_path.getDisk()->getName() + "\x1b[0m:" + "\x1b[1;31m" + disk_with_path.getCurrentPath() + "\x1b[0m$ ";
...
void DisksApp::parseAndCheckOptions(const std::vector<String> &arguments, const ProgramOptionsDescription &options_description,
                                    PositionalProgramOptionsDescription &positional_options_description, CommandLineOptions &options) {
...
void DisksApp::addOptions() {
  options_description.add_options()("help,h", "Print common help message")("config-file,C", po::value<String>(), "Set config file")(
      "disk", po::value<String>(), "Set disk name")("save-logs", "Save logs to a file")("log-level", po::value<String>(), "Logging level")(
      "query,q", po::value<String>(), "Query for a non-interactive mode")("test-mode", "Interactive interface in test regime");
...
void DisksApp::processOptions() {
  if (options.contains("config-file")) config().setString("config-file", options["config-file"].as<String>());
  if (options.contains("disk")) config().setString("disk", options["disk"].as<String>());
  if (options.contains("save-logs")) config().setBool("save-logs", true);
  if (options.contains("log-level")) config().setString("log-level", options["log-level"].as<String>());
  if (options.contains("test-mode")) config().setBool("test-mode", true);
  if (options.contains("query")) query = std::optional{options["query"].as<String>()};
}
...
void DisksApp::printCommandHelpMessage(String command_name) const { printCommandHelpMessage(getCommandByName(command_name)); }
...
String DisksApp::getDefaultConfigFileName() { return "/etc/clickhouse-server/config.xml"; }
...
int DisksApp::main(const std::vector<String> & /*args*/) {
  ...
  if (config().has("macros")) global_context->setMacros(std::make_unique<Macros>(config(), "macros", &logger()));
...
DisksApp::~DisksApp() {
  ...
  if (global_context) global_context->shutdown();
