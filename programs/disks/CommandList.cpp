 ...
 private:
  static void list(const DiskWithPath& disk, const std::string& path, bool show_hidden) {
    ...
    for (const auto& file_name : file_names)
      if (show_hidden || (!file_name.starts_with('.'))) selected_and_sorted_file_names.push_back(file_name);
  ...
  static void listRecursive(const DiskWithPath& disk, const std::string& relative_path, bool show_hidden) {
    ...
    for (const auto& file_name : selected_and_sorted_file_names) {
      auto path = [&]() -> String {
        if (relative_path.ends_with("/")) return relative_path + file_name;
...
CommandPtr makeCommandList() { return std::make_shared<DB::CommandList>(); }
}  // namespace DB
