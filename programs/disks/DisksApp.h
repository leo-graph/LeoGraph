...
class DisksApp : public Poco::Util::Application {
 ...
 private:
  ...
  static void parseAndCheckOptions(const std::vector<String>& arguments, const ProgramOptionsDescription& options_description,
  ...
  String history_file;
  UInt32 history_max_entries = 0;  /// Maximum number of entries in the history file. Needs to be initialized to 0 since we don't have a
                                   /// proper constructor. Worry not, actual value is set within the initializeHistoryFile method.
  ...
  const std::unordered_map<String, String> aliases = {{"cp", "copy"},
                                                      {"mv", "move"},
                                                      {"ls", "list"},
