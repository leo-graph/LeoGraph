...
class CommandHelp final : public ICommand {
 public:
  explicit CommandHelp(const DisksApp& disks_app_) : ICommand("CommandHelp"), disks_app(disks_app_) {
    ...
    options_description.add_options()("command", po::value<String>(),
                                      "A command to help with (optional, positional), if not specified, help lists all the commands");
...
CommandPtr makeCommandHelp(const DisksApp& disks_app) { return std::make_shared<DB::CommandHelp>(disks_app); }
