    ...
    if (disk.getDisk()->existsFile(path_from)) {
      ...
      if (!disk.getDisk()->existsDirectory(target_location)) {
        ...
        if (disk.getDisk()->existsFile(target_location)) {
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "cannot overwrite non-directory '{}' with directory '{}'", target_location, path_from);
        }
      ...
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "cannot stat '{}' on disk: '{}': No such file or directory", path_from,
                      disk.getDisk()->getName());
    }
  }
};

CommandPtr makeCommandMove() { return std::make_shared<DB::CommandMove>(); }
}  // namespace DB
