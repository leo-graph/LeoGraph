...
DiskWithPath::DiskWithPath(DiskPtr disk_, std::optional<String> path_) : disk(disk_) {
  if (path_.has_value()) {
    ...
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Initializing path {} (normalized path: {}) at disk {} is not a directory", path,
                    relative_path, disk->getName());
  } else {
    path = String{"/"};
  }
...
String DiskWithPath::validatePathAndGetAsRelative(const String& path) {
  ...
  if (lexically_normal_path.contains("..")) throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Path {} is not normalized", path);

  /// If path is absolute we should keep it as relative inside disk, so disk will look like
  /// an ordinary filesystem with root.
  if (fs::path(lexically_normal_path).is_absolute()) return lexically_normal_path.substr(1);
...
DisksClient::DisksClient(const Poco::Util::AbstractConfiguration& config_, ContextPtr context_)
  ...
  for (const auto& disk_name : keys) {
    ...
    uninitialized_disks.emplace(disk_name, std::pair{[disk_name, disk_config_prefix, &factory, this]() {
                                                       return factory.create(disk_name, config, disk_config_prefix, this->context,
                                                                             initialized_disks,
                                                                             /*attach*/ false,
                                                                             /*custom_disk*/ false, {"cache", "encrypted"});
                                                     },
                                                     std::nullopt});
  }
  if (!has_default_disk) {
    uninitialized_disks.emplace(DEFAULT_DISK_NAME, std::pair{[this, config_prefix]() -> DiskPtr {
                                                               return std::make_shared<DiskLocal>(
                                                                   DEFAULT_DISK_NAME, this->context->getPath(), 0, this->context,
                                                                   this->config, config_prefix);
                                                             },
                                                             std::nullopt});
  }

  if (!has_local_disk) {
    /// We mount the local disk at the root of the file system so it can be used as a local filesystem within the Disks app. Please note
    /// that the initial path matches the path from which you launch the Disks app, unless it is explicitly specified in the switch-disk
    /// command.
    uninitialized_disks.emplace(LOCAL_DISK_NAME, std::pair{[this, config_prefix]() {
                                                             return std::make_shared<DiskLocal>(LOCAL_DISK_NAME, "/", 0, this->context,
                                                                                                this->config, config_prefix);
                                                           },
...
std::vector<String> DisksClient::getAllDiskNames() const {
  ...
  answer.insert(answer.end(), std::make_move_iterator(uninitialized_disks_names.begin()),
                std::make_move_iterator(uninitialized_disks_names.end()));
  return answer;
}
