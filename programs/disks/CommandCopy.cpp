...
class CommandCopy final : public ICommand {
 public:
  explicit CommandCopy() : ICommand("CommandCopy") {
    ...
    options_description.add_options()("disk-from", po::value<String>(),
                                      "disk from which we copy is executed (default value is a current disk)")(
  ...
  void executeImpl(const CommandLineOptions& options, DisksClient& client) override {
    ...
    if (disk_from.getDisk()->existsFile(path_from)) {
      ...
      if (!disk_to.getDisk()->existsDirectory(target_location)) {
        LOG_INFO(log, "Copying file from disk '{}', file path '{}' to disk '{}', file path '{}'", disk_from.getDisk()->getName(), path_from,
                 disk_to.getDisk()->getName(), target_location);
        disk_from.getDisk()->copyFile(path_from, *disk_to.getDisk(), target_location,
                                      /* read_settings= */ {},
                                      /* write_settings= */ {},
                                      /* cancellation_hook= */ {});
      } else {
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "cannot overwrite directory '{}' with non-directory '{}'", target_location, path_from);
      }
      ...
      LOG_INFO(log, "Copying directory content from disk '{}', directory path '{}' to disk '{}', directory path '{}'",
               disk_from.getDisk()->getName(), path_from, disk_to.getDisk()->getName(), target_location);

      disk_from.getDisk()->copyDirectoryContent(path_from, disk_to.getDisk(), target_location,
                                                /* read_settings= */ {},
                                                /* write_settings= */ {},
                                                /* cancellation_hook= */ {});
    } else {
      throw Exception(ErrorCodes::FILE_DOESNT_EXIST, "cannot stat '{}' on disk '{}': No such file or directory", path_from,
                      disk_from.getDisk()->getName());
    }
  }
};

CommandPtr makeCommandCopy() { return std::make_shared<DB::CommandCopy>(); }
