...
class DiskWithPath {
 public:
  ...
  /// If ignore_exception is true, then the function will not throw an exception but can return incomplete result (though all returned paths
  /// are valid). This is useful for autocomplete which should not fail if there are any problems with disks.
