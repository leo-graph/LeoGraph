...
class CommandWrite final : public ICommand {
 public:
  CommandWrite() : ICommand("CommandWrite") {
    ...
    options_description.add_options()("path-from", po::value<String>(),
  ...
  void executeImpl(const CommandLineOptions& options, DisksClient& client) override {
    ...
    if (write_mode_param.has_value()) {
      if (*write_mode_param == "rewrite")
        ...
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "invalid `mode`, expected `rewrite` or `append`, actual '{}'", *write_mode_param);
    }

    auto in = [&]() -> std::unique_ptr<ReadBufferFromFileBase> {
      if (!path_from.has_value()) return std::make_unique<ReadBufferFromFileDescriptor>(STDIN_FILENO);
    ...
    LOG_INFO(log, "Writing file from '{}' to '{}' with mode '{}' at disk '{}'", path_from.value_or("stdin"), path_to, write_mode,
...
CommandPtr makeCommandWrite() { return std::make_shared<DB::CommandWrite>(); }
