...
int mainEntryClickHouseCompressor(int argc, char** argv) {
  ...
  try {
    ...
    desc.add_options()("help,h", "produce help message")("input", po::value<std::string>()->value_name("INPUT"), "input file")(
        "output", po::value<std::string>()->value_name("OUTPUT"), "output file")("decompress,d", "decompress")(
        "offset-in-compressed-file", po::value<size_t>()->default_value(0ULL),
        "offset to the compressed block (i.e. physical file offset)")(
        ...
        "hc", "use LZ4HC instead of LZ4")("zstd", "use ZSTD instead of LZ4")("codec", po::value<std::vector<std::string>>()->multitoken(),
                                                                             "use codecs combination instead of LZ4")(
        "level", po::value<int>(), "compression level for codecs specified via flags")("threads", po::value<size_t>()->default_value(1),
                                                                                       "number of threads for parallel compression")(
        "none", "use no compression instead of LZ4")("no-checksum-validation", "disable checksum validation")(
        "stat", "print block statistics of compressed data")("stacktrace", "print stacktrace of exception");
    ...
    if (options.contains("codec")) codecs = options["codec"].as<std::vector<std::string>>();
    ...
    if (num_threads < 1) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid value of `threads` parameter");
    ...
    if (options.contains("level")) level = options["level"].as<int>();
    ...
    if (stat_mode) {
      ...
      if (offset_in_compressed_file || offset_in_decompressed_block) {
        ...
        if (options.contains("no-checksum-validation")) compressed_file.disableChecksumming();
        ...
        if (options.contains("no-checksum-validation")) from.disableChecksumming();
        copyData(from, *wb);
      }
    } else {
      /// Compression
