...
int mainEntryClickHouseServer(int argc, char **argv) {
  ...
  if (argc > 0) {
    ...
    if (env_watchdog) {
      if (0 == strcmp(env_watchdog, "1")) app.shouldSetupWatchdog(argv[0]);
...
static std::string getCanonicalPath(std::string &&path) {
  ...
  if (path.empty()) throw Exception(ErrorCodes::INVALID_CONFIG_PARAMETER, "path configuration parameter is empty");
  if (path.back() != '/') path += '/';
...
Poco::Net::SocketAddress Server::socketBindListen(const ServerSettings &server_settings, Poco::Net::ServerSocket &socket,
                                                  const std::string &host, UInt16 port, [[maybe_unused]] bool secure) const {
...
Strings getInterserverListenHosts(const Poco::Util::AbstractConfiguration &config) {
  ...
  if (!interserver_listen_hosts.empty()) return interserver_listen_hosts;
...
void Server::createServer(Poco::Util::AbstractConfiguration &config, const std::string &listen_host, const char *port_name, bool listen_try,
                          bool start_server, std::vector<ProtocolServerAdapter> &servers, CreateServerFunc &&func) const {
  /// For testing purposes, user may omit tcp_port or http_port or https_port in configuration file.
  if (config.getString(port_name, "").empty()) return;

  /// If we already have an active server for this listen_host/port_name, don't create it again
  for (const auto &server : servers) {
    if (!server.isStopping() && server.getListenHost() == listen_host && server.getPortName() == port_name) return;
  }
...
void Server::initialize(Poco::Util::Application &self) {
  ConfigProcessor::registerEmbeddedConfig(
      "config.xml", std::string_view(reinterpret_cast<const char *>(resource_embedded_xml), std::size(resource_embedded_xml)));
  BaseDaemon::initialize(self);
  logger().information("starting up");

  LOG_INFO(&logger(), "OS name: {}, version: {}, architecture: {}", Poco::Environment::osName(), Poco::Environment::osVersion(),
           Poco::Environment::osArchitecture());
}

std::string Server::getDefaultCorePath() const { return getCanonicalPath(config().getString("path", DBMS_DEFAULT_PATH)) + "cores"; }
...
void checkForUsersNotInMainConfig(const Poco::Util::AbstractConfiguration &config, const ServerSettings &server_settings,
                                  const std::string &config_path, const std::string &users_config_path, LoggerPtr log) {
  if (server_settings[ServerSetting::skip_check_for_incorrect_settings]) return;
...
void sanityChecks(Server &server, const ServerSettings &server_settings) {
  ...
  try {
    ...
    if (readNumber(filename) < 30000)
      server.context()->addOrUpdateWarningMessage(Context::WarningType::LINUX_MAX_PID_TOO_LOW,
  ...
  try {
    ...
    if (readNumber(filename) == 0)
      server.context()->addOrUpdateWarningMessage(
          ...
          PreformattedMessage::create("Delay accounting is not enabled, OSIOWaitMicroseconds will not be gathered. You can enable it "
  ...
  if (getBlockDeviceType(dev_id) == BlockDeviceType::ROT && getBlockDeviceReadAheadBytes(dev_id) == 0)
    server.context()->addOrUpdateWarningMessage(
        ...
        PreformattedMessage::create("Rotational disk with disabled readahead is in use. Performance can be degraded. Used for data: {}",
                                    String(data_path)));
#endif

  try {
    if (getAvailableMemoryAmount() < (2l << 30))
      server.context()->addOrUpdateWarningMessage(Context::WarningType::AVAILABLE_MEMORY_TOO_LOW,
  ...
  if (server.context()->getMergeTreeSettings()[MergeTreeSetting::allow_remote_fs_zero_copy_replication]) {
    ...
    server.context()->addOrUpdateWarningMessage(Context::WarningType::SETTING_ZERO_COPY_REPLICATION_ENABLED,
...
void loadStartupScripts(const Poco::Util::AbstractConfiguration &config, const ServerSettings &server_settings, ContextMutablePtr context,
                        Poco::Logger *log) {
  try {
    ...
    for (const auto &key : keys) {
      if (key == "throw_on_error") continue;
    ...
    if (!skipped_startup_scripts.empty()) {
      context->addOrUpdateWarningMessage(
          ...
          PreformattedMessage::create("Skipped the following startup script(s): {} as the condition query for those returned values, "
                                      "which can't be interpreted as a boolean (`0`, `false`, `1`, `true`).",
    ...
    DimensionalMetrics::set(DimensionalMetrics::StartupScriptsFailureReason, {String(ErrorCodes::getName(getCurrentExceptionCode()))}, 1.0);
    ...
    if (server_settings[ServerSetting::startup_scripts_throw_on_error])
      throw Exception(ErrorCodes::STARTUP_SCRIPTS_ERROR,
                      "Cannot finish startup_script successfully. Use startup_scripts.throw_on_error setting to change this behavior");
  }
}

static void initializeAzureSDKLogger([[maybe_unused]] const ServerSettings &server_settings, [[maybe_unused]] int server_logs_level) {
#if USE_AZURE_BLOB_STORAGE
  if (!server_settings[ServerSetting::enable_azure_sdk_logging]) return;
...
int Server::main(const std::vector<std::string> & /*args*/) try {
#if USE_SSL
  ...
  LOG_INFO(log, "Available RAM: {}; logical cores: {}; used cores: {}.", formatReadableSizeWithBinarySuffix(physical_server_memory),
...
#  undef COLLECT_FLAG
  ...
  if (server_settings[ServerSetting::mlock_executable]) {
    ...
    if (physical_server_memory >= min_physical_server_memory_to_mlock) {
      ...
      LOG_INFO(log,
               "Skip mlock for the clickhouse executable, because the total memory in the system ({}) is less than the minimum configured "
               "threshold (`mlock_executable_min_total_memory_amount_bytes` = {})",
...
#if USE_JEMALLOC
  Jemalloc::verifySetup(server_settings[ServerSetting::jemalloc_enable_global_profiler],
  ...
  if (!libhdfs3_conf.empty()) {
    if (std::filesystem::path{libhdfs3_conf}.is_relative() && !std::filesystem::exists(libhdfs3_conf)) {
      ...
      if (std::filesystem::exists(config_dir / libhdfs3_conf)) libhdfs3_conf = std::filesystem::absolute(config_dir / libhdfs3_conf);
    }
  ...
  if (auto total_numa_memory = getNumaNodesTotalMemory(); total_numa_memory.has_value()) {
    LOG_INFO(log, "ClickHouse is bound to a subset of NUMA nodes. Total memory of all available nodes: {}",
  ...
  global_context->addOrUpdateWarningMessage(Context::WarningType::SERVER_BUILT_IN_DEBUG_MODE,
  ...
  global_context->addOrUpdateWarningMessage(Context::WarningType::SERVER_BUILT_WITH_SANITIZERS,
  ...
  global_context->addOrUpdateWarningMessage(Context::WarningType::SERVER_BUILT_WITH_COVERAGE,
  ...
  if (page_cache_max_size != 0 && (page_cache_min_size > page_cache_max_size)) {
    throw Exception(ErrorCodes::INVALID_CONFIG_PARAMETER,
                    ...
                    page_cache_min_size, page_cache_max_size);
  }
  ...
  GlobalThreadPool::initialize(server_settings[ServerSetting::max_thread_pool_size],
  ...
  Poco::ThreadPool server_pool(
      ...
      /* idleTime */ 60,
      /* stackSize */ POCO_THREAD_STACK_SIZE, server_settings[ServerSetting::global_profiler_real_time_period_ns],
  ...
  if (page_cache_max_size != 0) {
    global_context->setPageCache(std::chrono::milliseconds(Int64(server_settings[ServerSetting::page_cache_history_window_ms])),
                                 server_settings[ServerSetting::page_cache_policy], server_settings[ServerSetting::page_cache_size_ratio],
                                 server_settings[ServerSetting::page_cache_min_size], server_settings[ServerSetting::page_cache_max_size],
  ...
  DB::AsynchronousMetrics::ProtocolServerMetricsFunc asynchronous_metrics_protocol_server_metrics_func =
  ...
  if (async_metrics_keeper_metrics_only) {
    async_metrics = std::make_unique<KeeperAsynchronousMetrics>(global_context, async_metrics_update_period_s,
                                                                asynchronous_metrics_protocol_server_metrics_func,
                                                                async_metrics_update_jemalloc_epoch, async_metrics_update_rss);
  } else
#endif
  {
    async_metrics = std::make_unique<ServerAsynchronousMetrics>(
        global_context, async_metrics_update_period_s, server_settings[ServerSetting::asynchronous_metrics_enable_heavy_metrics],
        server_settings[ServerSetting::asynchronous_heavy_metrics_update_period_s], asynchronous_metrics_protocol_server_metrics_func,
        async_metrics_update_jemalloc_epoch, async_metrics_update_rss);
  }
  ...
  SCOPE_EXIT_SAFE({
    ...
    if (!servers_to_start_before_tables.empty()) {
      ...
      if (current_connections > 0)
        current_connections =
            waitServersToFinish(servers_to_start_before_tables, servers_lock, server_settings[ServerSetting::shutdown_wait_unfinished]);
  ...
  getIOThreadPool().initialize(server_settings[ServerSetting::max_io_thread_pool_size],
                               server_settings[ServerSetting::max_io_thread_pool_free_size],
                               server_settings[ServerSetting::io_thread_pool_queue_size]);

  getBackupsIOThreadPool().initialize(server_settings[ServerSetting::max_backups_io_thread_pool_size],
  ...
  getFetchPartitionThreadPool().initialize(server_settings[ServerSetting::max_fetch_partition_thread_pool_size],
  ...
  getActivePartsLoadingThreadPool().initialize(server_settings[ServerSetting::max_active_parts_loading_thread_pool_size],
  ...
  getOutdatedPartsLoadingThreadPool().initialize(server_settings[ServerSetting::max_outdated_parts_loading_thread_pool_size],
                                                 0,  // We don't need any threads once all the parts will be loaded
  ...
  getOutdatedPartsLoadingThreadPool().setMaxTurboThreads(server_settings[ServerSetting::max_active_parts_loading_thread_pool_size]);

  getUnexpectedPartsLoadingThreadPool().initialize(server_settings[ServerSetting::max_unexpected_parts_loading_thread_pool_size],
                                                   0,  // We don't need any threads once all the parts will be loaded
                                                   server_settings[ServerSetting::max_unexpected_parts_loading_thread_pool_size]);

  /// It could grow if we need to synchronously wait until all the data parts will be loaded.
  getUnexpectedPartsLoadingThreadPool().setMaxTurboThreads(server_settings[ServerSetting::max_active_parts_loading_thread_pool_size]);

  getPartsCleaningThreadPool().initialize(server_settings[ServerSetting::max_parts_cleaning_thread_pool_size],
                                          0,  // We don't need any threads one all the parts will be deleted
                                          server_settings[ServerSetting::max_parts_cleaning_thread_pool_size]);

  auto max_database_replicated_create_table_thread_pool_size =
      server_settings[ServerSetting::max_database_replicated_create_table_thread_pool_size]
          ? server_settings[ServerSetting::max_database_replicated_create_table_thread_pool_size]
          : getNumberOfCPUCoresToUse();
  getDatabaseReplicatedCreateTablesThreadPool().initialize(max_database_replicated_create_table_thread_pool_size,
                                                           0,  // We don't need any threads once all the tables will be created
                                                           max_database_replicated_create_table_thread_pool_size);

  getDatabaseCatalogDropTablesThreadPool().initialize(server_settings[ServerSetting::database_catalog_drop_table_concurrency],
  ...
  getFormatParsingThreadPool().initialize(server_settings[ServerSetting::max_format_parsing_thread_pool_size],
  ...
  assertProcessUserMatchesDataOwner(path_str, [&](const PreformattedMessage &message) {
    global_context->addOrUpdateWarningMessage(Context::WarningType::PROCESS_USER_MATCHES_DATA_OWNER, message);
  });
  ...
  if (loaded_config.has_zk_includes) {
    ...
    loaded_config = config_processor.loadConfigWithZooKeeperIncludes(main_config_zk_node_cache.get(), main_config_zk_changed_event,
  ...
  /// Inject and enable failpoints declared in the configuration
  {
    ...
    for (const auto &fail_point_name : fail_point_names) {
      if (ConfigHelper::getBool(conf, root_key + "." + fail_point_name)) FailPointInjection::enableFailPoint(fail_point_name);
    }
  }
  ...
  if (oom_score) setOOMScore(oom_score, log);
#endif

  std::unique_ptr<DB::BackgroundSchedulePoolTaskHolder> cancellation_task;

  SCOPE_EXIT({
    if (cancellation_task) CancellationChecker::getInstance().terminateThread();
  });
  ...
  if (server_settings[ServerSetting::background_schedule_pool_size] > 1) {
    auto cancellation_task_holder = global_context->getSchedulePool().createTask(
        StorageID::createEmpty(), "CancellationChecker", [] { CancellationChecker::getInstance().workerFunction(); });
  ...
  {
    rlimit rlim;
    if (getrlimit(RLIMIT_NOFILE, &rlim)) throw Poco::Exception("Cannot getrlimit");

    if (rlim.rlim_cur == rlim.rlim_max) {
      LOG_DEBUG(log, "rlimit on number of file descriptors is {}", rlim.rlim_cur);
    } else {
      rlim_t old = rlim.rlim_cur;
      rlim.rlim_cur = server_settings[ServerSetting::max_open_files].changed
                          ? static_cast<rlim_t>(server_settings[ServerSetting::max_open_files])
                          : rlim.rlim_max;
      int rc = setrlimit(RLIMIT_NOFILE, &rlim);
      if (rc != 0)
        LOG_WARNING(
            log,
            "Cannot set max number of file descriptors to {}. Try to specify max_open_files according to your system limits. error: {}",
            rlim.rlim_cur, errnoToString());
      else
  ...
  {
    rlimit rlim;
    if (getrlimit(RLIMIT_NPROC, &rlim)) throw Poco::Exception("Cannot getrlimit");
  ...
  if (config().has("macros")) global_context->setMacros(std::make_unique<Macros>(config(), "macros", log));
...

  {
    ...
    std::string dictionaries_lib_path =
        dictionaries_lib_path_setting.changed ? dictionaries_lib_path_setting.value : String(path / "dictionaries_lib/");
  ...
  /// top_level_domains_lists
  {
    ...
    const std::string &top_level_domains_path =
        top_level_domains_path_setting.changed ? top_level_domains_path_setting.value : String(path / "top_level_domains/");
  ...
  static const auto interserver_tags = {
      std::make_tuple("interserver_http_host", server_settings[ServerSetting::interserver_http_host], "interserver_http_port",
                      server_settings[ServerSetting::interserver_http_port], "http"),
      std::make_tuple("interserver_https_host", server_settings[ServerSetting::interserver_https_host], "interserver_https_port",
                      server_settings[ServerSetting::interserver_https_port], "https")};
  ...
  for (auto [host_tag, host_setting, port_tag, port_setting, scheme] : interserver_tags) {
    if (port_setting.changed) {
      ...
      if (this_host.empty()) {
        ...
        LOG_DEBUG(log, "Configuration parameter '{}' doesn't exist or exists and empty. Will use '{}' as replica host.", host_tag,
                  this_host);
      }

      UInt64 port = port_setting.value;

      if (port > 0xFFFF) throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "Out of range '{}': {}", String(port_tag), port);
  ...
  const size_t max_cache_size =
      static_cast<size_t>(static_cast<double>(physical_server_memory) * server_settings[ServerSetting::cache_size_to_ram_max_ratio]);
  ...
  if (uncompressed_cache_size > max_cache_size) {
    ...
    LOG_INFO(log, "Lowered uncompressed cache size to {} because the system has limited RAM",
  ...
  if (primary_index_cache_size > max_cache_size) {
    ...
    LOG_INFO(log, "Lowered primary index cache size to {} because the system has limited RAM",
  ...
  if (index_uncompressed_cache_size > max_cache_size) {
    ...
    LOG_INFO(log, "Lowered index uncompressed cache size to {} because the system has limited RAM",
             formatReadableSizeWithBinarySuffix(index_uncompressed_cache_size));
  }
  global_context->setIndexUncompressedCache(index_uncompressed_cache_policy, index_uncompressed_cache_size,
  ...
  if (index_mark_cache_size > max_cache_size) {
    ...
    LOG_INFO(log, "Lowered index mark cache size to {} because the system has limited RAM",
  ...
  if (vector_similarity_index_cache_size > max_cache_size) {
    ...
    LOG_INFO(log, "Lowered vector similarity index cache size to {} because the system has limited RAM",
             formatReadableSizeWithBinarySuffix(vector_similarity_index_cache_size));
  }
  global_context->setVectorSimilarityIndexCache(vector_similarity_index_cache_policy, vector_similarity_index_cache_size,
                                                vector_similarity_index_cache_max_entries, vector_similarity_index_cache_size_ratio);
  ...
  if (text_index_tokens_cache_size > max_cache_size) {
    ...
    LOG_INFO(log, "Lowered text index tokens cache size to {} because the system has limited RAM",
             formatReadableSizeWithBinarySuffix(text_index_tokens_cache_size));
  }
  global_context->setTextIndexTokensCache(text_index_tokens_cache_policy, text_index_tokens_cache_size, text_index_tokens_cache_max_count,
  ...
  if (text_index_header_cache_size > max_cache_size) {
    ...
    LOG_INFO(log, "Lowered text index header cache size to {} because the system has limited RAM",
             formatReadableSizeWithBinarySuffix(text_index_header_cache_size));
  }
  global_context->setTextIndexHeaderCache(text_index_header_cache_policy, text_index_header_cache_size, text_index_header_cache_max_count,
  ...
  if (text_index_postings_cache_size > max_cache_size) {
    ...
    LOG_INFO(log, "Lowered text index posting list cache size to {} because the system has limited RAM",
             formatReadableSizeWithBinarySuffix(text_index_postings_cache_size));
  }
  global_context->setTextIndexPostingsCache(text_index_postings_cache_policy, text_index_postings_cache_size,
                                            text_index_postings_cache_max_count, text_index_postings_cache_size_ratio);

  size_t mmap_cache_size = server_settings[ServerSetting::mmap_cache_size];
  if (mmap_cache_size > max_cache_size) {
    mmap_cache_size = max_cache_size;
    LOG_INFO(log, "Lowered mmap file cache size to {} because the system has limited RAM",
  ...
  if (iceberg_metadata_files_cache_size > max_cache_size) {
    ...
    LOG_INFO(log, "Lowered Iceberg metadata cache size to {} because the system has limited RAM",
             formatReadableSizeWithBinarySuffix(iceberg_metadata_files_cache_size));
  }
  global_context->setIcebergMetadataFilesCache(iceberg_metadata_files_cache_policy, iceberg_metadata_files_cache_size,
                                               iceberg_metadata_files_cache_max_entries, iceberg_metadata_files_cache_size_ratio);
#endif
#if USE_PARQUET
  ...
  if (parquet_metadata_cache_size > max_cache_size) {
    ...
    LOG_INFO(log, "Lowered Parquet metadata cache size to {} because the system has limited RAM",
             formatReadableSizeWithBinarySuffix(parquet_metadata_cache_size));
  }
  global_context->setParquetMetadataCache(parquet_metadata_cache_policy, parquet_metadata_cache_size, parquet_metadata_cache_max_entries,
  ...
  global_context->setAllowedDisksForTableEngines(
      std::unordered_set<String>(allowed_disks_table_engines.begin(), allowed_disks_table_engines.end()));
  ...
  if (query_condition_cache_size > max_cache_size) {
    ...
    LOG_INFO(log, "Lowered query condition cache size to {} because the system has limited RAM",
             formatReadableSizeWithBinarySuffix(query_condition_cache_size));
  }
  ...
  global_context->setQueryResultCache(query_result_cache_max_size_in_bytes, query_result_cache_max_entries,
                                      query_result_cache_max_entry_size_in_bytes, query_result_cache_max_entry_size_in_rows);
  ...
  try {
    ...
    if (wait_time != 0) cgroups_memory_usage_observer.emplace(std::chrono::seconds(wait_time));
  } catch (Exception &) {
  ...
  DNSResolver::instance().setFilterSettings(server_settings[ServerSetting::dns_allow_resolve_names_to_ipv4],
                                            server_settings[ServerSetting::dns_allow_resolve_names_to_ipv6]);
  ...
  if (server_settings[ServerSetting::disable_internal_dns_cache]) {
    ...
    dns_cache_updater = std::make_unique<DNSCacheUpdater>(global_context, server_settings[ServerSetting::dns_cache_update_period],
                                                          server_settings[ServerSetting::dns_max_consecutive_failures]);
    dns_cache_updater->start();
  }

  auto main_config_reloader = std::make_unique<ConfigReloader>(
      config_path, extra_paths, server_settings[ServerSetting::path], std::move(main_config_zk_node_cache), main_config_zk_changed_event,
      [&](ConfigurationPtr loaded_config, bool initial_loading) {
        ...
        const size_t current_physical_server_memory =
            getMemoryAmount();  /// With cgroups, the amount of memory available to the server can be changed dynamically.
        const size_t default_max_server_memory_usage =
            static_cast<size_t>(static_cast<double>(current_physical_server_memory) * max_server_memory_usage_to_ram_ratio);
        ...
        if (max_server_memory_usage == 0) {
          ...
          LOG_INFO(log,
                   ...
                   formatReadableSizeWithBinarySuffix(current_physical_server_memory), max_server_memory_usage_to_ram_ratio);
        ...
        size_t default_merges_mutations_server_memory_usage =
            static_cast<size_t>(static_cast<double>(current_physical_server_memory) *
        ...
        if (merges_mutations_memory_usage_soft_limit == 0) {
          ...
          LOG_INFO(log,
                   "Setting merges_mutations_memory_usage_soft_limit was set to {}"
        ...
        global_context->setMaxPendingMutationsExecutionTimeToWarn(
            new_server_settings[ServerSetting::max_pending_mutations_execution_time_to_warn]);
        ...
        global_context->setOSCPUOverloadSettings(new_server_settings[ServerSetting::min_os_cpu_wait_time_ratio_to_drop_connection],
                                                 new_server_settings[ServerSetting::max_os_cpu_wait_time_ratio_to_drop_connection]);
        ...
        auto [concurrent_threads_soft_limit, concurrency_control_scheduler] =
            global_context->setConcurrentThreadsSoftLimit(new_server_settings[ServerSetting::concurrent_threads_soft_limit_num],
        ...
        if (global_context->areBackgroundExecutorsInitialized()) {
          ...
          global_context->getMergeMutateExecutor()->increaseThreadsAndMaxTasksCount(
              new_pool_size, static_cast<size_t>(static_cast<double>(new_pool_size) * new_ratio));
          global_context->getMergeMutateExecutor()->updateSchedulingPolicy(
              new_server_settings[ServerSetting::background_merges_mutations_scheduling_policy].toString());
        }
        ...
        global_context->getBufferFlushSchedulePool().increaseThreadsCount(
        ...
        global_context->getDistributedSchedulePool().increaseThreadsCount(
            new_server_settings[ServerSetting::background_distributed_schedule_pool_size]);

        global_context->getAsyncLoader().setMaxThreads(TablesLoaderForegroundPoolId,
                                                       new_server_settings[ServerSetting::tables_loader_foreground_pool_size]);
        global_context->getAsyncLoader().setMaxThreads(TablesLoaderBackgroundLoadPoolId,
                                                       new_server_settings[ServerSetting::tables_loader_background_pool_size]);
        global_context->getAsyncLoader().setMaxThreads(TablesLoaderBackgroundStartupPoolId,
                                                       new_server_settings[ServerSetting::tables_loader_background_pool_size]);

        getIOThreadPool().reloadConfiguration(new_server_settings[ServerSetting::max_io_thread_pool_size],
        ...
        getBackupsIOThreadPool().reloadConfiguration(new_server_settings[ServerSetting::max_backups_io_thread_pool_size],
        ...
        getFetchPartitionThreadPool().reloadConfiguration(new_server_settings[ServerSetting::max_fetch_partition_thread_pool_size],
                                                          0,  // FETCH PARTITION is relatively rare, no need to keep threads
        ...
        getPartsCleaningThreadPool().reloadConfiguration(new_server_settings[ServerSetting::max_parts_cleaning_thread_pool_size],
                                                         0,  // We don't need any threads one all the parts will be deleted
        ...
        getFormatParsingThreadPool().reloadConfiguration(new_server_settings[ServerSetting::max_format_parsing_thread_pool_size],
        ...
        if (!initial_loading) {
          ...
          if (zkutil::hasZooKeeperConfig(config())) global_context->reloadZooKeeperIfChanged(loaded_config);
        ...
        DNSResolver::instance().setFilterSettings(new_server_settings[ServerSetting::dns_allow_resolve_names_to_ipv4],
                                                  new_server_settings[ServerSetting::dns_allow_resolve_names_to_ipv6]);

        if (global_context->isServerCompletelyStarted())
          CannotAllocateThreadFaultInjector::setFaultProbability(
              new_server_settings[ServerSetting::cannot_allocate_thread_fault_injection_probability]);
        ...
        /// Update core dump size limit.
        {
          ...
          if (getrlimit(RLIMIT_CORE, &rlim) == 0) {
            ...
            if (setrlimit(RLIMIT_CORE, &rlim)) LOG_WARNING(log, "Cannot set max size of core file to {}", rlim.rlim_cur);
          }
        }
  ...
  if (config().has("keeper_server.server_id")) {
    ...
    for (const auto &listen_host : listen_hosts) {
      ...
      createServer(config(), listen_host, port_name, listen_try, /* start_server: */ false, servers_to_start_before_tables,
                   [&](UInt16 port) -> ProtocolServerAdapter {
                     ...
                     return ProtocolServerAdapter(
                         listen_host, port_name, "Keeper (tcp): " + address.toString(),
                         std::make_unique<TCPServer>(
                             new KeeperTCPHandlerFactory(config_getter, global_context->getKeeperDispatcher(),
                                                         global_context->getSettingsRef()[Setting::receive_timeout].totalSeconds(),
                                                         global_context->getSettingsRef()[Setting::send_timeout].totalSeconds(), false),
                             server_pool, socket));
                   });

      const char *secure_port_name = "keeper_server.tcp_port_secure";
      createServer(config(), listen_host, secure_port_name, listen_try, /* start_server: */ false, servers_to_start_before_tables,
                   [&](UInt16 port) -> ProtocolServerAdapter {
#  if USE_SSL
                     ...
                     socket.setSendTimeout(Poco::Timespan(server_settings[ServerSetting::keeper_server_socket_send_timeout_sec], 0));
                     return ProtocolServerAdapter(
                         listen_host, secure_port_name, "Keeper with secure protocol (tcp_secure): " + address.toString(),
                         std::make_unique<TCPServer>(
                             new KeeperTCPHandlerFactory(config_getter, global_context->getKeeperDispatcher(),
                                                         global_context->getSettingsRef()[Setting::receive_timeout].totalSeconds(),
                                                         global_context->getSettingsRef()[Setting::send_timeout].totalSeconds(), true),
                             server_pool, socket));
#  else
                    UNUSED(port);
...
#  endif
      ...
      createServer(
          config(), listen_host, port_name, listen_try, /* start_server: */ false, servers_to_start_before_tables,
          [&](UInt16 port) -> ProtocolServerAdapter {
            ...
            return ProtocolServerAdapter(
                listen_host, port_name, "HTTP Control: http://" + address.toString(),
                std::make_unique<HTTPServer>(std::move(http_context),
                                             createKeeperHTTPHandlerFactory(*this, config_getter(), global_context->getKeeperDispatcher(),
                                                                            "KeeperHTTPHandler-factory"),
                                             server_pool, socket, http_params));
          });
    }
#else
...

  {
    ...
    createInterserverServers(config(), server_settings, interserver_listen_hosts, listen_try, server_pool, *async_metrics,
  ...
  /// 1. We create a socket for keeper secure protocol (w/o starting a server yet). That creates a SSL context (in particular, it might be
  /// the default context).
  ...
  /// 4. `CertificateReloader` will set its custom `cert_cb` to the default context if it is not initialized yet (see
  /// `CertificateReloader::init()`).
  /// 5. Items (2) and (4) are not synchronized, so there might be a data race for example (see
  /// https://github.com/ClickHouse/ClickHouse/issues/85412).
  ...
  if (!server_settings[ServerSetting::tmp_policy].value.empty()) {
    global_context->setTemporaryStoragePolicy(server_settings[ServerSetting::tmp_policy],
                                              server_settings[ServerSetting::max_temporary_data_on_disk_size]);
    ...
    if (!cache_name.empty()) {
      ...
      if (cache_data->getSettings()[FileCacheSetting::load_metadata_asynchronously]) {
        throw Exception(ErrorCodes::INVALID_SETTING_VALUE,
                        "Cache disk '{}' is used as a temporary storage, load_metadata_asynchronously is disallowed.", cache_name);
      }
    }
    global_context->setTemporaryStorageInCache(server_settings[ServerSetting::temporary_data_in_cache],
                                               server_settings[ServerSetting::max_temporary_data_on_disk_size]);
  } else {
  ...
  global_context->setStartServersCallback([&](const ServerType &server_type) {
    ...
    createServers(config(), server_settings, listen_hosts, listen_try, server_pool, *async_metrics, servers,
                  /* start_servers= */ true, server_type);
  });
  ...
  if (server_settings[ServerSetting::async_insert_threads]) {
    global_context->setAsynchronousInsertQueue(
        std::make_shared<AsynchronousInsertQueue>(global_context, server_settings[ServerSetting::async_insert_threads],
  ...
  if (!filesystem_caches_path.empty()) global_context->setFilesystemCachesPath(filesystem_caches_path);
  ...
  if (default_database.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "default_database cannot be empty");
  ...
  SCOPE_EXIT_SAFE(LOG_INFO(log, "Stopping AsyncLoader.");
  ...
  if (has_trace_collector) global_context->initializeTraceCollector();
...

  {
    ...
    if (has_zookeeper && config().has("distributed_ddl")) {
      ...
      if (pool_size < 1) throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "distributed_ddl.pool_size should be greater then 0");
      global_context->setDDLWorker(
          std::make_unique<DDLWorker>(pool_size, ddl_queue_path, ddl_replicas_path, global_context, &config(), "distributed_ddl", "default",
                                      "DDLWorker", &CurrentMetrics::MaxDDLEntryID, &CurrentMetrics::MaxPushedDDLEntryID),
    ...
    if (config().has("startup_scripts")) loadStartupScripts(config(), server_settings, global_context, log);
    ...
    CannotAllocateThreadFaultInjector::setFaultProbability(
        server_settings[ServerSetting::cannot_allocate_thread_fault_injection_probability]);
    ...
    SCOPE_EXIT_SAFE({
      ...
      if (!server_settings[ServerSetting::shutdown_wait_unfinished_queries]) global_context->getProcessList().killAllQueries();

      size_t wait_limit_seconds = server_settings[ServerSetting::shutdown_wait_unfinished];
      auto wait_start = std::chrono::steady_clock::now();

      if (current_connections) current_connections = waitServersToFinish(servers, servers_lock, wait_limit_seconds);
    ...
    for (const auto &graphite_key : DB::getMultipleKeysFromConfig(config(), "", "graphite")) {
      metrics_transmitters.emplace_back(std::make_unique<MetricsTransmitter>(global_context->getConfigRef(), graphite_key, *async_metrics));
    }
...
std::unique_ptr<TCPProtocolStackFactory> Server::buildProtocolStackFromConfig(const Poco::Util::AbstractConfiguration &config,
                                                                              const ServerSettings &server_settings,
                                                                              const std::string &protocol,
                                                                              Poco::Net::HTTPServerParams::Ptr http_params,
                                                                              AsynchronousMetrics &async_metrics, bool &is_secure) {
  auto create_factory = [&](const std::string &type, const std::string &conf_name) -> TCPServerConnectionFactory::Ptr {
    if (type == "tcp")
      return TCPServerConnectionFactory::Ptr(
          new TCPHandlerFactory(*this, false, false, ProfileEvents::InterfaceNativeReceiveBytes, ProfileEvents::InterfaceNativeSendBytes));
    ...
    if (type == "proxy1") return TCPServerConnectionFactory::Ptr(new ProxyV1HandlerFactory(*this, conf_name));
    if (type == "mysql")
      return TCPServerConnectionFactory::Ptr(new MySQLHandlerFactory(*this, server_settings[ServerSetting::mysql_require_secure_transport],
                                                                     ProfileEvents::InterfaceMySQLReceiveBytes,
                                                                     ProfileEvents::InterfaceMySQLSendBytes));
    if (type == "postgres")
#if USE_SSL
      return TCPServerConnectionFactory::Ptr(
          new PostgreSQLHandlerFactory(*this, server_settings[ServerSetting::postgresql_require_secure_transport], conf_name + ".",
                                       ProfileEvents::InterfacePostgreSQLReceiveBytes, ProfileEvents::InterfacePostgreSQLSendBytes));
#else
      return TCPServerConnectionFactory::Ptr(
          new PostgreSQLHandlerFactory(*this, server_settings[ServerSetting::postgresql_require_secure_transport],
                                       ProfileEvents::InterfacePostgreSQLReceiveBytes, ProfileEvents::InterfacePostgreSQLSendBytes));
#endif
    if (type == "http") {
      ...
      if (config.has(conf_name + ".handlers")) handlers_config_key = config.getString(conf_name + ".handlers");
      return TCPServerConnectionFactory::Ptr(new HTTPServerConnectionFactory(
          httpContext(), http_params, createHandlerFactory(*this, config, async_metrics, "HTTPHandler-factory", handlers_config_key),
          ProfileEvents::InterfaceHTTPReceiveBytes, ProfileEvents::InterfaceHTTPSendBytes));
    }
    if (type == "prometheus") {
      const std::string handler_name =
          server_settings[ServerSetting::prometheus_keeper_metrics_only] ? "KeeperPrometheusHandler-factory" : "PrometheusHandler-factory";
      return TCPServerConnectionFactory::Ptr(
          new HTTPServerConnectionFactory(httpContext(), http_params, createHandlerFactory(*this, config, async_metrics, handler_name),
                                          ProfileEvents::InterfacePrometheusReceiveBytes, ProfileEvents::InterfacePrometheusSendBytes));
    }
    if (type == "interserver")
      return TCPServerConnectionFactory::Ptr(new HTTPServerConnectionFactory(
          httpContext(), http_params, createHandlerFactory(*this, config, async_metrics, "InterserverIOHTTPHandler-factory"),
          ProfileEvents::InterfaceInterserverReceiveBytes, ProfileEvents::InterfaceInterserverSendBytes));
  ...
  while (true) {
    ...
    if (config.has(prefix + "type")) {
      ...
      if (type == "tls") {
        if (is_secure) throw Exception(ErrorCodes::INVALID_CONFIG_PARAMETER, "Protocol '{}' contains more than one TLS layer", protocol);
        is_secure = true;
      }
    ...
    if (!config.has(prefix + "impl")) break;
...
HTTPContextPtr Server::httpContext() const { return std::make_shared<HTTPContext>(context()); }

void Server::createServers(Poco::Util::AbstractConfiguration &config, const ServerSettings &server_settings, const Strings &listen_hosts,
                           bool listen_try, Poco::ThreadPool &server_pool, AsynchronousMetrics &async_metrics,
                           std::vector<ProtocolServerAdapter> &servers, bool start_servers, const ServerType &server_type) {
  ...
  for (const auto &protocol : protocols) {
    if (!server_type.shouldStart(ServerType::Type::CUSTOM, protocol)) continue;

    std::string prefix = "protocols." + protocol + ".";
    std::string port_name = prefix + "port";
    std::string description{"<undefined> protocol"};
    if (config.has(prefix + "description")) description = config.getString(prefix + "description");

    if (!config.has(prefix + "port")) continue;
    ...
    for (const auto &host : hosts) {
      ...
      if (stack->empty()) throw Exception(ErrorCodes::INVALID_CONFIG_PARAMETER, "Protocol '{}' stack empty", protocol);
      ...
      createServer(config, host, port_name.c_str(), listen_try, start_servers, servers, [&](UInt16 port) -> ProtocolServerAdapter {
        ...
        return ProtocolServerAdapter(
            host, port_name.c_str(), description + ": " + address.toString(),
            std::make_unique<TCPServer>(stack.release(), server_pool, socket, makeServerParams(server_settings), connection_filter));
      });
    }
  }
  ...
  for (const auto &listen_host : listen_hosts) {
    ...
    if (server_type.shouldStart(ServerType::Type::HTTP)) {
      ...
      createServer(config, listen_host, port_name, listen_try, start_servers, servers, [&](UInt16 port) -> ProtocolServerAdapter {
        ...
        return ProtocolServerAdapter(
            listen_host, port_name, "http://" + address.toString(),
            std::make_unique<HTTPServer>(httpContext(), createHandlerFactory(*this, config, async_metrics, "HTTPHandler-factory"),
                                         server_pool, socket, http_params, connection_filter, ProfileEvents::InterfaceHTTPReceiveBytes,
    ...
    if (server_type.shouldStart(ServerType::Type::HTTPS)) {
      ...
      createServer(config, listen_host, port_name, listen_try, start_servers, servers, [&](UInt16 port) -> ProtocolServerAdapter {
        ...
        socket.setSendTimeout(settings[Setting::http_send_timeout]);
        return ProtocolServerAdapter(
            listen_host, port_name, "https://" + address.toString(),
            std::make_unique<HTTPServer>(httpContext(), createHandlerFactory(*this, config, async_metrics, "HTTPSHandler-factory"),
                                         server_pool, socket, http_params, connection_filter, ProfileEvents::InterfaceHTTPReceiveBytes,
                                         ProfileEvents::InterfaceHTTPSendBytes));
#else
                UNUSED(port);
    ...
    if (server_type.shouldStart(ServerType::Type::TCP)) {
      ...
      createServer(config, listen_host, port_name, listen_try, start_servers, servers, [&](UInt16 port) -> ProtocolServerAdapter {
        ...
        socket.setSendTimeout(settings[Setting::send_timeout]);
        return ProtocolServerAdapter(
            listen_host, port_name, "native protocol (tcp): " + address.toString(),
            std::make_unique<TCPServer>(
                new TCPHandlerFactory(*this, /* secure */ false, /* proxy protocol */ false, ProfileEvents::InterfaceNativeReceiveBytes,
                                      ProfileEvents::InterfaceNativeSendBytes),
                server_pool, socket, makeServerParams(server_settings), connection_filter));
      });
    }
    ...
    if (server_type.shouldStart(ServerType::Type::TCP_WITH_PROXY)) {
      ...
      createServer(config, listen_host, port_name, listen_try, start_servers, servers, [&](UInt16 port) -> ProtocolServerAdapter {
        ...
        socket.setSendTimeout(settings[Setting::send_timeout]);
        return ProtocolServerAdapter(
            listen_host, port_name, "native protocol (tcp) with PROXY: " + address.toString(),
            std::make_unique<TCPServer>(
                new TCPHandlerFactory(*this, /* secure */ false, /* proxy protocol */ true, ProfileEvents::InterfaceNativeReceiveBytes,
                                      ProfileEvents::InterfaceNativeSendBytes),
                server_pool, socket, makeServerParams(server_settings), connection_filter));
      });
    }
    ...
    if (server_type.shouldStart(ServerType::Type::ARROW_FLIGHT)) {
      ...
      createServer(config, listen_host, port_name, listen_try, start_servers, servers, [&](UInt16 port) -> ProtocolServerAdapter {
        ...
        socket.setSendTimeout(settings[Setting::send_timeout]);
        return ProtocolServerAdapter(
            listen_host, port_name, "Arrow Flight compatibility protocol: " + address.toString(),
            std::unique_ptr<IGRPCServer>(new ArrowFlightHandler(*this, makeSocketAddress(listen_host, port, &logger()))), true);
      });
    }
#endif
    ...
    if (server_type.shouldStart(ServerType::Type::TCP_SECURE)) {
      ...
      createServer(config, listen_host, port_name, listen_try, start_servers, servers, [&](UInt16 port) -> ProtocolServerAdapter {
        ...
        socket.setSendTimeout(settings[Setting::send_timeout]);
        return ProtocolServerAdapter(
            listen_host, port_name, "secure native protocol (tcp_secure): " + address.toString(),
            std::make_unique<TCPServer>(
                new TCPHandlerFactory(*this, /* secure */ true, /* proxy protocol */ false, ProfileEvents::InterfaceNativeReceiveBytes,
                                      ProfileEvents::InterfaceNativeSendBytes),
                server_pool, socket, makeServerParams(server_settings), connection_filter));
#else
                UNUSED(port);
    ...
    if (server_type.shouldStart(ServerType::Type::TCP_SSH)) {
      ...
      createServer(config, listen_host, port_name, listen_try, start_servers, servers, [&](UInt16 port) -> ProtocolServerAdapter {
        ...
        return ProtocolServerAdapter(listen_host, port_name, "SSH PTY: " + address.toString(),
                                     std::make_unique<TCPServer>(new SSHPtyHandlerFactory(*this, config), server_pool, socket,
                                                                 makeServerParams(server_settings), connection_filter));
#else
                UNUSED(port);
    ...
    if (server_type.shouldStart(ServerType::Type::MYSQL)) {
      ...
      createServer(config, listen_host, port_name, listen_try, start_servers, servers, [&](UInt16 port) -> ProtocolServerAdapter {
        ...
        return ProtocolServerAdapter(
            listen_host, port_name, "MySQL compatibility protocol: " + address.toString(),
            std::make_unique<TCPServer>(new MySQLHandlerFactory(*this, secure_required, ProfileEvents::InterfaceMySQLReceiveBytes,
                                                                ProfileEvents::InterfaceMySQLSendBytes),
                                        server_pool, socket, makeServerParams(server_settings), connection_filter));
      });
    }
    ...
    if (server_type.shouldStart(ServerType::Type::POSTGRESQL)) {
      ...
      createServer(config, listen_host, port_name, listen_try, start_servers, servers, [&](UInt16 port) -> ProtocolServerAdapter {
        ...
        return ProtocolServerAdapter(
            listen_host, port_name, "PostgreSQL compatibility protocol: " + address.toString(),
#if USE_SSL
            std::make_unique<TCPServer>(
                new PostgreSQLHandlerFactory(*this, secure_required, Poco::Net::SSLManager::CFG_SERVER_PREFIX,
                                             ProfileEvents::InterfacePostgreSQLReceiveBytes, ProfileEvents::InterfacePostgreSQLSendBytes),
                server_pool, socket, makeServerParams(server_settings), connection_filter));
#else
                    std::make_unique<TCPServer>(
    ...
    if (server_type.shouldStart(ServerType::Type::GRPC)) {
      ...
      createServer(config, listen_host, port_name, listen_try, start_servers, servers, [&](UInt16 port) -> ProtocolServerAdapter {
        ...
        return ProtocolServerAdapter(listen_host, port_name, "gRPC protocol: " + server_address.toString(),
    ...
    if (server_type.shouldStart(ServerType::Type::PROMETHEUS)) {
      ...
      createServer(config, listen_host, port_name, listen_try, start_servers, servers, [&](UInt16 port) -> ProtocolServerAdapter {
        ...
        socket.setSendTimeout(settings[Setting::http_send_timeout]);
        return ProtocolServerAdapter(
            listen_host, port_name, "Prometheus: http://" + address.toString(),
            std::make_unique<HTTPServer>(httpContext(), createHandlerFactory(*this, config, async_metrics, handler_name), server_pool,
                                         socket, http_params, nullptr, ProfileEvents::InterfacePrometheusReceiveBytes,
...
void Server::createInterserverServers(Poco::Util::AbstractConfiguration &config, const ServerSettings &server_settings,
                                      const Strings &interserver_listen_hosts, bool listen_try, Poco::ThreadPool &server_pool,
                                      AsynchronousMetrics &async_metrics, std::vector<ProtocolServerAdapter> &servers, bool start_servers,
  ...
  for (const auto &interserver_listen_host : interserver_listen_hosts) {
    ...
    if (server_type.shouldStart(ServerType::Type::INTERSERVER_HTTP)) {
      ...
      createServer(
          config, interserver_listen_host, port_name, listen_try, start_servers, servers, [&](UInt16 port) -> ProtocolServerAdapter {
            Poco::Net::ServerSocket socket;
            auto address = socketBindListen(server_settings, socket, interserver_listen_host, port);
            socket.setReceiveTimeout(settings[Setting::http_receive_timeout]);
            socket.setSendTimeout(settings[Setting::http_send_timeout]);
            return ProtocolServerAdapter(
                interserver_listen_host, port_name, "replica communication (interserver): http://" + address.toString(),
                std::make_unique<HTTPServer>(httpContext(),
                                             createHandlerFactory(*this, config, async_metrics, "InterserverIOHTTPHandler-factory"),
                                             server_pool, socket, http_params, nullptr, ProfileEvents::InterfaceInterserverReceiveBytes,
    ...
    if (server_type.shouldStart(ServerType::Type::INTERSERVER_HTTPS)) {
      ...
      createServer(
          config, interserver_listen_host, port_name, listen_try, start_servers, servers, [&](UInt16 port) -> ProtocolServerAdapter {
            ...
            socket.setSendTimeout(settings[Setting::http_send_timeout]);
            return ProtocolServerAdapter(
                interserver_listen_host, port_name, "secure replica communication (interserver): https://" + address.toString(),
                std::make_unique<HTTPServer>(httpContext(),
...
void Server::stopServers(std::vector<ProtocolServerAdapter> &servers, const ServerType &server_type) const {
  ...
  auto check_server = [&log](const char prefix[], auto &server) {
    if (!server.isStopping()) return false;
    size_t current_connections = server.currentConnections();
    LOG_DEBUG(log, "Server {}{}: {} ({} connections)", server.getDescription(), prefix, !current_connections ? "finished" : "waiting",
              current_connections);
  ...
  for (auto &server : servers) {
    if (!server.isStopping()) {
      ...
      if (server_type.shouldStop(server_port_name)) server.stop();
    }
  }
...
void Server::updateServers(Poco::Util::AbstractConfiguration &config, const ServerSettings &server_settings, Poco::ThreadPool &server_pool,
                           AsynchronousMetrics &async_metrics, std::vector<ProtocolServerAdapter> &servers,
                           std::vector<ProtocolServerAdapter> &servers_to_start_before_tables) {
  LoggerRawPtr log = &logger();
  ...
  for (auto &server : servers) all_servers.push_back(&server);

  for (auto &server : servers_to_start_before_tables) all_servers.push_back(&server);
  ...
  for (auto *server : all_servers) {
    if (server->supportsRuntimeReconfiguration() && !server->isStopping()) {
      ...
      if (port_name.starts_with("protocols.")) {
        ...
        while (true) {
          if (config.has(prefix + "type")) {
            ...
            if (type == "http") {
              is_http = true;
              if (config.has(conf_name + ".handlers")) handlers_key = config.getString(conf_name + ".handlers");
              break;
            }
          }
          ...
          if (!pset.insert(conf_name).second)
            throw Exception(ErrorCodes::INVALID_CONFIG_PARAMETER, "Protocol '{}' configuration contains a loop on '{}'", protocol,
                            conf_name);
        }
      } else {
      ...
      if (!has_host) has_host = std::find(listen_hosts.begin(), listen_hosts.end(), server->getListenHost()) != listen_hosts.end();
      ...
      if (force_restart) LOG_TRACE(log, "<{}> had been changed, will reload {}", handlers_key, server->getDescription());
  ...
  createInterserverServers(config, server_settings, interserver_listen_hosts, listen_try, server_pool, async_metrics,
                           servers_to_start_before_tables, /* start_servers= */ true);
