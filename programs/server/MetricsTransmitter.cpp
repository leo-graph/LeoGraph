...
MetricsTransmitter::MetricsTransmitter(const Poco::Util::AbstractConfiguration& config, const std::string& config_name_,
...
void MetricsTransmitter::run() {
  ...
  while (true) {
    if (cond.wait_until(lock, get_next_time(interval_seconds), [this] { return quit; })) break;
...
void MetricsTransmitter::transmit(std::vector<ProfileEvents::Count>& prev_counters) {
  ...
  if (!key_vals.empty()) BaseDaemon::instance().writeToGraphite(key_vals, config_name);
}
