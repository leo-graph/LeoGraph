...
Client::Client() { fuzzer = QueryFuzzer(randomSeed(), &std::cout, &std::cerr); }
...
void Client::showWarnings() {
  try {
    ...
    if (!messages.empty()) {
      ...
      for (const auto &message : messages) output_stream << " * " << message << std::endl;
...
std::vector<String> Client::loadWarningMessages() {
  ...
  if (server_revision < min_server_revision_to_load_warnings) return {};
  ...
  connection->sendQuery(connection_parameters.timeouts,
                        ...
                        {} /* query_parameters */, "" /* query_id */, QueryProcessingStage::Complete, &client_context->getSettingsRef(),
    ...
    switch (packet.type) {
      case Protocol::Server::Data:
        if (!packet.block.empty()) {
          ...
          for (size_t i = 0; i < rows; ++i) messages.emplace_back(column[i].safeGet<String>());
        }
        continue;
...
Poco::Util::LayeredConfiguration &Client::getClientConfiguration() { return config(); }
...
void Client::initialize(Poco::Util::Application &self) {
  ...
  if (config_path.has_value()) {
    ...
    if (config().has("connection")) connection_name.emplace(config().getString("connection"));
    ...
    if (overrides.hostname.has_value()) configuration.setString("host", overrides.hostname.value());
    if (overrides.port.has_value()) configuration.setInt("port", overrides.port.value());
    ...
    if (overrides.user.has_value()) configuration.setString("user", overrides.user.value());
    if (overrides.password.has_value()) configuration.setString("password", overrides.password.value());
    if (overrides.database.has_value()) configuration.setString("database", overrides.database.value());
    if (overrides.history_file.has_value()) {
      auto history_file = overrides.history_file.value();
      if (history_file.starts_with("~/") && !home_path.empty()) history_file = home_path / history_file.substr(2);
      configuration.setString("history_file", history_file);
    }
    if (overrides.history_max_entries.has_value()) configuration.setUInt("history_max_entries", overrides.history_max_entries.value());
    ...
    if (overrides.prompt.has_value()) configuration.setString("prompt", overrides.prompt.value());

    config().add(loaded_config.configuration);

#if USE_JWT_CPP && USE_SSL
    /// If config file has user/password credentials, don't use auto-detected OAuth login for cloud endpoints
    if (login_was_auto_added && (loaded_config.configuration->has("user") || loaded_config.configuration->has("password"))) {
  ...
  if (env_user && !config().has("user")) config().setString("user", env_user);

  const char *env_password = getenv("CLICKHOUSE_PASSWORD");  // NOLINT(concurrency-mt-unsafe)
  if (env_password && !config().has("password")) config().setString("password", env_password);

  if (env_host && !config().has("host")) config().setString("host", env_host);

  /// settings and limits could be specified in config file, but passed settings has higher priority
  for (const auto &setting : client_context->getSettingsRef().getUnchangedNames()) {
    String name{setting};
    if (config().has(name)) client_context->setSetting(name, config().getString(name));
  }
  ...
  if (!config().has("no-warnings") && !config().getBool("warnings", true)) config().setBool("no-warnings", true);
}

int Client::main(const std::vector<std::string> & /*args*/) try {
  setupSignalHandler();
  ...
  for (;;) {
    try {
      ...
      bool should_ask_password = !asked_password && is_interactive &&
                                 ...
                                 !config().has("password") && !config().getBool("ask-password", false) && !config().has("ssh-key-file");
      ...
      if (should_ask_2fa) {
        asked_2fa = true;
        if (!connection_parameters.password.empty()) config().setString("password", connection_parameters.password);
  ...
  if (is_interactive && !config().has("no-warnings")) showWarnings();
...
void Client::connect() {
  ...
  for (size_t attempted_address_index = 0; attempted_address_index < hosts_and_ports.size(); ++attempted_address_index) {
    try {
      ...
      connection_parameters = ConnectionParameters(config(), host, database, hosts_and_ports[attempted_address_index].port);
      ...
      connection->getServerVersion(connection_parameters.timeouts, server_name, server_version_major, server_version_minor,
      ...
      if (e.code() == ErrorCodes::AUTHENTICATION_FAILED || e.code() == ErrorCodes::REQUIRED_PASSWORD) throw;

      if (attempted_address_index == hosts_and_ports.size() - 1) throw;
  ...
  if (server_display_name.empty()) server_display_name = config().getString("host", "localhost");
  ...
  /// - directly (possible as CLI parameter or in client.xml as top-level <prompt>...</prompt> or within client.xml's connection
  /// credentials)
  ...
  if (config().has("prompt"))
    ...
    if (config().has("prompt_by_server_display_name.default")) prompt = config().getRawString("prompt_by_server_display_name.default");
  ...
  for (const auto &[key, value] : prompt_substitutions) boost::replace_all(prompt, "{" + key + "}", value);
...
void Client::printHelpMessage(const OptionsDescription &options_description) {
  ...
  if (options_description.external_description.has_value()) output_stream << options_description.external_description.value() << "\n";
...
void Client::addExtraOptions(OptionsDescription &options_description) {
  ...
  options_description.main_description->add_options()("config,c", po::value<std::string>(), "config-file path (another shorthand)")(
      ...
      "secure,s", "Use TLS connection")("tls-sni-override", po::value<std::string>(),
                                        "Override the SNI host name used for TLS connections")("no-secure", "Don't use TLS connection")(
      "user,u", po::value<std::string>()->default_value("default"), "user")("password", po::value<std::string>(), "password")(
      "ask-password", "ask-password")("ssh-key-file", po::value<std::string>(),
      ...
      "jwt", po::value<std::string>(), "Use JWT for authentication")("one-time-password", po::value<std::string>(),
                                                                     "Time-based one-time password (TOTP) for two-factor authentication")
#if USE_JWT_CPP && USE_SSL
      ("login", po::bool_switch(), "Use OAuth 2.0 to login")("oauth-url", po::value<std::string>(),
                                                             "The base URL for the OAuth 2.0 authorization server")(
          ...
          ("max_client_network_bandwidth", po::value<int>(),
           "the maximum speed of data exchange over the network for the client in bytes per second.")(
              ...
              "opentelemetry-traceparent", po::value<std::string>(),
              "OpenTelemetry traceparent header as described by W3C Trace Context recommendation")(
              "opentelemetry-tracestate", po::value<std::string>(),
              "OpenTelemetry tracestate header as described by W3C Trace Context recommendation")(
              "no-warnings", "disable warnings when client connects to server")("no-server-client-version-message",
                                                                                "suppress server-client version mismatch message")
      /// TODO: Left for compatibility as it's used in upgrade check, remove after next release and use server setting
  ...
  options_description.external_description->add_options()(
      ...
      "file", po::value<std::string>(), "data file or - for stdin")("name", po::value<std::string>()->default_value("_data"),
                                                                    "name of the table")(
      "format", po::value<std::string>()->default_value("TabSeparated"), "data format")("structure", po::value<std::string>(), "structure")(
  ...
  options_description.hosts_and_ports_description->add_options()(
      ...
      "if there is no such host, the port will be attached to the next first host or to default host.")("port", po::value<UInt16>(),
                                                                                                        "server ports");
}

void Client::processOptions(const OptionsDescription &options_description, const CommandLineOptions &options,
  ...
  for (size_t i = 0; i < external_tables_arguments.size(); ++i) {
    ...
    try {
      ...
      if (external_tables.back().file == "-") ++number_of_external_tables_with_stdin_source;
  ...
  if (options.contains("config")) config().setString("config-file", options["config"].as<std::string>());
  if (options.contains("connection")) config().setString("connection", options["connection"].as<std::string>());
  if (options.contains("interleave-queries-file"))
    interleave_queries_files = options["interleave-queries-file"].as<std::vector<std::string>>();
  if (options.contains("secure")) config().setBool("secure", true);
  if (options.contains("tls-sni-override")) config().setString("tls-sni-override", options["tls-sni-override"].as<std::string>());
  if (options.contains("no-secure")) config().setBool("no-secure", true);
  if (options.contains("user") && !options["user"].defaulted()) config().setString("user", options["user"].as<std::string>());
  if (options.contains("password")) config().setString("password", options["password"].as<std::string>());
  if (options.contains("ask-password")) config().setBool("ask-password", true);
  if (options.contains("one-time-password")) config().setString("one-time-password", options["one-time-password"].as<std::string>());
  if (options.contains("ssh-key-file")) config().setString("ssh-key-file", options["ssh-key-file"].as<std::string>());
  if (options.contains("ssh-key-passphrase")) config().setString("ssh-key-passphrase", options["ssh-key-passphrase"].as<std::string>());
  if (options.contains("quota_key")) config().setString("quota_key", options["quota_key"].as<std::string>());
  if (options.contains("max_client_network_bandwidth")) max_client_network_bandwidth = options["max_client_network_bandwidth"].as<int>();
  if (options.contains("compression")) config().setBool("compression", options["compression"].as<bool>());
  if (options.contains("no-warnings")) config().setBool("no-warnings", true);
  if (options.contains("no-server-client-version-message")) config().setBool("no-server-client-version-message", true);
  if (options.contains("fake-drop")) config().setString("ignore_drop_queries_probability", "1");
  if (options.contains("jwt")) {
    if (!options["user"].defaulted()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "User and JWT flags can't be specified together");
    config().setString("jwt", options["jwt"].as<std::string>());
    config().setString("user", "");
  }
#if USE_JWT_CPP && USE_SSL
  if (options["login"].as<bool>()) {
    if (!options["user"].defaulted()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "User and login flags can't be specified together");
    if (config().has("jwt")) throw Exception(ErrorCodes::BAD_ARGUMENTS, "JWT and login flags can't be specified together");
    config().setBool("login", true);
    config().setString("user", "");
  }
  if (options.contains("oauth-url")) config().setString("oauth-url", options["oauth-url"].as<std::string>());
  if (options.contains("oauth-client-id")) config().setString("oauth-client-id", options["oauth-client-id"].as<std::string>());
  if (options.contains("oauth-audience")) config().setString("oauth-audience", options["oauth-audience"].as<std::string>());
#endif
...
void Client::processConfig() {
  ...
  if (stdin_is_a_tty && (delayed_interactive || (queries.empty() && queries_files.empty()))) {
    ...
    if (!query_id.empty()) client_context->setCurrentQueryId(query_id);
  }
...
void Client::readArguments(int argc, char **argv, Arguments &common_arguments, std::vector<Arguments> &external_tables_arguments,
    ...
    if (!first_arg.starts_with('-')) {
      ...
      if (isCloudEndpoint(hostname) && !has_auth_in_connection_string) {
        ...
        for (int i = 1; i < argc; ++i) {
          ...
          if (arg.starts_with("--user") || arg.starts_with("--password") || arg.starts_with("--jwt") || arg.starts_with("--ssh-key-file") ||
              arg == "-u") {
        ...
        if (!port.empty()) host_and_port.push_back("--port=" + port);
  ...
  bool has_connection_string = !is_hostname_argument && argc >= 2 &&
                               tryParseConnectionString(std::string_view(argv[1]), common_arguments, hosts_and_ports_arguments);
  ...
  for (int arg_num = start_argument_index; arg_num < argc; ++arg_num) {
    ...
    if (has_connection_string || is_hostname_argument) checkIfCmdLineOptionCanBeUsedWithConnectionString(arg);
    ...
    else if (in_external_group && (arg.starts_with("--file=") || arg.starts_with("--name=") || arg.starts_with("--format=") ||
                                   arg.starts_with("--structure=") || arg.starts_with("--types="))) {
    ...
    /// Options with value after whitespace.
      ...
      if (arg.starts_with("--param_") || arg.starts_with("--param-")) {
        ...
        if (arg == "--host" || arg == "-h") {
          ++arg_num;
          if (arg_num >= argc) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Host argument requires value");
          arg = argv[arg_num];
          host_arg = "--host=";
          host_arg.append(arg);
        } else
        ...
        if (!prev_port_arg.empty()) {
          ...
          if (!prev_host_arg.empty()) hosts_and_ports_arguments.push_back({prev_host_arg});
        ...
        if (arg == "--port") {
          port_arg.push_back('=');
          ++arg_num;
          if (arg_num >= argc) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Port argument requires value");
          arg = argv[arg_num];
          port_arg.append(arg);
        }
        ...
        if (!prev_host_arg.empty()) {
          ...
          if (!prev_port_arg.empty()) hosts_and_ports_arguments.push_back({prev_port_arg});
