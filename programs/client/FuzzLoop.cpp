...
bool Client::processWithASTFuzzer(std::string_view full_query) {
  ...
  try {
    ...
    orig_ast = parseQuery(begin, begin + full_query.size(), client_context->getSettingsRef(),
                          /*allow_multi_statements=*/true);
  } catch (const Exception &e) {
    if (e.code() != ErrorCodes::SYNTAX_ERROR && e.code() != ErrorCodes::TOO_DEEP_RECURSION) throw;
  }
  ...
  if (auto *q = orig_ast->as<ASTSetQuery>()) {
    if (auto *set_dialect = q->changes.tryGet("dialect"); set_dialect && set_dialect->safeGet<String>() == "kusto") return true;
  }
  ...
  for (size_t fuzz_step = 0; fuzz_step < this_query_runs; ++fuzz_step) {
    ...
    try {
      ...
      if (!ast_to_process) fmt::print(stderr, "Error while forming new query: {}\n", getCurrentExceptionMessage(true));
...
static void runExternalCommand(std::unique_ptr<BuzzHouse::ExternalIntegrations> &external_integrations, const uint64_t seed,
                               const bool async, const String &engine, const String &cname, const String &tname) {
  ...
  if (fuzz_config->read_log) {
    ...
    while (server_up && (no_timeout = (!deadline || clock::now() < *deadline)) &&
      ...
      if (full_query == restart_cmd) {
        ...
        UNUSED(x);
      } else if (startsWith(full_query, external_cmd) &&
    ...
    BuzzHouse::RandomGenerator rg(fuzz_config->seed, fuzz_config->min_string_length, fuzz_config->max_string_length,
                                  fuzz_config->random_limited_values);
    ...
    while (server_up && (no_timeout = (!deadline || clock::now() < *deadline))) {
      ...
      if (total_create_database_tries < 20 && nsuccessfull_create_database < max_initial_databases) {
        gen.generateNextCreateDatabase(rg, sq1.mutable_single_query()->mutable_explain()->mutable_inner_query()->mutable_create_database());
      ...
      } else if (gen.collectionHas<std::shared_ptr<BuzzHouse::SQLDatabase>>(gen.attached_databases) && total_create_table_tries < 300 &&
                 nsuccessfull_create_table < max_initial_tables) {
        gen.generateNextCreateTable(rg, false,
                                    sq1.mutable_single_query()->mutable_explain()->mutable_inner_query()->mutable_create_table());
        BuzzHouse::SQLQueryToString(full_query, sq1);
        fuzz_config->outf << full_query << std::endl;
        server_up &= processBuzzHouseQuery(full_query);
        ...
        const uint32_t dump_oracle =
            10 * static_cast<uint32_t>(fuzz_config->allow_query_oracles && fuzz_config->use_dump_table_oracle > 0 &&
        ...
        const uint32_t roundtrip_oracle =
        ...
        const uint32_t prob_space = correctness_oracle + settings_oracle + dump_oracle + peer_oracle + roundtrip_oracle + restart_client +
        ...
        if (correctness_oracle && nopt < (correctness_oracle + 1)) {
          ...
          const bool test_content = fuzz_config->use_dump_table_oracle > 1 && rg.nextBool() &&
                                    gen.collectionHas<BuzzHouse::SQLTable>(gen.attached_tables_to_compare_content);
          const auto &tbl = rg.pickRandomly(gen.filterCollection<BuzzHouse::SQLTable>(test_content ? gen.attached_tables_to_compare_content
                                                                                                   : gen.attached_tables_to_test_format));
          ...
          } else if (alter_update_table && nopt2 < (optimize_table + reattach_table + backup_restore_table + alter_update_table + 1)) {
            strategy = BuzzHouse::DumpOracleStrategy::ALTER_UPDATE;
          } else if (insert_count_table &&
        ...
        } else if (roundtrip_oracle && nopt < (correctness_oracle + settings_oracle + dump_oracle + peer_oracle + roundtrip_oracle + 1)) {
          /// Roundtrip oracle: check that encode/decode and encrypt/decrypt preserve data
          const auto &tbl = rg.pickRandomly(gen.filterCollection<BuzzHouse::SQLTable>(gen.attached_tables_to_compare_content));
        ...
        } else if (restart_client &&
                   nopt < (correctness_oracle + settings_oracle + dump_oracle + peer_oracle + roundtrip_oracle + restart_client + 1)) {
          fuzz_config->outf << restart_cmd << std::endl;
          gen.setInTransaction(false);
          server_up &= fuzzLoopReconnect();
        } else if (external_call && nopt < (correctness_oracle + settings_oracle + dump_oracle + peer_oracle + roundtrip_oracle +
                                            restart_client + external_call + 1)) {
          const uint64_t nseed = rg.nextInFullRange();
          const auto &tbl = rg.pickRandomly(gen.filterCollection<BuzzHouse::SQLTable>(gen.attached_tables_for_external_call)).get();
          ...
          fuzz_config->outf << external_cmd << (async ? "async " : "") << "with seed " << nseed << " to " << engine << " table " << ndname
                            << "." << ntname << std::endl;
          runExternalCommand(external_integrations, nseed, async, engine, ndname, ntname);
        } else if (health_check && nopt < (correctness_oracle + settings_oracle + dump_oracle + peer_oracle + roundtrip_oracle +
                                           restart_client + external_call + health_check + 1)) {
          fuzz_config->outf << health_check_cmd << std::endl;
          fuzz_config->validateClickHouseHealth();
        } else if (run_query && nopt < (correctness_oracle + settings_oracle + dump_oracle + peer_oracle + roundtrip_oracle +
                                        restart_client + external_call + health_check + run_query + 1)) {
...
#else
bool Client::buzzHouse() { throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Clickhouse was compiled without BuzzHouse enabled"); }
#endif
