...
int mainEntryClickHouseFormat(int argc, char** argv) {
  ...
  try {
    ...
    desc.add_options()("query", po::value<std::string>(), "query to format")("help,h", "produce help message")(
        "comments", "keep comments in the output")("hilite,highlight",
                                                   "add syntax highlight with ANSI terminal escape sequences (can also use --highlight)")(
        "oneline", "format in single line")("max_line_length", po::value<size_t>()->default_value(0),
                                            "format in single line queries with length less than specified")(
        "quiet,q", "just check syntax, no output on success")("multiquery,n", "allow multiple queries in the same file")(
        "obfuscate", "obfuscate instead of formatting")("backslash", "add a backslash at the end of each line of the formatted query")(
    ...
    if (options.contains("comments")) comments_callback = [](const std::string_view comment) { std::cout << comment << '\n'; };
    ...
    if (obfuscate) {
      ...
      for (auto* it = auto_time_zones; *it; ++it) {
        ...
        for (const auto& word : split)
          if (!word.empty()) additional_names.insert(word);
      }
      ...
      for (const auto& name : additional_names) additional_names_lowercase.insert(Poco::toLower(name));

      KnownIdentifierFunc is_known_identifier = [&](std::string_view name) {
        std::string what(name);

        if (FunctionFactory::instance().has(what) || AggregateFunctionFactory::instance().isAggregateFunctionName(what) ||
            TableFunctionFactory::instance().isTableFunctionName(what) || FormatFactory::instance().isOutputFormat(what) ||
            FormatFactory::instance().isInputFormat(what) || additional_names.contains(what))
          return true;
      ...
      while (pos != end) {
        ...
        ASTPtr res = parseQueryAndMovePosition(parser, pos, end, "query", multiple, cmd_settings[Setting::max_query_size],
                                               cmd_settings[Setting::max_parser_depth], cmd_settings[Setting::max_parser_backtracks]);
        ...
        if (!quiet) {
          if (!backslash) {
...
#if USE_REPLXX
            if (hilite) formatted_query = highlighted(formatted_query, *context);
#endif
            ...
            while (s_end > s_pos && isWhitespaceASCIIOneLine(*(s_end - 1))) --s_end;
