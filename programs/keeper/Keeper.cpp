...
Poco::Net::SocketAddress Keeper::socketBindListen(Poco::Net::ServerSocket &socket, const std::string &host, UInt16 port,
...
void Keeper::createServer(const std::string &listen_host, const char *port_name, bool listen_try, CreateServerFunc &&func) const {
  ...
  if (!config().has(port_name)) return;
...
void Keeper::initialize(Poco::Util::Application &self) {
  ConfigProcessor::registerEmbeddedConfig(
      "keeper_config.xml",
      std::string_view(reinterpret_cast<const char *>(keeper_resource_embedded_xml), std::size(keeper_resource_embedded_xml)));

  BaseDaemon::initialize(self);
  logger().information("starting up");

  LOG_INFO(&logger(), "OS Name = {}, OS Version = {}, OS Architecture = {}", Poco::Environment::osName(), Poco::Environment::osVersion(),
           Poco::Environment::osArchitecture());
}

std::string Keeper::getDefaultConfigFileName() const { return "keeper_config.xml"; }

bool Keeper::allowTextLog() const { return false; }
...
void Keeper::defineOptions(Poco::Util::OptionSet &options) {
  ...
  options.addOption(Poco::Util::Option("force-recovery", "force-recovery",
                                       "Force recovery mode allowing Keeper to overwrite cluster configuration without quorum")
                        .required(false)
                        .repeatable(false)
                        .noArgument()
...
HTTPContextPtr httpContext() { return std::make_shared<KeeperHTTPContext>(Context::getGlobalContextInstance()); }
...
String getKeeperPath(Poco::Util::LayeredConfiguration &config) {
  ...
  if (config.has("keeper_server.storage_path")) {
    ...
    throw Exception(ErrorCodes::NO_ELEMENTS_IN_CONFIG,
                    "By default 'keeper_server.storage_path' could be assigned to {}, but the directory {} already exists. Please specify "
                    "'keeper_server.storage_path' in the keeper configuration explicitly",
...
int Keeper::main(const std::vector<std::string> & /*args*/) try {
  ...
  if (auto total_numa_memory = getNumaNodesTotalMemory(); total_numa_memory.has_value()) {
    LOG_INFO(log, "Keeper is bound to a subset of NUMA nodes. Total memory of all available nodes: {}", ReadableSize(*total_numa_memory));
  }
  ...
  GlobalThreadPool::initialize(
      ...
      std::min(1000U, config().getUInt("max_thread_pool_size", 1000)), config().getUInt("max_thread_pool_free_size", 100),
  ...
  for (const auto &listen_host : listen_hosts) {
    ...
    createServer(listen_host, port_name, listen_try, [&](UInt16 port) {
      ...
      servers->emplace_back(listen_host, port_name, "Keeper (tcp): " + address.toString(),
                            std::make_unique<TCPServer>(new KeeperTCPHandlerFactory(config_getter, global_context->getKeeperDispatcher(),
                                                                                    tcp_receive_timeout, tcp_send_timeout, false),
                                                        server_pool, socket, tcp_params));
    });
    ...
    createServer(listen_host, secure_port_name, listen_try, [&](UInt16 port) {
      ...
      servers->emplace_back(listen_host, secure_port_name, "Keeper with secure protocol (tcp_secure): " + address.toString(),
                            std::make_unique<TCPServer>(new KeeperTCPHandlerFactory(config_getter, global_context->getKeeperDispatcher(),
    ...
    createServer(listen_host, port_name, listen_try, [&, my_http_context = std::move(http_context)](UInt16 port) mutable {
      ...
      servers->emplace_back(
          ...
          std::make_unique<HTTPServer>(std::move(my_http_context),
    ...
    createServer(listen_host, port_name, listen_try, [&](UInt16 port) mutable {
      ...
      socket.setSendTimeout(my_http_context->getSendTimeout());
      servers->emplace_back(
          listen_host, port_name, "HTTPS Control: https://" + address.toString(),
          std::make_unique<HTTPServer>(
              std::move(my_http_context),
  ...
  auto main_config_reloader = std::make_unique<ConfigReloader>(config_path, extra_paths, getKeeperPath(config()),
                                                               /* zk_node_cache_= */ nullptr, unused_event,
...
void Keeper::logRevision() const {
  LOG_INFO(getLogger("Application"), "Starting ClickHouse Keeper {} (revision: {}, git hash: {}, build id: {}), PID {}", VERSION_STRING,
           ClickHouseRevision::getVersionRevision(), GIT_HASH, build_id.empty() ? "<unknown>" : build_id, getpid());
}
