...
class Benchmark : public Poco::Util::Application {
 public:
  Benchmark(const boost::program_options::variables_map &options, String &&proto_send_chunked, String &&proto_recv_chunked,
            bool print_stacktrace_, Settings &&settings_)
  ...
  void initialize(Poco::Util::Application &self [[maybe_unused]]) override {
    ...
    if (connection_name.has_value()) {
      ...
      if (overrides.user.has_value() && !connection_arguments.user.has_value()) connection_arguments.user.emplace(overrides.user.value());
      ...
      if (connection_arguments.hosts.has_value()) {
        if (isCloudEndpoint(connection_arguments.hosts->front())) connection_arguments.secure.emplace(true);
      }
    }
 ...
 private:
  ...
  struct IntervalStats {
    ...
    IntervalStats(Benchmark &benchmark_, UInt64 start_ns_) : benchmark(benchmark_), start_ns(start_ns_) {}

    ~IntervalStats() { report(); }
    ...
    void report() {
      if (reported || end_ns == std::numeric_limits<UInt64>::max()) return;
      reported = true;
  ...
  void makeConnections() {
    ...
    if (env_user != nullptr) env_user_str.emplace(std::string(env_user));

    const char *env_password = getenv("CLICKHOUSE_PASSWORD");  // NOLINT(concurrency-mt-unsafe)
    if (env_password != nullptr) env_password_str.emplace(std::string(env_password));

    const char *env_host = getenv("CLICKHOUSE_HOST");  // NOLINT(concurrency-mt-unsafe)
    if (env_host != nullptr) env_host_str.emplace(std::string(env_host));

    const char *env_quota_key = getenv("CLICKHOUSE_QUOTA_KEY");  // NOLINT(concurrency-mt-unsafe)
    if (env_quota_key != nullptr) env_quota_key_str.emplace(std::string(env_quota_key));
    ...
    for (size_t i = 0; i < connections_cnt; ++i) {
      ...
      connections.emplace_back(std::make_unique<ConnectionPool>(
          max_concurrency, cur_host, cur_port, connection_arguments.database.value_or("default"),
          connection_arguments.user.value_or(env_user_str.value_or("default")),
          connection_arguments.password.value_or(env_password_str.value_or("")), connection_arguments.proto_send_chunked,
          connection_arguments.proto_recv_chunked, connection_arguments.quota_key.value_or(env_quota_key_str.value_or("")),
          /* cluster_= */ "",
          /* cluster_secret_= */ "",
          /* client_name_= */ std::string(DEFAULT_CLIENT_NAME), Protocol::Compression::Enable, secure,
          /* bind_host_= */ ""));

      if (!round_robin || total_stats.empty()) total_stats.emplace_back(std::make_shared<Stats>());
    }
  ...
  void readQueries() {
    if (query_to_execute.empty()) {
      ...
      while (!in.eof()) {
        ...
        if (!query.empty()) queries.emplace_back(std::move(query));
      }

      if (queries.empty()) throw Exception(ErrorCodes::EMPTY_DATA_PASSED, "Empty list of queries.");
    } else {
  ...
  bool tryPushQueryInteractively(const String &query, InterruptListener &interrupt_listener) {
    ...
    if (delay > 0 && delay_watch.elapsedSeconds() > delay) {
      ...
      if (concurrency < max_concurrency) {
        if (threads == max_concurrency) return false;  // Reached maximum concurrency
        try {
  ...
  void startNextInterval(bool first = false) {
    ...
    if (cumulative) {
      if (!first) report(total_stats, total_watch.elapsedSeconds(), threads);
    } else {
      // Report previous interval (if any)
      if (interval) {
        interval->close(now_ns, threads);
        if (!precise) interval->report();
      }
      ...
      for (size_t i = 0; i < (round_robin ? 1 : connections.size()); ++i) next_interval->stats.emplace_back(std::make_shared<Stats>());
      if (interval) interval->next = next_interval;
  ...
  void runBenchmark() {
    ...
    try {
      for (size_t i = 0; i < concurrency; ++i) pool.scheduleOrThrowOnError([this]() mutable { thread(); });
    } catch (...) {
      shutdown = true;
      pool.wait();
      throw;
  ...
  void thread() {
    ...
    while (true) {
      ...
      while (!extracted) {
        ...
        if (shutdown || (max_iterations && queries_executed == max_iterations)) return;
      }
      ...
      try {
        ...
        size_t info_index = round_robin ? 0 : connection_index;
        {
          ...
          log << getCurrentExceptionMessage(print_stacktrace, true /*check embedded stack trace*/) << "\n\n" << flush;

          ++total_stats[info_index]->errors;
        }

        std::lock_guard lock(interval_mutex);  // Locking order: first intervals_mutex, then mutex
        if (interval) ++interval->stats[info_index]->errors;
      }
  ...
  void execute(const Query &query, size_t connection_index) {
    ...
    if (should_reconnect) entry->disconnect();

    RemoteQueryExecutor executor(*entry, query, std::make_shared<const Block>(), global_context, nullptr, Scalars(), Tables(),
    ...
    ProfileInfo info;
    for (Block block = executor.readBlock(); !block.empty(); block = executor.readBlock()) info.update(block);

    executor.finish();

    watch.stop();
    double duration =
        (display_client_side_time || progress.elapsed_ns == 0) ? watch.elapsedSeconds() : static_cast<double>(progress.elapsed_ns) / 1e9;
    size_t info_index = round_robin ? 0 : connection_index;
    ...
    if (precise && cur_interval) {
      ...
      for (; cur_interval; cur_interval = cur_interval->next) {
        if (cur_interval->end_ns <= watch.getStart()) continue;
        if (cur_interval->start_ns >= watch.getEnd()) break;
        const double overlap_ns =
            static_cast<double>(std::min(cur_interval->end_ns, watch.getEnd()) - std::max(cur_interval->start_ns, watch.getStart()));
  ...
  void report(const MultiStats &infos, double seconds, size_t used_threads = 0) {
    ...
    for (int percent = 0; percent <= 90; percent += 10) print_percentile(percent);
 ...
 public:
  ~Benchmark() override { shutdown = true; }
};
...
int mainEntryClickHouseBenchmark(int argc, char **argv) {
  ...
  try {
    ...
    options_description.add_options()("help", "Print usage summary and exit; combine with --verbose to display all options")(
        "verbose", "Increase output verbosity")("query,q", value<std::string>()->default_value(""), "query to execute")(
        "concurrency,c", value<unsigned>()->default_value(1), "number of parallel queries")(
        "max_concurrency,C", value<unsigned>()->default_value(0),
        "gradually increase number of parallel queries up to specified value, making one report for every concurrency level")(
        "delay,d", value<double>()->default_value(1), "delay between intermediate reports in seconds (set 0 to disable reports)")(
        "precise", "enable precise per-interval reporting with weighted metrics")(
        "stage", value<std::string>()->default_value("complete"),
        "request query processing up to specified stage: "
        "complete,fetch_columns,with_mergeable_state,with_mergeable_state_after_aggregation,with_mergeable_state_after_aggregation_and_"
        "limit")("iterations,i", value<size_t>()->default_value(0), "amount of queries to be executed")(
        "timelimit,t", value<double>()->default_value(0.), "stop launch of queries after specified time limit")(
        "randomize,r", "randomize order of execution")("host,h", value<Strings>()->multitoken(), "list of hosts")(
        "port", value<Ports>()->multitoken(), "list of ports")("connection", value<std::string>(),
                                                               "connection name (define under connections_credentials in client config)")(
        "roundrobin",
        "Instead of comparing queries for different --host/--port just pick one random --host/--port for every query and send query to "
        "it.")("cumulative", "prints cumulative data instead of data per interval")("secure,s", "Use TLS connection")(
        ...
        "user,u", value<std::string>(), "")("password", value<std::string>(), "")("quota_key", value<std::string>(), "")(
        "database", value<std::string>(), "")("config", value<std::string>(), "Path to client config")("stacktrace",
                                                                                                       "print stack traces of exceptions")(
        "confidence", value<size_t>()->default_value(5),
        "set the level of confidence for T-test [0=80%, 1=90%, 2=95%, 3=98%, 4=99%, 5=99.5%(default)")(
        "query_id", value<std::string>()->default_value(""), "")("query_id_prefix", value<std::string>()->default_value(""), "")(
        ...
        "reconnect", value<size_t>()->default_value(0),
        "control reconnection behaviour: 0 (never reconnect), 1 (reconnect for every query), or N (reconnect after every N queries)")(
        "client-side-time",
        "display the time including network communication instead of server-side time; note that for server versions before 22.8 we always "
        "display client-side time")("proto_caps", value<std::string>(),
                                    "Enable/disable chunked protocol (comma-separated): chunked_optional, notchunked, notchunked_optional, "
                                    "send_chunked, send_chunked_optional, send_notchunked, send_notchunked_optional, recv_chunked, "
                                    "recv_chunked_optional, recv_notchunked, recv_notchunked_optional");
    ...
    for (const auto &op : parsed.options) {
      ...
      if (op.string_key.empty() && op.original_tokens[0].starts_with("--")) break;

      if (!op.unregistered && op.string_key.empty() && !op.original_tokens[0].starts_with("--") && !op.original_tokens[0].empty() &&
          !op.value.empty()) {
    ...
    Benchmark benchmark(options, std::move(proto_send_chunked), std::move(proto_recv_chunked), print_stacktrace, std::move(settings));
