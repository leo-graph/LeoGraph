...
static UInt64 maskBits(UInt64 x, size_t num_bits) { return x & ((1ULL << num_bits) - 1); }
...
static UInt64 feistelNetwork(UInt64 x, size_t num_bits, UInt64 seed, size_t num_rounds = 4) {
  ...
  for (size_t i = 0; i < num_rounds; ++i) bits = feistelRound(bits, num_bits, seed, i);
...
class UnsignedIntegerModel : public IModel {
 ...
 public:
  ...
  ColumnPtr generate(const IColumn &column) override {
    ...
    for (size_t i = 0; i < size; ++i) res->insert(transform(column.getUInt(i), seed));

    return res;
  }

  void updateSeed() override { seed = hash(seed); }
};
...
static Int64 transformSigned(Int64 x, UInt64 seed) {
  if (x >= 0) return transform(x, seed);
...
class SignedIntegerModel : public IModel {
 ...
 public:
  ...
  ColumnPtr generate(const IColumn &column) override {
    ...
    for (size_t i = 0; i < size; ++i) res->insert(transformSigned(column.getInt(i), seed));
...
class IdentityModel : public IModel {
 public:
  ...
  ColumnPtr generate(const IColumn &column) override { return column.cloneResized(column.size()); }
...
static void transformUUID(const UUID &src_uuid, UUID &dst_uuid, UInt64 seed) {
  ...
  UUIDHelpers::getLowBytes(dst_uuid) =
      (UUIDHelpers::getLowBytes(dst_uuid) & (0xffffffffffffffffull - trace[1])) | (UUIDHelpers::getLowBytes(src_uuid) & trace[1]);
  UUIDHelpers::getHighBytes(dst_uuid) =
      (UUIDHelpers::getHighBytes(dst_uuid) & (0xffffffffffffffffull - trace[0])) | (UUIDHelpers::getHighBytes(src_uuid) & trace[0]);
}
...
class FixedStringModel : public IModel {
 ...
 public:
  ...
  ColumnPtr generate(const IColumn &column) override {
    ...
    for (size_t i = 0; i < size; ++i) transformFixedString(&src_data[i * string_size], &res_data[i * string_size], string_size, seed);
...
class UUIDModel : public IModel {
 ...
 public:
  ...
  ColumnPtr generate(const IColumn &column) override {
    ...
    res_data.resize(src_data.size());
    for (size_t i = 0; i < src_column.size(); ++i) transformUUID(src_data[i], res_data[i], seed);
...
class MarkovModel {
 private:
  ...
  struct Histogram {
    ...
    CodePoint sample(UInt64 random, double end_multiplier) const {
      ...
      for (const auto &elem : buckets) {
        sum += elem.second;
        if (sum > random) return elem.first;
      }
  ...
  static bool writeCodePoint(CodePoint code, char *&pos, const char *end) {
    size_t length = (code & 0xFF000000) ? 4 : (code & 0xFFFF0000) ? 3 : (code & 0xFFFFFF00) ? 2 : 1;
 ...
 public:
  explicit MarkovModel(MarkovModelParameters params_) : params(std::move(params_)), code_points(params.order, BEGIN) {}
  ...
  void consume(const char *data, size_t size) {
    ...
    while (true) {
      ...
      if (inside) next_code_point = readCodePoint(pos, end);

      for (size_t context_size = 0; context_size < params.order; ++context_size) {
        NGramHash context_hash =
            hashContext(code_points.data() + code_points.size() - context_size, code_points.data() + code_points.size());
  ...
  void finalize() {
    ...
    if (params.frequency_add) {
      for (auto &elem : table) {
        ...
        for (auto &bucket : histogram.buckets) bucket.second += params.frequency_add;
    ...
    if (params.frequency_desaturate > 0.0) {
      for (auto &elem : table) {
        ...
        for (auto &bucket : histogram.buckets) {
          bucket.second = static_cast<UInt64>(static_cast<double>(bucket.second) * (1.0 - params.frequency_desaturate) +
                                              average * params.frequency_desaturate);
  ...
  size_t generate(char *data, size_t desired_size, size_t buffer_size, UInt64 seed, const char *determinator_data,
                  size_t determinator_size) {
    code_points.resize(params.order);
    ...
    while (pos < end) {
      ...
      while (true) {
        ...
        if (it && it->getMapped().total + it->getMapped().count_end != 0) break;

        if (context_size == 0) break;
        --context_size;
      }

      if (!it) throw Exception(ErrorCodes::LOGICAL_ERROR, "Logical error in markov model");
      ...
      size_t determinator_sliding_window_overflow = offset_from_begin_of_string + determinator_sliding_window_size > determinator_size
                                                        ? offset_from_begin_of_string + determinator_sliding_window_size - determinator_size
                                                        : 0;

      const char *determinator_sliding_window_begin =
          determinator_data + offset_from_begin_of_string - determinator_sliding_window_overflow;
      ...
      if (num_bytes_after_desired_size > 0) end_probability_multiplier = std::pow(1.25, num_bytes_after_desired_size);
      ...
      if (num_bytes_after_desired_size > 0) {
        ...
        if (code < 128 && !isAlphaNumericASCII(static_cast<char>(code))) break;
      }

      if (!writeCodePoint(code, pos, end)) break;
...
class StringModel : public IModel {
 ...
 public:
  ...
  void finalize() override { markov_model.finalize(); }
  ...
  ColumnPtr generate(const IColumn &column) override {
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      if (desired_string_size != 0)
        actual_size =
            markov_model.generate(new_string.data(), desired_string_size, new_string.size(), seed, src_string.data(), src_string.size());
  ...
  void serialize(WriteBuffer &out) const override { markov_model.serialize(out); }

  void deserialize(ReadBuffer &in) override { markov_model.deserialize(in); }
};
...
class ArrayModel : public IModel {
 ...
 public:
  ...
  void finalize() override { nested_model->finalize(); }
  ...
  void updateSeed() override { nested_model->updateSeed(); }

  void serialize(WriteBuffer &out) const override { nested_model->serialize(out); }

  void deserialize(ReadBuffer &in) override { nested_model->deserialize(in); }
};
 ...
 public:
  ModelPtr get(const IDataType &data_type, UInt64 seed, MarkovModelParameters markov_model_params) const {
    if (isInteger(data_type)) {
      if (isUInt(data_type)) return std::make_unique<UnsignedIntegerModel>(seed);

      return std::make_unique<SignedIntegerModel>(seed);
    }

    if (typeid_cast<const DataTypeFloat32 *>(&data_type)) return std::make_unique<FloatModel<Float32>>(seed);

    if (typeid_cast<const DataTypeFloat64 *>(&data_type)) return std::make_unique<FloatModel<Float64>>(seed);

    if (typeid_cast<const DataTypeDate *>(&data_type)) return std::make_unique<IdentityModel>();

    if (typeid_cast<const DataTypeDateTime *>(&data_type)) return std::make_unique<DateTimeModel>(seed);

    if (typeid_cast<const DataTypeString *>(&data_type)) return std::make_unique<StringModel>(seed, markov_model_params);

    if (typeid_cast<const DataTypeFixedString *>(&data_type)) return std::make_unique<FixedStringModel>(seed);

    if (typeid_cast<const DataTypeUUID *>(&data_type)) return std::make_unique<UUIDModel>(seed);
...
class Obfuscator {
 ...
 public:
  Obfuscator(const Block &header, UInt64 seed, MarkovModelParameters markov_model_params) {
    ...
    for (const auto &elem : header) models.emplace_back(factory.get(*elem.type, hash(seed, elem.name), markov_model_params));
  }

  void train(const Columns &columns) {
    size_t size = columns.size();
    for (size_t i = 0; i < size; ++i) models[i]->train(*columns[i]);
  }

  void finalize() {
    for (auto &model : models) model->finalize();
  }

  Columns generate(const Columns &columns) {
    size_t size = columns.size();
    Columns res(size);
    for (size_t i = 0; i < size; ++i) res[i] = models[i]->generate(*columns[i]);
    return res;
  }

  void updateSeed() {
    for (auto &model : models) model->updateSeed();
  }

  void serialize(WriteBuffer &out) const {
    for (const auto &model : models) model->serialize(out);
  }

  void deserialize(ReadBuffer &in) {
    for (auto &model : models) model->deserialize(in);
  }
};
...
int mainEntryClickHouseObfuscator(int argc, char **argv) try {
  using namespace DB;
  ...
  description.add_options()("help", "produce help message")("structure,S", po::value<std::string>(),
                                                            "structure of the initial table (list of column and type names)")(
      "input-format", po::value<std::string>(), "input format of the initial table data")("output-format", po::value<std::string>(),
                                                                                          "default output format")(
      "seed", po::value<std::string>(),
      "seed (arbitrary string), must be random string with at least 10 bytes length; note that a seed for each column is derived from this "
      "seed and a column name: you can obfuscate data for different tables and as long as you use identical seed and identical column "
      "names, the data for corresponding non-text columns for different tables will be transformed in the same way, so the data for "
      "different tables can be JOINed after obfuscation")(
      "limit", po::value<UInt64>(),
      "if specified - stop after generating that number of rows; the limit can be also greater than the number of source dataset - in this "
      "case it will process the dataset in a loop more than one time, using different seeds on every iteration, generating result as large "
      "as needed")("silent", po::value<bool>()->default_value(false), "don't print information messages to stderr")(
      "save", po::value<std::string>(),
      "save the models after training to the specified file. You can use --limit 0 to skip the generation step. The file is using binary, "
      "platform-dependent, opaque serialization format. The model parameters are saved, while the seed is not.")(
      "load", po::value<std::string>(),
      "load the models instead of training from the specified file. The table structure must match the saved file. The seed should be "
      "specified separately, while other model parameters are loaded.")("order", po::value<UInt64>()->default_value(5),
                                                                        "order of markov model to generate strings")(
      "frequency-cutoff", po::value<UInt64>()->default_value(5),
      "frequency cutoff for markov model: remove all buckets with count less than specified")(
      "num-buckets-cutoff", po::value<UInt64>()->default_value(0),
      "cutoff for number of different possible continuations for a context: remove all histograms with less than specified number of "
      "buckets")("frequency-add", po::value<UInt64>()->default_value(0),
                 "add a constant to every count to lower probability distribution skew")(
      "frequency-desaturate", po::value<double>()->default_value(0),
      "0..1 - move every frequency towards average to lower probability distribution skew")(
      "determinator-sliding-window-size", po::value<UInt64>()->default_value(8),
  ...
  if (options.contains("help") || !options.contains("seed") || !options.contains("input-format") || !options.contains("output-format")) {
    std::cout << documentation << "\n"
              ...
              << description << "\n"
              << "\nExample:\n    " << argv[0]
              << " --seed \"$(head -c16 /dev/urandom | base64)\" --input-format TSV --output-format TSV --structure 'CounterID UInt32, "
                 "URLDomain String, URL String, SearchPhrase String, Title String' < stats.tsv\n";
    return 0;
  }
  ...
  if (options.contains("structure")) structure = options["structure"].as<std::string>();
  ...
  UInt64 limit = 0;
  if (options.contains("limit")) limit = options["limit"].as<UInt64>();
  ...
  if (structure.empty()) {
    ...
    if (-1 == res) throw ErrnoException(ErrorCodes::CANNOT_SEEK_THROUGH_FILE, "Input must be seekable file (it will be read twice)");
  ...
  if (load_from_file.empty()) {
    ...
    while (executor.pull(block)) {
      ...
      if (!silent) std::cerr << "Processed " << source_rows << " rows\n";
    }

    obfuscator.finalize();
    rewind_needed = true;
  } else {
    if (!silent) std::cerr << "Loading models\n";
    ...
    if (version != 0) throw Exception(ErrorCodes::UNKNOWN_FORMAT_VERSION, "Unknown version of the model file");
    ...
    for (size_t i = 0; i < header_size; ++i) {
      ...
      if (type != data_types[i]) throw Exception(ErrorCodes::TYPE_MISMATCH, "The saved model was created for different types of columns");
    }
  ...
  if (!save_into_file.empty()) {
    if (!silent) std::cerr << "Saving models\n";
    ...
    for (const auto &type : data_types) writeBinary(type, model_out);
  ...
  if (!options.contains("limit")) limit = source_rows;
  ...
  while (processed_rows < limit) {
    ...
    if (rewind_needed) file_in.rewind();

    Pipe pipe(context->getInputFormat(input_format, file_in, header, max_block_size));

    if (processed_rows + source_rows > limit) {
      pipe.addSimpleTransform(
          [&](const SharedHeader &cur_header) { return std::make_shared<LimitTransform>(cur_header, limit - processed_rows, 0); });
    }
