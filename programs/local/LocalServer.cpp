...
Poco::Util::LayeredConfiguration &LocalServer::getClientConfiguration() { return config(); }
...
void LocalServer::processError(std::string_view) const {
  ...
  if (is_interactive) {
    ...
    if (client_exception) client_exception->rethrow();
  }
}
...
void LocalServer::initialize(Poco::Util::Application &self) {
...
#if USE_JEMALLOC
  Jemalloc::setup(server_settings[ServerSetting::jemalloc_enable_global_profiler],
  ...
  getActivePartsLoadingThreadPool().initialize(active_parts_loading_threads,
  ...
  getOutdatedPartsLoadingThreadPool().initialize(outdated_parts_loading_threads,
                                                 0,  // We don't need any threads one all the parts will be loaded
  ...
  getUnexpectedPartsLoadingThreadPool().initialize(unexpected_parts_loading_threads,
                                                   0,  // We don't need any threads one all the parts will be loaded
  ...
  getPartsCleaningThreadPool().initialize(cleanup_threads,
...
static DatabasePtr createClickHouseLocalDatabaseOverlay(const String &name_, ContextPtr context) {
  ...
  if (FS::isSymlinkNoThrow(existing_path_symlink)) {
    ...
    if (!symlink_path.has_filename() && symlink_path.has_parent_path()) symlink_path = symlink_path.parent_path();
  ...
  fs::path default_database_metadata_path =
      fs::weakly_canonical(context->getPath()) / DatabaseCatalog::getStoreDirPath(default_database_uuid);
...
void LocalServer::tryInitPath() {
  ...
  if (getClientConfiguration().has("path")) {
    ...
    /// we can have another clickhouse-local running simultaneously, even with the same PID (for ex. - several dockers mounting the same
    /// folder) or it can be some leftovers from other clickhouse-local runs as we can't accurately distinguish those situations we don't
    /// touch any existent folders we just try to pick some free name for our working folder

    default_path = parent_folder / fmt::format("clickhouse-local-{}", UUIDHelpers::generateV4());

    if (fs::exists(default_path))
      throw Exception(ErrorCodes::FILE_ALREADY_EXISTS, "Unsuccessful attempt to set up the working directory: {} already exists.",
                      default_path.string());
...
std::pair<std::string, std::string> LocalServer::getInitialCreateTableQuery() {
  ...
  if (!getClientConfiguration().has("table-file") || getClientConfiguration().getString("table-file") == "-") {
    ...
    table_file = "stdin";
    if (default_input_compression_method != CompressionMethod::None) compression = toContentEncodingName(default_input_compression_method);
  } else {
    /// Use regular file
  ...
  return {table_name, fmt::format("CREATE TEMPORARY TABLE {} {} ENGINE = File({}, {}, {});", backQuote(table_name), table_structure,
                                  data_format, table_file, compression)};
}
...
void LocalServer::setupUsers() {
  ...
  access_control.setEnabledUsersWithoutRowPoliciesCanReadRows(
      config.getBool("access_control_improvements.users_without_row_policies_can_read_rows", true));
  access_control.setOnClusterQueriesRequireClusterGrant(
      config.getBool("access_control_improvements.on_cluster_queries_require_cluster_grant", true));
  access_control.setSelectFromSystemDatabaseRequiresGrant(
      config.getBool("access_control_improvements.select_from_system_db_requires_grant", true));
  access_control.setSelectFromInformationSchemaRequiresGrant(
      config.getBool("access_control_improvements.select_from_information_schema_requires_grant", true));
  access_control.setSettingsConstraintsReplacePrevious(
      config.getBool("access_control_improvements.settings_constraints_replace_previous", true));
  ...
  access_control.setThrowOnInvalidReplicatedAccessEntities(
      config.getBool("access_control_improvements.throw_on_invalid_replicated_access_entities", true));
...
void LocalServer::connect() {
  connection_parameters =
      ConnectionParameters(config(), ConnectionParameters::Host{"localhost"}, ConnectionParameters::Database{default_database});
  ...
  connection = LocalConnection::createConnection(connection_parameters, client_context, in, need_render_progress,
                                                 need_render_profile_events, server_display_name);
}

int LocalServer::main(const std::vector<std::string> & /*args*/) try {
  ...
  is_interactive = stdin_is_a_tty && (getClientConfiguration().hasOption("interactive") ||
  ...
  if (!table_name.empty()) client_context->setSetting("implicit_table_at_top_level", table_name);
  ...
  if (is_interactive && !delayed_interactive) {
    ...
    if (delayed_interactive) runInteractive();
  }
#endif
...
void LocalServer::processConfig() {
  ...
  auto logging = (getClientConfiguration().has("logger.console") || getClientConfiguration().has("logger.level") ||
                  getClientConfiguration().has("log-level") || getClientConfiguration().has("logger.log"));
  ...
  /// Maybe useless
  if (getClientConfiguration().has("macros")) global_context->setMacros(std::make_unique<Macros>(getClientConfiguration(), "macros", log));
  ...
  const size_t default_max_server_memory_usage =
      static_cast<size_t>(static_cast<double>(physical_server_memory) * max_server_memory_usage_to_ram_ratio);
  ...
  if (max_server_memory_usage == 0) {
    ...
    LOG_INFO(log,
             ...
             formatReadableSizeWithBinarySuffix(max_server_memory_usage), formatReadableSizeWithBinarySuffix(physical_server_memory),
  ...
  if (!mark_cache_size) LOG_ERROR(log, "Too low mark cache size will lead to severe performance degradation.");
  ...
  global_context->setVectorSimilarityIndexCache(vector_similarity_index_cache_policy, vector_similarity_index_cache_size,
                                                vector_similarity_index_cache_max_count, vector_similarity_index_cache_size_ratio);
  ...
  if (!server_default_database.empty()) {
    ...
    if (UUID uuid = database->getUUID(); uuid != UUIDHelpers::Nil) DatabaseCatalog::instance().addUUIDMapping(uuid);
  ...
  if (getClientConfiguration().has("path")) {
    ...
    attachInformationSchema(global_context,
                            *createMemoryDatabaseIfNotExists(global_context, DatabaseCatalog::INFORMATION_SCHEMA_UPPERCASE));
    ...
    if (fs::exists(fs::path(path) / "user_defined")) global_context->getUserDefinedSQLObjectsStorage().loadObjects();
...
void LocalServer::printHelpMessage(const OptionsDescription &options_description) {
  ...
  if (options_description.main_description.has_value()) output_stream << options_description.main_description.value() << "\n";
          ...
          ("input-format", po::value<std::string>(), "Default input format. Takes precedence over --format.")
              ...
              ("logger.console", po::value<bool>()->implicit_value(true), "Log to console")(
                  ...
                  ("no-system-tables", "Do not attach system tables (better startup time)")(
                      ...
                      "Storage path. If it was not specified, we will use a temporary directory, that is cleaned up on exit.")(
...
void LocalServer::applyCmdSettings(ContextMutablePtr context) { context->applySettingsChanges(cmd_settings->changes()); }
...
void LocalServer::processOptions(const OptionsDescription &, const CommandLineOptions &options, const std::vector<Arguments> &,
  ...
  if (options.contains("path")) getClientConfiguration().setString("path", options["path"].as<std::string>());
  if (options.contains("table")) getClientConfiguration().setString("table-name", options["table"].as<std::string>());
  if (options.contains("file")) getClientConfiguration().setString("table-file", options["file"].as<std::string>());
  if (options.contains("structure")) getClientConfiguration().setString("table-structure", options["structure"].as<std::string>());
  if (options.contains("no-system-tables")) getClientConfiguration().setBool("no-system-tables", true);
  if (options.contains("only-system-tables")) getClientConfiguration().setBool("only-system-tables", true);

  if (options.contains("input-format")) getClientConfiguration().setString("table-data-format", options["input-format"].as<std::string>());
  if (options.contains("output-format")) getClientConfiguration().setString("output-format", options["output-format"].as<std::string>());

  if (options.contains("logger.console")) getClientConfiguration().setBool("logger.console", options["logger.console"].as<bool>());
  if (options.contains("logger.log")) getClientConfiguration().setString("logger.log", options["logger.log"].as<std::string>());
  if (options.contains("logger.level")) getClientConfiguration().setString("logger.level", options["logger.level"].as<std::string>());
  ...
  if (options.contains("copy")) {
    if (!queries.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Options '--copy' and '--query' cannot be specified at the same time");
  ...
  for (int arg_num = 1; arg_num < argc; ++arg_num) {
    ...
    if (arg.starts_with("--param_") || arg.starts_with("--param-")) {
      ...
      if (equal_pos == std::string::npos) {
        /// param_name value
        ++arg_num;
        if (arg_num >= argc) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Parameter requires value");
        arg = argv[arg_num];
        query_parameters.emplace(String(param_continuation), String(arg));
      } else {
        if (equal_pos == 0) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Parameter name cannot be empty");
