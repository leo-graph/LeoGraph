...
enum class FuzzerState { NONE, WAITING_FOR_INPUT, DATA_READY, FINISHED };
...
extern "C" int LLVMFuzzerInitialize(const int *argc, char ***argv) {
  ...
  for (int i = 1; i < *argc; ++i)
    ...
    else if (std::string_view arg{(*argv)[i]}; arg.substr(0, arg.find('=')) == "-ignore_remaining_args")
      ignore = true;

  {
    ...
    if (!cv.wait_for(lock, std::chrono::seconds(30), [] { return state == FuzzerState::WAITING_FOR_INPUT; })) abort();
  }
  ...
  while (true) {
    {
      ...
      if (state == FuzzerState::FINISHED) break;
