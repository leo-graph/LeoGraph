...
Runner::Runner(std::optional<size_t> concurrency_, const std::string &config_path, const std::string &input_request_log_,
               const std::string &setup_nodes_snapshot_path_, const Strings &hosts_strings_, std::optional<double> max_time_,
               std::optional<double> delay_, std::optional<bool> continue_on_error_, std::optional<size_t> max_iterations_)
    : input_request_log(input_request_log_), setup_nodes_snapshot_path(setup_nodes_snapshot_path_), info(std::make_shared<Stats>()) {
  ...
  if (!config_path.empty()) {
    ...
    if (config->has("generator")) generator.emplace(*config);
  } else {
  ...
  if (!hosts_strings_.empty()) {
    for (const auto &host : hosts_strings_) connection_infos.push_back({.host = host});
  } else {
    if (!config) throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "No config file or hosts defined");
  ...
  if (config) enable_tracing = config->getBool("enable_tracing", false);
...
void Runner::parseHostsFromConfig(const Poco::Util::AbstractConfiguration &config) {
  const auto fill_connection_details = [&](const std::string &key, auto &connection_info) {
    if (config.has(key + ".secure")) connection_info.secure = config.getBool(key + ".secure");

    if (config.has(key + ".session_timeout_ms")) connection_info.session_timeout_ms = config.getInt(key + ".session_timeout_ms");

    if (config.has(key + ".operation_timeout_ms")) connection_info.operation_timeout_ms = config.getInt(key + ".operation_timeout_ms");

    if (config.has(key + ".connection_timeout_ms")) connection_info.connection_timeout_ms = config.getInt(key + ".connection_timeout_ms");

    if (config.has(key + ".use_compression")) connection_info.use_compression = config.getBool(key + ".use_compression");

    if (config.has(key + ".use_xid_64")) connection_info.use_xid_64 = config.getBool(key + ".use_xid_64");
  };
  ...
  for (const auto &key : connections_keys) {
    ...
    if (key.starts_with("host")) {
      ...
      if (config.has(connection_key + ".sessions")) connection_info.sessions = config.getUInt64(connection_key + ".sessions");
  ...
  while (true) {
    ...
    Coordination::ResponseCallback callback = [&request_with_callbacks, promise](const Coordination::Response &response) {
      ...
      if (response.error == Coordination::Error::ZOK) {
        for (const auto &success_callback : request_with_callbacks.on_success_callbacks) success_callback();
        set_exception = false;
      }
    ...
    try {
      ...
      if (request) std::cerr << "For request:\n" << request->toString() << std::endl;
...
struct ZooKeeperRequestBlock {
  ...
  UInt64 getExecutorId(size_t row) const { return getSessionId(row); }

  std::string getHostname(size_t row) const { return getField(hostname_idx, row).safeGet<std::string>(); }

  UInt64 getThreadId(size_t row) const { return getField(thread_id_idx, row).safeGet<UInt64>(); }

  DB::DateTime64 getRequestEventTime(size_t row) const { return getField(request_event_time_idx, row).safeGet<DB::DateTime64>(); }

  DB::DateTime64 getResponseEventTime(size_t row) const { return getField(response_event_time_idx, row).safeGet<DB::DateTime64>(); }

  Int64 getSessionId(size_t row) const { return getField(session_id_idx, row).safeGet<Int64>(); }

  Int64 getXid(size_t row) const { return getField(xid_idx, row).safeGet<Int64>(); }

  bool hasWatch(size_t row) const { return getField(has_watch_idx, row).safeGet<UInt8>(); }

  Coordination::OpNum getOpNum(size_t row) const { return static_cast<Coordination::OpNum>(getField(op_num_idx, row).safeGet<Int64>()); }

  bool isEphemeral(size_t row) const { return getField(is_ephemeral_idx, row).safeGet<UInt8>(); }

  bool isSequential(size_t row) const { return getField(is_sequential_idx, row).safeGet<UInt8>(); }

  std::string getPath(size_t row) const { return getField(path_idx, row).safeGet<std::string>(); }

  std::string getData(size_t row) const { return getField(data_idx, row).safeGet<std::string>(); }

  UInt64 getRequestsSize(size_t row) const { return getField(requests_size_idx, row).safeGet<UInt64>(); }
...
struct ZooKeeperRequestFromLogReader {
  ZooKeeperRequestFromLogReader(const std::string &input_request_log, DB::ContextPtr context) {
    ...
    input_format = DB::FormatFactory::instance().getInput(
        format, *file_read_buf, header_block, context, context->getSettingsRef()[DB::Setting::max_block_size], format_settings,
        DB::FormatParserSharedResources::singleThreaded(context->getSettingsRef()), nullptr,
        /*is_remote_fs*/ false, DB::CompressionMethod::None, false);
  ...
  std::optional<RequestFromLog> getNextRequest(bool for_multi = false) {
    ...
    switch (op_num) {
      ...
      case Coordination::OpNum::CheckNotExists: {
        ...
        if (op_num == Coordination::OpNum::CheckNotExists) check_request->not_exists = true;
      ...
      case Coordination::OpNum::MultiRead: {
        if (for_multi) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Nested multi requests are not allowed");
...
struct SetupNodeCollector {
  explicit SetupNodeCollector(const std::string &setup_nodes_snapshot_path) {
    if (setup_nodes_snapshot_path.empty()) return;
    ...
    keeper_context->setSnapshotDisk(std::make_shared<DB::DiskLocal>("Keeper-snapshots", setup_nodes_snapshot_path));
  ...
  void processRequest(const RequestFromLog &request_from_log) {
    if (!request_from_log.expected_result.has_value()) return;

    auto process_request = [&](const Coordination::ZooKeeperRequest &request, const auto expected_result) {
      const auto &path = request.getPath();

      if (nodes_created_during_replay.contains(path)) return;
      ...
      if (op_num == Coordination::OpNum::Create) {
        ...
        if (expected_result == Coordination::Error::ZOK || expected_result == Coordination::Error::ZBADVERSION) addExpectedNode(path);
        ...
        if (expected_result == Coordination::Error::ZOK) addExpectedNode(path);
      }
    };
    ...
    if (request->getOpNum() == Coordination::OpNum::Multi || request->getOpNum() == Coordination::OpNum::MultiRead) {
      ...
      for (size_t i = 0; i < subrequests.size(); ++i) {
        ...
        if (subrequest_expected_result.has_value()) process_request(zookeeper_request, *subrequest_expected_result);
      }
    } else
  ...
  void addExpectedNode(const std::string &path) {
    ...
    if (initial_storage->container.contains(path)) return;
  ...
  void generateSnapshot() {
    ...
    DB::SnapshotMetadataPtr snapshot_meta =
        std::make_shared<DB::SnapshotMetadata>(initial_storage->getZXID(), 1, std::make_shared<nuraft::cluster_config>());
...
void dumpStats(std::string_view type, const RequestFromLogStats::Stats &stats_for_type) {
  std::cerr << fmt::format("{} requests: {} total, {} with unexpected results ({:.4}%)", type, stats_for_type.total.load(),
                           stats_for_type.unexpected_results.load(),
                           stats_for_type.total != 0
                               ? static_cast<double>(stats_for_type.unexpected_results) / static_cast<double>(stats_for_type.total) * 100
                               : 0.0)
            << std::endl;
};
...
void requestFromLogExecutor(std::shared_ptr<ConcurrentBoundedQueue<RequestFromLog>> queue, RequestFromLogStats &request_stats,
  ...
  while (queue->pop(request_from_log)) {
    ...
    Coordination::ResponseCallback callback = [&, request_promise, start_time, request = request_from_log.request,
                                               ...
                                               bench_info](const Coordination::Response &response) mutable {
      ...
      if (expected_result) {
        ...
        if (bench_info && *expected_result != response.error) bench_info->errors.fetch_add(1, std::memory_order_relaxed);
    ...
    if (request_from_log.has_watch) watch = std::make_shared<Coordination::WatchCallback>([](const Coordination::WatchResponse &) {});

    request_from_log.connection->executeGenericRequest(request_from_log.request, callback, watch);
  }

  if (last_request) last_request->wait();
}
...
void Runner::runBenchmarkFromLog() {
  ...
  auto get_zookeeper_connection = [&](int64_t session_id) {
    if (auto it = zookeeper_connections.find(session_id); it != zookeeper_connections.end() && !it->second->isExpired()) return it->second;
  ...
  if (!setup_nodes_snapshot_path.empty()) setup_nodes_collector.emplace(setup_nodes_snapshot_path);

  std::unordered_map<uint64_t, std::shared_ptr<ConcurrentBoundedQueue<RequestFromLog>>> executor_id_to_queue;

  SCOPE_EXIT_SAFE({
    for (const auto &[executor_id, executor_queue] : executor_id_to_queue) executor_queue->finish();
  ...
  auto push_request = [&](RequestFromLog request) {
    if (auto it = executor_id_to_queue.find(request.executor_id); it != executor_id_to_queue.end()) {
      ...
      if (!success) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Failed to push to the executor's queue");
      return;
    }
    ...
    auto scheduled =
        pool->trySchedule([&, executor_queue]() mutable { requestFromLogExecutor(std::move(executor_queue), stats, info.get()); });

    if (!scheduled) throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Failed to schedule worker, try to increase concurrency parameter");
...
void Runner::writeOutputString(const std::string &output_string, int64_t start_timestamp_ms) {
  if (print_to_stdout) std::cout << output_string << std::endl;
...
void Runner::createConnections() {
  ...
  for (size_t connection_info_idx = 0; connection_info_idx < connection_infos.size(); ++connection_info_idx) {
    ...
    std::cerr << fmt::format(
                     "Creating {} session(s) for:\n"
                     "- host: {}\n"
                     "- secure: {}\n"
                     "- session timeout: {}ms\n"
                     "- operation timeout: {}ms\n"
                     "- connection timeout: {}ms",
                     connection_info.sessions, connection_info.host, connection_info.secure, connection_info.session_timeout_ms,
                     connection_info.operation_timeout_ms, connection_info.connection_timeout_ms)
              << std::endl;
...
void removeRecursive(Coordination::ZooKeeper &zookeeper, const std::string &path) {
  ...
  while (!children_span.empty()) {
    ...
    auto multi_callback = [multi_promise](const Coordination::MultiResponse &) { multi_promise->set_value(); };
  ...
  auto remove_callback = [remove_promise](const Coordination::RemoveResponse &) { remove_promise->set_value(); };
    ...
    if (key.starts_with("node")) {
      ...
      if (config.has(node_key + ".repeat")) {
        if (!node->name.isRandom())
          throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Repeating node creation for key {}, but name is not randomly generated",
                              node_key);

        auto repeat_count = config.getUInt64(node_key + ".repeat");
        node->repeat_count = repeat_count;
        for (size_t i = 1; i < repeat_count; ++i) root_nodes.emplace_back(node->clone());
      }
...
std::shared_ptr<BenchmarkContext::Node> BenchmarkContext::parseNode(const std::string &key,
  ...
  for (const auto &node_key : node_keys) {
    ...
    if (config.has(node_key_string + ".repeat")) {
      ...
      for (size_t i = 1; i < repeat_count; ++i) node->children.push_back(child_node);
    }
  }
...
void BenchmarkContext::Node::dumpTree(int level) const {
  std::string data_string = data.has_value() ? fmt::format("{}", data->description()) : "no data";

  std::string repeat_count_string = repeat_count != 0 ? fmt::format(", repeated {} times", repeat_count) : "";

  std::cerr << fmt::format("{}name: {}, data: {}{}", std::string(level, '\t'), name.description(), data_string, repeat_count_string)
            << std::endl;
...
void BenchmarkContext::Node::createNode(Coordination::ZooKeeper &zookeeper, const std::string &parent_path,
  ...
  for (const auto &child : children) child->createNode(zookeeper, path, acls);
}

void BenchmarkContext::startup(Coordination::ZooKeeper &zookeeper) {
  if (root_nodes.empty()) return;
