...
std::string generateRandomString(size_t length) {
  ...
  while (length--) s += chars[pick(rng)];

  return s;
}
}  // namespace

NumberGetter NumberGetter::fromConfig(const std::string& key, const Poco::Util::AbstractConfiguration& config,
...
std::string NumberGetter::description() const {
  if (const auto* number = std::get_if<uint64_t>(&value)) return std::to_string(*number);
...
StringGetter StringGetter::fromConfig(const std::string& key, const Poco::Util::AbstractConfiguration& config) {
  ...
  if (config.has(key + ".random_string"))
    string_getter.value = NumberGetter::fromConfig(key + ".random_string.size", config);
  else
...
void StringGetter::setString(std::string name) { value = std::move(name); }
...
bool StringGetter::isRandom() const { return std::holds_alternative<NumberGetter>(value); }
...
PathGetter PathGetter::fromConfig(const std::string& key, const Poco::Util::AbstractConfiguration& config) {
  ...
  for (const auto& path_key : path_keys) {
    if (!path_key.starts_with(path_key_string)) continue;
...
void PathGetter::initialize(Coordination::ZooKeeper& zookeeper) {
  for (const auto& parent_path : parent_paths) {
    ...
    for (const auto& child : list_response.names) paths.push_back(std::filesystem::path(parent_path) / child);
  }
...
std::string PathGetter::getPath() const {
  if (!initialized) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "PathGetter is not initialized");
...
RequestGetter::RequestGetter(std::vector<RequestGeneratorPtr> request_generators_) : request_generators(std::move(request_generators_)) {}
...
RequestGetter RequestGetter::fromConfig(const std::string& key, const Poco::Util::AbstractConfiguration& config, bool for_multi) {
  ...
  for (const auto& generator_key : generator_keys) {
    ...
    if (generator_key.starts_with("create"))
      ...
      if (for_multi) throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Nested multi requests are not allowed");
      request_generator = std::make_unique<MultiRequestGenerator>();
    } else {
      if (for_multi) continue;
  ...
  if (generators.empty()) throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "No request generators found in config for key '{}'", key);
  ...
  if (use_weights) {
    ...
    for (size_t i = 1; i < generators.size(); ++i) weights.push_back(weights.back() + generators[i]->getWeight());
  }
...
RequestGeneratorPtr RequestGetter::getRequestGenerator() const {
  ...
  if (weights.empty()) return request_generators[random_number];

  for (size_t i = 0; i < request_generators.size(); ++i) {
    if (random_number <= weights[i]) return request_generators[i];
  }
...
std::string RequestGetter::description() const {
  ...
  for (const auto& request_generator : request_generators) description += fmt::format("\n{}\n", request_generator->description());
  return description + guard;
}

void RequestGetter::startup(Coordination::ZooKeeper& zookeeper) {
  for (const auto& request_generator : request_generators) request_generator->startup(zookeeper);
}

const std::vector<RequestGeneratorPtr>& RequestGetter::requestGenerators() const { return request_generators; }

void RequestGenerator::getFromConfig(const std::string& key, const Poco::Util::AbstractConfiguration& config) {
  if (config.has(key + ".weight")) weight = config.getUInt64(key + ".weight");
...
ZooKeeperRequestWithCallbacks RequestGenerator::generate(const Coordination::ACLs& acls) { return generateImpl(acls); }

void RequestGenerator::startup(Coordination::ZooKeeper& zookeeper) { startupImpl(zookeeper); }

size_t RequestGenerator::getWeight() const { return weight; }

CreateRequestGenerator::CreateRequestGenerator() : rng(randomSeed()), remove_picker(0, 1.0) {}
...
void CreateRequestGenerator::getFromConfigImpl(const std::string& key, const Poco::Util::AbstractConfiguration& config) {
  ...
  if (config.has(key + ".data")) data = StringGetter::fromConfig(key + ".data", config);

  if (config.has(key + ".remove_factor")) remove_factor = config.getDouble(key + ".remove_factor");
}

std::string CreateRequestGenerator::descriptionImpl() {
  std::string data_string = data.has_value() ? fmt::format("data for created nodes: {}", data->description()) : "no data for created nodes";
  std::string remove_factor_string = remove_factor.has_value() ? fmt::format("- remove factor: {}", *remove_factor) : "- without removes";
  return fmt::format(
      ...
      "- {}\n"
      "{}",
      parent_path.description(), name.description(), data_string, remove_factor_string);
}

void CreateRequestGenerator::startupImpl(Coordination::ZooKeeper& zookeeper) { parent_path.initialize(zookeeper); }
...
ZooKeeperRequestWithCallbacks CreateRequestGenerator::generateImpl(const Coordination::ACLs& acls) {
  ...
  if (data) request->data = data->getString();
...
void SetRequestGenerator::startupImpl(Coordination::ZooKeeper& zookeeper) { path.initialize(zookeeper); }
...
void GetRequestGenerator::startupImpl(Coordination::ZooKeeper& zookeeper) { path.initialize(zookeeper); }
...
void ListRequestGenerator::startupImpl(Coordination::ZooKeeper& zookeeper) { path.initialize(zookeeper); }

void MultiRequestGenerator::getFromConfigImpl(const std::string& key, const Poco::Util::AbstractConfiguration& config) {
  if (config.has(key + ".size")) size = NumberGetter::fromConfig(key + ".size", config);
...
std::string MultiRequestGenerator::descriptionImpl() {
  ...
  return fmt::format(
      ...
      "{}"
      "- requests:\n{}",
      size_string, request_getter.description());
}
...
ZooKeeperRequestWithCallbacks MultiRequestGenerator::generateImpl(const Coordination::ACLs& acls) {
  ...
  if (size) {
    ...
    for (size_t i = 0; i < request_count; ++i) {
      ...
      on_success_callbacks.insert(on_success_callbacks.end(), std::make_move_iterator(request_with_callbacks.on_success_callbacks.begin()),
...
void MultiRequestGenerator::startupImpl(Coordination::ZooKeeper& zookeeper) { request_getter.startup(zookeeper); }
...
ZooKeeperRequestWithCallbacks Generator::generate() { return request_getter.getRequestGenerator()->generate(default_acls); }
