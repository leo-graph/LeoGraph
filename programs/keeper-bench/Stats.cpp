...
double Stats::StatsCollector::getPercentile(double percent) { return sampler.quantileNearest(percent / 100.0) / 1000.0; }
...
void Stats::report(size_t concurrency) {
  ...
  if (0 == read_requests && 0 == write_requests) return;

  double read_rps = 0;
  double read_bps = 0;
  double write_rps = 0;
  double write_bps = 0;
  if (read_requests != 0) std::tie(read_rps, read_bps) = read_collector.getThroughput(concurrency);
  if (write_requests != 0) std::tie(write_rps, write_bps) = write_collector.getThroughput(concurrency);

  std::cerr << "read requests " << read_requests << ", write requests " << write_requests << ", ";
  if (errors) std::cerr << "errors " << errors << ", ";

  if (0 != read_requests) {
    std::cerr << "Read RPS: " << read_rps << ", "
              << "Read MiB/s: " << read_bps / 1048576;

    if (0 != write_requests) std::cerr << ", ";
  }
  ...
  const auto print_all_percentiles = [&](auto& collector) {
    for (int percent = 0; percent <= 90; percent += 10) print_percentile(percent, collector);
...
void Stats::writeJSON(DB::WriteBuffer& out, size_t concurrency, int64_t start_timestamp) {
  ...
  const auto get_results = [&](auto& collector) {
    ...
    for (int percent = 0; percent <= 90; percent += 10) add_percentile(percent);
  ...
  if (read_collector.requests != 0) results.AddMember("read_results", get_results(read_collector), results.GetAllocator());

  if (write_collector.requests != 0) results.AddMember("write_results", get_results(write_collector), results.GetAllocator());
