  ...
  try {
    ...
    if (snapshot_path.ends_with(".bin") || snapshot_path.ends_with(".bin.zstd")) {
      ...
      for (const auto &file : snapshot_files) {
        if (file.starts_with("snapshot_") && (file.ends_with(".bin") || file.ends_with(".bin.zstd"))) snapshot_paths.push_back(file);
      }

      if (snapshot_paths.empty()) throw DB::Exception(ErrorCodes::UNKNOWN_SNAPSHOT, "No snapshot files found in {}", snapshot_path);
    ...
    for (const auto &snapshot_file : snapshot_paths) {
      try {
        ...
        auto snapshot_manager = KeeperSnapshotManager<KeeperMemoryStorage>(std::numeric_limits<size_t>::max(),  // snapshots_to_keep
                                                                           keeper_context,
        ...
        auto result = snapshot_manager.deserializeSnapshotFromBuffer(
            snapshot_manager.deserializeSnapshotBufferFromDisk(getSnapshotPathUpToLogIdx(full_path)), full_storage);
        ...
        if (full_storage) {
          std::cout << fmt::format(
                           ...
                           "  Number of nodes: {}\n"
                           "  Digest: {}\n",
                           snapshot_meta->get_last_log_idx(), snapshot_meta->get_last_log_term(), result.storage->getNodesCount(),
          ...
          std::cout << fmt::format(
                           ...
                           "  Last committed log term: {}\n"
                           "  Number of paths: {}\n",
                           snapshot_meta->get_last_log_idx(), snapshot_meta->get_last_log_term(), result.paths.size())
                    << std::endl;
          ...
          if (with_node_stats) {
            ...
            for (const auto &[node_path, count] : subtree_sizes) {
              ...
              if (pq.size() > subtrees_limit) pq.pop();
            }
  ...
  try {
    ...
    for (const auto &file : log_files) {
      if (file.starts_with("changelog_")) {
        if (specific_changelog.empty() || file == specific_changelog) changelog_files.push_back(file);
      }
    }
    ...
    if (specific_changelog.empty()) std::sort(changelog_files.begin(), changelog_files.end(), std::greater<>());
...
void spliceChangelogFile(const std::string &source_path, const std::string &destination_path, uint64_t start_index, uint64_t end_index) {
  try {
    ...
    if (start_index == 0) start_index = source_desc->from_log_index;

    if (start_index >= end_index) throw DB::Exception(ErrorCodes::BAD_ARGUMENTS, "Start index must be less than end index");

    // Validate index range
    if (start_index < source_desc->from_log_index || end_index > source_desc->to_log_index + 1) {
      throw DB::Exception(ErrorCodes::BAD_ARGUMENTS, "Index range [{}, {}) is out of changelog range [{}, {}]", start_index, end_index,
                          source_desc->from_log_index, source_desc->to_log_index + 1);
    }

    // Generate destination filename based on source filename and index range
    std::string dest_filename = Changelog::formatChangelogPath("changelog", start_index, end_index - 1, source_desc->extension);
    ...
    throw DB::Exception(ErrorCodes::CANNOT_READ_ALL_DATA, "Failed to splice changelog from {} to {}: {}", source_path, destination_path,
                        e.message());
  }
}

void dumpSessions(const DB::KeeperMemoryStorage &storage, const std::string &output_file, const std::string &output_format,
  ...
  if (!output_file.empty()) {
    ...
    for (const auto &[session_id, timeout] : sessions) {
      ...
      if (auto it = ephemerals.find(session_id); it != ephemerals.end()) ephemeral_count = it->second.size();
    ...
    if (parallel_output)
      output_format_processor = DB::FormatFactory::instance().getOutputFormatParallelIfPossible(output_format, output_buf, data,
                                                                                                DB::Context::getGlobalContextInstance());
    else
      output_format_processor =
          DB::FormatFactory::instance().getOutputFormat(output_format, output_buf, data, DB::Context::getGlobalContextInstance());
...
void dumpNodes(const DB::KeeperMemoryStorage &storage, const std::string &output_file, const std::string &output_format,
               bool parallel_output, bool with_acl) {
  SharedContextHolder shared_context;
  ContextMutablePtr global_context;
  ...
  if (!output_file.empty()) {
    ...
    ColumnsWithTypeAndName columns{
        {std::make_shared<DataTypeString>(), "path"},          {std::make_shared<DataTypeInt64>(), "czxid"},
        {std::make_shared<DataTypeInt64>(), "mzxid"},          {std::make_shared<DataTypeDateTime>(), "ctime"},
        {std::make_shared<DataTypeDateTime>(), "mtime"},       {std::make_shared<DataTypeInt32>(), "version"},
        {std::make_shared<DataTypeInt32>(), "cversion"},       {std::make_shared<DataTypeInt32>(), "aversion"},
        {std::make_shared<DataTypeInt64>(), "ephemeralOwner"}, {std::make_shared<DataTypeInt32>(), "dataLength"},
        {std::make_shared<DataTypeInt32>(), "numChildren"},    {std::make_shared<DataTypeInt64>(), "pzxid"},
        {std::make_shared<DataTypeString>(), "data"},
    };

    if (with_acl) columns.emplace_back(std::make_shared<DataTypeString>(), "acl");
    ...
    if (parallel_output)
      output_format_processor =
          DB::FormatFactory::instance().getOutputFormatParallelIfPossible(output_format, output_buf, data, global_context);
    else
  ...
  print_function = [&](const auto &key, const auto &value) {
    ...
    std::cout << fmt::format(
        ...
        value.stats.version, value.stats.mtime, value.stats.ephemeralOwner(), value.stats.czxid, value.stats.mzxid,
        value.stats.numChildren(), value.stats.data_size);
...
int dumpStateMachine(const std::string &snapshot_path, const std::string &log_path, bool debug_mode, const std::string &output_file,
                     const std::string &output_format, bool parallel_output, bool with_acl,
                     uint64_t end_index = std::numeric_limits<uint64_t>::max(), bool dump_sessions = false) {
    ...
    for (size_t i = last_committed_index + 1; i < last_index_to_apply; ++i) {
      ...
      if (entry.get_val_type() == nuraft::log_val_type::app_log) {
        if (debug_mode) {
          ...
          LOG_INFO(logger, "Applying log entry {}: term={}, zxid={}, session_id={}\nrequest:\n{}", i, entry.get_term(), req->zxid,
                   req->session_id, req->request->toString());
        }
...
int deserializeChangelog(const std::string &changelog_path, const std::string &output_file, const std::string &output_format,
                         bool parallel_output, bool with_requests, uint64_t start_index, uint64_t end_index) {
  try {
    ...
    if (start_index == 0)
      ...
      throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "start_index {} is out of range for changelog {} which has range [{}, {}]",
                          start_index, changelog_path, desc->from_log_index, desc->to_log_index);
    }
    ...
    if (end_index == std::numeric_limits<uint64_t>::max())
      ...
      throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "end_index {} is out of range for changelog {} which has range [{}, {}]",
                          end_index, changelog_path, desc->from_log_index, desc->from_log_index + entry_storage.size());
    }
    ...
    if (!output_file.empty()) {
      ...
      // Define output columns
      ColumnsWithTypeAndName columns{
          {std::make_shared<DataTypeUInt64>(), "log_index"},   {std::make_shared<DataTypeUInt64>(), "term"},
          {std::make_shared<DataTypeString>(), "entry_type"},  {std::make_shared<DataTypeUInt64>(), "entry_size"},
      ...
      for (size_t i = start_index; i < end_index; ++i) {
        ...
        if (with_requests) {
          if (entry->get_val_type() != nuraft::log_val_type::app_log) {
            for (; col_idx < columns.size(); ++col_idx) res_columns[col_idx]->insertDefault();
      ...
      if (with_requests && entry->get_val_type() == nuraft::log_val_type::app_log) {
        try {
          if (auto buffer = entry->get_buf_ptr(); buffer) {
            ...
            if (request->digest) std::cout << fmt::format("Digest: {} ({})\n", request->digest->value, request->digest->version);
...
int mainEntryClickHouseKeeperUtils(int argc, char **argv) {
  ...
  try {
    ...
    if (cmd == "dump-state") {
      ...
      dump_state_options.add_options()("help,h", "Show help message")("debug-mode", "Enable debug output for log entries")(
          "snapshot-path", po::value<std::string>()->required(), "Path to snapshots directory")(
          "log-path", po::value<std::string>()->required(), "Path to logs directory")("output-file,o", po::value<std::string>(),
                                                                                      "Optional: Write output to file instead of stdout")(
          "output-format,f", po::value<std::string>()->default_value("CSVWithNamesAndTypes"),
          "Output format (default: CSVWithNamesAndTypes)")("parallel-output", po::bool_switch()->default_value(false),
                                                           "Enable parallel output format processing")(
          "with-acl", po::bool_switch()->default_value(false), "Include ACL information in the output (only for node output)")(
          "end-index", po::value<uint64_t>()->default_value(std::numeric_limits<uint64_t>::max()),
          "End index (exclusive) for changelog processing")("dump-sessions", po::bool_switch()->default_value(false),
      ...
      try {
        ...
        if (dump_state_vm.contains("output-file")) output_file = dump_state_vm["output-file"].as<std::string>();

        return dumpStateMachine(dump_state_vm["snapshot-path"].as<std::string>(), dump_state_vm["log-path"].as<std::string>(),
                                dump_state_vm.contains("debug-mode") > 0, output_file, dump_state_vm["output-format"].as<std::string>(),
                                dump_state_vm["parallel-output"].as<bool>(), dump_state_vm["with-acl"].as<bool>(),
                                dump_state_vm["end-index"].as<uint64_t>(), dump_state_vm["dump-sessions"].as<bool>());
      } catch (const std::exception &e) {
        std::cerr << "Error in dump-state: " << e.what() << "\n"
      ...
      analyzer_options.add_options()("help,h", "Show help message")("snapshot-path", po::value<std::string>()->required(),
      ...
      try {
        // Parse the remaining arguments (after the command)
        std::vector<std::string> subcommand_args;
        for (int i = 2; i < argc; ++i) subcommand_args.push_back(argv[i]);
        ...
        analyzeSnapshot(analyzer_vm["snapshot-path"].as<std::string>(), analyzer_vm["full-storage"].as<bool>(),
                        analyzer_vm["with-node-stats"].as<bool>(), analyzer_vm["subtrees-limit"].as<size_t>());
        return 0;
      } catch (const std::exception &e) {
      ...
      analyzer_options.add_options()("help,h", "Show help message")("log-path", po::value<std::string>()->required(),
      ...
      try {
        ...
        if (analyzer_vm.contains("changelog")) changelog_file = analyzer_vm["changelog"].as<std::string>();
      ...
      splicer_options.add_options()("help,h", "Show help message")("source", po::value<std::string>()->required(),
                                                                   "Path to source changelog file")(
      ...
      try {
        ...
        uint64_t start_index = splicer_vm.contains("start-index") ? splicer_vm["start-index"].as<uint64_t>()
                                                                  : 0;  // Will be set to actual start index in spliceChangelogFile

        spliceChangelogFile(source_path, splicer_vm["destination"].as<std::string>(), start_index, splicer_vm["end-index"].as<uint64_t>());
      ...
      deserializer_options.add_options()("help,h", "Show help message")("changelog-path", po::value<std::string>()->required(),
                                                                        "Path to the changelog file to deserialize")(
          ...
          "end-index", po::value<uint64_t>()->default_value(std::numeric_limits<uint64_t>::max()),
          "End index (exclusive) of entries to process");
      ...
      try {
        ...
        std::string output_file;
        if (deserializer_vm.contains("output-file")) output_file = deserializer_vm["output-file"].as<std::string>();

        uint64_t start_idx = deserializer_vm["start-index"].as<uint64_t>();
        uint64_t end_idx = deserializer_vm["end-index"].as<uint64_t>();

        if (start_idx >= end_idx) throw po::error("start-index must be less than end-index");

        return deserializeChangelog(deserializer_vm["changelog-path"].as<std::string>(), output_file,
                                    deserializer_vm["output-format"].as<std::string>(), deserializer_vm["parallel-output"].as<bool>(),
                                    deserializer_vm["with-requests"].as<bool>(), start_idx, end_idx);
      } catch (const std::exception &e) {
