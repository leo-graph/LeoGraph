...
void prependDoubleQuoteForPath(replxx::Replxx &rx) {
  ...
  {
    auto it = word_begin;
    while (it != text.begin() && std::isspace(*(it - 1))) --it;
  ...
  if (std::distance(word_begin, word_end) < 1) {
    ...
    if (cursor < text.size() && text[cursor] == open_quote) text.erase(cursor, 1);
  }
...
std::vector<String> KeeperClient::getCompletions(String prefix) const {
  ...
  if (probe_pos->type != TokenType::BareWord) return registered_commands_and_four_letter_words;

  ++probe_pos;

  /// If there is nothing after the command word (e.g. "ls" or "get_d"), return command completions.
  if (probe_pos->isEnd()) return registered_commands_and_four_letter_words;
  ...
  char quote_char = '"';
  {
    ...
    std::string_view last_word =
        (last_word_pos == String::npos) ? std::string_view{prefix} : std::string_view{prefix}.substr(last_word_pos + 1);
    if (!last_word.empty() && (last_word[0] == '\'' || last_word[0] == '"')) quote_char = last_word[0];
  }
  ...
  if (!parent_prefix.ends_with('/')) parent_prefix.append("/");
...
void KeeperClient::defineOptions(Poco::Util::OptionSet &options) {
  ...
  options.addOption(Poco::Util::Option("help", "", "show help and exit").binding("help"));

  options.addOption(Poco::Util::Option("host", "h", "server hostname. default `localhost`").argument("<host>").binding("host"));

  options.addOption(Poco::Util::Option("port", "p", "server port. default `9181`").argument("<port>").binding("port"));

  options.addOption(Poco::Util::Option("password", "", "password to connect to keeper server").argument("<password>").binding("password"));

  options.addOption(Poco::Util::Option("query", "q", "will execute given query, then exit.").argument("<query>").binding("query"));

  options.addOption(Poco::Util::Option("connection-timeout", "", "set connection timeout in seconds. default 10s.")
                        .argument("<seconds>")
                        .binding("connection-timeout"));

  options.addOption(Poco::Util::Option("session-timeout", "", "set session timeout in seconds. default 10s.")
                        .argument("<seconds>")
                        .binding("session-timeout"));

  options.addOption(Poco::Util::Option("operation-timeout", "", "set operation timeout in seconds. default 10s.")
                        .argument("<seconds>")
                        .binding("operation-timeout"));

  options.addOption(Poco::Util::Option("use-xid-64", "", "use 64-bit XID. default false.").binding("use-xid-64"));
  ...
  options.addOption(Poco::Util::Option("history-file", "", "set path of history file. default `~/.keeper-client-history`")
                        .argument("<file>")
                        .binding("history-file"));

  options.addOption(Poco::Util::Option("log-level", "", "set log level").argument("<level>").binding("log-level"));

  options.addOption(
      Poco::Util::Option("no-confirmation", "",
                         "if set, will not require a confirmation on several commands. default false for interactive and true for query")
          .binding("no-confirmation"));

  options.addOption(
      Poco::Util::Option(
          "tests-mode", "",
          "run keeper-client in a special mode for tests. all commands output are separated by special symbols. default false")
          .binding("tests-mode"));
  ...
  options.addOption(
      Poco::Util::Option("secure", "s", "use secure connection (adds secure:// prefix to host). default false").binding("secure"));

  options.addOption(Poco::Util::Option("tls-cert-file", "", "path to TLS certificate file for secure connection")
                        .argument("<file>")
                        .binding("tls-cert-file"));

  options.addOption(Poco::Util::Option("tls-key-file", "", "path to TLS private key file for secure connection")
                        .argument("<file>")
                        .binding("tls-key-file"));

  options.addOption(Poco::Util::Option("tls-ca-file", "", "path to TLS CA certificate file for secure connection")
                        .argument("<file>")
                        .binding("tls-ca-file"));
...
bool KeeperClient::processQueryText(const String &text, bool is_interactive) {
  if (exit_strings.contains(text)) return false;
  ...
  while (true) {
    try {
      if (i > 0) connectToKeeper();

      if (waiting_confirmation) {
        waiting_confirmation = false;
        if (text.size() == 1 && (text == "y" || text == "Y")) confirmation_callback();
        return true;
      }
      ...
      while (begin < end) {
        String message;
        ASTPtr res = tryParseQuery(parser, begin, end,
      ...
      if (!is_interactive || !Coordination::isHardwareError(err.code)) break;
...
void KeeperClient::runInteractiveReplxx() {
  ...
  while (true) {
    ...
    if (!processQueryText(input, /*is_interactive=*/true)) break;
  }
...
void KeeperClient::connectToKeeper() {
  ...
  if (config().has("tls-cert-file") || config().has("tls-key-file") || config().has("tls-ca-file") ||
      config().has("accept-invalid-certificate")) {
    ...
    auto context = Poco::Net::Context::Ptr(new Poco::Net::Context(
        Poco::Net::Context::TLSV1_2_CLIENT_USE, config().getString("tls-key-file", ""), config().getString("tls-cert-file", ""),
        config().getString("tls-ca-file", ""), verification_mode, 9, true, "ALL:!ADH:!LOW:!EXP:!MD5:@STRENGTH"));
  ...
  if (!config().has("host") && !config().has("port") && !keys.empty()) {
    ...
    for (const auto &key : keys) {
      ...
      if (clickhouse_config.configuration->has(prefix + ".secure") || config().has("secure")) host = "secure://" + host;
    ...
    if (config().has("secure")) host = "secure://" + host;
  ...
  if (!new_zk_args.identity.empty()) new_zk_args.auth_scheme = "digest";
  zk_args = new_zk_args;
...
int KeeperClient::main(const std::vector<String> & /* args */) {
  ...
  if (config().has("no-confirmation") || config().has("query")) ask_confirmation = false;
