...
| ClickHouse-native AST reuse | Helper boundary exists for `ASTIdentifier`, `ASTLiteral`, `ASTFunction`, and `ASTExpressionList`, but historical expression output still mostly uses `GQLExpr`. | Expression lowering must handle both `GQLExpr` and ClickHouse-native expression nodes once migration starts. | For interpreter MVP, support current `GQLExpr` first. Add native-AST handling behind the same expression-lowering visitor, not as a separate path. | Incrementally migrate only semantically identical SQL expression pieces. |
