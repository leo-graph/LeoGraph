...
| ClickHouse-native AST reuse | `ExpressionLowering` now handles `ASTIdentifier`, `ASTLiteral`, `ASTFunction`, and `ASTExpressionList` in the same lowering path as `GQLExpr`, but historical parser output still mostly uses `GQLExpr`. | Clauses can consume either current `GQLExpr` nodes or future semantically identical ClickHouse-native expression nodes. | Keep native-AST support behind the shared expression-lowering visitor, not as a separate path. | Incrementally migrate only semantically identical SQL expression pieces. |
