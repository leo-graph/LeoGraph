...
- `ExpressionLowering` supports both current `GQLExpr` nodes and semantically identical ClickHouse-native `ASTIdentifier`, `ASTLiteral`, `ASTFunction`, and `ASTExpressionList` nodes through the same `ActionsDAG` path. `WHERE` / `FILTER`, `ORDER BY`, `OFFSET`, and `LIMIT` lowering consume expressions as `IAST`, so filter and paging paths inherit the same expression contract.
