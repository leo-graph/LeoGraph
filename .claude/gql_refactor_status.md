...
- `FILTER` clauses now keep their own `GQLWhereClause::Type::Filter` kind instead of normalizing to `WHERE`, while the interpreter still reuses the common predicate-lowering path.
