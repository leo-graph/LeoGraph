...
4. `SELECT FROM` source-list handling now has a dedicated `SourceCompositionLowering` module with an entry-classification pass. It can lower same-graph graph-match source lists into one `GraphMatch` source, while different graph references, mixed source kinds, and true multi-source composition still need real operator semantics.
