...
void MakeColumnsFromVector(T *ptr) {
  if (ptr->dataHolder.empty()) {
    ...
    for (size_t i = 0; i < ptr->num_cols; ++i) fields.push_back({nullptr, 0});
    ptr->fieldHolder.push_back(fields);
  } else {
    for (const auto &row : ptr->dataHolder) {
      std::vector<ClickHouseLibrary::Field> fields;
      for (const auto &field : row) fields.push_back({&field, sizeof(field)});
      ptr->fieldHolder.push_back(fields);
    }
  }
...
void *ClickHouseDictionary_v3_loadIds(void *data_ptr, ClickHouseLibrary::CStrings *settings, ClickHouseLibrary::CStrings *columns,
                                      const struct ClickHouseLibrary::VectorUInt64 *ids) {
  auto ptr = static_cast<DataHolder *>(data_ptr);

  if (ids) LOG(ptr->lib->log, "loadIds lib call ptr=" << data_ptr << " => " << ptr << " size=" << ids->size);
...
void *ClickHouseDictionary_v3_libNew(ClickHouseLibrary::CStrings * /*settings*/,
                                     void (*logFunc)(ClickHouseLibrary::LogLevel, ClickHouseLibrary::CString)) {
