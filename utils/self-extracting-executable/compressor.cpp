...
int doCompress(char* input, char* output, off_t& in_offset, off_t& out_offset, off_t input_size, off_t output_size, ZSTD_CCtx* cctx) {
...
int compress(int in_fd, int out_fd, int level, off_t& pointer, const struct stat& info_in, uint64_t& compressed_size) {
  ...
  char* output = static_cast<char*>(mmap(nullptr, 2 * max_block_size, PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE, -1, 0));
  ...
  while (in_offset < info_in.st_size) {
    ...
    if (0 != doCompress(input, output, in_offset, current_block_size, size, ZSTD_compressBound(size), cctx)) {
      ...
      if (0 != munmap(output, 2 * max_block_size)) perror("munmap");
      ZSTD_freeCCtx(cctx);
      return 1;
    }
  ...
  std::cout << "Compressed size: " << compressed_size << ", compression rate: " << std::fixed << std::setprecision(2)
            << static_cast<float>(compressed_size) / static_cast<float>(total_size) * 100 << "%" << std::endl;

  if (0 != munmap(input, info_in.st_size) || 0 != munmap(output, 2 * max_block_size)) {
    perror("munmap");
    ZSTD_freeCCtx(cctx);
    return 1;
  }
...
int saveMetaData(const char* filenames[], int count, int output_fd, const MetaData& metadata, FileData* files_data, size_t pointer,
  ...
  char* output = static_cast<char*>(mmap(nullptr, pointer + count * sizeof(FileData) + sum_file_size + sizeof(MetaData),
...
int compressFiles(const char* out_name, const char* exec, char* filenames[], int count, int output_fd, int level,
                  const struct stat& info_out) {
  MetaData metadata;
  ...
  for (int i = 0; i <= count; ++i) {
    ...
    if (!is_exec && !exec && strcmp(names[i], out_name) == 0) files_data[i].exec = true;
...
const char* get_param(int argc, char* const argv[], const char* name) {
  if (nullptr == name || name[0] == 0) return nullptr;
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (nullptr == eq) eq = arg + arg_len;

    if (name_len != static_cast<size_t>(eq - arg - 2)) continue;

    if (0 == memcmp(name, arg + 2, name_len)) return *eq == 0 ? eq : eq + 1;
  }
...
int main(int argc, char* argv[]) {
  ...
  if (decompressor != nullptr) {
    if (decompressor[0] == 0) decompressor = nullptr;
    ++start_of_files;
  }

  /// Specified executable
  const char* exec = get_param(argc, argv, "exec");
  if (exec != nullptr) ++start_of_files;
  ...
  if (decompressor != nullptr) {
    if (copy_decompressor_file(decompressor, output_fd)) return 1;
  } else {
    if (copy_decompressor_self(argv[0], output_fd)) return 1;
  }
