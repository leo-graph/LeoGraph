...
/// decompress part
int doDecompress(char *input, char *output, off_t &in_offset, off_t &out_offset, off_t input_size, off_t output_size, ZSTD_DCtx *dctx) {
...
int decompress(char *input, char *output, off_t start, off_t end, size_t max_number_of_forks = 10) {
  ...
  while (in_pointer < end && !error_happened) {
    ...
    if (-1 == pid) {
      ...
      if (0 != doDecompress(input, output, in_pointer, out_pointer, size, decompressed_size, dctx)) _exit(1);
      _exit(0);
    } else {
      ++number_of_forks;
  ...
  while (number_of_forks > 0) {
    ...
    if (WIFEXITED(status)) {
      if (WEXITSTATUS(status) != 0) error_happened = true;
    } else {
      error_happened = true;
  ...
  if (in_pointer < end || error_happened) return 1;

  return 0;
}

bool isSudo() { return geteuid() == 0; }

/// Read data about files and decompress them.
int decompressFiles(int input_fd, char *path, char *name, bool &have_compressed_analoge, bool &has_exec, char *decompressed_suffix,
                    uint64_t *decompressed_umask) {
  ...
  if (0 != fstatvfs(input_fd, &fs_info)) {
    perror("fstatvfs");
    if (0 != munmap(input, info_in.st_size)) perror("munmap");
    return 1;
  }
  /// Available space in bytes = free blocks * block size
  if (fs_info.f_bavail * fs_info.f_frsize < decompressed_full_size) {
    std::cerr << "Not enough space for decompression. Have " << fs_info.f_bavail * fs_info.f_frsize << ", need " << decompressed_full_size
              << std::endl;
    return 1;
  }
  ...
  for (size_t i = 0; i < le64toh(metadata.number_of_files); ++i) {
    ...
    if (file_info.exec) {
      ...
      if (strcmp(name, input + files_pointer) == 0) same_name = true;
    ...
    char *output =
        static_cast<char *>(mmap(nullptr, le64toh(file_info.uncompressed_size), PROT_READ | PROT_WRITE, MAP_SHARED, output_fd, 0));
    ...
    if (0 != decompress(input, output, le64toh(file_info.start), le64toh(file_info.end))) {
      if (0 != munmap(input, info_in.st_size)) perror("munmap");
      if (0 != munmap(output, le64toh(file_info.uncompressed_size))) perror("munmap");
      return 1;
    }

    if (0 != munmap(output, le64toh(file_info.uncompressed_size))) perror("munmap");
    if (0 != fsync(output_fd)) perror("fsync");
    if (0 != close(output_fd)) perror("close");

    if (is_sudo) chown(file_name.data(), info_in.st_uid, info_in.st_gid);
  }
...
int read_exe_path(char *exe, size_t buf_sz) {
  ...
  if (_NSGetExecutablePath(apple.data(), &size) != 0) return 1;
  if (realpath(apple.data(), exe) == nullptr) return 1;
  return 0;
}
...
uint64_t getInode(const char *self) {
  ...
  for (std::string line; std::getline(maps, line);) {
    ...
    std::string path;
    uint64_t inode = 0;
    if (ss >> addr >> mode >> offset >> id >> inode >> path && path == self) return inode;
  }
...
int main(int /* argc*/, char *argv[]) {
  ...
  if (input_info.st_ino & 0xFFFFFFFF00000000 && !(inode & 0xFFFFFFFF00000000)) input_info.st_ino &= 0x00000000FFFFFFFF;
  ...
  if (input_info.st_ino != inode) {
    ...
    if (lock_info.st_size == 1) execv(self, argv);  // NOLINT(clang-analyzer-optin.taint.GenericTaint)
  ...
  if (0 != decompressFiles(input_fd, path, name, have_compressed_analoge, has_exec, decompressed_suffix, &decompressed_umask)) {
    ...
    if (0 != close(input_fd)) perror("close");
    return 1;
  }
  ...
  if (!have_compressed_analoge)
    ...
    if (isSudo()) chown(static_cast<char *>(self), input_info.st_uid, input_info.st_gid);
