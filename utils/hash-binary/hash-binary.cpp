...
uint64_t rotl64(uint64_t value, unsigned int count) { return (value << count) | (value >> (64U - count)); }
...
class SipHash {
 ...
 public:
  ...
  void update(const char *data, uint64_t size) {
    ...
    switch (end - data) {
      case 7:
        current_bytes[6] = static_cast<uint8_t>(data[6]);
        [[fallthrough]];
      case 6:
        current_bytes[5] = static_cast<uint8_t>(data[5]);
        [[fallthrough]];
      case 5:
        current_bytes[4] = static_cast<uint8_t>(data[4]);
        [[fallthrough]];
      case 4:
        current_bytes[3] = static_cast<uint8_t>(data[3]);
        [[fallthrough]];
      case 3:
        current_bytes[2] = static_cast<uint8_t>(data[2]);
        [[fallthrough]];
      case 2:
        current_bytes[1] = static_cast<uint8_t>(data[1]);
        [[fallthrough]];
      case 1:
        current_bytes[0] = static_cast<uint8_t>(data[0]);
        [[fallthrough]];
      default:
        break;
    }
  ...
  void update(uint64_t value) { update(reinterpret_cast<const char *>(&value), sizeof(value)); }
...
bool hashExecutableSegments(std::istream &in, SipHash &hash) {
  ...
  for (size_t i = 0; i < ehdr.e_phnum; ++i) {
    ...
    if (phdr.p_type == PT_LOAD && (phdr.p_flags & PF_X)) {
      std::cerr << "Found executable segment: offset=0x" << std::hex << phdr.p_offset << ", filesz=0x" << phdr.p_filesz << std::dec << " ("
                << phdr.p_filesz << " bytes)\n";
...
int main(int argc, char **argv) {
  try {
    ...
    SipHash hash;
    if (!hashElfFile(file, hash)) return 1;
    ...
    std::cout << std::uppercase << std::hex << std::setfill('0') << std::setw(16) << hi << std::setw(16) << lo;
    return 0;
